...
use wellfriendpdf_engine::{
    analyze_semantic_layout, line_break_text, preview_reflow, prompt33_feature_matrix,
    GeometricReflowRequest, LayoutConstraint,
};

const PROMPT33_SOURCE_FIXTURE: &[u8] =
...
fuzz_target!(|data: &[u8]| {
    ...
    let _ = preview_reflow(PROMPT33_SOURCE_FIXTURE, &request);
    let _ = analyze_semantic_layout(PROMPT33_SOURCE_FIXTURE, Some(&request));
    let _ = serde_json::to_string(&request);
    let _ = prompt33_feature_matrix();
});
