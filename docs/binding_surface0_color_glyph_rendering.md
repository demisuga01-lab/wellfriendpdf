# Multilingual Color Glyphs Color Glyph Rendering

Porterduff Radial Color Glyph closes the remaining safe COLRv1 color-glyph work with gradients,
...
- `PaintComposite` with `SourceOver`, Porter-Duff modes, `Plus`, and the PDF
  blend modes already supported by Transparency Rendering/07B
...
the CJK RTL Color Glyph Closeout and 10C synthetic sbix fixtures.

Colrv Svg Bitmap adds `sbix` JPEG strike rendering through the existing bounded DCT
...
Colrv Svg Bitmap also adds safe static SVG-in-OpenType rendering for simple paths,
basic shapes, finite transforms, fill/stroke, and opacity. It never executes
active SVG content.

## Exact Unsupported Rows

No Multilingual Color Glyphs COLRv1 composite operator remains broadly unsupported. Malformed
...
```text
cjk_rtl_color_glyph_closeout_color_glyph_cjk_rtl_fidelity_closure
color_glyph_hinting_color_glyph_hinting_cff_closure
colrv_svg_bitmap_full_colrv1_svg_color_glyph_closure
colrv_gradient_composite_colrv1_gradient_clip_composite_closure
porterduff_radial_color_glyph_colrv1_porterduff_radial_closure
```
