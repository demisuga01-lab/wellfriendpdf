# Pades LTV Fuzz — cargo-fuzz Blocker Resume Audit

Schema: `pades_ltv_fuzz.cargo-fuzz-blocker-closure.v1`

Pades LTV Fuzz resumes the incomplete roadmap closure 25 run to resolve the single
outstanding closure gap — sanitizer-backed `cargo-fuzz` execution under the
4 GiB memory cap — without redoing Pades LTV and without discarding any work.

## Starting state (preserved, not modified)

- HEAD: `6bc409a5e926d8e6168b3acd07ccf21dd78fb717`
  ("Close roadmap closure 24 certificate trust pades ocsp crl").
- Branch `main`, remote `https://github.com/demisuga01-lab/wellfriend-parser.git`.
- Worktree was dirty with the full Pades LTV implementation (signature/timestamp/
  DSS/VRI/LTV/MDP/edit surfaces across Rust, CLI, Python, C ABI, .NET, Java, WASM),
  plus untracked Pades LTV docs, scripts, two new fuzz targets, and gitignored
  `target/pades_ltv-signature-ltv-edits/` evidence.
...
`target/pades_ltv-signature-ltv-edits/pades_ltv_fuzz-resume-start-state.json`.

## Recovery snapshot (before any change)

An external recovery archive was created outside the repo and hash-recorded:

- Path: `E:\wellpdfsdk-pades_ltv-recovery\pades_ltv_fuzz-fuzz-blocker-resume-20260721T115902.zip`
...
- Contents: `tracked-dirty.patch`, `tracked-dirty-files/`, `untracked-files/`,
  `generated-pades_ltv-artifacts/` (80.85 MB), `inventory.json`.

The archive is not committed and is not a substitute for the closure commit.
See `target/pades_ltv-signature-ltv-edits/pades_ltv_fuzz-recovery-artifact.json`.
...
These match the two failing commands recorded during the incomplete Pades LTV
...
`docs/pades_ltv_fuzz_cargo_fuzz_root_cause.md`. In short: keep the **default address
...
the current Windows/MSVC toolchain: all four Pades LTV signature fuzz targets
...
cargo-fuzz; Windows/MSVC cargo-fuzz was never a release requirement. Pades LTV Fuzz
additionally adds the Signature Validation/25 signature targets (`signature_evidence`,
