# Prompt 06B Semantic Inventory

Prompt 06B is an additive closure pass on the Prompt 06 semantic text model. It
does not change flat text extraction, table detection, form handling, rendering,
or redaction application.

| Feature | Prompt 06 status | Gap | Prompt 06B target | Tests needed | Memory/cap concern |
| --- | --- | --- | --- | --- | --- |
...
| MCID-to-content mapping | `TextCollector::collect_marked` existed | Prompt 06 model used unmarked chunks | build model from marked chunks when structure is enabled | MCID char/search assertions | `max_mcid_entries` |
...
| RTL/vertical semantic fields | direction flags existed | unchanged | preserve existing fields | existing Prompt 06 tests | none |
...
| redaction-planning provenance | search quads existed | match source unclear | search results identify ActualText/hidden/fallback/tag source | search fixture | redaction apply remains Prompt 07 |
