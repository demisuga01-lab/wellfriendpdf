...
Prompt 34 routes tables, approved math and OCR text edits through Prompt 33,
annotation appearance regeneration through Prompt 17, form data through the
...
`prompt34_analyze` returns the canonical table projection, born-digital math
trees, OCR layer classification, interactive annotation/form inventory, and
XFA inventory/extraction/runtime evidence. `prompt34_plan` accepts the same
typed request consumed by `prompt34_apply`; it does not invent a second
document model.
...
editor or Prompt 33 transaction path and reopens its output before returning.
