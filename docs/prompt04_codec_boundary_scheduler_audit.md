# Combined Prompt 04 Codec Boundary And Scheduler Audit

## Starting Checkpoint

- Starting HEAD: `79b15aa`.
- Starting commit: `79b15aa Close combined prompt 03B wasm-pack packaging gate`.
...
- `crates/engine/src/lib.rs` has `#![forbid(unsafe_code)]`, so Prompt 04 acceleration and policy enforcement must remain safe in the engine crate.
...
## Prompt 04 Implementation Bounds

- This prompt must not implement renderer parity features such as transparency parity, shadings, patterns, font raster parity, color glyphs, or PDFium/MuPDF comparison.
- This prompt must not add native/C codec dependencies by default.
