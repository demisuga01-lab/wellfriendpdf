...
- Oxide `native-cmm-lcms2`
- Poppler
- PDFium
- MuPDF

Reference renderers often flatten Separation and DeviceN color into RGB preview
and do not expose Oxide's internal plate framebuffer. Those differences are
classified, not treated as automatic failures.

The audit policy:

- when references agree on visual preview and Oxide is an outlier, classify it
  as an Oxide outlier.
- when references disagree because spot/DeviceN flattening policy differs,
  classify the disagreement and use Oxide internal plate artifacts as plate
  evidence.
