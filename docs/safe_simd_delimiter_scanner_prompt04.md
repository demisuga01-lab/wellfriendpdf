# Prompt 04 Safe Delimiter Scanner

## Scope

The Prompt 04 scanner accelerates raw PDF marker candidate discovery. It does not replace PDF parsing and does not decide whether a marker is valid in lexical context.
...
Prompt 04 routes parser/reader marker searches through `find_marker_accelerated()` / `rfind_marker_accelerated()` for stream, endstream, xref, trailer, startxref, and object-header recovery scans.
...
- `target/prompt04-codec-boundary-scheduler/scanner-parity-report.json`
- `target/prompt04-codec-boundary-scheduler/scanner-benchmark-report.json`
