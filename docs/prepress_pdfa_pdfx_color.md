...
Decode Scheduler adds color-specific reporting and validation hooks. It does not claim
...
- `PdfA`: emits `color.output_intent.missing` if no output intent exists and
  ...
  `DestOutputProfile`. Color Management also diagnoses output-intent `/S` values that
  are not `GTS_PDFA1`.
- `PdfX`: same required output-intent/profile checks as `PdfA`, plus Color Management
...
is acceptable for preview but is not a claim of true spot-plate output. Roadmap task
...
Prepress CMM adds a sparse separation framebuffer side-channel. Separation and
DeviceN colorant names, tint values, alternate preview RGB, alpha, and
provenance are preserved in
`prepress_cmm_prepress_cmm_device_link_separation_plates`. RGB page output remains a
visual preview. The plate report is the source of truth for Prepress CMM
separation evidence.

## Overprint

Decode Scheduler parses and preserves:
...
`DrawState` carries them for future device backends. Color Management adds a bounded
...
Color Management keeps the default backend as safe Rust plus qcms. ICC transforms are
...
Prepress CMM extends ICC inventory with profile class detection for input,
...
overprint usage, plus the Color Management color-specific PDF/A and PDF/X
...
Prepress CMM does not change that standards boundary. It improves prepress
structure preservation and reporting, but it is not certification-grade PDF/X
validation. Prepress Proofing reports bounded overprint simulation, but it still does
