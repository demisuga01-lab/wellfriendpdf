...
| Binding | Surface |
| --- | --- |
| Rust | `wellfriendpdf_engine::prompt21_*` functions and `prompt21` module types |
...
| C ABI | `wellfriendpdf_document_prompt21_*` functions |
| WASM | `prompt21*Json` methods and `prompt21PackObjectStreams()` |
| .NET | `WellfriendDocument.Prompt21*` methods |
| Java | `WellfriendPdf.Document.prompt21*` methods |
