# Font Subsystem Font Fidelity Closure

Font Subsystem closes the font-foundation leftovers that can be handled safely in
...
Font Subsystem does not rewrite sfnt `glyf`/`loca`/`cmap`/`hmtx`/`maxp`/`head`
...
color glyph rendering is not implemented in Font Subsystem because it intersects
the Decode Scheduler color pipeline and image compositing.

## Benchmark Baseline

Codec Boundary baseline on the text/font Poppler slice:
...
Font Subsystem reran the same capped slice after rebuilding `target\release\wellfriendpdf.exe`:
...
- artifact path: `target/font_subsystem-font-render-benchmark/`

This is not a visual-fidelity improvement over Codec Boundary. The code changes close
...
## Font Failure Analysis Follow-Up

Font Failure Analysis implemented the named sfnt/glyf subsetting campaign for the generated
...
The Font Failure Analysis benchmark reran the same 24-file Poppler font/text slice:

- visual pass: 45.83% -> 45.83%
- weighted score: 45.21 -> 45.21
- peak Wellfriend memory: 11.28 MB -> 11.54 MB
- artifact path: `target/font_failure_analysis-font-render-benchmark-v2/`
...
two blank-reference mismatches. See `docs/font_font_failure_analysis_failure_analysis.md`
and `docs/font_subsystem_font_failure_analysis.md`.
