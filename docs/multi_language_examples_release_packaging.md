# Release Packaging Multi-Language Examples

The example suite uses public SDK surfaces only. Existing Binding Surface/02 examples remain the broad workflow examples; Release Packaging adds codec isolation examples and records all workflows in the release matrix generated under `target/release_packaging-packaging-codec-isolation/`.

## Example Inventory

| Surface | General workflow example | Release Packaging codec example | Package context |
| --- | --- | --- | --- |
| Rust | `crates/engine/examples/sdk_reports.rs` | `crates/engine/examples/release_packaging_codec_isolation.rs` | `cargo package -p wellfriendpdf-engine --allow-dirty` |
...
| WASM | `crates/wellfriendpdf-wasm/examples/browser` | `codec_isolation_report.mjs` | `scripts/wasm_packaging_wasm_pack_gate.ps1` wasm-pack web/Node packages |
| .NET | `bindings/dotnet/examples/BindingParityReports.cs` | `ReleasePackagingCodecIsolation.cs` | `dotnet pack` artifact |
| Java | `bindings/java/examples/BindingParityReports.java` | `ReleasePackagingCodecIsolation.java` | Maven and Gradle JARs |
...
```powershell
cargo run -p wellfriendpdf-engine --example release_packaging_codec_isolation -- in_process
```
...
node scripts\wasm_packaging_wasm_pack_node_smoke.mjs target\release_packaging-packaging-codec-isolation\wasm-pack\node-pkg crates\engine\tests\fixtures\minimal.pdf target\release_packaging-packaging-codec-isolation\wasm-pack\wasm-pack-node-smoke.json
```

The Wasm Packaging WASM gate imports the generated `nodejs` package directory, not
