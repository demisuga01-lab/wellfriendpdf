...
Update: GA Prompt 4 reran the same 265-entry slice after targeted renderer and benchmark-attribution fixes. Prompt 3 then reran the same slice after raw-image decode and conservative text-AA calibration fixes. The latest recorded result is 86.94% visual pass / 91.82 weighted score; see `docs/ga4_renderer_fidelity.md`. The Prompt 11 baseline below is preserved for provenance.
