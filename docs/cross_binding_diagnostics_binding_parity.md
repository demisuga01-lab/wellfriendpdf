# Cross-Binding Diagnostics Binding Parity

Binding Parity preserves diagnostics by routing public binding methods through the
...
Binding Parity does not introduce surface-specific schema forks. Convenience layers
...
Java Packaging makes progress and cancellation posture machine-readable in the
shared feature report. Progress is reported as `progress_not_supported` because
the Binding Parity report/output facade has no phase or step callback events.
Cancellation is reported as
`cancellation_not_supported_for_binding_parity_bindings`: engine render internals
already have `render_page_cancellable` and
`render_display_list_cancellable_with_mode`, but WASM/.NET/Java Binding Parity
...
`target/binding_parity-binding-parity/cross-binding-parity.json` compares SHA-256
