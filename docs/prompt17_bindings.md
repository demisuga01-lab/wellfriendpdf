...
- Rust: typed functions and reports in `wellfriendpdf_engine::prompt17` and crate-root re-exports.
...
- C ABI: versioned `wellfriendpdf_document_*_json` functions with explicit input lengths and owned output/free rules; all Prompt 17 declarations are published in `crates/wellfriendpdf-capi/include/wellfriendpdf.h` and the header passes an MSVC C syntax check.
