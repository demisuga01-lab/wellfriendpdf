# Prompt 18 secure mutation audit

Prompt 18 starts from verified commit `d0842aae76b536f8ccc82d26f1a5a8054889ad49` with a clean worktree. The canonical implementation paths are `crates/engine/src/editing.rs` for page/image content mutation, `crates/engine/src/prompt18.rs` for associated files and signature policy, `crates/engine/src/writer.rs` for full and incremental serialization, and `crates/engine/src/sdk.rs` for binding parity.
...
`crates/engine/tests/prompt18_secure_mutation.rs` reopens output, checks affected mask references, decodes rewritten inline samples, compares deterministic output bytes, exercises add/extract/dedup/remove/rescan, and verifies an incremental output begins with the exact original byte sequence. `scripts/prompt18_secure_mutation_audit.py` runs that suite serially and creates the required target-local audit bundle.

The stable feature matrix is `target/prompt18-mask-inline-associated-signatures/prompt18-feature-matrix.json`. No Prompt 18 row is blocked. Exact bounded limits are documented in `docs/prompt18_known_limits.md`.
