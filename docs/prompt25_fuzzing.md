...
Prompt 25 fuzz bins compile and in-engine hostile seed smoke passes.

Prompt 25B resolution: sanitizer-backed `cargo-fuzz` now builds and smoke-runs
...
`[profile.dev]`. All four Prompt 25 signature fuzz targets pass with
...
production defects. Details: `docs/prompt25b_cargo_fuzz_root_cause.md` and
`docs/prompt25b_fuzz_blocker_resume_audit.md`.
...
consolidated, narrow targets cover the nine logical Prompt 25 fuzz areas
...
policy); see `target/prompt25-signature-ltv-edits/cargo-fuzz-target-inventory-prompt25b.json`.
