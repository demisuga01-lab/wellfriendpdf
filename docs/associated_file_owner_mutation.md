...
Wellfriend supports full-rewrite add, update, and unlink for catalog `/AF`, page `/AF`, annotation `/FS` and `/AF`, structure-element `/AF`, Form XObject `/AF`, and supported image XObject `/AF`. Catalog EmbeddedFiles indexing remains distinct; owner mutation does not silently canonicalize every association into that name tree.
