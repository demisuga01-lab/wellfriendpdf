# Advanced Rendering Editing And Conversion Inventory

Advanced Rendering starts from the Transparency Closeout checkpoint and adds a shared editable
...
| Feature | Current state | Advanced Rendering target | Tests | Risk | Later boundary |
| --- | --- | --- | --- | --- | --- |
| logical document model | `parse::Document` and Reference Renderer semantic model exist | feed a Advanced Rendering editable model | `advanced_rendering_editing_conversion` | medium | deeper ML classification later |
...
| footnotes/endnotes | heuristic from Native Renderer | represented only when parse roles expose them | docs | medium | full note linkage later |
...
| reading order | Reference Renderer model | preserved from parse body order | existing gates | medium | no perfect untagged order claim |
...
| real table grid model | Transparency Rendering table cells/spans | editable table cells export to Markdown/HTML/Office | Office package test | low | complex table style recreation later |
...
| page-faithful DOCX mode | Type3 CID Rendering implements `DocxLayout::PageFaithful` | positioned OOXML anchors/text boxes | `type3_cid_rendering_editing_conversion` | medium | exact Word pagination later |
...
| RAG/chunk export | Reference Renderer chunks exist | model keeps provenance/sections for chunk input | docs | low | chunking tune later |
...
| incremental save | editor supports append-only overlays | expose CLI and test prefix preservation | integration test | medium | signature-aware preservation in Annotation Ocg Rendering |
...
| undo/redo model | Type3 CID Rendering patch/checkpoint transaction log | model text edit undo/redo | unit test | low | external transaction replay later |
...
| Zopfli/object-stream optimization | writer modes exist | bounded as optimization, not Advanced Rendering core | docs | medium | compression phase later |

Baseline artifacts were recorded under `target/advanced_rendering-baseline/` before code
changes. Advanced Rendering artifacts are written under `target/advanced_rendering-artifacts/`.
