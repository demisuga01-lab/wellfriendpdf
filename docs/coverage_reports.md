...
`scripts/run_malformed_coverage_coverage.py` records coverage tool support, coverage summary, and a low-coverage risk register.
...
The risk register identifies areas where line or region coverage is still not sufficient for final release claims. Malformed Coverage coverage is a hardening signal, not proof that every real-world malformed PDF path has been exercised.
