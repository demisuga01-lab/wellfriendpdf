# Prompt 04 Renderer Decode Scheduler Adoption

## Scope

Prompt 04 adopts the decode scheduler boundary for renderer decode work without changing rendering semantics or enabling opportunistic parallel composition.
...
`RenderCache` remains a final tile-buffer cache, not a decoded image-stream cache. Prompt 04 does not introduce speculative parallel renderer predecode.
...
The renderer still composes in content-stream order. The scheduler is used for memory admission and metrics, not for racing paint operations. The Prompt 04 report generator renders the `image_only.pdf` fixture twice and records matching output hashes.
...
- `target/prompt04-codec-boundary-scheduler/renderer-scheduler-report.json`
