...
This audit maps the parser state used for Binding Surface. It is based on the current
...
| Xref streams | Present, hardened in this roadmap task | `reader.rs::{read_xref_stream, parse_xref_stream_entries}` | Supports `/W`, `/Index`, entry types 0/1/2, zero-width type fields. This roadmap task adds duplicate `/Index` object-number rejection. |
| Object streams | Present, hardened in this roadmap task | `reader.rs::{parse_object_stream, parse_object_stream_data}` | Lazy-decoded through bounded `BoundedObjectStreamCache`. This roadmap task rejects duplicate object IDs inside object streams and keeps `/N` preallocation bounded. |
...
| Encryption detection/hooks | Present | `reader.rs::setup_encryption`, `EncryptionContext`, `encrypt_dictionary` | Detects `/Encrypt`, verifies Standard Security Handler passwords, decrypts strings/streams on object access. Encrypted content without keys returns structured `WellfriendError`. |
...
| Parser errors | Present, now supplemented | `error.rs::WellfriendError`, `parser_report.rs::ParserDiagnostic` | Existing errors are stable enough for callers; this roadmap task adds structured diagnostics with severity/category/code/offset/recovery fields. |
| Repair/recovery | Present, hardened in this roadmap task | `rebuild_xref_from_object_scan`, `repair_uncompressed_xref_offsets`, `scan_indirect_object_headers` | Tiers implemented today: trusted xref, near xref repair, object scan/trailer synthesis. This roadmap task records repair actions and ignores object-looking bytes inside normal stream spans. |
...
| Fuzzing | Present, extended | `fuzz/`, `crates/engine/src/fuzz.rs` | Existing out-of-workspace cargo-fuzz targets cover document parser, filters, writer, rendering-adjacent paths. This roadmap task adds COS object, parser report, xref stream, and object stream targets. |
| Corpus/differential | Present scripts, extended | `scripts/poppler_compare.py`, `scripts/differential_fuzz.py`, `scripts/parser_corpus_runner.py`, `scripts/parser_differential.py` | SafeDocs-compatible bounded JSONL corpus runner and parser-specific differential harness over `wellfriendpdf parser-report`. External tools remain opt-in unless required by flag. |

## Fragile Assumptions Found

- Repair object scanning previously treated object-looking byte sequences inside
  stream data as possible indirect-object headers. This roadmap task now skips normal
  `stream`...`endstream` spans in the repair scanner.
- Xref stream `/Index` ranges could overlap and silently produce duplicate object
  numbers in one section. This roadmap task rejects duplicate object numbers.
- Object streams could declare duplicate object IDs, with last-writer wins in the
  temporary map. This roadmap task rejects duplicates.
- `/Prev` traversal had loop detection but no explicit depth ceiling. This roadmap task
  adds a 256-section cap.
- The public parser surface had no structured diagnostic report. This roadmap task adds
  `ParserReport`, `ParserDiagnostic`, and `wellfriendpdf parser-report`.
