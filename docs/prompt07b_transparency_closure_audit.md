...
|---|---|---|---|---|---|
| `alpha_image` | Wellfriend was an outlier while references agreed | Closed | Image XObject painting now multiplies decoded/SMask alpha by graphics-state `/ca` before `PixelBuffer::blend_pixel` | `alpha_image`; `prompt07b-render-results.json` | None for DeviceRGB alpha constants |
...
| `multi_reference_closure` | One `alpha_image` outlier plus partial rows | Closed | Poppler/PDFium/MuPDF/Wellfriend audit rerun on Prompt 07 plus 07B fixtures | `prompt07b-reference-disagreement-summary.json`, `prompt07b-html-report/index.html` | Malformed recursive group remains classified as malformed/reference failure |
