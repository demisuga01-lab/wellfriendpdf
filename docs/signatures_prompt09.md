...
- DocMDP/FieldMDP permission evaluation remains bounded; Prompt 09 reports when permission evaluation is unavailable.
- Canonical full rewrite invalidates existing signed byte ranges by design.

## Tests

Existing signature tests cover valid, tampered, timestamp, DSS/LTV, revoked CRL, and incremental signing cases. Prompt 09 adds check-bit assertions for ByteRange, digest, CMS, chain, timestamp, and LTV status separation.
