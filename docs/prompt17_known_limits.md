...
- Sub-byte and stencil images are decoded defensively for rendering, but Prompt 17 does not claim sample-space rewriting for them; intersecting invocations are removed or the strict policy fails closed.
...
- Prior byte-range signatures are invalidated by every Prompt 17 full rewrite.
