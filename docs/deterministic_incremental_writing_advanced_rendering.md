# Deterministic And Incremental Writing In Advanced Rendering

Advanced Rendering uses the existing writer instead of inventing another serializer.

Deterministic full rewrite:

- stable object/resource ordering comes from the existing writer modes.
- Advanced Rendering tests compare resource digests from repeated identical text
  replacements.
- conversion packages are emitted deterministically for the same input/options
  within the native writer's supported subset.
- Type3 CID Rendering adds `DeterministicSaveOptions` and
...
- Type3 CID Rendering tests repeated incremental output for identical input/edit/options
  and compares SHA-256 digests.

Signature boundary:

- edits can invalidate signatures.
- Advanced Rendering does not perform ByteRange/CMS/PAdES/LTV validation.
- Annotation Ocg Rendering owns full signature preservation and validation decisions.
...
- object-stream packing and Zopfli-class compression are not Advanced Rendering core.
