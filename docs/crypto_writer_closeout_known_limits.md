# crypto writer closeout Known Limits

These are exact supported-boundary limits after the final crypto writer closeout closure
pass. They are not hidden blockers for crypto writer closeout.

- RecipientInfo classes outside the supported `/Adobe.PubSec`
  `/adbe.pkcs7.s5` KeyTrans profile are classified in
  `pubsec-recipient-profile-matrix-crypto_writer_closeout.json` as not applicable or exact
