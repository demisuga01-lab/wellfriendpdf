# Codec Boundary RLBox/WASM Sandbox Feasibility

## Verdict

RLBox/WASM codec sandboxing is hard-blocked for Codec Boundary. Wellfriend must not claim RLBox support from documentation alone.

The practical sandbox boundary remains the Release Packaging `wellfriendpdf-codec-worker` subprocess protocol with input caps, output caps, timeout handling, worker crash containment, and fail-closed policy modes.

## Evidence

The Codec Boundary feasibility wrapper records local command evidence in:
