...
- Example: `bindings/java/examples/BindingParityReports.java`
...
Java Packaging added password-aware open overloads. Passwords are encoded as UTF-8
...
`bindings/java/target/wellfriendpdf-sdk-0.1.0.jar`. The Java Packaging package script
(`scripts/java_packaging_java_package_smoke.ps1`) downloads Maven 3.9.9 into
`target/java_packaging-tools` if no host `mvn` exists, runs `mvn clean test` and
...
`bindings/java/build/libs/wellfriendpdf-sdk-0.1.0.jar`. The Gradle Packaging package script
(`scripts/gradle_packaging_gradle_package_smoke.ps1`) downloads pinned Gradle 9.6.1
into `target/gradle_packaging-tools` if no host `gradle` exists, verifies the archive
...
manifest fields can differ. Gradle Packaging requires public class/API equivalence,
matching `Automatic-Module-Name`, clean package contents, and successful runtime
smokes for both artifacts.

Progress and cancellation are not exposed because the current Binding Parity
...
`cancellation_not_supported_for_binding_parity_bindings` statuses.
