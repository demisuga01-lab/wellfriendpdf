# Native Renderer Renderer Failure Taxonomy

The machine-readable taxonomy is written to
`target/native_renderer-renderer-native-replay/failure-taxonomy.json`.
...
Reference Renderer writes the multi-reference taxonomy to
...
Advanced Rendering updates the current renderer gap posture: text clipping, common
shadings, mesh/patch shadings, and tiling patterns are no longer vague
`later_owned` buckets. Type3 CID Rendering closes the common Type3/CID text-clipping and
