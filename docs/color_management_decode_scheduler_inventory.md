# Decode Scheduler Color Inventory

Starting checkpoint: `16af5ac Complete Font Final Parity final font parity audit`.

Decode Scheduler found that Wellfriend already had a useful internal color layer:
...
| color feature | parser support | rendering support | image conversion | shading support | writer support | PDF/A/PDF/X reporting | diagnostics before Decode Scheduler | Decode Scheduler target |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
| ICCBased | DONE | qcms to sRGB for Gray/RGB when valid; native LittleCMS for CMYK when enabled | qcms to RGB/Gray preview; CMYK ICC requires native LittleCMS | named color path plus SVG/PS Gray/RGB vector shadings in default qcms and CMYK vector shadings with native LittleCMS | output intents reported, authoring output intent still subset | profile presence/basic validity | limited | cap profiles, expose backend decision |
...
Decode Scheduler deliberately did not introduce LittleCMS/native FFI. The engine crate
