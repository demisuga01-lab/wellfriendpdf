...
| CLI contract controls | PARTIAL_ADVANCED | Bounded contract/raw-surface rendering, grayscale output, print-profile selection, contract background, full schema-v1 field builders, and exact full-field JSON input are exposed; active CPU execution for non-default transform/page-box/CMM/overprint/exactness/determinism and non-pixel resource-budget semantics remains incomplete. |
