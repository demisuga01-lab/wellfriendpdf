...
| CLI contract controls | PARTIAL_ADVANCED | Bounded contract/raw-surface rendering, print-profile selection, and exact full-field JSON input are exposed; ergonomic field builders for matrix, page box, background, CMM, overprint, exactness, determinism, and resource-budget subfields remain incomplete. |
