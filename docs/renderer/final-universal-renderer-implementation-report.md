...
| RV-30 | PARTIAL | PARTIAL_ADVANCED | Rust API/CLI capabilities | active / active | contract Rust API plus CLI contract/raw surface path and exact JSON input active | contract rejects unsupported policy; CLI raw layout flags require raw output; JSON input rejects builder mixing | CLI now covers clip, raw caller surface, sidecar/input contract JSON, grayscale, byte order, halftone, print-profile, annotations, forms, and max-pixel budget; ergonomic matrix/page-box/background/CMM/overprint/non-pixel-budget builders remain incomplete |
