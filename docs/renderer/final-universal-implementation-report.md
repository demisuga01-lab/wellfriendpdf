...
| RV-30 | PARTIAL | PARTIAL_ADVANCED | Rust/CLI render APIs | partial | contract/raw surface route for raster CLI output | focused local CLI contract test passes | Rust/CLI partial | max pixels and contract validation | Full matrix/page-box/background/CMM/overprint/resource-budget builders still missing |
...
| RV-30 Rust/API controls | PARTIAL_ADVANCED | Contract JSON, caller-owned buffer render, reverse byte order, adaptive progressive entry, and bounded CLI raw contract rendering are active in source. | CLI still lacks builders for matrix, page box, background, CMM, overprint, exactness, determinism, and resource-budget subfields. |
