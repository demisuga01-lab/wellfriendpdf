...
| RV-30 | PARTIAL | PARTIAL_ADVANCED | Rust/CLI render APIs | partial | contract/raw surface route plus exact JSON input for raster CLI output | focused local CLI contract round-trip test passes | Rust/CLI partial | max pixels and contract validation | Ergonomic matrix/page-box/background/CMM/overprint/non-pixel-budget builders still missing |
...
| RV-30 Rust/API controls | PARTIAL_ADVANCED | Contract JSON, caller-owned buffer render, grayscale/raw layout, reverse byte order, adaptive progressive entry, bounded CLI raw contract rendering, max-pixel budget, and exact CLI JSON input are active in source. | CLI still lacks ergonomic builders for matrix, page box, background, CMM, overprint, exactness, determinism, and non-pixel resource-budget subfields. |
