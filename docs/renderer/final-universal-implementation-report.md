...
| RV-30 | PARTIAL | PARTIAL_ADVANCED | Rust/CLI render APIs | partial | contract/raw surface route plus exact JSON input for raster CLI output | focused local CLI contract round-trip test passes | Rust/CLI partial | resource-budget fields and contract validation | CLI schema builders now cover matrix, page-box, background, CMM, overprint, exactness, determinism, and non-pixel budgets; non-white background is honored; other non-default execution parity remains incomplete |
...
| RV-30 Rust/API controls | PARTIAL_ADVANCED | Contract JSON, caller-owned buffer render, grayscale/raw layout, reverse byte order, adaptive progressive entry, bounded CLI raw contract rendering, exact CLI JSON input, contract background, and CLI schema builders for matrix, page box, CMM, overprint, exactness, determinism, and resource budgets are active in source. | Active CPU execution for non-default transform/page-box/CMM/overprint/exactness/determinism and non-pixel resource-budget semantics remains incomplete. |
