...
| CLI render-contract controls | `render` used the legacy raster route and lacked contract/caller-surface controls. | Raster `render` can route through schema-v1 `RenderContract`, emit raw caller-owned surfaces, include contract and font-substitution sidecars, read an exact contract JSON file, and build schema fields for page box, device transform, background, clip, pixel format, grayscale, byte-order, halftone, print-profile, overprint, rendering intent, color management, exactness, determinism, annotation, form, and resource budgets. | `crates/cli/src/main.rs`, `crates/cli/tests/tool_surface.rs`, `crates/engine/src/engine.rs`, `render/font_substitution_report.rs` | `wellfriendpdf render --render-contract`, `--format raw`, `--write-contract-json`, `--contract-json`, `--font-substitution-report`, `--page-box`, `--device-transform`, `--background`, `--grayscale`, `--print-profile`, `--overprint`, `--rendering-intent`, `--color-management`, `--exactness`, `--determinism`, `--max-render-pixels`, `--max-decoded-bytes`, `--max-temporary-bytes`, `--max-cache-bytes` | Raw-surface layout flags require `--format raw`; vector SVG/PS/EPS paths reject contract-only flags rather than ignoring them; JSON input cannot be mixed with builder flags. | Sidecar/input JSON records the exact contract; font-substitution sidecar records bounded events and overflow; engine validation enforces max-pixel budget; contract background and Media/Crop page boxes are honored. | CLI and Rust engine. | Non-default execution remains incomplete for device transform, Bleed/Trim/Art boxes, CMM, overprint, exactness, determinism, and non-pixel resource budgets unless the active CPU renderer already honors that field. |
...
| Contract-builder parity status | PARTIAL_ADVANCED: CLI can build the schema-v1 geometry, surface, prepress, exactness, determinism, and resource-budget fields and can replay exact full-field contract JSON; full non-default execution parity remains incomplete. |
