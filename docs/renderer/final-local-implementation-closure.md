...
| CLI render-contract controls | `render` used the legacy raster route and lacked contract/caller-surface controls. | Raster `render` can route through schema-v1 `RenderContract`, emit raw caller-owned surfaces, include contract JSON sidecars, read an exact contract JSON file, and expose clip, pixel format, byte-order, halftone, print-profile, annotation, form, and max-pixel budget controls. | `crates/cli/src/main.rs`, `crates/cli/tests/tool_surface.rs`, `crates/engine/src/engine.rs` | `wellfriendpdf render --render-contract`, `--format raw`, `--write-contract-json`, `--contract-json`, `--print-profile`, `--max-render-pixels` | Raw-surface layout flags require `--format raw`; vector SVG/PS/EPS paths reject contract-only flags rather than ignoring them; JSON input cannot be mixed with builder flags. | Sidecar/input JSON records the exact contract; engine validation enforces max-pixel budget and still gives typed refusals for unsupported policies. | CLI. | Ergonomic full field builders remain incomplete for matrix, page box, background, CMM, overprint, exactness, determinism, and non-pixel resource-budget subfields. |
