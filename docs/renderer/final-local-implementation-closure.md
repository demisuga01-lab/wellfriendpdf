...
| CLI render-contract controls | `render` used the legacy raster route and lacked contract/caller-surface controls. | Raster `render` can route through schema-v1 `RenderContract`, emit raw caller-owned surfaces, include contract JSON sidecars, read an exact contract JSON file, and expose clip, pixel format, byte-order, halftone, print-profile, annotation, and form controls. | `crates/cli/src/main.rs`, `crates/cli/tests/tool_surface.rs` | `wellfriendpdf render --render-contract`, `--format raw`, `--write-contract-json`, `--contract-json`, `--print-profile` | Raw-surface layout flags require `--format raw`; vector SVG/PS/EPS paths reject contract-only flags rather than ignoring them; JSON input cannot be mixed with builder flags. | Sidecar/input JSON records the exact contract; engine validation still enforces contract identity and typed refusals. | CLI. | Ergonomic full field builders remain incomplete for matrix, page box, background, CMM, overprint, exactness, determinism, and resource-budget subfields. |
