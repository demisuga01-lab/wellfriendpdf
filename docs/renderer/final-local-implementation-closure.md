...
| CLI render-contract controls | `render` used the legacy raster route and lacked contract/caller-surface controls. | Raster `render` can route through schema-v1 `RenderContract`, emit raw caller-owned surfaces, include contract JSON sidecars, read an exact contract JSON file, and expose clip, pixel format, byte-order, halftone, annotation, and form controls. | `crates/cli/src/main.rs`, `crates/cli/tests/tool_surface.rs` | `wellfriendpdf render --render-contract`, `--format raw`, `--write-contract-json`, `--contract-json` | Raw-surface layout flags require `--format raw`; vector SVG/PS/EPS paths reject contract-only flags rather than ignoring them; JSON input cannot be mixed with builder flags. | Sidecar/input JSON records the exact contract; engine validation still enforces contract identity and typed refusals. | CLI. | Ergonomic full field builders remain incomplete for matrix, page box, background, CMM, overprint, exactness, determinism, and resource-budget subfields. |
...
| Print-profile status | PARTIAL_ADVANCED: screen halftone is active; full CMYK/DeviceN/proof execution remains incomplete. |
...
| Contract-builder parity status | PARTIAL_ADVANCED: CLI can build bounded contract/raw-surface requests and can replay exact full-field contract JSON; ergonomic full-field builders remain incomplete. |
