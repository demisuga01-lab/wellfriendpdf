# Binding Examples — Binding Surface
...
`target/binding_surface-binding-core/rust-api-smoke.json`).
...
`target/binding_surface-binding-core/c-abi-smoke.json`). Verified compiled with MSVC and
run against the real DLL in this roadmap task.

## Cross-surface parity check

```python
import json
c = json.load(open("target/binding_surface-binding-core/c-abi-smoke.json"))
p = json.load(open("target/binding_surface-binding-core/python-api-smoke.json"))
r = json.load(open("target/binding_surface-binding-core/rust-api-smoke.json"))
