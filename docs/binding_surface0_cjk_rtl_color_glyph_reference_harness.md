# Multilingual Color Glyphs CJK/RTL/Color Glyph Reference Harness

Multilingual Color Glyphs extends the renderer parity campaign into complex text rasterization
...
All Multilingual Color Glyphs artifacts are written under:
...
- `reference-tool-manifest-multilingual_color_glyphs.json`: Poppler, PDFium, and MuPDF discovery,
  versions, checksum posture, command templates, DPI, timeout, and output format.
- `corpus-manifest-multilingual_color_glyphs.json`: available CJK/RTL fixtures plus explicit
  policy rows for missing or unsupported color-glyph cases.
- `multilingual_color_glyphs-capability-matrix.json`: CJK, RTL, color glyph, PDFium, and MuPDF
  status matrix.
- `multi-reference-render-results-multilingual_color_glyphs.json`: per-page Wellfriend/Poppler/PDFium/
  MuPDF render commands, artifacts, and raw classifications.
- `multi-reference-diff-metrics-multilingual_color_glyphs.json`: per-page pairwise image metrics.
- `reference-disagreement-summary-multilingual_color_glyphs.json`: reference disagreement and
  Wellfriend outlier summary.
- `html-report/index.html`: browsable summary table.
- `public-feature-report-multilingual_color_glyphs.json`: feature report captured through the CLI.
- `binding-report-parity-multilingual_color_glyphs.json`: shared public report surface mapping.
...
Run the full Multilingual Color Glyphs harness:

```powershell
python scripts/multilingual_color_glyphs_cjk_rtl_color_glyph_reference_harness.py --dpi 72 --timeout 120
```

Useful development options:

```powershell
python scripts/multilingual_color_glyphs_cjk_rtl_color_glyph_reference_harness.py --skip-render
python scripts/multilingual_color_glyphs_cjk_rtl_color_glyph_reference_harness.py --limit 3
```

The full harness requires all three reference renderers. It reuses the Roadmap task
06B target-local bootstrap policy through
`scripts/reference_renderer_bootstrap_reference_renderers.ps1`, with Multilingual Color Glyphs-specific
tool and artifact paths. Missing Poppler, PDFium, or MuPDF is a failed Roadmap task
10 audit, not a silent skip.

## CJK Raster And Hinting Posture

The Multilingual Color Glyphs corpus includes real-safe fixtures for Type0/CID fonts,
...
CJK RTL Color Glyph Closeout supersedes the original detection-only posture. The renderer now
supports the bounded safe/common paths documented in
`docs/multilingual_color_glyphs_color_glyph_rendering.md`: COLR/CPAL v0 solid layers and bounded
...
Multilingual Color Glyphs status is exposed through the shared SDK feature report facade, so the
...
```text
multilingual_color_glyphs_cjk_rtl_color_glyph_reference_harness
```
...
- CJK RTL Color Glyph Closeout closes the Korean and Hebrew visual page fixture gaps.
