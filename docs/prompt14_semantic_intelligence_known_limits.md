...
- Prompt 14B adds production dictionary provider and external pack loading support, but large third-party CJK dictionaries are still not bundled without explicit license clarity.
- Production dictionary quality depends on the user-supplied pack's license, source, entries, normalization, and versioning discipline.
...
- Prompt 14B reports local real-runtime support as `unsupported_reported_no_runtime`; no ONNX/Torch/LayoutParser/DocLayNet runtime or model weights are bundled.
