...
- `within_tolerance`: Oxide and the reference render are dimension-compatible
...
- `oxide_outlier`: supported Oxide output is outside the reference cluster.
- `unclassified_failure`: any unsupported, structural, or visual failure that
  has not been assigned a precise cause.

Prompt 20B acceptance requires zero `oxide_outlier`, zero
