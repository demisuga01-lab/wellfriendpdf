# Annotation Ocg Rendering Fuzzing, Metamorphic, and Differential Testing

## Coverage-Guided Fuzzing

Existing fuzz bins cover parser, filters, crypto, writer, parser-report, color-report, PDF/A, editing, signature validation, structured PDFs, and display lists. Annotation Ocg Rendering keeps the standard smoke:
...
`scripts/annotation_ocg_rendering_structure_mutator.py` creates deterministic mutations for:
...
Outputs are written under `target/annotation_ocg_rendering-structure-mutations/`.

## Metamorphic Tests

Annotation Ocg Rendering adds tests for:
