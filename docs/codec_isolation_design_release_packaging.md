# Release Packaging Codec Isolation Design

## Scope

Release Packaging isolates the narrow lossless filter path first. These filters are centralized in `filters.rs` and can cross a process boundary as `(filter name, encoded bytes, limits) -> decoded bytes/report` without moving PDF object ownership or renderer state.
