# Large-File Performance Binding Parity Report
...
Binding Parity after-profile source: the working tree containing this report and the streaming reader changes.

Release Packaging follow-up: `docs/large_file_performance_release_packaging.md` records the target-scale streaming-content and bounded-parallel results.

## Headline

Target: process a single 3-4 GB PDF and documents with thousands of pages inside a hard 2 GB resident-memory envelope.

Binding Surface ceiling under the 2 GB cap:
...
Binding Parity ceiling under the same 2 GB cap:
...
Plainly: Binding Parity removed the full-file heap buffer as the dominant bottleneck and proved bounded open/page-count behavior beyond 2 GB. The remaining gap to the 3-4 GB target is per-page content-stream materialization on size-heavy files with very large single-page streams. Release Packaging should target streaming content tokenization/decode, bounded parallel page windows, and remaining throughput work.
...
- The current file source uses a `Mutex<File>`, so concurrent reads are safe but serialized. Release Packaging should revisit this when adding bounded-window parallelism.
...
- `large-file-profile/results/binding_parity-before/`

After snapshot directory:

- `large-file-profile/results/binding_parity-after/`

Output-diff artifacts:

- `large-file-profile/results/binding_parity-output-diff/`

Machine/tool provenance is unchanged from the Binding Surface baseline:
...
This is the fresh Binding Parity before snapshot from the Binding Surface commit, using the same 2 GB cap.
...
| Scenario | Binding Surface | Binding Parity | Result |
|---|---:|---:|---|
...
| Scenario | Binding Parity ceiling | What breaks next |
|---|---:|---|
| Open/page-count, size-heavy PDFs | at least 3.0 GB tested | 4.0 GB not yet profiled in Binding Parity |
...
## Bottleneck Attribution After Binding Parity
...
1. Per-page content stream materialization. The 3.0 GB synthetic file has four pages with 805,305,344 bytes of uncompressed content each. Binding Parity reaches page 1 and fails allocating that page stream. The failure has moved from "cannot open file" to "cannot hold this single active page stream."
...
3. The file-backed source serializes file reads through one `Mutex<File>`. That is acceptable for Binding Parity correctness, but Release Packaging parallel extraction should avoid a global read lock becoming the throughput bottleneck.

## Release Packaging Input List
...
7. Re-run the Binding Surface harness unchanged after each major Release Packaging change, including the real 80 MB, 290 MB, and 703 MB files to prove the common case did not regress.
