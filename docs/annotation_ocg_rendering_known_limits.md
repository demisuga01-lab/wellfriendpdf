# Annotation Ocg Rendering Known Limits

Renderer Validation removes the remaining vague future buckets for annotation rendering, optional content, progressive resume, and tile/cache behavior. Remaining limits are intentionally narrow, report-visible, and assigned.
...
- Global image/Form/pattern/shading/clip-mask/transparency-group surface caches are not introduced in Annotation Ocg Rendering. The implemented cache remains a byte-budgeted render-tile cache with deterministic eviction.
- Parallel tile rendering is not enabled by default; Annotation Ocg Rendering preserves deterministic output above speed.
