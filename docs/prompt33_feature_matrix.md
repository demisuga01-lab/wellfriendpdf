...
pairwise-disjoint same-page vector paths through the existing Prompt 20 mutator when the caller
...
`font_policy=preserve_original_per_run` route uses the existing Prompt 20
...
Prompt 31 provenance; unrelated paragraphs elsewhere in the same document do
...
format/check/clippy/test, focused Prompt 33 behavior, canonical writer impact,
Prompt 31/32 impact filters, C ABI, CLI, fresh-wheel Python, WASM, .NET,
