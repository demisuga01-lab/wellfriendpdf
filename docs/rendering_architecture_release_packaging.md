# Release Packaging Rendering Architecture

Release Packaging moves Wellfriend's renderer toward a display-list and device-based
...
Current rendering path before Release Packaging:
...
- `crates/engine/src/render/image_painter.rs` paints decoded images through the
  Binding Parity central decode/cap layer.
...
Key architectural gap before Release Packaging:
...
Release Packaging adds `crates/engine/src/render/display_list.rs`.
...
Release Packaging exposes an explicit real replay path and keeps the proven default
behavior stable.
...
Result during Release Packaging implementation: 7 passed.

## Reference Measurement

Baseline before Release Packaging edits:
...
python renderer-benchmark\scripts\renderer_benchmark.py `
  ...
  --output-dir target\release_packaging-baseline-benchmark
```

After Release Packaging display-list architecture:
...
python renderer-benchmark\scripts\renderer_benchmark.py `
  ...
  --max-pages-per-file 1 --limit 5 --determinism-sample 2 `
  --threshold-profile renderer `
  --output-dir target\release_packaging-after-benchmark
```
...
| Bezier flattening | Already present | Existing `path` tests | Covered by existing render benchmark | Release Packaging did not change algorithm |
...
- Does not bypass Binding Parity decoding.
...
renderer campaign remains in `docs/rendering_fidelity_baseline.md`; Release Packaging's
new code did not add a full-document or full-file rendering path.

## CPU Performance Direction

Release Packaging establishes the seam needed for Blend2D-style future optimization:
...
Exact remaining limits after Release Packaging:
...
- Text rendering still depends on the immediate renderer and current font
  subsystem; Codec Boundary should add display-list glyph/run operations.
- Image XObjects and inline images still render through the immediate renderer;
  a future display-list op should carry decoded-image references under Binding Parity
  decode limits.
...
## Wasm Packaging Update

Wasm Packaging keeps the Release Packaging vector-native display-list path and broadens the
...
replay gap without moving font shaping or color management work into Wasm Packaging.
...
Wasm Packaging does not claim mature-renderer parity. Its expanded Poppler-backed
...
JPEG 2000/image-codec, and function-based shading cases that belong to Roadmap task
04, Decode Scheduler, or later renderer fidelity iterations.
