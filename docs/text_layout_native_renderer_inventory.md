# Native Renderer Text/Layout Inventory

Starting checkpoint: `69ceef5 Complete Color Management color prepress closure`.

Baseline before edits:

- `cargo build --release -p wellfriendpdf-cli`: passed.
- 200-file text slice: `target\competitive-benchmark\native_renderer-text-before`.
- `char_similarity`: 0.92743.
- `word_f1`: 1.0.
- `line_recall`: 1.0.
- `reading_order`: 0.96019.

Native Renderer target: keep the flat text gates unchanged while adding a geometry-backed semantic model for search, redaction preview, JSON diagnostics, and downstream RAG/conversion use.

| capability | current state before Native Renderer | missing capability | Native Renderer target | tests | metric impact |
| --- | --- | --- | --- | --- | --- |
...
| marked content / MCID | tagged extraction uses `MarkedTextChunk` | Native Renderer model does not yet attach MCID path | keep tagged extractor; bounded limit for text model | tagged semantic tests remain | unchanged |
...
| vertical writing | Codec Boundary font writing mode exists; reading-order path handles vertical | no vertical flag in text model | vertical direction and counters | unit coverage through chunk flags | unchanged |
...
| redaction matching | editing can redact regions | text match-to-quad prep incomplete | expose stable match quads; no apply in Native Renderer | integration test | new API |
| Markdown/HTML/JSON output | parse/document model supports outputs | Native Renderer text model absent | add `model-json`; keep parse outputs canonical | CLI smoke | unchanged |
...
- Tagged PDF role paths remain in `semantic.rs`; Native Renderer does not merge every StructTree element into the new char-level model.
