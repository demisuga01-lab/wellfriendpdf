# advanced editing public surfaces and bindings

The shared Rust implementation lives in `crates/engine/src/advanced_editing.rs`.
...
and secure mutation closeout signature preflight. The CLI exposes the corresponding mutation
commands plus `advanced_editing-report`.

Python, C ABI, WASM, .NET, and Java expose the versioned advanced editing report,
...
The mutation entry points are `advanced_editing_text_edit`, `advanced_editing_vector_edit`, and
`advanced_editing_ink_fit` (idiomatically cased in .NET and Java). Text mode and option
