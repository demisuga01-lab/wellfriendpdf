# Prompt 04 Native/C Codec Boundary Policy
...
Prompt 04 adds `CodecBackendRegistryEntry` and `CodecBackendSelectionReport`. Current registered backends are the existing Rust implementations:
...
The native dependency allowlist is intentionally empty in Prompt 04. `validate_codec_registry_policy()` checks that any future native entry is feature-gated, worker-required, not enabled by default, and allowlisted.
...
- `target/prompt04-codec-boundary-scheduler/native-codec-boundary-report.json`

The SDK envelope version remains `1`; Prompt 04 adds inner report fields only.
