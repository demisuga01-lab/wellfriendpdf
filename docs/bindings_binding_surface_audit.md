# roadmap closure 01 — Binding Core Surfaces: Audit Note

This document is the evidence trail for the binding surface audit and the
Rust / Python / C-ABI stabilization performed in roadmap closure 01
(roadmap items 001–004). It is updated during the work, not only at the end.

## Starting checkpoint

- Starting HEAD: `cd67cbc` (`Complete Annotation Ocg Rendering enterprise trust hardening`)
...
The core problem this roadmap task solves is **binding divergence**: the CLI can
...
roadmap task introduces **one shared Rust facade** —
...
| Python SDK | `crates/wellfriendpdf-py/src/lib.rs` | `wellfriendpdf.Document`, module fns | pyo3; ergonomic but report-light before this roadmap task. |
...
| WASM | `crates/wellfriendpdf-wasm` | wasm-bindgen | Out of scope for this roadmap task (documented gap). |
| .NET / Java | `bindings/dotnet`, `bindings/java` | thin over C ABI | Out of scope; inherit C ABI additions. |

## Existing binding coverage (pre-binding-surface baseline)
...
**Gaps common to both bindings (the core of this roadmap task):** security report,
...
See [`public_api_rust_binding_surface.md`](public_api_rust_binding_surface.md),
[`python_sdk_binding_surface.md`](python_sdk_binding_surface.md),
[`c_abi_binding_surface.md`](c_abi_binding_surface.md), and
[`report_schema_versioning_binding_surface.md`](report_schema_versioning_binding_surface.md).

## Follow-up / bounded limits (for later roadmap closures)

- WASM and .NET/Java parity with the new report facade — **deferred** to a later
  binding roadmap task; C ABI additions are the substrate the managed bindings wrap.
- Streaming / progress callbacks and cancellation tokens over the C ABI and
  ...
  cancel token and a Python callback is out of this roadmap task's core scope and is
