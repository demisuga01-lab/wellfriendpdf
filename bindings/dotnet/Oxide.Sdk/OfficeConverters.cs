...
public static class OfficeConverters
{
    ...
    public static OxideBinaryResult Prompt22ToPdf(string path, string format) =>
    ...
    public static string Prompt22InspectJson(byte[] bytes, string format)
    {
        ...
        try
        {
            var status = NativeMethods.oxide_prompt22_office_inspect_json(
    ...
    public static OxideBinaryResult Prompt22ToPdf(byte[] bytes, string format)
    {
        ArgumentNullException.ThrowIfNull(bytes);
        ArgumentException.ThrowIfNullOrWhiteSpace(format);
        var formatPtr = NativeMethods.StringToNativeOrNull(format);
        try
        {
            var status = NativeMethods.oxide_prompt22_office_to_pdf(
