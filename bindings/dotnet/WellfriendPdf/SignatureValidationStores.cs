...
public sealed class SignatureTrustStore : IDisposable
{
    ...
    {
        _handle = NativeMethods.wellfriendpdf_signature_trust_store_new(out var error);
    ...
    public void AddTrustAnchorDer(byte[] der)
    {
        ...
        var status = NativeMethods.wellfriendpdf_signature_trust_store_add_anchor_der(
    ...
    public void AddDistrustedCertificateSha256(string fingerprint)
    {
        ...
        try
        {
            var status = NativeMethods.wellfriendpdf_signature_trust_store_add_distrusted_certificate_sha256(
...
public sealed class SignatureIntermediateStore : IDisposable
{
    ...
    {
        _handle = NativeMethods.wellfriendpdf_signature_intermediate_store_new(out var error);
        if (_handle.IsInvalid) NativeMethods.ThrowIfError(2, error);
    }
    ...
    public void AddDer(byte[] der)
    {
        ...
        ThrowIfDisposed();
        if (der.Length == 0) throw new ArgumentException("DER input must not be empty.", nameof(der));
        var status = NativeMethods.wellfriendpdf_signature_intermediate_store_add_der(
            _handle, der, (UIntPtr)der.Length, out var error);
        NativeMethods.ThrowIfError(status, error);
    }
...
public sealed class SignatureEvidenceStore : IDisposable
{
    ...
    {
        _handle = NativeMethods.wellfriendpdf_signature_evidence_store_new(out var error);
        if (_handle.IsInvalid) NativeMethods.ThrowIfError(2, error);
    }
    ...
    public void AddOcspDer(byte[] der) => AddDer(der, NativeMethods.wellfriendpdf_signature_evidence_store_add_ocsp_der);

    public void AddCrlDer(byte[] der) => AddDer(der, NativeMethods.wellfriendpdf_signature_evidence_store_add_crl_der);
    ...
    public void ImportBundleJson(string bundleJson)
    {
        ...
        try
        {
            var status = NativeMethods.wellfriendpdf_signature_evidence_store_set_bundle_json(
                _handle, native, out var error);
            NativeMethods.ThrowIfError(status, error);
        }
        finally
...
public sealed class SignatureRetrievalPolicy : IDisposable
{
    ...
    {
        _handle = NativeMethods.wellfriendpdf_signature_retrieval_policy_new(out var error);
        if (_handle.IsInvalid) NativeMethods.ThrowIfError(2, error);
    }
    ...
    public void SetJson(string policyJson)
    {
        ...
        try
        {
            var status = NativeMethods.wellfriendpdf_signature_retrieval_policy_set_json(
                _handle, native, out var error);
            NativeMethods.ThrowIfError(status, error);
        }
        finally
...
public sealed class SignatureValidationCancellation : IDisposable
{
    ...
    {
        _handle = NativeMethods.wellfriendpdf_signature_validation_cancellation_new(out var error);
        if (_handle.IsInvalid) NativeMethods.ThrowIfError(2, error);
    }
    ...
    {
        ThrowIfDisposed();
        var status = NativeMethods.wellfriendpdf_signature_validation_cancellation_cancel(_handle, out var error);
        NativeMethods.ThrowIfError(status, error);
    }
