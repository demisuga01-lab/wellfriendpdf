//! A **Python-implemented OCR backend** for the Wellfriend seam.
//!
//! This is the integration that matters most for the local-AI and cloud-AI
//! audiences: their model code is Python, and this lets that code *be* Wellfriend's
...
//! ```
//!
//! method to a parse call (`ocr=...`), and Wellfriend drives it exactly like the
...
//! into the interpreter, so Wellfriend OCRs pages one at a time when this backend is
...
//! # Robustness
//!
//! A Python exception in `recognize` is converted to a clean [`WellfriendError`] and
//! contained by the engine's seam ([`wellfriendpdf_engine::ocr::dispatch`]): the page
...
impl PyOcrEngine {
    ...
    /// errors are `PyErr` (mapped to `WellfriendError` by the caller).
    fn recognize_gil(
        &self,
        py: Python<'_>,
        image: &OcrImage,
