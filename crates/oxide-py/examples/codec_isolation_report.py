...
Build/install the wheel, then run:
    ...
    python crates/oxide-py/examples/codec_isolation_report.py

The example uses the public `oxide.codec_isolation_report` helper and prints the
...
def main() -> int:
    ...
    encoded = zlib.compress(b"hello oxide")
    envelope = oxide.codec_isolation_report("FlateDecode", encoded, policy=policy)
