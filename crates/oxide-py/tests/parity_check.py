"""Installed-wheel parity check against the Oxide CLI.
...
def run_cli(oxide_bin: Path, args: list[str]) -> str:
...
def check_file(oxide_bin: Path, pdf: Path) -> tuple[bool, str]:
    doc = oxide.open(pdf)

    py_text = norm_text(doc.extract_text())
    cli_text = norm_text(run_cli(oxide_bin, ["extract-text", str(pdf)]))
    if py_text != cli_text:
        return False, f"text mismatch for {pdf.name}"

    py_tables = doc.extract_tables()
    cli_table_payload = cli_tables(run_cli(oxide_bin, ["extract-tables", str(pdf), "--format", "json"]))
    if py_tables != cli_table_payload:
        return False, f"table mismatch for {pdf.name}"

    py_fields = doc.extract_fields()
    cli_fields = json.loads(run_cli(oxide_bin, ["extract-fields", str(pdf), "--format", "json"]))
...
def main() -> int:
    ...
    parser.add_argument("--oxide-bin", type=Path, default=Path(os.environ.get("OXIDE_BIN", ROOT / "target" / "release" / "oxide.exe")))
    ...
    for index, pdf in enumerate(pdfs, start=1):
        ok, message = check_file(args.oxide_bin, pdf)
        if not ok:
