...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let out_dir = std::env::args()
        .nth(1)
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from("target/prompt04-codec-boundary-scheduler"));
...
fn sdk_report_parity() -> Result<serde_json::Value, Box<dyn std::error::Error>> {
    ...
    Ok(json!({
        ...
        "rust_sdk_prompt04_fields": {
            "native_codec_boundary": feature["report"]["prompt04"]["native_codec_boundary"].is_object(),
            "scanner": feature["report"]["prompt04"]["scanner"]["default_implementation"],
            "renderer_decode_scheduler": feature["report"]["prompt04"]["renderer_decode_scheduler"]["status"],
            "rlbox_wasm": feature["report"]["prompt04"]["rlbox_wasm"]["status"]
        },
        ...
        "schema_versioning": "report envelope remains version 1; Prompt 04 adds inner report fields"
    }))
}
