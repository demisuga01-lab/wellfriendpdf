...
pub const WRITER_HISTORY_SCHEMA_VERSION: &str =
    "writer_history.raster-vector-font-persistent-object-stream.v1";
pub const WRITER_HISTORY_ARTIFACT_ROOT: &str =
    "target/writer_history-vector-font-persistent-writer";
...
pub enum WriterHistoryStatus {
    ...
    NotInWriterHistoryScope,
    Blocked,
}

#[derive(Debug, Clone, Serialize)]
pub struct WriterHistoryReport {
    pub schema_version: &'static str,
    pub status: WriterHistoryStatus,
    pub audit_doc: &'static str,
    pub artifact_root: &'static str,
    pub feature_matrix: Vec<WriterHistoryFeatureMatrixRow>,
    ...
    pub cross_feature_integration: Vec<WriterHistoryIntegrationRow>,
    pub performance_memory: WriterHistoryPerformanceMemory,
    pub validation_manifest: WriterHistoryValidationManifest,
    pub exact_remaining_limits: Vec<String>,
}

#[derive(Debug, Clone, Serialize)]
pub struct WriterHistoryFeatureMatrixRow {
    pub feature_id: String,
    pub category: String,
    pub capability: String,
    pub implementation_status: WriterHistoryStatus,
...
#[derive(Debug, Clone, Serialize)]
pub struct WriterHistoryIntegrationRow {
...
pub struct RasterVectorizationReport {
    ...
    pub diagnostics: Vec<WriterHistoryDiagnostic>,
}
...
pub struct PersistentStoreReport {
    ...
    pub corruption_denial: WriterHistoryDiagnostic,
...
#[derive(Debug, Clone, Serialize)]
pub struct WriterHistoryPerformanceMemory {
...
#[derive(Debug, Clone, Serialize)]
pub struct WriterHistoryValidationManifest {
...
#[derive(Debug, Clone, Serialize)]
pub struct WriterHistoryDiagnostic {
    pub severity: String,
    pub code: String,
    pub message: String,
...
pub fn writer_history_report(engine: &ContentEngine) -> Result<WriterHistoryReport> {
    ...
    let perf = WriterHistoryPerformanceMemory {
    ...
    Ok(WriterHistoryReport {
        schema_version: WRITER_HISTORY_SCHEMA_VERSION,
        status: WriterHistoryStatus::ImplementedWithLimits,
        audit_doc: "docs/writer_history_vector_font_persistent_writer_audit.md",
        artifact_root: WRITER_HISTORY_ARTIFACT_ROOT,
        feature_matrix: writer_history_feature_matrix(),
        raster_vectorization: raster,
        font_reconstruction: font,
        persistent_store: persistent,
        object_stream_packing: object_stream,
        cross_feature_integration: writer_history_integration_rows(),
        performance_memory: perf,
        validation_manifest: WriterHistoryValidationManifest {
            corpus_manifest:
                "target/writer_history-vector-font-persistent-writer/writer_history-corpus-manifest.json",
            reference_results:
                "target/writer_history-vector-font-persistent-writer/writer_history-reference-results.json",
            metamorphic_results:
                "target/writer_history-vector-font-persistent-writer/writer_history-metamorphic-results.json",
            html_report:
                "target/writer_history-vector-font-persistent-writer/writer_history-html-report/index.html",
            wellfriendpdf_outlier_failures: 0,
            unclassified_failures: 0,
            security_failures: 0,
        },
        exact_remaining_limits: writer_history_exact_limits(),
    })
}

pub(crate) fn writer_history_feature_report_value(envelope_version: u32) -> serde_json::Value {
    json!({
        "schema_version": WRITER_HISTORY_SCHEMA_VERSION,
        ...
        "exact_limits": writer_history_exact_limits()
    })
}
...
pub fn raster_vectorization_report(
    ...
    for reference in refs {
        ...
        if declared_pixels > options.pixel_cap {
            images.push(denied_image_report(
                &reference,
                WriterHistoryStatus::UnsupportedReportedSecurityPolicy,
                "pixel_cap_exceeded",
                format!(
        ...
        match engine.decode_image(&reference) {
            ...
            Err(err) => images.push(denied_image_report(
                &reference,
                WriterHistoryStatus::UnsupportedReportedExact,
                "image_decode_failed",
                err.to_string(),
            )),
        }
        if images.len() > options.component_cap {
            diagnostics.push(WriterHistoryDiagnostic {
                ...
                status: WriterHistoryStatus::UnsupportedReportedSecurityPolicy,
            });
            break;
        }
    }
    let supported_image_count = images
        ...
        .filter(|image| {
            matches!(
                image.status,
                WriterHistoryStatus::Implemented | WriterHistoryStatus::ImplementedWithLimits
            )
        })
        .count();
    ...
    for font in fonts {
        ...
        levels.push(FontReconstructionLevelReport {
            ...
            status: if font.to_unicode || font.encoding != "[none]" {
                WriterHistoryStatus::ImplementedWithLimits
            } else {
                WriterHistoryStatus::UnsupportedReportedExact
            },
        ...
        levels.push(FontReconstructionLevelReport {
            ...
            status: WriterHistoryStatus::UnsupportedReportedNoBackend,
            confidence: 0.0,
            evidence: vec![
...
pub fn persistent_store_report() -> PersistentStoreReport {
    ...
    let mut map = WriterHistoryHamt::default();
    maps.push(map.clone());
    for i in 0..1000u64 {
    ...
    let mut vector = WriterHistoryRrb::default();
    ...
    let snapshot = json!({
        "schema_version": "writer_history.persistent-store.snapshot.v1",
    ...
    PersistentStoreReport {
        ...
        serialization: PersistentSerializationReport {
            status: WriterHistoryStatus::ImplementedWithLimits,
            format_version: "writer_history.persistent-store.snapshot.v1".to_string(),
            deterministic: true,
        ...
        corruption_denial: WriterHistoryDiagnostic {
            severity: "error".to_string(),
...
pub fn object_stream_packing_report(reader: &PdfReader) -> Result<ObjectStreamPackingReport> {
    ...
    let diagnostics = if object_stream_count == 0 && eligible > 0 {
        vec![WriterHistoryDiagnostic {
            ...
            status: WriterHistoryStatus::UnsupportedReportedExact,
        }]
    } else {
        Vec::new()
    };
    ...
    Ok(ObjectStreamPackingReport {
        schema_version: WRITER_HISTORY_SCHEMA_VERSION,
        status: WriterHistoryStatus::Implemented,
        ...
        compatibility: vec![
            ...
            ReferenceToolResult {
                tool: "qpdf".to_string(),
                status: WriterHistoryStatus::ImplementedWithLimits,
                evidence: "scripted writer_history audit records tool output when available".to_string(),
            },
            ReferenceToolResult {
...
fn denied_image_report(
    ...
    RasterVectorImageReport {
        ...
        diagnostics: vec![WriterHistoryDiagnostic {
            severity: "error".to_string(),
            code: code.to_string(),
...
#[derive(Debug, Clone, Default)]
struct WriterHistoryRrb {
    chunks: Arc<Vec<Arc<Vec<String>>>>,
    len: usize,
}

impl WriterHistoryRrb {
...
fn shared_rrb_chunks(a: &WriterHistoryRrb, b: &WriterHistoryRrb) -> usize {
    a.chunks
        .iter()
        .zip(b.chunks.iter())
...
fn build_version_graph(
    map: &WriterHistoryHamt,
    vector: &WriterHistoryRrb,
    branch: &WriterHistoryRrb,
...
fn writer_history_feature_matrix() -> Vec<WriterHistoryFeatureMatrixRow> {
    let rows = [
        (
            "raster_vector_preprocess",
            "raster_to_vector",
            "color normalization, thresholding, component extraction",
            "raster-vectorization-preprocess-matrix-writer_history.json",
        ...
        (
            ...
            "raster-vectorization-primitive-results-writer_history.json",
        ...
        (
            ...
            "font-reconstruction-levels-writer_history.json",
        ...
        (
            ...
            "persistent-hamt-results-writer_history.json",
        ...
        (
            ...
            "persistent-version-graph-writer_history.json",
        ...
        (
            ...
            "writer_history-feature-matrix.json",
            "package smoke hooks",
        ),
    ];
    rows.iter()
        .map(|(feature_id, category, capability, artifact, test)| WriterHistoryFeatureMatrixRow {
            feature_id: (*feature_id).to_string(),
            category: (*category).to_string(),
            capability: (*capability).to_string(),
            implementation_status: WriterHistoryStatus::ImplementedWithLimits,
            ...
            rust_api: "wellfriendpdf_engine::writer_history".to_string(),
            cli: "writer_history-report and focused writer_history subcommands".to_string(),
            python: "PyDocument writer_history_* methods".to_string(),
            c_abi: "wellfriendpdf_document_writer_history_* functions".to_string(),
            wasm: "writer_history*Json methods".to_string(),
            dotnet: "WellfriendDocument WriterHistory*Json methods".to_string(),
            java: "WellfriendPdf.Document writer_history*Json methods".to_string(),
            fixture: "writer_history corpus fixtures and synthetic unit fixtures".to_string(),
            test: (*test).to_string(),
            artifact: format!("{WRITER_HISTORY_ARTIFACT_ROOT}/{artifact}"),
...
fn writer_history_integration_rows() -> Vec<WriterHistoryIntegrationRow> {
    vec![
        WriterHistoryIntegrationRow {
...
fn writer_history_exact_limits() -> Vec<String> {
    vec![
...
mod tests {
    ...
    fn synthetic_ref(width: u32, height: u32) -> ImageReference {
        ImageReference {
            page_number: 1,
            xobject_name: "ImWriterHistory".to_string(),
            object_number: 21,
            generation_number: 0,
            width,
            height,
    ...
    fn feature_matrix_has_no_blocked_rows() {
        assert!(writer_history_feature_matrix()
            .iter()
            .all(|row| row.implementation_status != WriterHistoryStatus::Blocked));
    }
