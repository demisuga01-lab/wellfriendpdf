...
pub const SECURE_MUTATION_SCHEMA_VERSION: &str =
    "secure_mutation.mask-inline-associated-signature-policy.v1";
pub const SECURE_MUTATION_CLOSEOUT_SCHEMA_VERSION: &str =
...
/// incremental primitive used by secure mutation tests: the original bytes remain an
...
pub fn plan_signature_preserving_form_fill(
    ...
    Ok(SignaturePreservingEditPlan {
        ...
        policy,
        exact_limits: vec![
            "Pades LTV form-fill preservation is append-only and revalidates original signatures after reopen".to_string(),
...
pub fn secure_mutation_report(engine: &ContentEngine) -> Result<serde_json::Value> {
    Ok(serde_json::json!({
        "schema_version": SECURE_MUTATION_SCHEMA_VERSION,
        ...
        "feature": secure_mutation_feature_report_value(crate::sdk::REPORT_ENVELOPE_VERSION),
    }))
}

pub fn secure_mutation_closeout_report(engine: &ContentEngine) -> Result<serde_json::Value> {
    Ok(serde_json::json!({
        "schema_version": SECURE_MUTATION_CLOSEOUT_SCHEMA_VERSION,
        "kind": "secure_mutation_closeout_report",
        "closure": secure_mutation_closeout_feature_report_value(crate::sdk::REPORT_ENVELOPE_VERSION),
        "mask_redaction": mask_redaction_inventory(engine)?,
        "associated_files": associated_files_inventory(engine)?,
        "signature_policy": analyze_edit_policy(engine, EditOperation::IncrementalSave)?,
    }))
...
pub(crate) fn secure_mutation_closeout_feature_report_value(
    ...
    serde_json::json!({
        ...
        "public_report_schema": "additive_feature_report_secure_mutation_closeout"
    })
}

pub(crate) fn secure_mutation_feature_report_value(envelope_version: u32) -> serde_json::Value {
    serde_json::json!({
        "schema_version": SECURE_MUTATION_SCHEMA_VERSION,
        "envelope_version": envelope_version,
        "status": "complete_extended_by_secure_mutation_closeout",
        "coverage": {
            "mask_softmask_redaction": "implemented_with_secure_mutation_closeout_packed_icc_extension",
            "inline_image_partial_redaction": "implemented_with_limits",
            "associated_files": "implemented_with_secure_mutation_closeout_owner_mutation",
            "signature_safe_edit_policy": "implemented_with_secure_mutation_closeout_active_enforcement",
            "docmdp_fieldmdp_structural_policy": "implemented_with_secure_mutation_closeout_active_enforcement",
        ...
        "public_report_schema": "additive_feature_report_secure_mutation"
    })
}
...
mod tests {
    ...
    fn feature_report_has_no_blocked_rows() {
        let report = secure_mutation_feature_report_value(1);
