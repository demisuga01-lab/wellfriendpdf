...
pub const ANNOTATION_MEDIA_REDACTION_SCHEMA_VERSION: &str =
...
pub enum AnnotationMediaRedactionSupportStatus {
    Implemented,
    ImplementedWithLimits,
    UnsupportedReportedSecurityPolicy,
    UnsupportedReportedNoSafeDecoder,
    UnsupportedReportedExact,
    NotInAnnotationMediaRedactionScope,
    Blocked,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct AnnotationMediaRedactionDiagnostic {
    pub code: String,
    pub severity: String,
    pub message: String,
...
impl AnnotationMediaRedactionDiagnostic {
    ...
    for page in &pages {
            ...
            .enumerate()
        {
            ...
            let Some(dict) = object.as_dict().cloned() else {
                diagnostics.push(AnnotationMediaRedactionDiagnostic::warning(
...
fn parse_annotation_node(
    node: &XmlNode,
    diagnostics: &mut Vec<AnnotationMediaRedactionDiagnostic>,
...
pub(crate) fn annotation_media_redaction_feature_report_value(
    envelope_version: u32,
) -> serde_json::Value {
    serde_json::json!({
        "schema_version": ANNOTATION_MEDIA_REDACTION_SCHEMA_VERSION,
        ...
        "public_report_schema": "additive_feature_report_annotation_media_redaction"
    })
}
