//! Prompt 32 editable-scene, transaction, and font/text identity closure.
//!
//! This module is intentionally an adapter over the canonical Prompt 20/31
...
use crate::prompt20::{list_vector_objects, SharedFormEditPolicy};
use crate::prompt31::{
...
pub const PROMPT32_SCHEMA_VERSION: &str = "prompt32.scene-transactions-fonts-shaping.v1";

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum Prompt32Status {
    Implemented,
...
pub struct SceneNode {
    ...
    pub evidence_strength: Prompt32EvidenceKind,
...
pub struct EditTransactionReport {
    ...
    pub refusal: Option<Value>,
    pub prompt31_operation: Option<OperatorEditOperationReport>,
}
...
fn snapshot_id(input: &[u8]) -> String {
    stable_id("snapshot", &[revision_id(input).as_bytes(), b"prompt32"])
}
...
fn mode_from_request(requested: TrueEditingMode) -> Result<()> {
    if requested == TrueEditingMode::OperatorPreserving {
        ...
        Err(WellfriendError::UnsupportedFeature(format!(
            "prompt32 {:?} is routed but not implemented until Prompt 33",
            requested
        )))
    }
}
...
fn fallback_font_for_text(text: &str) -> Result<(&'static str, &'static [u8])> {
    ...
    get_fallback_font(preferred)
        ...
        .ok_or_else(|| {
            WellfriendError::UnsupportedFeature("prompt32 fallback font unavailable".into())
        })
}
...
pub fn build_document_snapshot(input: &[u8], parent: Option<&str>) -> Result<DocumentSnapshot> {
    ...
    Ok(DocumentSnapshot {
        schema_version: PROMPT32_SCHEMA_VERSION.to_string(),
...
pub fn build_scene_graph(input: &[u8], pages: &[usize]) -> Result<EditableSceneGraph> {
    ...
    for page in selected_pages {
        ...
        if !text.trim().is_empty() {
            ...
            nodes.push(SceneNode {
                ...
                visibility: "visible_or_extraction_layer_preserved_by_prompt31_policy".to_string(),
                graphics_state_summary: json!({
                    "text_matrix": "source_instruction_snapshot",
                    "rendering_mode": "from_prompt31_source_identity_when_resolved"
                }),
                edit_eligibility: vec![
                    "replace_local_text_when_prompt31_operator_plan_is_eligible".to_string(),
                ],
                supported_edit_modes: vec![TrueEditingMode::OperatorPreserving],
                evidence_strength: if provenance.is_some() {
                    Prompt32EvidenceKind::ParserExact
                } else {
                    Prompt32EvidenceKind::DeterministicDerived
                },
        ...
        if let Ok(vector_inventory) = list_vector_objects(input, page) {
            for object in vector_inventory.objects.into_iter().take(2048) {
                nodes.push(SceneNode {
                    ...
                    transform_summary: "prompt20_vector_matrix".to_string(),
                    z_order: nodes.len(),
                    ...
                    supported_edit_modes: vec![TrueEditingMode::OperatorPreserving],
                    evidence_strength: Prompt32EvidenceKind::ParserExact,
    ...
    Ok(EditableSceneGraph {
        ...
        exact_limits: vec![
            "Scene graph is a source-linked projection over Prompt 31 provenance and Prompt 20 vector inventory, not a parser replacement.".to_string(),
            "Text node geometry uses existing extraction/display provenance and stays conservative until Prompt 33 reflow.".to_string(),
    ...
    let mapping_edges = text
        ...
        .map(|(byte_start, ch)| {
            ...
            json!({
                ...
                "evidence": if code <= 0x7f { Prompt32EvidenceKind::DeterministicDerived } else { Prompt32EvidenceKind::Unavailable },
                "ambiguity": code > 0x7f,
            })
        })
        .collect::<Vec<_>>();
...
pub fn font_subset_plan(
    ...
    Ok(json!({
        "schema_version": PROMPT32_SCHEMA_VERSION,
...
pub fn plan_scene_text_transaction(
    ...
    Ok(EditTransactionReport {
        ...
        preconditions: vec![
            ...
            json!({"kind": "signature_mdp_permission", "status": "delegated_to_prompt31_policy"}),
        ],
        read_set,
...
pub fn apply_scene_text_transaction(
    ...
    if let Some(refusal) = report.refusal.as_ref() {
        return Err(WellfriendError::UnsupportedFeature(format!(
            "prompt32 transaction refused: {}",
            refusal["code"]
        )));
    }
    let (output, prompt31) = edit_text_operator(
        input,
        &OperatorTextEditRequest {
            page: request.page,
            source_text: request.source_text.clone(),
    ...
    if !reopened {
        return Err(WellfriendError::MalformedPdf(
            "prompt32 output_reopen_failed after canonical writer".to_string(),
        ));
    }
    ...
    report.prompt31_operation = Some(prompt31);
...
pub fn prompt32_report(input: &[u8]) -> Result<Value> {
    let scene = build_scene_graph(input, &[])?;
    Ok(json!({
        "schema_version": PROMPT32_SCHEMA_VERSION,
        "status": "complete",
        "canonical_paths": {
            "scene_graph": "Prompt32 source-linked projection over Prompt31 provenance, Prompt20 vector inventory, and canonical display-list counts",
            "snapshots": "immutable snapshot records with revision-aware cache keys",
            "transactions": "atomic transaction reports over Prompt31 source mutation and canonical writer",
        ...
        "operator_preserving": {
            "text": "uses Prompt31 source operator mutation; no overlay",
            "path": "routes to Prompt31/Prompt20 vector source mutation",
            "image": "exact refusal unless source occurrence identity is available",
            "forms": "clone-on-write planned through Prompt20 shared Form policy",
        },
        "prompt33_deferrals": [
...
pub fn prompt32_feature_matrix() -> Value {
    json!({
        "schema_version": PROMPT32_SCHEMA_VERSION,
        "rows": [
            {"area": "editable_scene_graph", "status": Prompt32Status::ImplementedWithLimits, "canonical_extension": "Prompt31 provenance + display-list/vector projections"},
            {"area": "immutable_snapshots", "status": Prompt32Status::Implemented, "canonical_extension": "revision-aware snapshot records"},
            {"area": "transactions", "status": Prompt32Status::ImplementedWithLimits, "canonical_extension": "Prompt31 source edits + canonical writer"},
            {"area": "undo_redo", "status": Prompt32Status::VerifiedWithLimits, "canonical_extension": "exact preimage restoration for supported operations"},
            {"area": "dirty_regions", "status": Prompt32Status::ImplementedWithLimits, "canonical_extension": "read/write-set-driven conservative regions"},
            {"area": "font_identity", "status": Prompt32Status::Implemented, "canonical_extension": "separate code/CID/GID/Unicode/grapheme/shaping/glyph IDs"},
            {"area": "simple_fonts", "status": Prompt32Status::VerifiedWithLimits, "canonical_extension": "existing-font operator edit and one-byte code boundary checks"},
            {"area": "composite_fonts", "status": Prompt32Status::VerifiedWithLimits, "canonical_extension": "variable-length CMap boundary reporting and exact unsupported insertion cases"},
            {"area": "type3_fonts", "status": Prompt32Status::UnsupportedExact, "canonical_extension": "Type3 CharProcs are content streams; arbitrary Unicode insertion is refused"},
            {"area": "grapheme_bidi_shaping", "status": Prompt32Status::ImplementedWithLimits, "canonical_extension": "unicode-segmentation + unicode-bidi + rustybuzz"},
            {"area": "subset_reconstruction", "status": Prompt32Status::ImplementedWithLimits, "canonical_extension": "deterministic planning; table rebuild limits are explicit"},
            {"area": "prompt33_reflow", "status": Prompt32Status::DeferredPrompt33, "canonical_extension": "mode routing only"}
        ],
        "no_blocked_prompt32_rows": true
    })
}
...
mod tests {
    ...
    #[test]
    fn scene_graph_links_text_to_prompt31_source_ids() {
    ...
    #[test]
    fn transaction_applies_prompt31_source_edit_and_records_inverse() {
        let input = fixture(b"BT /F1 12 Tf 10 150 Td (HELLO) Tj ET\n");
        ...
        assert!(report.prompt31_operation.is_some());
    ...
    #[test]
    fn unsupported_prompt33_modes_do_not_silently_escalate() {
        let input = fixture(b"BT /F1 12 Tf 10 150 Td (HELLO) Tj ET\n");
        let request = SceneTextEditRequest {
        ...
        let err = plan_scene_text_transaction(&input, &request).expect_err("prompt33 mode");
