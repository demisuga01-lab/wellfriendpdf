//! Prompt 35 tagged-PDF repair, accessibility, redaction, and sanitization.
//!
//! The module is intentionally an adapter over existing Wellfriend subsystems:
//! tagged extraction and ParentTree recovery, Prompt 33/34 semantic mutation
//! paths, the canonical full-rewrite editor/redactor, the sanitizer, and the
//! writer. It keeps Prompt 35 operations source-linked and refuses boundaries
...
pub const PROMPT35_SCHEMA_VERSION: &str = "prompt35.accessibility-redaction-sanitization.v1";

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
pub enum Prompt35Subsystem {
    TaggedPdf,
    AccessibilityRepair,
    Redaction,
    Sanitization,
...
pub struct Prompt35Request {
    pub subsystem: Prompt35Subsystem,
    #[serde(default)]
    pub action: Option<Prompt35Action>,
    #[serde(default)]
    pub approved: bool,
    #[serde(default)]
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Prompt35AnalysisReport {
    ...
    pub prompt34_integration: Value,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Prompt35OperationReport {
    pub schema_version: String,
    pub operation: String,
    pub subsystem: Prompt35Subsystem,
    pub status: Prompt35Status,
    pub typed_result: Prompt35TypedResult,
    pub source_sha256: String,
...
pub struct ResidualFinding {
    pub check: String,
    pub classification: Prompt35TypedResult,
    pub object: Option<String>,
    pub page: Option<usize>,
    pub term_hash: Option<String>,
}

pub fn prompt35_feature_matrix() -> Value {
    json!({
        "schema_version": PROMPT35_SCHEMA_VERSION,
        "verdict": "implementation_complete_validation_deferred",
        "implementation_scope": {
            "tagged_pdf_model": "canonical semantic/StructTreeRoot extraction plus Prompt35 source mutation",
            ...
            "accessibility_repair": "PDF/UA-oriented language/MarkInfo/StructTree repair and Prompt31-34 hook reporting",
            ...
            "undo": "pre-serialization immutable preimage restoration for Prompt35 operations",
            "bindings": "SDK facade is binding-neutral; Prompt35 wrappers mirror Prompt34 surfaces"
        },
        "deferred_to_prompt36": [
            "large PDF/UA corpus",
...
pub fn analyze_prompt35(input: &[u8]) -> Result<Prompt35AnalysisReport> {
    ...
    let risky_content =
        serde_json::to_value(crate::security::scan_risky_content(engine.document())?)
            .map_err(json_err)?;
    let prompt34_integration =
        serde_json::to_value(crate::prompt34::analyze_prompt34(input)?).map_err(json_err)?;
    Ok(Prompt35AnalysisReport {
        schema_version: PROMPT35_SCHEMA_VERSION.to_string(),
        ...
        exact_limits: prompt35_exact_limits(),
    })
}

pub fn plan_prompt35(input: &[u8], request: &Prompt35Request) -> Result<Prompt35OperationReport> {
    let mut report = operation_report(input, request, "plan_prompt35")?;
    report.status = Prompt35Status::Planned;
    report.details = json!({
        ...
        "prompt32_transaction": "preimage/read_set/write_set/inverse recorded by Prompt35 operation report"
    });
    Ok(report)
}

pub fn apply_prompt35(
    input: &[u8],
    request: &Prompt35Request,
) -> Result<(Vec<u8>, Prompt35OperationReport)> {
    let action = request
        .action
        .as_ref()
        .ok_or_else(|| WellfriendError::invalid_input("prompt35 action is required for apply"))?;
    if requires_approval(Some(action)) && !request.approved {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt35 review_required: action requires explicit approval".to_string(),
        ));
    }
    if requires_full_rewrite(Some(action)) && !request.full_rewrite_acknowledged {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt35 destructive_full_rewrite_required: caller must acknowledge full rewrite and signature/history impact".to_string(),
        ));
    }
    let source_sha256 = resource_digest(input);
    let mut report = operation_report(input, request, action_name(action))?;
    let output = match action {
        Prompt35Action::InspectStructure
        | Prompt35Action::PlanRedactionText { .. }
        | Prompt35Action::InspectSanitizationFeatures
        | Prompt35Action::UndoBeforeSerialization => input.to_vec(),
        Prompt35Action::VerifyRedaction { terms } => {
            report.verification = Some(verify_residual_data(input, terms)?);
            input.to_vec()
        }
        Prompt35Action::VerifySanitization { preset } => {
            ...
            if risky.risky_total() > 0 {
                report.typed_result = Prompt35TypedResult::ActiveContentStillPresent;
                report.status = Prompt35Status::VerifiedPresent;
            }
            input.to_vec()
        }
        Prompt35Action::RepairTaggedStructure {
            lang,
            rebuild_parent_tree,
        } => {
        ...
        Prompt35Action::RebuildParentTree { lang } => {
            let language = language_or_default(lang.as_ref().or(request.language.as_ref()))?;
            report.changed_structure_nodes = 1;
            report
                .write_set
        ...
        Prompt35Action::RepairAfterMutation { mutation, lang } => {
            let language = language_or_default(lang.as_ref().or(request.language.as_ref()))?;
            ...
            report.details = json!({
                ...
                "repair_hooks": [
                    "prompt31_text_edit",
                    "prompt33_reflow_page_flow",
                    "prompt34_tables_math_ocr_forms_annotations"
                ],
                "structure_child_order": "derived_from_tagged_or_prompt33_reading_order",
                "parent_tree_rebuilt": true
            });
            output
        }
        Prompt35Action::RedactText {
            terms,
            pages,
            strict,
        } => {
        ...
        Prompt35Action::RedactSemanticNode {
            selector,
            verification_terms,
        } => {
        ...
        Prompt35Action::RedactAnnotation { page, rect } => {
        ...
        Prompt35Action::RedactFormField { field_name } => {
            let output = crate::prompt34::apply_prompt34(
                input,
                &crate::prompt34::Prompt34Request {
                    subsystem: crate::prompt34::Prompt34Subsystem::FormData,
                    action: Some(crate::prompt34::Prompt34Action::FormDelete {
        ...
        Prompt35Action::RedactMetadata { verification_terms } => {
        ...
        Prompt35Action::RedactAttachment => {
        ...
        Prompt35Action::SanitizeDocument { preset } => {
            let engine = ContentEngine::open_bytes(input.to_vec())?;
        ...
        Prompt35Action::FullRewriteHistoryRemoval => {
            let engine = ContentEngine::open_bytes(input.to_vec())?;
    ...
    ContentEngine::open_bytes(output.clone()).map_err(|err| {
        WellfriendError::MalformedPdf(format!("prompt35 output_reopen_failed: {err}"))
    })?;
    if !matches!(
        report.status,
        Prompt35Status::VerifiedPresent | Prompt35Status::Refused
    ) {
        report.status = if let Some(verification) = &report.verification {
            if verification.verified_absent {
                Prompt35Status::VerifiedAbsent
            } else {
                Prompt35Status::VerifiedPresent
            }
        } else {
            Prompt35Status::Applied
        };
    }
...
pub fn undo_prompt35(
    ...
    if output.is_empty() {
        return Err(WellfriendError::invalid_input(
            "prompt35 undo requires the operation output bytes",
        ));
    }
    let mut report = operation_report(input, request, "undo_prompt35")?;
    report.status = Prompt35Status::Applied;
...
pub fn verify_residual_data(bytes: &[u8], terms: &[String]) -> Result<ResidualVerificationReport> {
    ...
    for hit in &verification.extractable_hits {
        ...
        findings.push(ResidualFinding {
            ...
            classification: Prompt35TypedResult::ResidualDataDetected,
            object: None,
            page: Some(hit.page),
    ...
    if risky.risky_total() > 0 {
        findings.push(ResidualFinding {
            ...
            classification: Prompt35TypedResult::ActiveContentStillPresent,
            object: None,
            page: None,
            term_hash: None,
        });
    ...
    Ok(ResidualVerificationReport {
        ...
        typed_result: if verified_absent {
            Prompt35TypedResult::Ok
        } else {
            Prompt35TypedResult::ResidualDataDetected
        },
...
fn operation_report(
    ...
    request: &Prompt35Request,
    operation: &str,
) -> Result<Prompt35OperationReport> {
    let engine = ContentEngine::open_bytes(input.to_vec())?;
    Ok(Prompt35OperationReport {
        schema_version: PROMPT35_SCHEMA_VERSION.to_string(),
        operation: operation.to_string(),
        subsystem: request.subsystem,
        status: Prompt35Status::Planned,
        typed_result: Prompt35TypedResult::Ok,
        source_sha256: resource_digest(input),
        output_sha256: None,
        changed_objects: 0,
        ...
        details: json!({
            ...
            "prompt32_transaction": "source preimage, read_set, write_set, inverse restore, output hash",
...
fn validate_language(lang: &str) -> Result<String> {
            ...
            .all(|ch| ch.is_ascii_alphanumeric() || ch == '-')
    {
        return Err(WellfriendError::MalformedPdf(format!(
            "prompt35 language_invalid: '{value}' is not a supported BCP 47 tag"
        )));
    }
    Ok(value.to_string())
}
...
fn action_name(action: &Prompt35Action) -> &'static str {
    match action {
        Prompt35Action::InspectStructure => "inspect_structure",
        Prompt35Action::RepairTaggedStructure { .. } => "repair_tagged_structure",
        Prompt35Action::SetDocumentLanguage { .. } => "set_document_language",
        Prompt35Action::SetStructureMetadata { .. } => "set_structure_metadata",
        Prompt35Action::RebuildParentTree { .. } => "rebuild_parent_tree",
        Prompt35Action::RepairAfterMutation { .. } => "repair_after_mutation",
        Prompt35Action::PlanRedactionText { .. } => "plan_redaction_text",
        Prompt35Action::RedactText { .. } => "redact_text",
        Prompt35Action::RedactRegion { .. } => "redact_region",
        Prompt35Action::RedactSemanticNode { .. } => "redact_semantic_node",
        Prompt35Action::RedactAnnotation { .. } => "redact_annotation",
        Prompt35Action::RedactFormField { .. } => "redact_form_field",
        Prompt35Action::RedactMetadata { .. } => "redact_metadata",
        Prompt35Action::RedactAttachment => "redact_attachment",
        Prompt35Action::SanitizeDocument { .. } => "sanitize_document",
        Prompt35Action::InspectSanitizationFeatures => "inspect_sanitization_features",
        Prompt35Action::VerifyRedaction { .. } => "verify_redaction",
        Prompt35Action::VerifySanitization { .. } => "verify_sanitization",
        Prompt35Action::FullRewriteHistoryRemoval => "full_rewrite_history_removal",
        Prompt35Action::UndoBeforeSerialization => "undo_before_serialization",
    }
}

fn action_mutates(action: Option<&Prompt35Action>) -> bool {
    !matches!(
        action,
        None | Some(Prompt35Action::InspectStructure)
            | Some(Prompt35Action::PlanRedactionText { .. })
            | Some(Prompt35Action::InspectSanitizationFeatures)
            | Some(Prompt35Action::VerifyRedaction { .. })
            | Some(Prompt35Action::VerifySanitization { .. })
            | Some(Prompt35Action::UndoBeforeSerialization)
    )
}

fn requires_full_rewrite(action: Option<&Prompt35Action>) -> bool {
    matches!(
        action,
        Some(
            Prompt35Action::RedactText { .. }
                | Prompt35Action::RedactRegion { .. }
                | Prompt35Action::RedactSemanticNode { .. }
                | Prompt35Action::RedactAnnotation { .. }
                | Prompt35Action::RedactFormField { .. }
                | Prompt35Action::RedactMetadata { .. }
                | Prompt35Action::RedactAttachment
                | Prompt35Action::SanitizeDocument { .. }
                | Prompt35Action::FullRewriteHistoryRemoval
        )
    )
}

fn requires_approval(action: Option<&Prompt35Action>) -> bool {
    matches!(
        action,
        Some(
            Prompt35Action::SetStructureMetadata { .. }
...
fn rect_from_bounds(bounds: [f64; 4]) -> Result<ImageRect> {
    if bounds.iter().any(|v| !v.is_finite()) {
        return Err(WellfriendError::MalformedPdf(
            "prompt35 invalid_geometry: rectangle contains non-finite coordinates".to_string(),
        ));
    }
    ...
    if x1 <= x0 || y1 <= y0 {
        return Err(WellfriendError::MalformedPdf(
            "prompt35 invalid_geometry: rectangle has no positive area".to_string(),
        ));
    }
    Ok(ImageRect {
        x: x0,
...
fn redact_text_pdf(
    ...
    if terms.is_empty() {
        return Err(WellfriendError::MalformedPdf(
            "prompt35 redaction_target_not_resolved: at least one term is required".to_string(),
        ));
    }
    let engine = ContentEngine::open_bytes(input.to_vec())?;
    ...
    if matches_found == 0 {
        return Err(WellfriendError::MalformedPdf(
            "prompt35 redaction_target_not_resolved: no matching source text with geometry"
                .to_string(),
        ));
    }
    ...
    if strict && !verification.verified_absent {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt35 redaction_verification_failed: residual text remains after strict redaction"
                .to_string(),
        ));
    }
    Ok(output)
...
fn redact_semantic_node_pdf(
    ...
    let Some((page, bbox)) = find_semantic_bbox(&semantic.elements, selector) else {
        return Err(WellfriendError::MalformedPdf(
            "prompt35 structure_element_not_found: selected semantic node has no source geometry"
                .to_string(),
        ));
    };
...
fn ensure_catalog_tagging(
    ...
    let root_index = objects
        ...
        .ok_or_else(|| WellfriendError::MalformedPdf("prompt35 catalog missing".to_string()))?;
    let struct_root_ref;
    {
        let catalog = objects[root_index].object.as_dict_mut().ok_or_else(|| {
            WellfriendError::MalformedPdf("prompt35 catalog is not a dictionary".to_string())
        })?;
...
fn set_structure_metadata_pdf(
    ...
    if !matched {
        return Err(WellfriendError::MalformedPdf(
            "prompt35 structure_element_not_found: selector matched no StructElem".to_string(),
        ));
    }
    Ok(output)
}
...
fn prompt35_exact_limits() -> Vec<String> {
    vec![
        "PDF/UA and WTPDF conformance are not certified in Prompt 35; exhaustive validation is Prompt 36 scope".to_string(),
        ...
        "OCR residual inspection uses available searchable text and provider interfaces; unavailable OCR providers return exact provider_unavailable in Prompt 36 validation".to_string(),
    ]
}
...
mod tests {
    ...
    #[test]
    fn prompt35_repairs_minimal_structure_and_language() {
        let input = basic_pdf("Accessible");
        let request = Prompt35Request {
            subsystem: Prompt35Subsystem::AccessibilityRepair,
            action: Some(Prompt35Action::RepairTaggedStructure {
        ...
        let (output, report) = apply_prompt35(&input, &request).expect("repair");
        assert!(output.starts_with(b"%PDF-"));
        assert_eq!(report.status, Prompt35Status::Applied);
    ...
    #[test]
    fn prompt35_redacts_text_and_blocks_residual() {
        let input = basic_pdf("SECRET public");
        let request = Prompt35Request {
            subsystem: Prompt35Subsystem::Redaction,
            action: Some(Prompt35Action::RedactText {
        ...
        let (output, report) = apply_prompt35(&input, &request).expect("redact");
        assert!(output.starts_with(b"%PDF-"));
        assert!(
            report
                .verification
    ...
    #[test]
    fn prompt35_sanitizes_active_content_with_policy_report() {
        let input = basic_pdf("Safe");
        let request = Prompt35Request {
            subsystem: Prompt35Subsystem::Sanitization,
            action: Some(Prompt35Action::SanitizeDocument {
        ...
        let (output, report) = apply_prompt35(&input, &request).expect("sanitize");
        assert!(output.starts_with(b"%PDF-"));
        assert_eq!(report.status, Prompt35Status::Applied);
    }

    #[test]
    fn prompt35_undo_returns_preimage() {
        let input = basic_pdf("Undo");
        let request = Prompt35Request {
            subsystem: Prompt35Subsystem::TaggedPdf,
            action: Some(Prompt35Action::UndoBeforeSerialization),
            approved: false,
            language: None,
            full_rewrite_acknowledged: false,
        };
        let edited = basic_pdf("Edited");
        let (restored, report) = undo_prompt35(&input, &edited, &request).expect("undo");
