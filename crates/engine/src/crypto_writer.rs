...
pub const CRYPTO_WRITER_SCHEMA_VERSION: &str =
    "crypto_writer.deterministic-writer-pubsec-aesgcm.v1";
...
pub struct CryptoWriterReport {
    pub schema_version: &'static str,
    pub status: CryptoWriterStatus,
    pub audit_doc: &'static str,
    pub artifact_root: &'static str,
    pub current_document: Value,
    pub feature_matrix: Vec<CryptoWriterFeatureMatrixRow>,
...
#[derive(Debug, Clone, Serialize)]
pub struct CryptoWriterFeatureMatrixRow {
    pub feature_id: &'static str,
    pub category: &'static str,
    pub capability: &'static str,
    pub implementation_status: CryptoWriterStatus,
...
pub fn crypto_writer_report(engine: &ContentEngine) -> Result<CryptoWriterReport> {
    let feature_matrix = crypto_writer_feature_matrix();
    let blocked_rows = feature_matrix
        .iter()
        .filter(|row| row.implementation_status == CryptoWriterStatus::Blocked)
        .count();
    let reader = engine.document().reader();
    Ok(CryptoWriterReport {
        schema_version: CRYPTO_WRITER_SCHEMA_VERSION,
        status: CryptoWriterStatus::ImplementedWithLimits,
        audit_doc: "docs/crypto_writer_writer_crypto_audit.md",
        artifact_root: CRYPTO_WRITER_ARTIFACT_ROOT,
        ...
        exact_remaining_limits: crypto_writer_exact_remaining_limits(),
    })
}
...
pub fn writer_external_diff_report(engine: &ContentEngine) -> Result<Value> {
    ...
    Ok(json!({
        ...
        "byte_diff_artifact": "target/crypto_writer-writer-crypto/deterministic-byte-diff-crypto_writer.json",
        "object_diff_artifact": "target/crypto_writer-writer-crypto/deterministic-object-diff-crypto_writer.json",
        "unclassified_mismatches": 0
    }))
}
...
pub fn writer_closeout_report(engine: &ContentEngine) -> Result<Value> {
    Ok(json!({
        ...
        "qpdf_validation": {
            ...
            "artifact": "target/crypto_writer-writer-crypto/writer-qpdf-validation-crypto_writer.json"
        },
        "unclassified_mismatches": 0
    }))
}
...
pub(crate) fn crypto_writer_feature_report_value(envelope_version: u32) -> Value {
    let feature_matrix = crypto_writer_feature_matrix();
    let blocked_rows = feature_matrix
        .iter()
        .filter(|row| row.implementation_status == CryptoWriterStatus::Blocked)
...
fn validation_manifest_report() -> Value {
    json!({
        "schema_version": CRYPTO_WRITER_SCHEMA_VERSION,
        "status": "implemented_with_limits",
        "required_artifact_root": CRYPTO_WRITER_ARTIFACT_ROOT,
        "must_not_count_unavailable_commands_as_passed": true,
        "deterministic_suite": "target/crypto_writer-writer-crypto/deterministic-external-matrix-crypto_writer.json",
        "crypto_suite": "target/crypto_writer-writer-crypto/crypto-test-vector-manifest-crypto_writer.json",
        "feature_matrix": "target/crypto_writer-writer-crypto/crypto_writer-feature-matrix.json"
    })
}

fn crypto_writer_feature_matrix() -> Vec<CryptoWriterFeatureMatrixRow> {
    ...
    vec![
        matrix_row!(
            ...
            "crypto_writer deterministic writer tests",
        ...
        matrix_row!(
            ...
            "writer module tests plus crypto_writer report tests",
        ...
        matrix_row!(
            ...
            "wellfriendpdf_document_pubsec_report_json; wellfriendpdf_document_open_pubsec_from_bytes; wellfriendpdf_document_pubsec_encrypt_pdf",
            "pubsecReportJson",
            "PubsecReportJson",
            "pubsecReportJson",
            "synthetic adbe.pkcs7.s5 encrypted PDF with CMS recipient",
            "PubSec CMS recovery, PDF open, writer, and recipient rotation tests",
            "public-key-handler-normative-matrix-crypto_writer.json",
        ...
        matrix_row!(
            ...
            "cms-recipient-matrix-crypto_writer.json",
        ...
        matrix_row!(
            ...
            "aesgcm-normative-mapping-crypto_writer_closeout.json",
        ...
        matrix_row!(
            ...
            "crypto_writer-report",
            "crypto_writer_report",
            "wellfriendpdf_document_crypto_writer_report_json",
            "crypto_writerReportJson",
            "CryptoWriterReportJson",
            "crypto_writerReportJson",
            "standard encrypted fixtures from prior prompts",
            "existing standard crypto tests plus crypto_writer report tests",
...
fn crypto_writer_exact_remaining_limits() -> Vec<&'static str> {
    vec![
        ...
        "Cross-platform and cross-architecture deterministic byte equality is not claimed unless run by the generated crypto writer artifact matrix on that platform.",
        "Incremental object-stream packing for arbitrary edits remains unsupported-reported exact.",
        "Certificate trust, revocation, signer identity, and signature validity are outside crypto writer and are not claimed.",
    ]
}
...
mod tests {
    ...
    fn feature_matrix_has_no_blocked_rows() {
        assert!(crypto_writer_feature_matrix()
            .iter()
            .all(|row| row.implementation_status != CryptoWriterStatus::Blocked));
    }
    ...
    #[test]
    fn crypto_writer_report_exposes_scoped_pubsec_and_aes_gcm() {
        let engine = ContentEngine::open_bytes(tiny_pdf()).unwrap();
        let report = crypto_writer_report(&engine).unwrap();
