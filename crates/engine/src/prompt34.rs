...
/// Concrete Prompt 34 mutation requests.  These deliberately carry only
...
pub enum Prompt34Action {
    /// Replace the text in one detected source-linked table cell.  The table
    /// geometry is resolved again from the current snapshot before Prompt 33
    ...
    /// resolved table cell.  This shares table-cell provenance and Prompt 33
    ...
    /// The annotation's canonical Prompt 17 appearance is regenerated; the
    ...
    /// Reposition source-linked cell text with a bounded Prompt 33 alignment
    ...
    /// by the Prompt 32/33 path rather than painted as a replacement overlay.
    MathReplace {
        replacement_text: String,
    },
    /// Move or resize one resolved born-digital mathematical source region.
    /// The expression text is retained and rewritten through Prompt 33 rather
    ...
    /// replacing the original scan. Prompt 33 rewrites the text instruction at
    ...
    /// the viewer-independent appearance state through Prompt 17.
    AnnotationCreate {
        page: usize,
        subtype: String,
        rect: [f64; 4],
    ...
    /// Move or resize an existing stable XFDF annotation.  Canonical Prompt 17
    ...
    /// the canonical Prompt 17 importer.
    AnnotationXfdf {
        xfdf: String,
        #[serde(default)]
...
pub struct Prompt34Request {
    ...
    #[serde(default)]
    pub action: Option<Prompt34Action>,
    #[serde(default)]
...
/// existing table-analysis cells; Prompt 34 adds stable page/table identity and
    ...
    for page in 1..=engine.page_count()? {
            ...
            .enumerate()
        {
            ...
            let provenance = json!({
                ...
                "marked_content": "preserved_by_prompt33_source_rewrite",
            });
            let bounds = [
                chunk.x,
                chunk.y,
    ...
    let expression = analyze_math_expressions(input)?
        ...
        .ok_or_else(|| {
            WellfriendError::UnsupportedFeature(
                "prompt34 math_structure_not_resolved: source text is not a resolved born-digital mathematical expression"
                    .to_string(),
            )
        })?;
...
fn matrix_cell_reflow(
    ...
    let mut matrix = bracket_matrix_rows(&supplied.source_text).ok_or_else(|| {
        WellfriendError::UnsupportedFeature(
            "prompt34 math_structure_not_resolved: matrix cell editing requires resolved [[row];[row]] source syntax"
                .to_string(),
        )
    })?;
    let row_cells = matrix.get_mut(row).ok_or_else(|| {
        WellfriendError::UnsupportedFeature(format!(
            "prompt34 math_structure_not_resolved: matrix row {row} is outside the resolved matrix"
        ))
    })?;
    let cell = row_cells.get_mut(col).ok_or_else(|| {
        WellfriendError::UnsupportedFeature(format!(
            "prompt34 math_structure_not_resolved: matrix column {col} is outside the resolved matrix"
        ))
    })?;
        ...
        || replacement_text.contains(']')
    {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 invalid_field_value: matrix cell content must be a nonempty scalar source fragment"
                .to_string(),
        ));
    }
...
fn matrix_structure_reflow(
    ...
    let matrix = bracket_matrix_rows(&supplied.source_text).ok_or_else(|| {
        WellfriendError::UnsupportedFeature(
            "prompt34 math_structure_not_resolved: matrix structure editing requires resolved [[row];[row]] source syntax"
                .to_string(),
        )
    })?;
    ...
    match operation {
        "insert_row" => {
            if index > rows || values.len() != columns || !scalar_values() {
                return Err(WellfriendError::UnsupportedFeature(format!(
                    "prompt34 math_structure_not_resolved: inserted matrix row needs exactly {columns} nonempty scalar values"
                )));
            }
        ...
        "delete_row" => {
            if index >= rows || rows <= 1 || !values.is_empty() {
                return Err(WellfriendError::UnsupportedFeature(
                    "prompt34 math_structure_not_resolved: delete_row requires an existing row, no values, and at least one remaining row"
                        .to_string(),
                ));
            }
            matrix.remove(index);
        ...
        "insert_column" => {
            if index > columns || values.len() != rows || !scalar_values() {
                return Err(WellfriendError::UnsupportedFeature(format!(
                    "prompt34 math_structure_not_resolved: inserted matrix column needs exactly {rows} nonempty scalar values"
                )));
            }
        ...
        "delete_column" => {
            if index >= columns || columns <= 1 || !values.is_empty() {
                return Err(WellfriendError::UnsupportedFeature(
                    "prompt34 math_structure_not_resolved: delete_column requires an existing column, no values, and at least one remaining column"
                        .to_string(),
                ));
            }
        ...
        _ => {
            return Err(WellfriendError::UnsupportedFeature(
                "prompt34 math_structure_not_resolved: matrix operation must be insert_row, delete_row, insert_column, or delete_column"
                    .to_string(),
            ))
        }
    }
...
fn fraction_part_reflow(
    ...
    let (numerator, denominator) = supplied.source_text.trim().split_once('/').ok_or_else(|| {
        WellfriendError::UnsupportedFeature(
            "prompt34 math_structure_not_resolved: fraction editing requires one resolved numerator/denominator slash"
                .to_string(),
        )
    })?;
    ...
    {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 invalid_field_value: a fraction part requires one nonempty single-line scalar source fragment"
                .to_string(),
        ));
    }
    let rebuilt = match part {
        ...
        _ => return Err(WellfriendError::UnsupportedFeature(
            "prompt34 math_structure_not_resolved: fraction part must be numerator or denominator"
                .to_string(),
        )),
    };
    resolved_math_reflow(input, request, &rebuilt)
...
fn math_script_reflow(
    ...
    {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 invalid_field_value: a script requires one nonempty scalar source fragment"
                .to_string(),
        ));
    }
    let separator = match kind {
        ...
        "subscript" => '_',
        _ => return Err(WellfriendError::UnsupportedFeature(
            "prompt34 math_structure_not_resolved: script kind must be superscript or subscript"
                .to_string(),
        )),
    };
    let (base, _) = supplied.source_text.trim().split_once(separator).ok_or_else(|| {
        WellfriendError::UnsupportedFeature(format!(
            "prompt34 math_structure_not_resolved: requested {kind} is not present in the resolved source"
        ))
    })?;
    if base.trim().is_empty() {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 math_structure_not_resolved: script source has no resolved base expression"
                .to_string(),
        ));
    }
...
fn math_fenced_inner_reflow(
    ...
    let open = chars.next().ok_or_else(|| {
        WellfriendError::UnsupportedFeature(
            "prompt34 math_structure_not_resolved: fenced expression source is empty".to_string(),
        )
    })?;
    ...
    {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 math_structure_not_resolved: fenced editing requires one resolved non-matrix (), [], or {} source expression"
                .to_string(),
        ));
    }
...
fn math_radicand_reflow(
    ...
    {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 invalid_field_value: a radical radicand requires one nonempty single-line source fragment"
                .to_string(),
        ));
    }
        ...
        .is_some()
    {
        ...
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 math_structure_not_resolved: radical editing requires resolved √radicand or sqrt(radicand) source notation"
                .to_string(),
        ));
    };
    resolved_math_reflow(input, request, &rebuilt)
...
fn supported_ocr_add_searchable_words(
    ...
    if provider_id.trim().is_empty() {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 provider_unavailable: searchable-layer creation requires an identified canonical OCR provider"
                .to_string(),
        ));
    }
    if words.is_empty() || words.len() > MAX_OCR_LAYER_WORDS {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 resource_limit_exceeded: searchable layer requires 1..=20000 recognized words"
                .to_string(),
        ));
    }
    if language.is_some_and(|tag| tag.trim().is_empty() || tag.len() > 35) {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 unsupported_script: OCR language tag must be a bounded nonempty BCP 47 token"
                .to_string(),
        ));
    }
    let engine = ContentEngine::open_bytes(input.to_vec())?;
    let page_source = crate::classify_document(&engine, &[], &crate::ClassifyConfig::default())?
        ...
        .ok_or_else(|| {
            WellfriendError::UnsupportedFeature(format!(
                "prompt34 scan_not_resolved: page {page} is not present in the current snapshot"
            ))
        })?;
    if page_source.source != crate::PageSource::Scanned {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 searchable_layer_conflict: a searchable or born-digital text layer already owns this page"
                .to_string(),
        ));
    }
    if !engine.extract_page_words(page)?.is_empty() {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 duplicate_text_layer: page already has extractable source text".to_string(),
        ));
    }
    ...
    for (index, word) in words.iter().enumerate() {
        if word.text.trim().is_empty() || !word.text.is_ascii() {
            return Err(WellfriendError::UnsupportedFeature(format!(
                "prompt34 unsupported_script: OCR word {index} requires nonempty exact-ASCII text until a canonical ToUnicode-capable OCR font route is selected"
            )));
        }
        if !word.confidence.is_finite() || !(0.0..=1.0).contains(&word.confidence) {
            return Err(WellfriendError::UnsupportedFeature(format!(
                "prompt34 confidence_below_threshold: OCR word {index} has an invalid confidence"
            )));
        }
        if word.confidence < 0.80 {
            return Err(WellfriendError::UnsupportedFeature(format!(
                "prompt34 confidence_below_threshold: OCR word {index} requires review before searchable-layer creation"
            )));
        }
        if !word.font_size.is_finite() || word.font_size <= 0.0 || word.font_size > 288.0 {
            return Err(WellfriendError::UnsupportedFeature(format!(
                "prompt34 invalid_geometry: OCR word {index} font size must be finite and within 0..288"
            )));
        }
...
fn import_static_xfa_datasets_pdf(input: &[u8], datasets_xml: &str) -> Result<(Vec<u8>, Value)> {
    ...
    {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 invalid_field_value: XFA datasets import requires bounded XML with a datasets root"
                .to_string(),
        ));
    }
    ...
    if !before.inventory.present {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 field_not_found: document has no XFA datasets packet".to_string(),
        ));
    }
    if before.inventory.classification.dynamic_xfa {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 dynamic_xfa_unsupported: datasets import is limited to resolved static XFA"
                .to_string(),
        ));
    }
    if !before.template_parsed || !before.datasets_parsed {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 xfa_conversion_lossy: static template and existing datasets packet must both parse before import"
                .to_string(),
        ));
    }
    ...
    let acroform = reader.resolve(catalog.get("AcroForm").cloned().ok_or_else(|| {
        WellfriendError::UnsupportedFeature(
            "prompt34 field_not_found: document has no AcroForm/XFA dictionary".to_string(),
        )
    })?)?;
    let xfa = acroform
        .as_dict()
        ...
        .ok_or_else(|| {
            WellfriendError::UnsupportedFeature(
                "prompt34 field_not_found: document has no XFA packet array".to_string(),
            )
        })?;
    ...
    let dataset_reference = packet_items
        ...
        .ok_or_else(|| {
            WellfriendError::UnsupportedFeature(
                "prompt34 xfa_conversion_lossy: datasets import requires an indirect datasets packet"
                    .to_string(),
            )
        })?;
    ...
    if !changed {
        return Err(WellfriendError::MalformedPdf(
            "prompt34 structure_update_failed: resolved XFA datasets packet was not a writable stream"
                .to_string(),
        ));
    }
    ...
    if !after.datasets_parsed || after.inventory.classification.dynamic_xfa {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 structure_update_failed: imported static XFA datasets did not reopen as a resolved static packet"
                .to_string(),
        ));
    }
    ...
    if preserved_before != preserved_after {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 structure_update_failed: static XFA datasets import changed a non-datasets packet"
                .to_string(),
        ));
    }
    Ok((
...
fn table_cell_reflow(
    ...
    let graph = editable_tables(input)?
        ...
        .ok_or_else(|| {
            WellfriendError::UnsupportedFeature(format!(
                "prompt34 table_not_resolved: source-linked table {table_id} was not found in the current snapshot"
            ))
        })?;
    let cell = graph
        .source
        ...
        .ok_or_else(|| {
            WellfriendError::UnsupportedFeature(format!(
                "prompt34 table_not_resolved: origin cell ({row},{col}) is not present in {table_id}"
            ))
        })?;
    if cell.text.trim().is_empty() {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 table_not_resolved: an empty cell has no source text instruction to rewrite"
                .to_string(),
        ));
    }
...
fn table_cell_alignment_reflow(
    ...
    if !matches!(
        ...
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 invalid_field_value: table cell alignment must be left, right, center, justify, start, or end"
                .to_string(),
        ));
    }
    let graph = editable_tables(input)?
    ...
    if cell.text.trim().is_empty() {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 table_not_resolved: an empty cell has no source text instruction to realign"
                .to_string(),
        ));
    }
    let mut reflow = reflow_required(request, "table_not_resolved")?.clone();
...
fn table_cell_padding_reflow(
    ...
    {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 invalid_geometry: table cell padding must be finite and nonnegative"
                .to_string(),
        ));
    }
    let graph = editable_tables(input)?
    ...
    if cell.text.trim().is_empty() {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 table_not_resolved: an empty cell has no source text instruction to reflow"
                .to_string(),
        ));
    }
    ...
    if x1 - x0 < 4.0 || y1 - y0 < 4.0 {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 table_overflow: requested cell padding leaves no usable source reflow region"
                .to_string(),
        ));
    }
    let mut reflow = reflow_required(request, "table_not_resolved")?.clone();
...
fn append_simple_ruled_table_row(
    ...
    if table.source.source != crate::analysis::tables::TableSource::Ruled {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 decorative_layout_not_table: row insertion requires a resolved ruled grid"
                .to_string(),
        ));
    }
    ...
    if columns == 0 || values.len() != columns {
        return Err(WellfriendError::UnsupportedFeature(format!(
            "prompt34 table_not_resolved: appended row requires exactly {columns} values"
        )));
    }
    if table
        .source
    ...
    {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 merged_cell_conflict: appended rows are not safe for a grid with merged cells"
                .to_string(),
        ));
    }
    ...
    let column_bounds = column_bounds
        ...
        .ok_or_else(|| {
            WellfriendError::UnsupportedFeature(
                "prompt34 grid_ambiguous: table columns do not have finite source bounds"
                    .to_string(),
            )
        })?;
    ...
    if !row_height.is_finite() || !(12.0..=144.0).contains(&row_height) {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 table_overflow: row height must be finite and within 12..=144 points"
                .to_string(),
        ));
    }
    if values
    ...
    {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 table_overflow: appended row values must be single-line; use cell reflow for wrapped content"
                .to_string(),
        ));
    }
    ...
    if y_bottom < page_bottom {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 table_overflow: no explicit page-space capacity exists below the ruled table"
                .to_string(),
        ));
    }
    let intersects =
    ...
    {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 locked_object_conflict: source text occupies the requested appended-row region"
                .to_string(),
        ));
    }
    ...
    {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 locked_object_conflict: an annotation occupies the requested appended-row region"
                .to_string(),
        ));
    }
    ...
    if classification.is_some_and(|item| item.image_coverage > 0.0) {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 locked_object_conflict: pages with image occurrences require an explicit scene dependency before table expansion"
                .to_string(),
        ));
    }
    let mut editor = PdfEditor::open_bytes(input.to_vec())?;
    for (column, ((x0, x1), value)) in column_bounds.iter().zip(values).enumerate() {
        ...
        if width <= 0.0 {
            return Err(WellfriendError::UnsupportedFeature(
                "prompt34 grid_ambiguous: appended table cell has nonpositive width".to_string(),
            ));
        }
        // A bounded fixed-width source insertion is intentional here: wrapped
        // text belongs to `table_edit_cell` and Prompt 33 reflow, not an
        // implicit font-size reduction during row insertion.
        if value.chars().count() as f64 * 6.0 > width - 8.0 {
            return Err(WellfriendError::UnsupportedFeature(format!(
                "prompt34 table_overflow: appended value for column {column} exceeds the resolved single-line cell width"
            )));
        }
        editor.draw_rect(
            table.page,
...
fn append_simple_ruled_table_column(
    ...
    if table.source.source != crate::analysis::tables::TableSource::Ruled {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 decorative_layout_not_table: column insertion requires a resolved ruled grid"
                .to_string(),
        ));
    }
    ...
    if rows == 0 || values.len() != rows {
        return Err(WellfriendError::UnsupportedFeature(format!(
            "prompt34 table_not_resolved: appended column requires exactly {rows} values"
        )));
    }
    if table
        .source
    ...
    {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 merged_cell_conflict: appended columns are not safe for a grid with merged cells"
                .to_string(),
        ));
    }
    if values
    ...
    {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 table_overflow: appended column values must be single-line; use cell reflow for wrapped content"
                .to_string(),
        ));
    }
    ...
    let row_bounds = row_bounds
        ...
        .ok_or_else(|| {
            WellfriendError::UnsupportedFeature(
                "prompt34 grid_ambiguous: table rows do not have finite source bounds".to_string(),
            )
        })?;
    ...
    if !column_width.is_finite() || !(18.0..=288.0).contains(&column_width) {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 table_overflow: column width must be finite and within 18..=288 points"
                .to_string(),
        ));
    }
    ...
    if x_right > page_right {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 table_overflow: no explicit page-space capacity exists beside the ruled table"
                .to_string(),
        ));
    }
    ...
    {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 locked_object_conflict: source text occupies the requested appended-column region"
                .to_string(),
        ));
    }
    let interactive = interactive_report(&engine)?;
    ...
    {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 locked_object_conflict: an annotation occupies the requested appended-column region"
                .to_string(),
        ));
    }
    let classification = crate::classify_document(&engine, &[], &crate::ClassifyConfig::default())?
    ...
    for (row, ((y0, y1), value)) in row_bounds.iter().zip(values).enumerate() {
        ...
        if height <= 0.0 {
            return Err(WellfriendError::UnsupportedFeature(
                "prompt34 grid_ambiguous: appended table cell has nonpositive height".to_string(),
            ));
        }
        if value.chars().count() as f64 * 6.0 > column_width - 8.0 {
            return Err(WellfriendError::UnsupportedFeature(format!(
                "prompt34 table_overflow: appended value for row {row} exceeds the resolved single-line cell width"
            )));
        }
        editor.draw_rect(
            table.page,
...
fn rect_from_pdf_bounds(bounds: [f64; 4]) -> Result<ImageRect> {
    ...
    {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 invalid_geometry: annotation geometry must be finite with positive extent"
                .to_string(),
        ));
    }
...
fn supported_annotation_create(
    ...
    // Prompt 17 owns the source-level writer for annotations whose appearance
    ...
    if let Some(element) = direct_xfdf_annotation_element(subtype) {
        if page == 0 {
            return Err(WellfriendError::UnsupportedFeature(
                "prompt34 invalid_geometry: annotation pages are one-based".to_string(),
            ));
        }
        ...
        let xfdf = format!(
            ...
            element = element,
            page_zero = page - 1,
            annotation_id = xml_escape_prompt34(&annotation_id),
            rect = xfdf_rect(bounds),
            contents = xml_escape_prompt34(contents),
        );
        ...
        return Ok((
            output,
            json!({
                "source_edit": "canonical_prompt17_xfdf_import",
    ...
    match subtype.to_ascii_lowercase().as_str() {
        ...
        "link" => {
            let target = uri.ok_or_else(|| {
                WellfriendError::UnsupportedFeature(
                    "prompt34 invalid_geometry: Link creation requires an explicit URI".to_string(),
                )
            })?;
        ...
        unsupported => {
            return Err(WellfriendError::UnsupportedFeature(format!(
                "prompt34 unsupported_annotation_type: {unsupported}"
            )))
        }
    }
...
fn supported_annotation_create_reply(
    ...
    if parent_annotation_id.trim().is_empty() || page == 0 {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 reply_relationship_invalid: replies require a stable parent ID and one-based page"
                .to_string(),
        ));
    }
    ...
    {
        return Err(WellfriendError::UnsupportedFeature(format!(
            "prompt34 reply_relationship_invalid: parent annotation {parent_annotation_id} is not in the current snapshot"
        )));
    }
    let fingerprint = format!(
    ...
    let xfdf = format!(
        ...
        page_zero = page - 1,
        reply_id = xml_escape_prompt34(&reply_id),
        rect = xfdf_rect(bounds),
        parent = xml_escape_prompt34(parent_annotation_id),
        contents = xml_escape_prompt34(contents),
    );
    let options = AnnotationXfdfImportOptions {
        fail_on_unsupported: true,
    ...
    Ok((
        output,
        json!({
            "source_edit": "canonical_prompt17_xfdf_reply_import",
            "reply_id": reply_id,
...
fn xml_escape_prompt34(value: &str) -> String {
...
fn form_field_reference_by_name(
    ...
    if depth > 32 {
        return Err(WellfriendError::ResourceLimit(
            "prompt34 resource_limit_exceeded: AcroForm field hierarchy exceeds depth 32"
                .to_string(),
        ));
    }
...
fn rename_form_field_pdf(
    ...
    {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 invalid_field_value: a field rename requires a nonempty ASCII terminal name without hierarchy separators"
                .to_string(),
        ));
    }
    let document = PdfDocument::open_bytes(input.to_vec())?;
    ...
    let acroform = catalog.get("AcroForm").ok_or_else(|| {
        WellfriendError::UnsupportedFeature(
            "prompt34 field_not_found: document has no AcroForm field tree".to_string(),
        )
    })?;
    ...
    let target = target.ok_or_else(|| {
        WellfriendError::UnsupportedFeature(format!(
            "prompt34 field_not_found: {field_name} is not in the current AcroForm field tree"
        ))
    })?;
    ...
    {
        return Err(WellfriendError::UnsupportedFeature(format!(
            "prompt34 duplicate_field_name: {output_name} already exists in the current AcroForm field tree"
        )));
    }
    let mut changed = false;
    let (objects, root, info) = rewrite_document_objects(reader, &mut |number, object| {
    ...
    if !changed {
        return Err(WellfriendError::MalformedPdf(
            "prompt34 structure_update_failed: resolved field object was not a mutable dictionary"
                .to_string(),
        ));
    }
    let output = PdfWriter::new(objects, root)
...
fn set_form_default_pdf(input: &[u8], field_name: &str, value: &str) -> Result<(Vec<u8>, Value)> {
    if !value.is_ascii() || value.len() > 4096 || value.chars().any(char::is_control) {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 invalid_field_value: default values currently require at most 4096 printable ASCII characters"
                .to_string(),
        ));
    }
    let document = PdfDocument::open_bytes(input.to_vec())?;
...
fn set_form_button_default_pdf(
    ...
    if widgets.len() != 1 {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 appearance_state_mismatch: button default editing requires exactly one resolved widget"
                .to_string(),
        ));
    }
    ...
    let normal_states = widget
        ...
        .ok_or_else(|| {
            WellfriendError::UnsupportedFeature(
                "prompt34 appearance_state_mismatch: button has no named normal appearance states"
                    .to_string(),
            )
        })?;
    let state = if checked {
        normal_states
            ...
            .ok_or_else(|| {
                WellfriendError::UnsupportedFeature(
                    "prompt34 export_value_mismatch: button has no non-Off export appearance state"
                        .to_string(),
                )
            })?
    } else {
    ...
    if normal_states.get(&state).is_none() {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 appearance_state_mismatch: requested button default state is absent from /AP /N"
                .to_string(),
        ));
    }
    let mut changed = false;
    ...
    if !changed {
        return Err(WellfriendError::MalformedPdf(
            "prompt34 structure_update_failed: button default did not update the resolved field dictionary"
                .to_string(),
        ));
    }
    let output = PdfWriter::new(objects, root)
...
fn set_form_calculation_order_pdf(
    ...
    {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 invalid_field_value: calculation order requires 1..=512 unique nonempty resolved field names"
                .to_string(),
        ));
    }
    let engine = ContentEngine::open_bytes(input.to_vec())?;
    ...
    for field_name in field_names {
        ...
        if field.is_signature {
            return Err(WellfriendError::UnsupportedFeature(
                "prompt34 signature_permission_violation: signature fields cannot enter a calculation order"
                    .to_string(),
            ));
        }
    }
    ...
    let acroform_reference = acroform_source.as_reference().ok_or_else(|| {
        WellfriendError::UnsupportedFeature(
            "prompt34 structure_update_failed: calculation-order mutation requires an indirect AcroForm dictionary"
                .to_string(),
        )
    })?;
            ...
            fields
                ...
                .ok_or_else(|| {
                    WellfriendError::MalformedPdf(format!(
                        "prompt34 structure_update_failed: calculation-order field {field_name} disappeared during resolution"
                    ))
                })
        })
    ...
    if !changed {
        return Err(WellfriendError::MalformedPdf(
            "prompt34 structure_update_failed: calculation-order update did not reach the AcroForm dictionary"
                .to_string(),
        ));
    }
    let output = PdfWriter::new(objects, root)
    ...
    if forms.calculation_order_len != field_names.len() {
        return Err(WellfriendError::MalformedPdf(
            "prompt34 structure_update_failed: calculation-order cardinality did not survive reopen"
                .to_string(),
        ));
    }
    Ok((
...
fn collect_form_subtree_references(
    ...
    let Some(reference) = source.as_reference() else {
        return Err(WellfriendError::MalformedPdf(
            "prompt34 structure_update_failed: field subtree must use indirect objects".to_string(),
        ));
    };
    ...
    let Some(dict) = resolved.as_dict() else {
        return Err(WellfriendError::MalformedPdf(
            "prompt34 structure_update_failed: field subtree object is not a dictionary"
                .to_string(),
        ));
    };
...
fn delete_form_field_pdf(input: &[u8], field_name: &str) -> Result<(Vec<u8>, Value)> {
    ...
    if !changed {
        return Err(WellfriendError::MalformedPdf(
            "prompt34 structure_update_failed: field deletion did not alter any source field-tree or annotation object"
                .to_string(),
        ));
    }
    let output = PdfWriter::new(objects, root)
...
fn prompt34_object_remap(reader: &PdfReader) -> BTreeMap<u32, u32> {
...
fn prompt34_pdf_numbers(values: &[f64]) -> PdfObject {
    PdfObject::Array(values.iter().copied().map(PdfObject::Real).collect())
}

fn prompt34_pdf_literal(value: &str) -> Result<String> {
    if !value.is_ascii() || value.chars().any(char::is_control) {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 unsupported_script: viewer-independent text-field creation currently requires exact ASCII values"
                .to_string(),
        ));
    }
    Ok(value
...
fn create_text_form_field_pdf(
    ...
    {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 duplicate_field_name: text-field creation requires a unique nonempty ASCII root field name"
                .to_string(),
        ));
    }
    let rect = rect_from_pdf_bounds(bounds)?;
    let value_literal = prompt34_pdf_literal(value)?;
    let document = PdfDocument::open_bytes(input.to_vec())?;
    let reader = document.reader();
    let engine = ContentEngine::open_bytes(input.to_vec())?;
    if crate::forms_report(&engine)?
    ...
    {
        return Err(WellfriendError::UnsupportedFeature(format!(
            "prompt34 duplicate_field_name: {field_name} already exists in the current AcroForm field tree"
        )));
    }
    ...
    if acroform_source.is_some() && acroform_dict.is_none() {
        return Err(WellfriendError::MalformedPdf(
            "prompt34 structure_update_failed: catalog AcroForm is not a dictionary".to_string(),
        ));
    }
    let remap = prompt34_object_remap(reader);
    ...
    let acroform_number = if acroform_source.is_some() {
        acroform_source
            ...
            .ok_or_else(|| {
                WellfriendError::UnsupportedFeature(
                    "prompt34 structure_update_failed: indirect AcroForm is required for field creation"
                        .to_string(),
                )
            })?
    } else {
    ...
    let page_output_number = remap.get(&page_source_number).copied().ok_or_else(|| {
        WellfriendError::MalformedPdf(
            "prompt34 structure_update_failed: page object is missing from canonical remap"
                .to_string(),
        )
    })?;
    ...
    if !changed {
        return Err(WellfriendError::MalformedPdf(
            "prompt34 structure_update_failed: field creation could not update catalog, field tree, or page annotations"
                .to_string(),
        ));
    }
    ...
    appearance.insert(
        "BBox",
        prompt34_pdf_numbers(&[0.0, 0.0, rect.width, rect.height]),
    );
    ...
    widget.insert("Rect", prompt34_pdf_numbers(&bounds));
    widget.insert(
        "P",
        PdfObject::Reference {
...
fn create_signature_form_field_pdf(
    ...
    let field_reference = fields
        ...
        .ok_or_else(|| {
            WellfriendError::MalformedPdf(
                "prompt34 structure_update_failed: created signature field cannot be resolved"
                    .to_string(),
            )
        })?;
    let mut changed = false;
    ...
    if !changed {
        return Err(WellfriendError::MalformedPdf(
            "prompt34 structure_update_failed: signature creation did not update its terminal field"
                .to_string(),
        ));
    }
    let output = PdfWriter::new(objects, root)
    ...
    let field = crate::forms_report(&reopened)?
        ...
        .ok_or_else(|| {
            WellfriendError::MalformedPdf(
                "prompt34 structure_update_failed: created signature field is absent after reopen"
                    .to_string(),
            )
        })?;
    if !field.is_signature {
        return Err(WellfriendError::MalformedPdf(
            "prompt34 structure_update_failed: created field is not reported as a signature field"
                .to_string(),
        ));
    }
    Ok((
...
fn checkbox_appearance(width: f64, height: f64, marked: bool) -> PdfObject {
    ...
    dict.insert("BBox", prompt34_pdf_numbers(&[0.0, 0.0, width, height]));
...
fn create_checkbox_form_field_pdf(
    ...
    if rect.width < 8.0 || rect.height < 8.0 {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 invalid_geometry: checkbox creation requires at least an 8 by 8 point rectangle"
                .to_string(),
        ));
    }
    ...
    let field_reference = fields
        ...
        .ok_or_else(|| {
            WellfriendError::MalformedPdf(
                "prompt34 structure_update_failed: created checkbox field cannot be resolved"
                    .to_string(),
            )
        })?;
    ...
    if widgets.len() != 1 {
        return Err(WellfriendError::MalformedPdf(
            "prompt34 structure_update_failed: created checkbox needs exactly one indirect widget"
                .to_string(),
        ));
    }
    ...
    let off_source = widget
        ...
        .ok_or_else(|| {
            WellfriendError::MalformedPdf(
                "prompt34 structure_update_failed: created checkbox normal appearance is not indirect"
                    .to_string(),
            )
        })?;
    let remap = prompt34_object_remap(reader);
    let off_number = remap.get(&off_source.0).copied().ok_or_else(|| {
        WellfriendError::MalformedPdf(
            "prompt34 structure_update_failed: checkbox normal appearance is absent from canonical remap"
                .to_string(),
        )
    })?;
    ...
    if !changed {
        return Err(WellfriendError::MalformedPdf(
            "prompt34 structure_update_failed: checkbox creation did not update field, widget, and appearance"
                .to_string(),
        ));
    }
    objects.push(OutputObject {
...
fn create_radio_form_field_pdf(
    ...
    {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 export_value_mismatch: radio export values require 1..=64 ASCII name-safe characters"
                .to_string(),
        ));
    }
    ...
    let field_reference = fields
        ...
        .ok_or_else(|| {
            WellfriendError::MalformedPdf(
                "prompt34 structure_update_failed: created radio field cannot be resolved"
                    .to_string(),
            )
        })?;
    let field = reader.get_and_resolve(field_reference.0, field_reference.1)?;
    ...
    if widgets.len() != 1 {
        return Err(WellfriendError::MalformedPdf(
            "prompt34 structure_update_failed: created radio field needs exactly one indirect widget"
                .to_string(),
        ));
    }
    let widget_reference = widgets[0].as_reference().ok_or_else(|| {
    ...
    let normal = widget
        ...
        .ok_or_else(|| {
            WellfriendError::MalformedPdf(
                "prompt34 structure_update_failed: created radio normal appearance states are missing"
                    .to_string(),
            )
        })?;
    let off_source = normal
        ...
        .ok_or_else(|| {
            WellfriendError::MalformedPdf(
                "prompt34 structure_update_failed: created radio off appearance is not indirect"
                    .to_string(),
            )
        })?;
    let on_source = normal
        ...
        .ok_or_else(|| {
            WellfriendError::MalformedPdf(
                "prompt34 structure_update_failed: created radio on appearance is not indirect"
                    .to_string(),
            )
        })?;
    let remap = prompt34_object_remap(reader);
    let off_number = remap.get(&off_source.0).copied().ok_or_else(|| {
        WellfriendError::MalformedPdf(
            "prompt34 structure_update_failed: radio off appearance is absent from canonical remap"
                .to_string(),
        )
    })?;
    let on_number = remap.get(&on_source.0).copied().ok_or_else(|| {
        WellfriendError::MalformedPdf(
            "prompt34 structure_update_failed: radio on appearance is absent from canonical remap"
                .to_string(),
        )
    })?;
    ...
    if !changed {
        return Err(WellfriendError::MalformedPdf(
            "prompt34 structure_update_failed: radio creation did not update field and widget state"
                .to_string(),
        ));
    }
    let output = PdfWriter::new(objects, root)
...
fn create_choice_form_field_pdf(
    ...
    {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 invalid_field_value: choice creation requires 1..=100 unique printable ASCII options"
                .to_string(),
        ));
    }
    ...
    if !options.iter().any(|option| option == selected) {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 invalid_field_value: selected choice value must occur in the declared option array"
                .to_string(),
        ));
    }
    ...
    let field_reference = fields
        ...
        .ok_or_else(|| {
            WellfriendError::MalformedPdf(
                "prompt34 structure_update_failed: created choice field cannot be resolved"
                    .to_string(),
            )
        })?;
    ...
    if !changed {
        return Err(WellfriendError::MalformedPdf(
            "prompt34 structure_update_failed: choice creation did not update the source field dictionary"
                .to_string(),
        ));
    }
    let output = PdfWriter::new(objects, root)
...
fn set_choice_options_pdf(
    ...
    {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 invalid_field_value: choice options require 1..=100 unique printable ASCII values"
                .to_string(),
        ));
    }
    let selected = selected.unwrap_or(&options[0]);
    if !options.iter().any(|option| option == selected) {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 invalid_field_value: selected choice value must occur in the replacement option array"
                .to_string(),
        ));
    }
    let document = PdfDocument::open_bytes(input.to_vec())?;
    ...
    if !changed {
        return Err(WellfriendError::MalformedPdf(
            "prompt34 structure_update_failed: choice option update did not reach the resolved field dictionary"
                .to_string(),
        ));
    }
...
fn create_push_button_form_field_pdf(
    ...
    let field_reference = fields
        ...
        .ok_or_else(|| {
            WellfriendError::MalformedPdf(
                "prompt34 structure_update_failed: created push button cannot be resolved"
                    .to_string(),
            )
        })?;
    let field = reader.get_and_resolve(field_reference.0, field_reference.1)?;
    ...
    if widgets.len() != 1 {
        return Err(WellfriendError::MalformedPdf(
            "prompt34 structure_update_failed: created push button needs exactly one indirect widget"
                .to_string(),
        ));
    }
    let widget_reference = widgets[0].as_reference().ok_or_else(|| {
    ...
    let appearance_reference = widget
        ...
        .ok_or_else(|| {
            WellfriendError::MalformedPdf(
                "prompt34 structure_update_failed: created push-button normal appearance is not indirect"
                    .to_string(),
            )
        })?;
    let output_appearance = prompt34_object_remap(reader)
        .get(&appearance_reference.0)
        .copied()
        .ok_or_else(|| {
            WellfriendError::MalformedPdf(
                "prompt34 structure_update_failed: push-button appearance is absent from canonical remap"
                    .to_string(),
            )
        })?;
    let mut changed = false;
    ...
    if !changed {
        return Err(WellfriendError::MalformedPdf(
            "prompt34 structure_update_failed: push-button creation did not update field and widget appearances"
                .to_string(),
        ));
    }
    let output = PdfWriter::new(objects, root)
...
fn move_resize_form_widget_pdf(
    ...
    if widgets.len() != 1 {
        return Err(WellfriendError::UnsupportedFeature(format!(
            "prompt34 field_not_found: field {field_name} requires exactly one resolved widget on page {page}"
        )));
    }
    let widget = widgets
        .into_iter()
    ...
    Ok((
        ...
        json!({
            ...
            "valid_existing_appearance": "preserved_or_regenerated_under_prompt17_policy",
        }),
    ))
}

fn supported_form_edit(input: &[u8], action: &Prompt34Action) -> Result<(Vec<u8>, Value)> {
    if let Prompt34Action::FormCreateTextInTableCell {
        table_id,
        row,
        col,
        field_name,
    ...
    } = action
    {
        let table = editable_tables(input)?
            ...
            .ok_or_else(|| {
                WellfriendError::UnsupportedFeature(format!(
                    "prompt34 table_not_resolved: table {table_id} is not in the current source snapshot"
                ))
            })?;
        let cell = table
            .source
            ...
            .ok_or_else(|| {
                WellfriendError::UnsupportedFeature(format!(
                    "prompt34 grid_ambiguous: table {table_id} has no uniquely resolved cell at row {row}, column {col}"
                ))
            })?;
        ...
        {
            return Err(WellfriendError::UnsupportedFeature(
                "prompt34 invalid_geometry: resolved table-cell bounds are not usable for a form widget".into(),
            ));
        }
        return create_text_form_field_pdf(input, field_name, table.page, cell.bbox, value);
    }
    if let Prompt34Action::FormCreateCheckboxInTableCell {
        table_id,
        row,
        col,
        field_name,
    ...
    if let Prompt34Action::FormCreateChoiceInTableCell {
        table_id,
        row,
        col,
        field_name,
    ...
    if let Prompt34Action::FormCreateSignature {
        field_name,
        page,
        rect,
    } = action
    ...
    if let Prompt34Action::FormSetCalculationOrder { field_names } = action {
        return set_form_calculation_order_pdf(input, field_names);
    }
    let engine = ContentEngine::open_bytes(input.to_vec())?;
    let form_report = crate::forms_report(&engine)?;
    let requested_field = match action {
        Prompt34Action::FormSetText { field_name, .. }
        | Prompt34Action::FormSetChoice { field_name, .. }
        | Prompt34Action::FormSetChoiceOptions { field_name, .. }
        | Prompt34Action::FormSetCheckbox { field_name, .. }
        | Prompt34Action::FormSetDefault { field_name, .. }
        | Prompt34Action::FormSetButtonDefault { field_name, .. }
        | Prompt34Action::FormRename { field_name, .. }
        | Prompt34Action::FormDelete { field_name }
        | Prompt34Action::FormMoveResizeWidget { field_name, .. } => Some(field_name.as_str()),
        Prompt34Action::FormImportData { .. }
        | Prompt34Action::FormSetCalculationOrder { .. }
        | Prompt34Action::FormReset { .. }
        | Prompt34Action::FormFlatten
        | Prompt34Action::FormCreateText { .. }
        | Prompt34Action::FormCreateTextInTableCell { .. }
        | Prompt34Action::FormCreateCheckbox { .. }
        | Prompt34Action::FormCreateCheckboxInTableCell { .. }
        | Prompt34Action::FormCreateChoiceInTableCell { .. }
        | Prompt34Action::FormCreateChoice { .. }
        | Prompt34Action::FormCreatePushButton { .. }
        | Prompt34Action::FormCreateRadio { .. }
        | Prompt34Action::FormCreateSignature { .. } => None,
        _ => None,
    };
    if let Some(field_name) = requested_field {
        ...
        if field.is_signature {
            return Err(WellfriendError::UnsupportedFeature(
                "prompt34 signature_permission_violation: signature field values are not mutable"
                    .to_string(),
            ));
        }
        let compatible = matches!(
            (action, field.field_type.as_str()),
            (Prompt34Action::FormSetText { .. }, "text")
                | (Prompt34Action::FormSetChoice { .. }, "choice")
                | (Prompt34Action::FormSetChoiceOptions { .. }, "choice")
                | (Prompt34Action::FormSetCheckbox { .. }, "checkbox" | "radio")
                | (Prompt34Action::FormSetDefault { .. }, "text" | "choice")
                | (
                    Prompt34Action::FormSetButtonDefault { .. },
                    "checkbox" | "radio"
                )
                | (Prompt34Action::FormRename { .. }, _)
                | (Prompt34Action::FormDelete { .. }, _)
                | (Prompt34Action::FormMoveResizeWidget { .. }, _)
        );
        if !compatible {
            return Err(WellfriendError::UnsupportedFeature(format!(
                "prompt34 invalid_field_value: action is incompatible with {field_name} field type {}",
                field.field_type
            )));
        }
    }
    if let Prompt34Action::FormRename {
        field_name,
        new_name,
    } = action
    {
    ...
    if let Prompt34Action::FormSetDefault { field_name, value } = action {
        return set_form_default_pdf(input, field_name, value);
    }
    if let Prompt34Action::FormSetButtonDefault {
        field_name,
        checked,
    } = action
    {
    ...
    if let Prompt34Action::FormSetChoiceOptions {
        field_name,
        options,
        selected,
        editable_combo,
    ...
    if let Prompt34Action::FormDelete { field_name } = action {
        return delete_form_field_pdf(input, field_name);
    }
    if let Prompt34Action::FormCreateText {
        field_name,
        page,
        rect,
        value,
    ...
    if let Prompt34Action::FormCreateCheckbox {
        field_name,
        page,
        rect,
        checked,
    ...
    if let Prompt34Action::FormCreateChoice {
        field_name,
        page,
        rect,
        options,
    ...
    if let Prompt34Action::FormCreatePushButton {
        field_name,
        page,
        rect,
        caption,
    ...
    if let Prompt34Action::FormCreateRadio {
        field_name,
        page,
        rect,
        export_value,
    ...
    if let Prompt34Action::FormMoveResizeWidget {
        field_name,
        page,
        rect,
    } = action
    ...
    if let Prompt34Action::FormImportData { data, format } = action {
        let format = FormDataFormat::parse(format).ok_or_else(|| {
            WellfriendError::UnsupportedFeature(format!(
                "prompt34 unsupported_exact: unsupported form data format {format}"
            ))
        })?;
    ...
    if let Prompt34Action::FormReset { field_name } = action {
        ...
        if selected.is_empty() {
            return Err(WellfriendError::UnsupportedFeature(format!(
                "prompt34 field_not_found: {} is not in the current AcroForm field tree",
    ...
    let operation = match action {
        Prompt34Action::FormSetText { field_name, value } => {
            editor.set_form_text(field_name, value);
            "set_text"
        }
        Prompt34Action::FormSetChoice { field_name, value } => {
            editor.set_form_choice(field_name, value);
            "set_choice"
        }
        Prompt34Action::FormSetChoiceOptions { .. } => {
            unreachable!("choice options handled above")
        }
        Prompt34Action::FormSetCheckbox {
            field_name,
            checked,
        } => {
        ...
        Prompt34Action::FormSetDefault { .. } => unreachable!("default value handled above"),
        Prompt34Action::FormSetButtonDefault { .. } => {
            unreachable!("button default handled above")
        }
        Prompt34Action::FormRename { .. } => unreachable!("rename handled above"),
        Prompt34Action::FormDelete { .. } => unreachable!("delete handled above"),
        Prompt34Action::FormCreateText { .. } => unreachable!("create handled above"),
        Prompt34Action::FormCreateCheckbox { .. } => unreachable!("create handled above"),
        Prompt34Action::FormCreateChoice { .. } => unreachable!("create handled above"),
        Prompt34Action::FormCreatePushButton { .. } => unreachable!("create handled above"),
        Prompt34Action::FormCreateRadio { .. } => unreachable!("create handled above"),
        Prompt34Action::FormMoveResizeWidget { .. } => unreachable!("widget move handled above"),
        Prompt34Action::FormReset { .. } => unreachable!("reset handled above"),
        Prompt34Action::FormFlatten => {
        ...
        _ => {
            return Err(WellfriendError::UnsupportedFeature(
                "prompt34 invalid_field_value: request action is not a canonical form operation"
                    .to_string(),
            ))
        }
    };
...
fn action_subsystem(action: &Prompt34Action) -> Prompt34Subsystem {
    match action {
        Prompt34Action::TableEditCell { .. }
        | Prompt34Action::TableEditMathCell { .. }
        | Prompt34Action::TableMoveLinkedAnnotation { .. }
        | Prompt34Action::TableSetCellAlignment { .. }
        | Prompt34Action::TableSetCellPadding { .. }
        | Prompt34Action::TableAddCellBorder { .. }
        | Prompt34Action::TableSetCellFill { .. }
        | Prompt34Action::TableAppendRow { .. }
        | Prompt34Action::TableAppendColumn { .. } => Prompt34Subsystem::Table,
        Prompt34Action::MathReplace { .. }
        | Prompt34Action::MathMoveResize { .. }
        | Prompt34Action::MathEditMatrixCell { .. }
        | Prompt34Action::MathEditMatrixStructure { .. }
        | Prompt34Action::MathEditFractionPart { .. }
        | Prompt34Action::MathEditScript { .. }
        | Prompt34Action::MathEditFencedInner { .. }
        | Prompt34Action::MathEditRadicand { .. } => Prompt34Subsystem::Math,
        Prompt34Action::OcrCorrectText { .. }
        | Prompt34Action::OcrCorrectGeometry { .. }
        | Prompt34Action::OcrAddSearchableText { .. }
        | Prompt34Action::OcrAddSearchableTextWithLink { .. }
        | Prompt34Action::OcrAddSearchableWords { .. } => Prompt34Subsystem::OcrSearchableLayer,
        Prompt34Action::AnnotationCreate { .. }
        | Prompt34Action::AnnotationEditContents { .. }
        | Prompt34Action::AnnotationMoveResize { .. }
        | Prompt34Action::AnnotationCreateReply { .. }
        | Prompt34Action::AnnotationDeleteInRect { .. }
        | Prompt34Action::AnnotationXfdf { .. }
        | Prompt34Action::AnnotationFlatten { .. } => Prompt34Subsystem::AnnotationAppearance,
        Prompt34Action::FormSetText { .. }
        | Prompt34Action::FormSetChoice { .. }
        | Prompt34Action::FormSetChoiceOptions { .. }
        | Prompt34Action::FormSetCheckbox { .. }
        | Prompt34Action::FormSetDefault { .. }
        | Prompt34Action::FormSetButtonDefault { .. }
        | Prompt34Action::FormRename { .. }
        | Prompt34Action::FormDelete { .. }
        | Prompt34Action::FormCreateText { .. }
        | Prompt34Action::FormCreateTextInTableCell { .. }
        | Prompt34Action::FormCreateCheckbox { .. }
        | Prompt34Action::FormCreateCheckboxInTableCell { .. }
        ...
        | Prompt34Action::FormCreatePushButton { .. }
        | Prompt34Action::FormCreateRadio { .. }
        | Prompt34Action::FormCreateSignature { .. }
        | Prompt34Action::FormMoveResizeWidget { .. }
        | Prompt34Action::FormImportData { .. }
        | Prompt34Action::FormSetCalculationOrder { .. }
        | Prompt34Action::FormReset { .. }
        | Prompt34Action::FormFlatten => Prompt34Subsystem::FormData,
        Prompt34Action::XfaInventory
        | Prompt34Action::XfaImportDatasets { .. }
        | Prompt34Action::XfaFlattenStatic { .. } => Prompt34Subsystem::XfaPreservation,
    }
}
...
fn apply_explicit_action(
    input: &[u8],
    request: &Prompt34Request,
    action: &Prompt34Action,
) -> Result<(Vec<u8>, Prompt34OperationReport)> {
    if action_subsystem(action) != request.subsystem {
        return Err(WellfriendError::UnsupportedFeature(
            "prompt34 unsupported_exact: action does not match the selected subsystem".to_string(),
        ));
    }
    ...
    let (output, operation, transaction, appearance_effect, xfa_effect, changed_pages) =
        match action {
            Prompt34Action::TableMoveLinkedAnnotation {
                ...
                {
                    return Err(WellfriendError::UnsupportedFeature(
                        "prompt34 invalid_geometry: resolved table-cell bounds are not usable for annotation movement".into(),
                    ));
                }
                let (output, report) =
                ...
                (
                    ...
                    json!({
                        "normal_rollover_down": "canonical_prompt17_generation",
            ...
            Prompt34Action::TableEditMathCell {
                ...
                if !approved {
                    return Err(WellfriendError::UnsupportedFeature(
                        "prompt34 formula_review_required: table-cell mathematical replacement requires explicit approval".into(),
                    ));
                }
                let (reflow, table) =
                ...
                if !math_like(&reflow.source_text) {
                    return Err(WellfriendError::UnsupportedFeature(
                        "prompt34 math_structure_not_resolved: resolved table cell is not born-digital mathematical source".into(),
                    ));
                }
                ...
                (
                    ...
                    json!({
                        ...
                        "math": {
                            ...
                            "layout": "prompt32_shaped_prompt33_cell_reflow"
                        }
                    }),
                    json!({}),
                    json!({"preserved": true}),
            ...
            Prompt34Action::TableSetCellFill {
                ...
                {
                    return Err(WellfriendError::UnsupportedFeature(
                        "prompt34 invalid_geometry: table-cell fill color and opacity must be finite values in 0..=1".into(),
                    ));
                }
                let table = editable_tables(input)?
                    .into_iter()
            ...
            Prompt34Action::TableAddCellBorder {
                ...
                if !line_width.is_finite() || *line_width <= 0.0 || *line_width > 36.0 {
                    return Err(WellfriendError::UnsupportedFeature(
                        "prompt34 invalid_geometry: table-cell border width must be finite and within 0..=36 PDF units".into(),
                    ));
                }
                let table = editable_tables(input)?
                    .into_iter()
            ...
            Prompt34Action::TableAppendColumn {
                table_id,
                values,
                column_width,
            } => {
            ...
            Prompt34Action::MathMoveResize { bounds } => {
                if !request.approved {
                    return Err(WellfriendError::UnsupportedFeature(
                        "prompt34 formula_review_required: mathematical source movement requires explicit approval"
                            .to_string(),
                    ));
                }
                ...
                {
                    return Err(WellfriendError::UnsupportedFeature(
                        "prompt34 invalid_geometry: mathematical destination bounds are not usable"
                            .to_string(),
                    ));
                }
                ...
                (
                    output,
                    "math_source_move_resize_with_prompt32_shaping",
                    json!({
                        "prompt32_transaction": report,
                    ...
                    json!({
                        "math_layout": "source_text_shaped_by_prompt32",
                        "geometry": "prompt33_source_region_rewrite",
                    }),
                    json!({"preserved": true}),
                    vec![reflow.page],
                )
            }
            Prompt34Action::MathReplace { replacement_text } => {
                if !request.approved {
                    return Err(WellfriendError::UnsupportedFeature(
                        "prompt34 formula_review_required: mathematical source replacement requires explicit approval"
                            .to_string(),
                    ));
                }
                ...
                (
                    output,
                    "math_source_rewrite_with_prompt32_shaping",
                    json!({"prompt32_transaction": report, "source_text": reflow.source_text}),
                    json!({
                        "math_layout": "source_text_shaped_by_prompt32",
            ...
            Prompt34Action::MathEditMatrixCell {
                ...
                (
                    output,
                    "math_matrix_cell_source_rewrite_with_prompt32_shaping",
                    json!({
                        "prompt32_transaction": report,
                        "source_text": reflow.source_text,
                        "cell": {"row": row, "col": col},
            ...
            Prompt34Action::MathEditMatrixStructure {
                ...
                (
                    output,
                    "math_matrix_structure_source_rewrite_with_prompt32_shaping",
                    json!({
                        "prompt32_transaction": report,
                        "source_text": reflow.source_text,
                        "operation": operation,
            ...
            Prompt34Action::MathEditFractionPart {
                ...
                (
                    output,
                    "math_fraction_part_source_rewrite_with_prompt32_shaping",
                    json!({
                        "prompt32_transaction": report,
                        "source_text": reflow.source_text,
                        "part": part,
            ...
            Prompt34Action::MathEditScript {
                ...
                (
                    output,
                    "math_script_source_rewrite_with_prompt32_shaping",
                    json!({
                        "prompt32_transaction": report,
                        "source_text": reflow.source_text,
            ...
            Prompt34Action::MathEditFencedInner { replacement_text } => {
                if !request.approved {
                    return Err(WellfriendError::UnsupportedFeature(
                        "prompt34 formula_review_required: mathematical source replacement requires explicit approval"
                            .to_string(),
                ...
                (
                    output,
                    "math_fenced_inner_source_rewrite_with_prompt32_shaping",
                    json!({
                        "prompt32_transaction": report,
                        "source_text": reflow.source_text,
            ...
            Prompt34Action::MathEditRadicand { replacement_text } => {
                if !request.approved {
                    return Err(WellfriendError::UnsupportedFeature(
                        "prompt34 formula_review_required: mathematical source replacement requires explicit approval"
                            .to_string(),
                ...
                (
                    output,
                    "math_radicand_source_rewrite_with_prompt32_shaping",
                    json!({
                        "prompt32_transaction": report,
                        "source_text": reflow.source_text,
                    }),
            ...
            Prompt34Action::OcrCorrectText { replacement_text } => {
                if !request.approved {
                    return Err(WellfriendError::UnsupportedFeature(
                        "prompt34 reconstruction_review_required: searchable-layer correction requires explicit approval"
                            .to_string(),
                    ));
                }
            ...
            Prompt34Action::OcrCorrectGeometry { bounds } => {
                if !request.approved {
                    return Err(WellfriendError::UnsupportedFeature(
                        "prompt34 reconstruction_review_required: searchable-layer geometry correction requires explicit approval"
                            .to_string(),
                    ));
                }
                rect_from_pdf_bounds(*bounds)?;
            ...
            Prompt34Action::OcrAddSearchableText {
                ...
                if !request.approved {
                    return Err(WellfriendError::UnsupportedFeature(
                        "prompt34 reconstruction_review_required: searchable-layer creation requires explicit approval"
                            .to_string(),
                    ));
                }
            ...
            Prompt34Action::OcrAddSearchableTextWithLink {
                ...
                if !request.approved {
                    return Err(WellfriendError::UnsupportedFeature(
                        "prompt34 reconstruction_review_required: OCR searchable text with link creation requires explicit approval"
                            .to_string(),
                    ));
                }
                if uri.trim().is_empty() || uri.len() > 4_096 {
                    return Err(WellfriendError::UnsupportedFeature(
                        "prompt34 invalid_geometry: OCR-linked annotation URI must be a bounded nonempty value"
                            .to_string(),
                    ));
                }
            ...
            Prompt34Action::OcrAddSearchableWords {
                page,
                words,
                provider_id,
                provider_version,
            ...
            Prompt34Action::AnnotationDeleteInRect { page, rect } => {
                let mut editor = PdfEditor::open_bytes(input.to_vec())?;
            ...
            Prompt34Action::AnnotationXfdf { xfdf, delete_ids } => {
            ...
            Prompt34Action::AnnotationFlatten { subtypes } => {
                let mut editor = PdfEditor::open_bytes(input.to_vec())?;
            ...
            Prompt34Action::XfaInventory => {
                return Err(WellfriendError::UnsupportedFeature(
                    "prompt34 dynamic_xfa_unsupported: inventory is analyze-only; destructive conversion requires an explicit future conversion policy"
                        .to_string(),
                ))
            }
            Prompt34Action::XfaImportDatasets { datasets_xml } => {
            ...
            Prompt34Action::XfaFlattenStatic {
                remove_original_packets,
            } => {
                if !request.approved {
                    return Err(WellfriendError::UnsupportedFeature(
                        "prompt34 replacement_not_approved: static XFA flattening requires explicit approval"
                            .to_string(),
                    ));
                }
        ...
        match request.subsystem {
            ...
            Prompt34Subsystem::XfaPreservation => {
                ...
                if before_packets != after_packets {
                    return Err(WellfriendError::UnsupportedFeature(
                        "prompt34 structure_update_failed: unrelated edit changed byte-preserved XFA packet content"
                            .to_string(),
                    ));
                }
                (
...
pub fn undo_prompt34(
    ...
    let proof = match request.action.as_ref() {
        Some(Prompt34Action::TableEditMathCell {
            table_id,
            row,
            col,
            replacement_text,
            ...
            if restored != original {
                return Err(WellfriendError::UnsupportedFeature(
                    "prompt34 undo_failed: table-cell math replay did not restore original bytes"
                        .into(),
                ));
            }
            json!({"inverse": "prompt33_reflow_replay", "undo": undo})
        }
        Some(Prompt34Action::TableEditCell {
            table_id,
            row,
            col,
            replacement_text,
            ...
            if restored != original {
                return Err(WellfriendError::MalformedPdf(
                    "prompt34 undo_failed: table reflow inverse did not restore the input snapshot"
                        .to_string(),
                ));
            }
            json!({"inverse": "prompt33_reflow_replay", "undo": undo})
        }
        Some(Prompt34Action::TableSetCellAlignment {
            table_id,
            row,
            col,
            alignment,
            ...
            if restored != original {
                return Err(WellfriendError::MalformedPdf(
                    "prompt34 undo_failed: table alignment inverse did not restore the input snapshot"
                        .to_string(),
                ));
            }
            json!({"inverse": "prompt33_reflow_replay", "undo": undo})
        }
        Some(Prompt34Action::TableSetCellPadding {
            table_id,
            row,
            col,
            padding,
            ...
            if restored != original {
                return Err(WellfriendError::MalformedPdf(
                    "prompt34 undo_failed: table padding inverse did not restore the input snapshot"
                        .to_string(),
                ));
            }
            json!({"inverse": "prompt33_reflow_replay", "undo": undo})
        }
        Some(Prompt34Action::MathReplace { replacement_text })
        | Some(Prompt34Action::OcrCorrectText { replacement_text }) => {
            let typed = if matches!(
                request.action.as_ref(),
                Some(Prompt34Action::MathReplace { .. })
            ) {
            ...
            if restored != original {
                return Err(WellfriendError::MalformedPdf(
                    "prompt34 undo_failed: source reflow inverse did not restore the input snapshot"
                        .to_string(),
                ));
            }
            json!({"inverse": "prompt33_reflow_replay", "undo": undo})
        }
        Some(Prompt34Action::OcrCorrectGeometry { bounds }) => {
            let mut reflow = reflow_required(request, "scan_not_resolved")?.clone();
            rect_from_pdf_bounds(*bounds)?;
            reflow.region = Some(*bounds);
            reflow.replacement_text = reflow.source_text.clone();
            ...
            if restored != original {
                return Err(WellfriendError::MalformedPdf(
                    "prompt34 undo_failed: OCR geometry source reflow inverse did not restore the input snapshot"
                        .to_string(),
                ));
            }
            json!({"inverse": "prompt33_reflow_replay", "undo": undo})
        }
        Some(Prompt34Action::MathEditMatrixCell {
            row,
            col,
            replacement_text,
        }) => {
            ...
            if restored != original {
                return Err(WellfriendError::MalformedPdf(
                    "prompt34 undo_failed: matrix source reflow inverse did not restore the input snapshot"
                        .to_string(),
                ));
            }
            json!({"inverse": "prompt33_reflow_replay", "undo": undo})
        }
        Some(Prompt34Action::MathEditMatrixStructure {
            operation,
            index,
            values,
        }) => {
            ...
            if restored != original {
                return Err(WellfriendError::MalformedPdf(
                    "prompt34 undo_failed: matrix structure reflow inverse did not restore the input snapshot"
                        .to_string(),
                ));
            }
            json!({"inverse": "prompt33_reflow_replay", "undo": undo})
        }
        Some(Prompt34Action::MathEditFractionPart {
            part,
            replacement_text,
        }) => {
            ...
            if restored != original {
                return Err(WellfriendError::MalformedPdf(
                    "prompt34 undo_failed: fraction source reflow inverse did not restore the input snapshot"
                        .to_string(),
                ));
            }
            json!({"inverse": "prompt33_reflow_replay", "undo": undo})
        }
        Some(Prompt34Action::MathEditScript {
            script_kind,
            replacement_text,
        }) => {
            ...
            if restored != original {
                return Err(WellfriendError::MalformedPdf(
                    "prompt34 undo_failed: script source reflow inverse did not restore the input snapshot"
                        .to_string(),
                ));
            }
            json!({"inverse": "prompt33_reflow_replay", "undo": undo})
        }
        Some(Prompt34Action::MathEditFencedInner { replacement_text }) => {
            let reflow = math_fenced_inner_reflow(original, request, replacement_text)?;
            let (restored, undo) = undo_reflow_from_replay(original, output, &reflow)?;
            if restored != original {
                return Err(WellfriendError::MalformedPdf(
                    "prompt34 undo_failed: fenced-expression source reflow inverse did not restore the input snapshot"
                        .to_string(),
                ));
            }
            json!({"inverse": "prompt33_reflow_replay", "undo": undo})
        }
        Some(Prompt34Action::MathEditRadicand { replacement_text }) => {
            let reflow = math_radicand_reflow(original, request, replacement_text)?;
            let (restored, undo) = undo_reflow_from_replay(original, output, &reflow)?;
            if restored != original {
                return Err(WellfriendError::MalformedPdf(
                    "prompt34 undo_failed: radical source reflow inverse did not restore the input snapshot"
                        .to_string(),
                ));
            }
            json!({"inverse": "prompt33_reflow_replay", "undo": undo})
        ...
        Some(_) => {
            ...
            if replayed != output {
                return Err(WellfriendError::MalformedPdf(
                    "prompt34 stale_snapshot_conflict: output does not match the deterministic action replay"
                        .to_string(),
                ));
            }
            json!({
        ...
        None => {
            ...
            if restored != original {
                return Err(WellfriendError::MalformedPdf(
                    "prompt34 undo_failed: legacy reflow inverse did not restore the input snapshot"
                        .to_string(),
                ));
            }
            json!({"inverse": "prompt33_reflow_replay", "undo": undo})
...
pub fn prompt34_feature_matrix() -> Value {
    json!({
        ...
        "annotations": "PdfEditor source edits, Prompt17 XFDF/reply/move-resize import, appearance regeneration, and explicit flattening",
...
mod tests {
    ...
    fn formula_reflow() -> GeometricReflowRequest {
        ...
        .expect("Prompt34 formula reflow request")
    }
    ...
    fn matrix_reflow() -> GeometricReflowRequest {
        ...
        .expect("Prompt34 matrix reflow request")
    }
    ...
    fn table_math_cell_uses_math_review_and_cell_reflow() {
        ...
        let table = editable_tables(&input)
            ...
            .expect("table");
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::Table,
            action: Some(Prompt34Action::TableEditMathCell {
                table_id: table.table_id,
                row: 0,
                col: 0,
        ...
        let (output, report) = apply_prompt34(&input, &request).expect("table math rewrite");
        ...
        let (restored, _) = undo_prompt34(&input, &output, &request).expect("undo table math");
    ...
    fn source_linked_table_cell_action_rewrites_real_cell_source_and_undoes() {
        ...
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::Table,
            action: Some(Prompt34Action::TableEditCell {
        ...
        let (output, report) = apply_prompt34(&input, &request).expect("cell source rewrite");
        ...
        assert!(ContentEngine::open_bytes(output.clone())
            ...
            .contains("Renamed"));
        let (restored, _) = undo_prompt34(&input, &output, &request).expect("table undo");
        assert_eq!(restored, input);
    }
    ...
    fn source_linked_table_cell_alignment_rewrites_and_undoes() {
        ...
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::Table,
            action: Some(Prompt34Action::TableSetCellAlignment {
                table_id: table.table_id,
                row: 0,
                col: 0,
        ...
        let (output, report) = apply_prompt34(&input, &request).expect("cell alignment rewrite");
        assert_eq!(report.operation, "table_cell_alignment_source_rewrite");
        assert!(ContentEngine::open_bytes(output.clone()).is_ok());
        let (restored, _) = undo_prompt34(&input, &output, &request).expect("alignment undo");
        assert_eq!(restored, input);
    }
    ...
    fn source_linked_table_cell_padding_rewrites_and_undoes() {
        ...
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::Table,
            action: Some(Prompt34Action::TableSetCellPadding {
                table_id: table.table_id,
                row: 0,
                col: 0,
        ...
        let (output, report) = apply_prompt34(&input, &request).expect("cell padding rewrite");
        assert_eq!(report.operation, "table_cell_padding_source_rewrite");
        assert!(ContentEngine::open_bytes(output.clone()).is_ok());
        let (restored, _) = undo_prompt34(&input, &output, &request).expect("padding undo");
        assert_eq!(restored, input);
    }
    ...
    fn source_linked_table_cell_fill_writes_real_underlay_and_undoes() {
        ...
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::Table,
            action: Some(Prompt34Action::TableSetCellFill {
                table_id: table.table_id,
                row: 0,
                col: 0,
        ...
        let (output, report) = apply_prompt34(&input, &request).expect("append table fill");
        assert_eq!(
            report.operation,
            "table_cell_fill_source_instruction_append"
        );
        ...
        let (restored, _) = undo_prompt34(&input, &output, &request).expect("table fill undo");
        assert_eq!(restored, input);
    }
    ...
    fn source_linked_table_cell_border_writes_real_path_and_undoes() {
        ...
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::Table,
            action: Some(Prompt34Action::TableAddCellBorder {
                table_id: table.table_id,
                row: 0,
                col: 0,
                line_width: 1.5,
        ...
        let (output, report) = apply_prompt34(&input, &request).expect("append table border");
        assert_eq!(
            report.operation,
            "table_cell_border_source_instruction_append"
        );
        ...
        let (restored, _) = undo_prompt34(&input, &output, &request).expect("table border undo");
        assert_eq!(restored, input);
    }
    ...
    fn simple_ruled_table_append_row_writes_borders_text_and_undoes() {
        ...
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::Table,
            action: Some(Prompt34Action::TableAppendRow {
                table_id: table.table_id,
        ...
        let (output, report) = apply_prompt34(&input, &request).expect("append table row");
        ...
        let (restored, _) = undo_prompt34(&input, &output, &request).expect("append row undo");
        assert_eq!(restored, input);
    }
    ...
    fn simple_ruled_table_append_column_writes_borders_text_and_undoes() {
        ...
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::Table,
            action: Some(Prompt34Action::TableAppendColumn {
                table_id: table.table_id,
        ...
        let (output, report) = apply_prompt34(&input, &request).expect("append table column");
        ...
        let (restored, _) = undo_prompt34(&input, &output, &request).expect("append column undo");
        assert_eq!(restored, input);
    }
    ...
    fn math_move_resize_rewrites_source_geometry_and_undoes() {
        ...
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::Math,
            action: Some(Prompt34Action::MathMoveResize {
        ...
        let (output, report) = apply_prompt34(&input, &request).expect("move mathematical source");
        assert_eq!(
            report.operation,
            "math_source_move_resize_with_prompt32_shaping"
        );
        ...
        let (restored, _) = undo_prompt34(&input, &output, &request).expect("math movement undo");
        assert_eq!(restored, input);
    }
    ...
    fn math_fenced_inner_edit_preserves_delimiters_and_undoes() {
        ...
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::Math,
            action: Some(Prompt34Action::MathEditFencedInner {
        ...
        let (output, report) = apply_prompt34(&input, &request).expect("edit fenced math");
        assert_eq!(
            report.operation,
            "math_fenced_inner_source_rewrite_with_prompt32_shaping"
        );
        assert!(ContentEngine::open_bytes(output.clone())
            ...
            .contains("(y=2)"));
        let (restored, _) = undo_prompt34(&input, &output, &request).expect("fenced math undo");
        assert_eq!(restored, input);
    }
    ...
    fn math_radicand_edit_uses_resolved_source_and_undoes() {
        ...
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::Math,
            action: Some(Prompt34Action::MathEditRadicand {
        ...
        let (output, report) = apply_prompt34(&input, &request).expect("edit radical");
        assert_eq!(
            report.operation,
            "math_radicand_source_rewrite_with_prompt32_shaping"
        );
        assert!(ContentEngine::open_bytes(output.clone())
            ...
            .contains("sqrt(y+1)"));
        let (restored, _) = undo_prompt34(&input, &output, &request).expect("radical undo");
        assert_eq!(restored, input);
    }
    ...
    fn explicit_math_ocr_and_annotation_actions_use_canonical_editors() {
        ...
        for (source, action, reflow_request) in [
            ...
            let (output, _) = apply_prompt34(source, &request).expect("source-linked action");
            assert!(ContentEngine::open_bytes(output).is_ok());
        }
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::AnnotationAppearance,
            action: Some(Prompt34Action::AnnotationCreate {
                page: 1,
        ...
        let (output, report) = apply_prompt34(&input, &request).expect("annotation action");
        assert_eq!(
            report.operation,
            "annotation_create_and_appearance_regeneration"
        );
        ...
        let (output, report) = apply_prompt34(&input, &request).expect("free text action");
        assert_eq!(
            report.operation,
            "annotation_create_and_appearance_regeneration"
        );
    ...
    fn matrix_cell_edit_rewrites_source_and_undoes() {
        ...
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::Math,
            action: Some(Prompt34Action::MathEditMatrixCell {
                row: 1,
                col: 0,
        ...
        let (output, report) = apply_prompt34(&input, &request).expect("matrix cell source edit");
        assert_eq!(
            report.operation,
            "math_matrix_cell_source_rewrite_with_prompt32_shaping"
        );
        ...
        let (restored, _) = undo_prompt34(&input, &output, &request).expect("matrix undo");
        assert_eq!(restored, input);
    }
    ...
    fn matrix_structure_edit_rewrites_source_and_undoes() {
        ...
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::Math,
            action: Some(Prompt34Action::MathEditMatrixStructure {
        ...
        let (output, report) = apply_prompt34(&input, &request).expect("matrix structure edit");
        assert_eq!(
            report.operation,
            "math_matrix_structure_source_rewrite_with_prompt32_shaping"
        );
        ...
        let (restored, _) =
            undo_prompt34(&input, &output, &request).expect("matrix structure undo");
        assert_eq!(restored, input);
    }
    ...
    fn fraction_part_edit_rewrites_resolved_math_source_and_undoes() {
        ...
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::Math,
            action: Some(Prompt34Action::MathEditFractionPart {
        ...
        let (output, report) = apply_prompt34(&input, &request).expect("fraction part source edit");
        assert_eq!(
            report.operation,
            "math_fraction_part_source_rewrite_with_prompt32_shaping"
        );
        ...
        let (restored, _) = undo_prompt34(&input, &output, &request).expect("fraction undo");
        assert_eq!(restored, input);
    }
    ...
    fn script_edit_rewrites_resolved_math_source_and_undoes() {
        ...
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::Math,
            action: Some(Prompt34Action::MathEditScript {
        ...
        let (output, report) = apply_prompt34(&input, &request).expect("script source edit");
        assert_eq!(
            report.operation,
            "math_script_source_rewrite_with_prompt32_shaping"
        );
        ...
        let (restored, _) = undo_prompt34(&input, &output, &request).expect("script undo");
        assert_eq!(restored, input);
    }
    ...
    fn table_linked_annotation_moves_only_to_resolved_cell_and_undoes() {
        ...
        let (created, _) = apply_prompt34(&input, &create).expect("create source annotation");
        let (xfdf, _) = crate::prompt17::export_annotation_xfdf(
            &ContentEngine::open_bytes(created.clone()).expect("open created annotation"),
        )
        .expect("export source annotation");
        let annotation_id = crate::prompt17::parse_annotation_xfdf(&xfdf)
        ...
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::Table,
            action: Some(Prompt34Action::TableMoveLinkedAnnotation {
                table_id: table.table_id,
                row: 0,
                col: 0,
        ...
        let (output, report) =
            apply_prompt34(&created, &request).expect("move linked table annotation");
        assert_eq!(
            report.operation,
            "table_linked_annotation_source_move_and_appearance_regeneration"
        );
        ...
        let moved = crate::prompt17::parse_annotation_xfdf(&xfdf)
        ...
        let (restored, _) =
            undo_prompt34(&created, &output, &request).expect("table annotation undo");
    ...
    fn annotation_move_resize_preserves_source_identity_and_undoes() {
        ...
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::AnnotationAppearance,
            action: Some(Prompt34Action::AnnotationMoveResize {
                annotation_id: annotation_id.clone(),
                page: 1,
        ...
        let (output, report) = apply_prompt34(&created, &request).expect("move source annotation");
        assert_eq!(
            report.operation,
            "annotation_move_resize_source_update_and_appearance_regeneration"
        );
        ...
        let (restored, _) = undo_prompt34(&created, &output, &request).expect("annotation undo");
        assert_eq!(restored, created);
    }
    ...
    fn annotation_reply_uses_canonical_parent_relationship_and_undoes() {
        ...
        let (created, _) = apply_prompt34(&input, &create).expect("create parent annotation");
        ...
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::AnnotationAppearance,
            action: Some(Prompt34Action::AnnotationCreateReply {
        ...
        let (output, report) = apply_prompt34(&created, &request).expect("create reply annotation");
        assert_eq!(
            report.operation,
            "annotation_reply_source_update_and_appearance_regeneration"
        );
        ...
        let (restored, _) = undo_prompt34(&created, &output, &request).expect("reply undo");
        assert_eq!(restored, created);
    }
    ...
    fn provider_recorded_ocr_searchable_layer_preserves_scan_and_undoes() {
        ...
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::OcrSearchableLayer,
            action: Some(Prompt34Action::OcrAddSearchableText {
                page: 1,
        ...
        let (output, report) = apply_prompt34(&input, &request).expect("searchable layer");
        ...
        assert!(ContentEngine::open_bytes(output.clone())
            ...
            .contains("Scanned"));
        let (restored, _) = undo_prompt34(&input, &output, &request).expect("searchable undo");
        assert_eq!(restored, input);
    }
    ...
    fn searchable_ocr_geometry_correction_rewrites_source_and_undoes() {
        ...
        let (searchable, _) = apply_prompt34(&input, &create).expect("create searchable layer");
        let geometry = Prompt34Request {
            subsystem: Prompt34Subsystem::OcrSearchableLayer,
            action: Some(Prompt34Action::OcrCorrectGeometry {
        ...
        let (output, report) =
            apply_prompt34(&searchable, &geometry).expect("correct searchable geometry");
        assert_eq!(
            report.operation,
            "ocr_searchable_layer_source_geometry_correction"
        );
        assert!(ContentEngine::open_bytes(output.clone())
            ...
            .contains("Scanned"));
        let (restored, _) =
            undo_prompt34(&searchable, &output, &geometry).expect("geometry correction undo");
    ...
    fn provider_recorded_ocr_text_with_link_preserves_scan_and_undoes() {
        ...
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::OcrSearchableLayer,
            action: Some(Prompt34Action::OcrAddSearchableTextWithLink {
                page: 1,
                text: "Scanned".to_string(),
                rect,
                font_size: 12.0,
        ...
        let (output, report) = apply_prompt34(&input, &request).expect("OCR text with link");
        assert_eq!(
            report.operation,
            "ocr_searchable_layer_with_source_link_annotation"
        );
        ...
        assert!(reopened
            ...
            .contains("Scanned"));
        let (xfdf, _) =
            crate::prompt17::export_annotation_xfdf(&reopened).expect("export OCR link annotation");
        assert!(crate::prompt17::parse_annotation_xfdf(&xfdf)
            .expect("parse OCR link annotation")
            .annotations
            .iter()
            .any(|annotation| annotation.subtype == "Link" && annotation.rect == Some(rect)));
        let (restored, _) = undo_prompt34(&input, &output, &request).expect("OCR link undo");
        assert_eq!(restored, input);
    }
    ...
    fn provider_recorded_ocr_word_layer_is_atomic_and_searchable() {
        ...
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::OcrSearchableLayer,
            action: Some(Prompt34Action::OcrAddSearchableWords {
                page: 1,
                words: vec![
                    OcrSearchableWord {
                        text: "Scanned".to_string(),
        ...
        let (output, report) = apply_prompt34(&input, &request).expect("word layer");
        assert_eq!(report.operation, "ocr_searchable_layer_creation");
        let extracted = ContentEngine::open_bytes(output.clone())
            .expect("reopen searchable output")
            .get_page_text(1)
        ...
        let (restored, _) = undo_prompt34(&input, &output, &request).expect("word layer undo");
        assert_eq!(restored, input);
    }
    ...
    fn canonical_form_action_reopens_and_exposes_inverse_proof() {
        ...
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::FormData,
            action: Some(Prompt34Action::FormSetText {
                field_name: field.full_name.clone(),
                value: "Prompt34".to_string(),
            }),
            reflow: None,
            approved: true,
            form_data: None,
        ...
        let (output, report) = apply_prompt34(&input, &request).expect("form source edit");
        assert_eq!(report.operation, "acroform_source_edit");
        assert!(ContentEngine::open_bytes(output.clone()).is_ok());
        let (restored, inverse) = undo_prompt34(&input, &output, &request).expect("form inverse");
        assert_eq!(restored, input);
        assert_eq!(inverse["byte_exact_restoration"], Value::Bool(true));

        let default_request = Prompt34Request {
            subsystem: Prompt34Subsystem::FormData,
            action: Some(Prompt34Action::FormSetDefault {
                field_name: field.full_name.clone(),
                value: "Prompt34Default".to_string(),
            }),
            reflow: None,
            approved: true,
            form_data: None,
        ...
        let (defaulted, default_report) =
            apply_prompt34(&input, &default_request).expect("form default value");
        ...
        .any(|candidate| {
            candidate.full_name == field.full_name
                && candidate.default_value.as_deref() == Some("Prompt34Default")
        }));
        let (restored, _) =
            undo_prompt34(&input, &defaulted, &default_request).expect("form default undo");
        assert_eq!(restored, input);

        let import_request = Prompt34Request {
            subsystem: Prompt34Subsystem::FormData,
            action: Some(Prompt34Action::FormImportData {
        ...
        let (imported, import_report) =
            apply_prompt34(&input, &import_request).expect("form data import");
        assert_eq!(import_report.operation, "acroform_source_edit");
        assert!(ContentEngine::open_bytes(imported.clone()).is_ok());
        let (restored, _) =
            undo_prompt34(&input, &imported, &import_request).expect("form import undo");
        assert_eq!(restored, input);

        let rename_request = Prompt34Request {
            subsystem: Prompt34Subsystem::FormData,
            action: Some(Prompt34Action::FormRename {
                field_name: field.full_name.clone(),
                new_name: "Prompt34Renamed".to_string(),
            }),
            reflow: None,
            approved: true,
            form_data: None,
        ...
        let (renamed, rename_report) =
            apply_prompt34(&input, &rename_request).expect("form rename");
        ...
        .fields
        .iter()
        .any(|candidate| candidate.full_name.ends_with("Prompt34Renamed")));
        let (restored, _) =
            undo_prompt34(&input, &renamed, &rename_request).expect("form rename undo");
        assert_eq!(restored, input);

        let delete_request = Prompt34Request {
            subsystem: Prompt34Subsystem::FormData,
            action: Some(Prompt34Action::FormDelete {
                field_name: field.full_name.clone(),
            }),
            reflow: None,
            approved: true,
        ...
        let (deleted, delete_report) =
            apply_prompt34(&input, &delete_request).expect("form delete");
        ...
        let (restored, _) =
            undo_prompt34(&input, &deleted, &delete_request).expect("form delete undo");
        assert_eq!(restored, input);

        let reset_request = Prompt34Request {
            subsystem: Prompt34Subsystem::FormData,
            action: Some(Prompt34Action::FormReset {
        ...
        let (reset, reset_report) = apply_prompt34(&input, &reset_request).expect("form reset");
        assert_eq!(reset_report.operation, "acroform_source_edit");
        assert!(ContentEngine::open_bytes(reset.clone()).is_ok());
        let (restored, _) = undo_prompt34(&input, &reset, &reset_request).expect("form reset undo");
        assert_eq!(restored, input);
    }
    ...
    fn form_calculation_order_rewrites_co_and_undoes() {
        ...
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::FormData,
            action: Some(Prompt34Action::FormSetCalculationOrder {
        ...
        let (output, report) = apply_prompt34(&input, &request).expect("set calculation order");
        assert_eq!(report.operation, "acroform_source_edit");
        assert_eq!(
            crate::forms_report(
        ...
        let (restored, _) =
            undo_prompt34(&input, &output, &request).expect("calculation-order undo");
        assert_eq!(restored, input);
    }
    ...
    fn form_choice_widget_in_table_cell_uses_resolved_cell_bounds_and_undoes() {
        ...
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::FormData,
            action: Some(Prompt34Action::FormCreateChoiceInTableCell {
                table_id: table.table_id,
                row: 0,
                col: 0,
                field_name: "Prompt34TableChoice".to_string(),
        ...
        let (output, report) = apply_prompt34(&input, &request).expect("create table cell choice");
        assert_eq!(report.operation, "acroform_source_edit");
        ...
        let created = interactive
            .fields
            .iter()
            .find(|field| field.full_name == "Prompt34TableChoice")
        ...
        let (restored, _) =
            undo_prompt34(&input, &output, &request).expect("table choice creation undo");
        assert_eq!(restored, input);
    }
    ...
    fn form_checkbox_widget_in_table_cell_uses_resolved_cell_bounds_and_undoes() {
        ...
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::FormData,
            action: Some(Prompt34Action::FormCreateCheckboxInTableCell {
                table_id: table.table_id,
                row: 0,
                col: 0,
                field_name: "Prompt34TableCheck".to_string(),
                checked: true,
            }),
            reflow: None,
            approved: true,
        ...
        let (output, report) =
            apply_prompt34(&input, &request).expect("create table cell checkbox");
        assert_eq!(report.operation, "acroform_source_edit");
        let interactive = crate::forms_report(
        ...
        let created = interactive
            .fields
            .iter()
            .find(|field| field.full_name == "Prompt34TableCheck")
        ...
        let (restored, _) =
            undo_prompt34(&input, &output, &request).expect("table checkbox creation undo");
        assert_eq!(restored, input);
    }
    ...
    fn form_text_widget_in_table_cell_uses_resolved_cell_bounds_and_undoes() {
        ...
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::FormData,
            action: Some(Prompt34Action::FormCreateTextInTableCell {
                table_id: table.table_id,
                row: 0,
                col: 0,
                field_name: "Prompt34TableCell".to_string(),
        ...
        let (output, report) = apply_prompt34(&input, &request).expect("create table cell field");
        assert_eq!(report.operation, "acroform_source_edit");
        let interactive = crate::forms_report(
        ...
        let created = interactive
            .fields
            .iter()
            .find(|field| field.full_name == "Prompt34TableCell")
            .expect("created table field");
        assert!(created.widgets.iter().any(|widget| {
            widget.has_appearance && widget.rect.is_some_and(|rect| rect == cell_bounds)
        }));
        let (restored, _) =
            undo_prompt34(&input, &output, &request).expect("table field creation undo");
        assert_eq!(restored, input);
    }
    ...
    fn form_create_text_creates_widget_appearance_and_undoes() {
        ...
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::FormData,
            action: Some(Prompt34Action::FormCreateText {
                field_name: "Prompt34Text".to_string(),
                page: 1,
        ...
        let (output, report) = apply_prompt34(&input, &request).expect("create text field");
        assert_eq!(report.operation, "acroform_source_edit");
        let interactive = crate::forms_report(
        ...
        let created = interactive
            .fields
            .iter()
            .find(|field| field.full_name == "Prompt34Text")
            .expect("created field");
        assert_eq!(created.field_type, "text");
        assert!(created.widgets.iter().any(|widget| widget.has_appearance));
        let move_request = Prompt34Request {
            subsystem: Prompt34Subsystem::FormData,
            action: Some(Prompt34Action::FormMoveResizeWidget {
                field_name: "Prompt34Text".to_string(),
                page: 1,
        ...
        let (moved, _) = apply_prompt34(&output, &move_request).expect("move created widget");
        assert!(crate::forms_report(
        ...
        let (restored_created, _) =
            undo_prompt34(&output, &moved, &move_request).expect("created widget move undo");
        assert_eq!(restored_created, output);
        let update_request = Prompt34Request {
            subsystem: Prompt34Subsystem::FormData,
            action: Some(Prompt34Action::FormSetText {
                field_name: "Prompt34Text".to_string(),
        ...
        let (updated, _) = apply_prompt34(&output, &update_request).expect("update created field");
        assert!(crate::forms_report(
        ...
        .iter()
        .any(
            |field| field.full_name == "Prompt34Text" && field.value.as_deref() == Some("Updated")
        ));
        let (restored_created, _) =
            undo_prompt34(&output, &updated, &update_request).expect("created field update undo");
        assert_eq!(restored_created, output);
        let (restored, _) = undo_prompt34(&input, &output, &request).expect("created field undo");
        assert_eq!(restored, input);
    }
    ...
    fn form_create_checkbox_writes_named_appearances_and_undoes() {
        ...
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::FormData,
            action: Some(Prompt34Action::FormCreateCheckbox {
                field_name: "Prompt34Check".to_string(),
                page: 1,
        ...
        let (output, report) = apply_prompt34(&input, &request).expect("create checkbox");
        assert_eq!(report.operation, "acroform_source_edit");
        ...
        .fields
        .into_iter()
        .find(|field| field.full_name == "Prompt34Check")
        ...
        let default_request = Prompt34Request {
            subsystem: Prompt34Subsystem::FormData,
            action: Some(Prompt34Action::FormSetButtonDefault {
                field_name: "Prompt34Check".to_string(),
                checked: false,
            }),
            reflow: None,
        ...
        let (defaulted, _) =
            apply_prompt34(&output, &default_request).expect("set checkbox default");
        assert!(crate::forms_report(
        ...
        .iter()
        .any(|field| {
            field.full_name == "Prompt34Check"
                && field.value.as_deref() == Some("Yes")
                && field.default_value.as_deref() == Some("Off")
        }));
        let (restored_default, _) =
            undo_prompt34(&output, &defaulted, &default_request).expect("checkbox default undo");
        assert_eq!(restored_default, output);
        let (restored, _) = undo_prompt34(&input, &output, &request).expect("checkbox undo");
        assert_eq!(restored, input);
    }
    ...
    fn form_create_choice_writes_options_and_undoes() {
        ...
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::FormData,
            action: Some(Prompt34Action::FormCreateChoice {
                field_name: "Prompt34Choice".to_string(),
                page: 1,
        ...
        let (output, report) = apply_prompt34(&input, &request).expect("create choice");
        assert_eq!(report.operation, "acroform_source_edit");
        let created = crate::forms_report(
        ...
        .fields
        .into_iter()
        .find(|field| field.full_name == "Prompt34Choice")
        .expect("created choice field");
        assert_eq!(created.field_type, "choice");
        assert_eq!(created.value.as_deref(), Some("Two"));
        assert!(created.widgets.iter().any(|widget| widget.has_appearance));
        let option_request = Prompt34Request {
            subsystem: Prompt34Subsystem::FormData,
            action: Some(Prompt34Action::FormSetChoiceOptions {
                field_name: "Prompt34Choice".to_string(),
        ...
        let (updated, _) =
            apply_prompt34(&output, &option_request).expect("replace choice options");
        assert!(
            crate::forms_report(
            ...
            .fields
            .iter()
            .any(|field| field.full_name == "Prompt34Choice"
                && field.value.as_deref() == Some("Four"))
        );
        let (restored_choice, _) =
            undo_prompt34(&output, &updated, &option_request).expect("choice options undo");
        assert_eq!(restored_choice, output);
        let (restored, _) = undo_prompt34(&input, &output, &request).expect("choice undo");
        assert_eq!(restored, input);
    }
    ...
    fn form_create_push_button_writes_all_appearance_states_and_undoes() {
        ...
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::FormData,
            action: Some(Prompt34Action::FormCreatePushButton {
                field_name: "Prompt34Button".to_string(),
                page: 1,
        ...
        let (output, report) = apply_prompt34(&input, &request).expect("create push button");
        assert_eq!(report.operation, "acroform_source_edit");
        let created = crate::forms_report(
        ...
        .fields
        .into_iter()
        .find(|field| field.full_name == "Prompt34Button")
        .expect("created push button field");
        assert_eq!(created.field_type, "push_button");
        assert!(created.widgets.iter().any(|widget| widget.has_appearance));
        let (restored, _) = undo_prompt34(&input, &output, &request).expect("button undo");
        assert_eq!(restored, input);
    }
    ...
    fn form_create_radio_writes_export_state_and_undoes() {
        ...
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::FormData,
            action: Some(Prompt34Action::FormCreateRadio {
                field_name: "Prompt34Radio".to_string(),
                page: 1,
        ...
        let (output, report) = apply_prompt34(&input, &request).expect("create radio");
        assert_eq!(report.operation, "acroform_source_edit");
        let created = crate::forms_report(
        ...
        .fields
        .into_iter()
        .find(|field| field.full_name == "Prompt34Radio")
        .expect("created radio field");
        assert_eq!(created.field_type, "radio");
        assert_eq!(created.value.as_deref(), Some("Selected"));
        assert!(created.widgets.iter().any(|widget| widget.has_appearance));
        let (restored, _) = undo_prompt34(&input, &output, &request).expect("radio undo");
        assert_eq!(restored, input);
    }
    ...
    fn form_create_signature_field_is_unsigned_and_undoes() {
        ...
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::FormData,
            action: Some(Prompt34Action::FormCreateSignature {
                field_name: "Prompt34Signature".to_string(),
                page: 1,
        ...
        let (output, report) = apply_prompt34(&input, &request).expect("create signature field");
        assert_eq!(report.operation, "acroform_source_edit");
        ...
        .fields
        .into_iter()
        .find(|field| field.full_name == "Prompt34Signature")
        .expect("created signature field");
        assert!(field.is_signature);
        assert!(field.value.is_none());
        let (restored, _) = undo_prompt34(&input, &output, &request).expect("signature undo");
        assert_eq!(restored, input);
    }
    ...
    fn static_xfa_datasets_import_rewrites_only_datasets_packet_and_undoes() {
        ...
        let request = Prompt34Request {
            subsystem: Prompt34Subsystem::XfaPreservation,
            action: Some(Prompt34Action::XfaImportDatasets {
        ...
        let (output, report) = apply_prompt34(&input, &request).expect("import datasets");
        ...
        let (restored, _) = undo_prompt34(&input, &output, &request).expect("datasets undo");
        assert_eq!(restored, input);
    }
}
