//! Prompt 31 canonical provenance and operator-preserving editing adapters.
//!
//! This module deliberately composes the existing Prompt 20 parser-backed
...
pub const PROMPT31_SCHEMA_VERSION: &str = "prompt31.provenance-operator-editing.v1";

/// The requested editing contract.  Only [`OperatorPreserving`] is executable
/// in Prompt 31; callers receive an explicit route for later modes.
...
fn identity_from_candidate(
    input: &[u8],
    candidate: &crate::prompt20::SameWidthPatchEligibility,
...
pub fn operator_text_provenance(
    ...
    Ok(ProvenanceSelectionReport {
        schema_version: PROMPT31_SCHEMA_VERSION.to_string(),
        ...
        // do not yet carry stable source instruction IDs.  Prompt 32 owns that
        ...
        display_item_note: "The canonical display list is reused for rendering; stable display-item-to-instruction IDs are deferred to Prompt 32.".to_string(),
...
fn refusal_code(candidate: &crate::prompt20::SameWidthPatchEligibility) -> &'static str {
...
pub fn edit_text_operator(
    ...
    if !applied.output_reopened || !applied.replacement_extracts || !applied.old_text_absent {
        return Err(WellfriendError::MalformedPdf(
            "prompt31 validation_failed: source operator mutation did not reopen/extract cleanly"
                .to_string(),
        ));
    }
    ...
    Ok((
        ...
        OperatorEditOperationReport {
            ...
            validation: serde_json::json!({
                ...
                "replacement_extracts": applied.replacement_extracts,
                "old_text_absent": applied.old_text_absent,
                "canonical_writer": "prompt20_incremental_writer",
            }),
            output_revision,
        },
    ))
...
pub fn operator_path_provenance(input: &[u8], page: usize) -> Result<serde_json::Value> {
    Ok(serde_json::json!({
        "schema_version": PROMPT31_SCHEMA_VERSION,
        "requested_mode": TrueEditingMode::OperatorPreserving,
...
pub fn operator_image_eligibility(input: &[u8], page: usize) -> serde_json::Value {
    serde_json::json!({
        ...
        "prompt32_owner": "scene occurrence and image source-resolution closure",
    })
}

pub fn prompt31_report() -> serde_json::Value {
    serde_json::json!({
        "schema_version": PROMPT31_SCHEMA_VERSION,
        "status": "implemented_with_limits",
        "canonical_paths": {
            "text": "prompt20 same-width parser-backed stream operand patch",
            "path_and_graphics": "prompt20 vector source range mutation",
            "forms": "prompt20 explicit shared Form/appearance clone policy",
            "writer": "canonical incremental writer",
            "semantic": "prompt20 multi-run parser source spans",
        },
