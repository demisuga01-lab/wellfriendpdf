//! text reflow geometric and semantic reflow closure.
//!
//! This module extends the source editing/32 source-linked editing stack. It is a
...
//! flow edges, then compiles supported edits back through source editing/32 source
...
pub enum TextReflowStatus {
    Implemented,
    ImplementedWithLimits,
    Verified,
    VerifiedWithLimits,
    DeferredDocumentSubsystems,
    DeferredDocumentSecurity,
    UnsupportedExact,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum TextReflowEvidenceKind {
    ExactSourceFact,
...
/// a narrow extension of advanced editing's canonical vector mutator: callers name a
/// stable vector identity and an approved semantic dependency rather than
/// asking text reflow to move nearby/unknown artwork.  The current transaction
...
pub struct GeometricReflowRequest {
    ...
    /// bounded collision-free same-page path set through advanced editing's canonical
...
pub struct LayoutLine {
    ...
    pub source_link_status: TextReflowEvidenceKind,
}
...
pub struct LineBreakRecord {
    ...
    /// advanced editing source writer without changing logical extraction. Supported
    ...
    pub confidence: TextReflowEvidenceKind,
    pub reason: String,
}
...
pub struct SemanticRegionNode {
    ...
    pub evidence_kind: TextReflowEvidenceKind,
    pub confidence: Value,
...
pub struct SemanticRegionEdge {
    ...
    pub relationship: String,
    pub confidence: f64,
    pub exact_inferred_or_user_supplied: TextReflowEvidenceKind,
    pub source_evidence: Value,
    pub alternatives: Vec<Value>,
}
...
pub struct SemanticLayoutReport {
    ...
    /// the canonical Native Renderer/32 projections on every analysis, rather than
    /// being a documentation-only promise.
    pub region_graph_invariants: Value,
    pub review_required: Vec<Value>,
    pub document_subsystems_boundaries: Vec<String>,
    pub document_security_boundaries: Vec<String>,
}
...
pub struct ReflowTransactionReport {
    ...
    pub editing_transactions_transaction: Option<EditTransactionReport>,
}

/// Outcome of an executable text reflow undo.  The session stores no replacement
    ...
    pub fn with_max_operations(input: Vec<u8>, max_operations: usize) -> Result<Self> {
        if max_operations == 0 || max_operations > 100_000 {
            return Err(WellfriendError::ResourceLimit(
                "text_reflow reflow session operation limit must be between 1 and 100000"
                    .to_string(),
            ));
        }
    ...
    pub fn undo_reflow(&mut self) -> Result<ReflowUndoReport> {
        ...
        if edited_sha256 != checkpoint.after_sha256 || checkpoint.before_bytes > self.current.len()
        {
            return Err(WellfriendError::MalformedPdf(
                "text_reflow stale_snapshot_conflict: reflow undo checkpoint does not match current output"
                    .to_string(),
            ));
        }
        ...
        if restored_sha256 != checkpoint.before_sha256 {
            return Err(WellfriendError::MalformedPdf(
                "text_reflow undo_validation_failed: incremental truncation did not match recorded preimage"
                    .to_string(),
            ));
        }
        ...
        if page_count != checkpoint.before_page_count {
            return Err(WellfriendError::MalformedPdf(
                "text_reflow undo_validation_failed: restored page count does not match checkpoint"
                    .to_string(),
            ));
        }
    ...
    fn commit(&mut self, output: Vec<u8>, report: &ReflowTransactionReport) -> Result<()> {
        if self.cursor >= self.max_operations {
            return Err(WellfriendError::ResourceLimit(format!(
                "text_reflow resource_limit_exceeded: reflow session operation cap {} reached",
                self.max_operations
            )));
        }
...
/// Execute a binding-safe text reflow undo from immutable byte inputs.  Public
...
pub fn undo_reflow_from_replay(
    ...
    if session.bytes() != edited {
        return Err(WellfriendError::MalformedPdf(
            "text_reflow stale_snapshot_conflict: supplied output does not match deterministic reflow replay"
                .to_string(),
        ));
    }
    ...
    if !undo.undone || !undo.byte_exact_restoration || session.bytes() != input {
        return Err(WellfriendError::MalformedPdf(
            "text_reflow undo_validation_failed: replay session did not restore the exact preimage"
                .to_string(),
        ));
    }
...
fn sanitize_region(region: [f64; 4]) -> Result<[f64; 4]> {
    if region.iter().any(|v| !v.is_finite()) {
        return Err(WellfriendError::MalformedPdf(
            "text_reflow region geometry contains non-finite coordinates".to_string(),
        ));
    }
    ...
    if (x1 - x0) <= 0.0 || (y1 - y0) <= 0.0 {
        return Err(WellfriendError::MalformedPdf(
            "text_reflow region geometry has zero area".to_string(),
        ));
    }
    Ok([x0, y0, x1, y1])
}
...
fn allowed_expansion_for_request(
    ...
    if !contains_source || !inside_page {
        return Err(WellfriendError::UnsupportedFeature(
            "text_reflow constraint_infeasible: allowed expansion must contain the source region and remain inside the page box"
                .to_string(),
        ));
    }
    Ok(Some(expansion))
    ...
    if overflow_status == OverflowStatus::FitAfterRegionExpansion {
        return allowed_expansion_for_request(input, request, source)?.ok_or_else(|| {
            WellfriendError::MalformedPdf(
                "text_reflow expanded layout lacks an explicit validated expansion region"
                    .to_string(),
            )
        });
    }
...
/// Measure the exact shaped advance used by the advanced editing generated-Type0
...
fn shaped_advance(text: &str, direction: &str, font_size: f64) -> Result<f64> {
    let analysis = crate::advanced_editing::analyze_advanced_text_reflow(
        text,
        shaping_mode(direction),
        None,
        crate::advanced_editing::TextReflowLimits::default(),
    )?;
    if !analysis.missing_glyph_clusters.is_empty() {
        return Err(WellfriendError::UnsupportedFeature(
            "text_reflow shaping_failed: selected canonical generated font lacks one or more candidate clusters"
                .to_string(),
        ));
    }
    Ok(analysis
    ...
    boundaries
        ...
        .map(|offset| {
            ...
            LineBreakRecord {
                ...
                confidence: TextReflowEvidenceKind::DeterministicGeometry,
...
/// text reflow intentionally ships only two independently reviewed pattern
...
fn load_hyphenator(language: Language) -> Result<&'static Standard> {
    let cell = match language {
        ...
        _ => {
            return Err(WellfriendError::UnsupportedFeature(
                "text_reflow hyphenation_unavailable: no audited embedded dictionary for requested language"
                    .to_string(),
            ));
        }
    };
    Ok(cell.get_or_init(|| {
        Standard::from_embedded(language).expect(
            "TextReflow embeds only audited hyphenation dictionaries selected at compile time",
        )
    }))
}
...
fn range_advance(
    ...
    if *measured_spans >= MAX_FINAL_LAYOUT_CANDIDATE_SPANS {
        return Err(WellfriendError::UnsupportedFeature(format!(
            "text_reflow resource_limit_exceeded: final layout candidate span limit {} reached",
    ...
    ranges
        ...
        .map(|(index, (start, end))| -> Result<LayoutLine> {
            ...
            Ok(LayoutLine {
                ...
                hyphen_inserted,
                source_link_status: TextReflowEvidenceKind::DeterministicGeometry,
            })
        })
        .collect()
}
...
pub fn line_break_text(
    ...
    {
        return Err(WellfriendError::MalformedPdf(
            "text_reflow line layout received invalid dimensions".to_string(),
        ));
    }
    ...
    Ok(LineBreakingResult {
        ...
        pipeline: vec![
            ...
            "advanced_editing_rustybuzz_shaped_candidate_metrics".into(),
            "greedy_preview".into(),
...
pub fn analyze_geometric_region(
    ...
    Ok(GeometricTextRegion {
        ...
        style_runs: vec![json!({
            "source": "EditingTransactions font identity and scene style summary",
        ...
        // Only callers that name a AdvancedEditing stable vector identity plus an
        ...
        exclusion_zones: vec![json!({
            ...
            "locked": true,
            "reason": "TextReflow does not move unknown obstacles",
        })],
...
fn preview_reflow_internal(
    ...
    if request.requested_mode == TrueEditingMode::OperatorPreserving {
        return Err(WellfriendError::UnsupportedFeature(
            "text_reflow owns GeometricBlock and SemanticDocument; OperatorPreserving remains SourceEditing"
                .to_string(),
        ));
    }
...
fn refusal_for(
    ...
    if region.source_instructions.is_empty() {
        return Some(json!({
            ...
            "message": "requested text could not be linked to SourceEditing source instructions",
...
fn conformance_impact(_request: &GeometricReflowRequest) -> Value {
    json!({
        ...
        "pdfua": "tagged_structure_preserved_or_document_security_boundary_reported",
...
fn source_reflow_options(
    ...
    if max_lines == 0 {
        return Err(WellfriendError::UnsupportedFeature(
            "text_reflow constraint_infeasible: region has no complete line-height slot"
                .to_string(),
        ));
    }
    let alignment = match request.alignment.trim().to_ascii_lowercase().as_str() {
        ...
        other => {
            return Err(WellfriendError::UnsupportedFeature(format!(
                "text_reflow unsupported_alignment: {other}; supported values are left, right, center, start, end, justify"
            )))
        }
    };
    ...
    Ok(AdvancedTextEditOptions {
        ...
        // for Type0 text positioning. text reflow's line-height is deliberately
...
fn unique_scalar_range(haystack: &str, needle: &str) -> Result<[usize; 2]> {
    if needle.is_empty() {
        return Err(WellfriendError::UnsupportedFeature(
            "text_reflow paragraph_not_resolved: preserve_original_per_run requires nonempty source text"
                .to_string(),
        ));
    }
    ...
    if matches.len() != 1 {
        return Err(WellfriendError::UnsupportedFeature(format!(
            "text_reflow paragraph_not_resolved: preserve_original_per_run requires one unambiguous page logical source range; found {}",
            matches.len()
        )));
    }
...
fn apply_source_linked_reflow(
    ...
    if request.requested_mode != required_mode {
        return Err(WellfriendError::UnsupportedFeature(
            "text_reflow requested edit mode does not match the explicit source-linked application path"
                .to_string(),
        ));
    }
    ...
    // Both AdvancedEditing mutations are pure byte-to-byte transformations until this
    ...
    let (
        ...
        (
            ...
            json!({"path": "advanced_editing_source_token_removal_plus_generated_type0_text_stream", "detail": apply}),
            json!(apply.line_adjustments.clone()),
        )
    };
    if ContentEngine::open_bytes(output.clone()).is_err() {
        return Err(WellfriendError::MalformedPdf(
            "text_reflow output_reopen_failed after source reflow rewrite".to_string(),
        ));
    }
    let editing_transactions_request = SceneTextEditRequest {
    ...
    report.validation_evidence = json!({
        ...
        "independent_tools": "pending_text_reflow_vps_validation",
    });
    ...
    let editing_transactions_proxy = EditTransactionReport {
        schema_version: crate::editing_transactions::EDITING_TRANSACTIONS_SCHEMA_VERSION
            .to_string(),
        transaction_id: stable_id(
            "editing_transactions-proxy",
            &[request.source_text.as_bytes()],
        ),
    ...
    report.undo_proof = undo_restoration_report(input, &output, &editing_transactions_proxy);
    report.editing_transactions_transaction = Some(editing_transactions_proxy);
    Ok((output, report))
}
...
fn apply_single_paragraph_existing_target_flow(
    ...
    {
        return Err(WellfriendError::UnsupportedFeature(
            "text_reflow review_required: existing-next-page flow requires explicit SemanticDocument review approval"
                .to_string(),
        ));
    }
    let engine = ContentEngine::open_bytes(input.to_vec())?;
    if target_page == 0 || target_page > engine.page_count()? {
        return Err(WellfriendError::UnsupportedFeature(
            "text_reflow next_region_unavailable: the explicit downstream target page is unavailable"
                .to_string(),
        ));
    }
    ...
    {
        return Err(WellfriendError::UnsupportedFeature(
            "text_reflow structure_update_failed: existing-next-page flow refuses tagged structure until a source-linked structure repair transaction exists"
                .to_string(),
        ));
    }
    if target_semantic
    ...
    {
        return Err(WellfriendError::UnsupportedFeature(
            "text_reflow locked_object_conflict: canonical semantic text geometry occupies the proposed downstream target region"
                .to_string(),
        ));
    }
    ...
    if let Some(node) = target_scene.nodes.iter().find(|node| {
        node.page == target_page
            ...
            && node.node_kind != crate::editing_transactions::SceneNodeKind::TextObject
            && rects_intersect(node.bounds_user_space, target_region)
    }) {
        return Err(WellfriendError::UnsupportedFeature(
            format!(
                "text_reflow locked_object_conflict: the proposed downstream target region intersects visible {} scene node {}",
    ...
    {
        return Err(WellfriendError::UnsupportedFeature(
            "text_reflow structure_update_failed: existing-target flow refuses tagged structure until a source-linked structure repair transaction exists"
                .to_string(),
        ));
    }
    ...
    if max_lines == 0 || layout.lines.len() <= max_lines {
        return Err(WellfriendError::UnsupportedFeature(
            "text_reflow next_region_unavailable: requested paragraph does not have eligible bounded downstream overflow"
                .to_string(),
        ));
    }
    ...
    {
        return Err(WellfriendError::UnsupportedFeature(
            "text_reflow overflow_unresolved: downstream target capacity cannot hold the shaped continuation"
                .to_string(),
        ));
    }
    ...
    if format!("{first_text}{continuation_text}") != request.replacement_text {
        return Err(WellfriendError::MalformedPdf(
            "text_reflow downstream split did not preserve exact logical replacement text"
                .to_string(),
        ));
    }
    let options = source_reflow_options(request, source_region)?;
...
fn apply_single_paragraph_existing_next_region_flow(
    ...
    let target_region = request.next_region.ok_or_else(|| {
        WellfriendError::UnsupportedFeature(
            "text_reflow next_region_unavailable: no explicit next_region was supplied".to_string(),
        )
    })?;
    ...
    {
        return Err(WellfriendError::UnsupportedFeature(
            "text_reflow reading_order_ambiguous: next_region must be a disjoint below-source rectangle inside the source page box"
                .to_string(),
        ));
    }
...
fn apply_single_paragraph_existing_next_column_flow(
    ...
    if !matches!(direction.as_str(), "left_to_right" | "right_to_left") {
        return Err(WellfriendError::UnsupportedFeature(
            "text_reflow unsupported_writing_mode: explicit next-column flow supports only horizontal left-to-right or right-to-left text"
                .to_string(),
        ));
    }
    let source_region = region_for_request(input, request)?;
    let target_region = request.next_column.ok_or_else(|| {
        WellfriendError::UnsupportedFeature(
            "text_reflow next_column_unavailable: no explicit next_column was supplied".to_string(),
        )
    })?;
    let target_region = sanitize_region(target_region)?;
    let page = page_bounds(input, request.page)?;
        ...
        || !follows_source_in_column_order
    {
        return Err(WellfriendError::UnsupportedFeature(
            format!(
                "text_reflow reading_order_ambiguous: next_column must be a disjoint {} rectangle overlapping the source reading band",
...
fn apply_single_paragraph_existing_next_page_flow(
    ...
    if target_page > engine.page_count()? {
        return Err(WellfriendError::UnsupportedFeature(
            "text_reflow next_page_unavailable: no existing next page is available for this bounded flow"
                .to_string(),
        ));
    }
    let source_region = region_for_request(input, request)?;
    if page_bounds(input, request.page)? != page_bounds(input, target_page)? {
        return Err(WellfriendError::UnsupportedFeature(
            "text_reflow next_page_unavailable: existing-next-page flow requires identical page boxes"
                .to_string(),
        ));
    }
    apply_single_paragraph_existing_target_flow(
...
fn apply_single_paragraph_page_creation(
    ...
    if request.requested_mode != TrueEditingMode::SemanticDocument || !request.allow_page_creation {
        return Err(WellfriendError::UnsupportedFeature(
            "text_reflow page_creation_not_permitted: SemanticDocument plus explicit allow_page_creation is required"
                .to_string(),
        ));
    }
    if !request.approve_low_confidence_structure {
        return Err(WellfriendError::UnsupportedFeature(
            "text_reflow review_required: page creation requires explicit approval of the semantic structure"
                .to_string(),
        ));
    }
    let engine = ContentEngine::open_bytes(input.to_vec())?;
    ...
    if max_lines == 0 || line_breaking.lines.len() <= max_lines {
        return Err(WellfriendError::UnsupportedFeature(
            "text_reflow page_creation_not_required: no eligible page overflow exists for the explicit policy"
                .to_string(),
        ));
    }
    if line_breaking.lines.iter().any(|line| line.advance > width) {
        return Err(WellfriendError::UnsupportedFeature(
            "text_reflow overflow_unresolved: page creation cannot repair an over-wide unbreakable line"
                .to_string(),
        ));
    }
    ...
    {
        return Err(WellfriendError::MalformedPdf(
            "text_reflow page flow split did not preserve exact replacement text".to_string(),
        ));
    }
...
pub fn apply_reflow_document(
    ...
    if request.requested_mode != TrueEditingMode::SemanticDocument {
        return Err(WellfriendError::UnsupportedFeature(
            "text_reflow apply_semantic_reflow requires requested_mode=semantic_document"
                .to_string(),
        ));
    }
    ...
    if preliminary.overflow_status == OverflowStatus::UnresolvedOverflow {
        ...
        {
            return Err(WellfriendError::UnsupportedFeature(
                "text_reflow font_reconstruction_failed: preserve_original_per_run is supported only for a single local region; downstream, column, page, and page-creation flow require the generated Type0 font policy"
                    .to_string(),
            ));
        }
        if request.next_region.is_some() {
    ...
    // Native Renderer's exact glyph/word spans are deliberately not re-labelled as
    // source editing instruction IDs.  The application boundary therefore uses the
    // existing exact source editing provenance for *this requested source text* and
    ...
    // text reflow's incremental invalidation boundary.
    ...
    if source_region.source_instructions.is_empty() || paragraph_nodes.len() != 1 {
        return Err(WellfriendError::UnsupportedFeature(
            "text_reflow paragraph_not_resolved: SemanticDocument local application requires exactly one page-local semantic paragraph whose exact text matches the provenance-resolved source selection; duplicate or partial paragraph selections remain review-required"
                .to_string(),
        ));
    }
...
fn scene_node_kind_for_figure(
    kind: crate::editing_transactions::SceneNodeKind,
) -> Option<&'static str> {
    match kind {
        crate::editing_transactions::SceneNodeKind::ImageObject => Some("image"),
        crate::editing_transactions::SceneNodeKind::PathObject => Some("path"),
        _ => None,
    }
}
...
/// semantic block quads.  text reflow deliberately keeps this as a geometry
/// derivation layered on the existing Native Renderer layout model: it does not
...
/// is started.  This uses the existing advanced editing vector inventory and Roadmap task
...
fn validate_downstream_vector_moves(
    ...
    {
        return Err(WellfriendError::ResourceLimit(format!(
            "text_reflow resource_limit_exceeded: executable downstream movement supports at most {MAX_EXECUTABLE_DOWNSTREAM_VECTOR_MOVES} explicitly linked path object per transaction"
        )));
    }
    let engine = ContentEngine::open_bytes(input.to_vec())?;
    ...
    if interactive.forms.has_acroform || !interactive.annotations.annotations.is_empty() {
        return Err(WellfriendError::UnsupportedFeature(
            "text_reflow destination_update_failed: downstream vector movement refuses documents with forms or annotations until a source-linked association/rectangle repair transaction is available"
                .to_string(),
        ));
    }
    let page_box = engine.page_box(request.page).map_err(|_| {
        WellfriendError::UnsupportedFeature(
            "text_reflow region_not_resolved: downstream movement source page is unavailable"
                .to_string(),
        )
    })?;
    let source_region = region_for_request(input, request)?;
    ...
    for movement in &request.downstream_vector_moves {
        ...
        {
            return Err(WellfriendError::UnsupportedFeature(
                "text_reflow locked_object_conflict: downstream movement requires a non-empty source vector stable ID and explicit dependency edge ID"
                    .to_string(),
            ));
        }
        if !seen.insert(movement.vector_stable_id.clone()) {
            return Err(WellfriendError::UnsupportedFeature(
                "text_reflow locked_object_conflict: one source vector may not be moved twice in the same transaction"
                    .to_string(),
            ));
        }
        if !approved_downstream_vector_relationship(&movement.relationship) {
            return Err(WellfriendError::UnsupportedFeature(format!(
                "text_reflow ambiguous_layout: unsupported downstream dependency relationship {}; explicit approved relationships are keep_with_next, heading_for, list_continuation, caption_of, footnote_of, next_region, next_column, and next_page",
        ...
        {
            return Err(WellfriendError::MalformedPdf(
                "text_reflow constraint_infeasible: downstream movement requires finite non-zero dx or dy"
                    .to_string(),
            ));
        }
        let vector = inventory
            ...
            .ok_or_else(|| {
                WellfriendError::UnsupportedFeature(format!(
                    "text_reflow locked_object_conflict: explicit downstream vector {} is not a source-resolved editable path on page {}",
        ...
        {
            return Err(WellfriendError::UnsupportedFeature(
                "text_reflow structure_update_failed: downstream movement refuses vector paths in clipping, marked-content, or optional-content contexts"
                    .to_string(),
            ));
        }
        ...
        {
            return Err(WellfriendError::UnsupportedFeature(
                "text_reflow locked_object_conflict: a downstream vector owned by a Form XObject requires an explicit AdvancedEditing shared_form_policy"
                    .to_string(),
            ));
        }
        let own_scene_nodes = scene
            ...
            .iter()
            .filter(|node| {
                node.node_kind == crate::editing_transactions::SceneNodeKind::PathObject
                    && rects_nearly_equal(node.bounds_user_space, vector.bbox)
            })
            .collect::<Vec<_>>();
        if own_scene_nodes.len() != 1 {
            return Err(WellfriendError::UnsupportedFeature(format!(
                "text_reflow ambiguous_layout: vector {} does not have exactly one matching EditingTransactions scene occurrence",
        ...
        {
            return Err(WellfriendError::UnsupportedFeature(
                "text_reflow constraint_infeasible: downstream vector target lies outside the page box"
                    .to_string(),
            ));
        }
        if rects_intersect(target, source_region) {
            return Err(WellfriendError::UnsupportedFeature(
                "text_reflow locked_object_conflict: downstream vector target overlaps the edited text region"
                    .to_string(),
            ));
        }
        if let Some(node) = scene.nodes.iter().find(|node| {
            ...
            return Err(WellfriendError::UnsupportedFeature(format!(
                "text_reflow locked_object_conflict: downstream vector target overlaps source-linked {} scene node {}",
                format!("{:?}", node.node_kind).to_ascii_lowercase(),
                node.node_id
            )));
        }
        ...
        {
            return Err(WellfriendError::UnsupportedFeature(format!(
                "text_reflow locked_object_conflict: downstream vector target overlaps source-linked text line with hash {}",
        ...
        plans.push(json!({
            ...
            "evidence": TextReflowEvidenceKind::UserCorrection,
        }));
    }
    for (index, left) in plans.iter().enumerate() {
        let Some(left_bounds) = left["after_bounds"].as_array() else {
            return Err(WellfriendError::MalformedPdf(
                "text_reflow constraint_infeasible: downstream vector plan has invalid target bounds"
                    .to_string(),
            ));
        };
        let left_bounds = [
        ...
        for right in plans.iter().skip(index + 1) {
            ...
            if rects_intersect(left_bounds, right_bounds) {
                return Err(WellfriendError::UnsupportedFeature(
                    "text_reflow locked_object_conflict: two explicitly movable downstream vectors have overlapping target bounds"
                        .to_string(),
                ));
            }
        }
...
fn apply_downstream_vector_moves(
    ...
    // AdvancedEditing vector IDs include their decoded source operation range.  For
    ...
    for (index, movement) in execution {
        ...
        if reopened.page_count()? == 0 {
            return Err(WellfriendError::MalformedPdf(
                "text_reflow output_reopen_failed: downstream vector movement output has no page"
                    .to_string(),
            ));
        }
        output = next_output;
...
fn validate_downstream_link_moves(
    ...
    {
        return Err(WellfriendError::ResourceLimit(format!(
            "text_reflow resource_limit_exceeded: executable source-linked Link movement supports at most {MAX_EXECUTABLE_DOWNSTREAM_LINK_MOVES} annotation per transaction"
        )));
    }
    let engine = ContentEngine::open_bytes(input.to_vec())?;
    let source_region = region_for_request(input, request)?;
    let page_box = engine.page_box(request.page).map_err(|_| {
        WellfriendError::UnsupportedFeature(
            "text_reflow region_not_resolved: source-associated Link page is unavailable"
                .to_string(),
        )
    })?;
    let interactive = interactive_report(&engine)?;
    ...
    for movement in &request.downstream_link_moves {
        ...
        {
            return Err(WellfriendError::UnsupportedFeature(
                "text_reflow ambiguous_layout: source-associated Link movement requires a non-empty dependency edge and an approved relationship"
                    .to_string(),
            ));
        }
        ...
        {
            return Err(WellfriendError::MalformedPdf(
                "text_reflow constraint_infeasible: source-associated Link movement requires finite expected geometry and a non-zero finite delta"
                    .to_string(),
            ));
        }
        if !seen.insert(movement.annotation_index) {
            return Err(WellfriendError::UnsupportedFeature(
                "text_reflow locked_object_conflict: one Link annotation may not be moved twice in the same transaction"
                    .to_string(),
            ));
        }
        let annotation = interactive
            ...
            .ok_or_else(|| {
                WellfriendError::UnsupportedFeature(format!(
                    "text_reflow locked_object_conflict: Link annotation {} is not present on source page {}",
                    movement.annotation_index, request.page
                ))
            })?;
        if annotation.subtype != "Link" {
            return Err(WellfriendError::UnsupportedFeature(format!(
                "text_reflow locked_object_conflict: annotation {} is {}, not a source-associated Link",
                movement.annotation_index, annotation.subtype
            )));
        }
        let actual = annotation.rect.ok_or_else(|| {
            WellfriendError::MalformedPdf(
                "text_reflow structure_update_failed: source-associated Link has no finite /Rect"
                    .to_string(),
            )
        })?;
        if !rects_nearly_equal(actual, movement.expected_rect) {
            return Err(WellfriendError::UnsupportedFeature(
                "text_reflow stale_snapshot: source-associated Link rectangle differs from the request preimage"
                    .to_string(),
            ));
        }
        if !rects_intersect(actual, source_region) {
            return Err(WellfriendError::UnsupportedFeature(
                "text_reflow ambiguous_layout: explicit Link rectangle does not overlap the selected source region"
                    .to_string(),
            ));
        }
        let target = [
        ...
        {
            return Err(WellfriendError::UnsupportedFeature(
                "text_reflow constraint_infeasible: source-associated Link target lies outside the page box"
                    .to_string(),
            ));
        }
        if let Some(other) = interactive.annotations.annotations.iter().find(|other| {
            ...
            return Err(WellfriendError::UnsupportedFeature(format!(
                "text_reflow locked_object_conflict: source-associated Link target overlaps locked annotation {} on page {}",
    ...
    for (index, left) in plans.iter().enumerate() {
        let left_rect = left["after_rect"].as_array().ok_or_else(|| {
            WellfriendError::MalformedPdf(
                "text_reflow constraint_infeasible: source-associated Link plan has invalid target geometry"
                    .to_string(),
            )
        })?;
        let left_rect = [
        ...
        for right in plans.iter().skip(index + 1) {
            ...
            if rects_intersect(left_rect, right_rect) {
                return Err(WellfriendError::UnsupportedFeature(
                    "text_reflow locked_object_conflict: two explicitly movable source-associated Links have overlapping target rectangles"
                        .to_string(),
                ));
            }
        }
...
fn semantic_layout_from_graph(
    ...
    let mut add_node = |node: SemanticRegionNode| -> Result<String> {
        if nodes.len() >= MAX_RUNTIME_SEMANTIC_NODES {
            return Err(WellfriendError::ResourceLimit(format!(
                "text_reflow resource_limit_exceeded: semantic runtime node cap {MAX_RUNTIME_SEMANTIC_NODES}"
            )));
        }
    ...
    let mut add_edge = |source: String,
                        ...
                        relationship: &str,
                        confidence: f64,
                        evidence: TextReflowEvidenceKind,
    ...
    for page in semantic.pages {
        ...
        for (column_index, candidate) in column_candidates.iter().enumerate() {
            ...
            add_edge(
                ...
                TextReflowEvidenceKind::DeterministicGeometry,
        ...
        for scene_node in graph.nodes.iter().filter(|node| {
            ...
            let figure = add_node(SemanticRegionNode {
                ...
                evidence_kind: TextReflowEvidenceKind::ExactSourceFact,
            ...
            add_edge(
                ...
                "contains",
                0.98,
                TextReflowEvidenceKind::ExactSourceFact,
                json!({"parent": "page_region", "source": "editing_transactions_scene_occurrence"}),
            )?;
        ...
        for block in page.blocks {
            ...
            let list_parent = if block_kind == "list" {
                ...
                let list_node = add_node(SemanticRegionNode {
                    ...
                    bounds: quad_bounds(block.quad),
                    text_hash: digest_hex(block.text.as_bytes()),
                    evidence_kind: TextReflowEvidenceKind::HeuristicInference,
                    confidence: json!({
                        "geometry": 0.98,
                ...
                add_edge(
                    ...
                    "contains",
                    0.72,
                    TextReflowEvidenceKind::HeuristicInference,
            ...
            for line in &block.lines {
                ...
                for character in &line.chars {
                    ...
                    let glyph_node = add_node(SemanticRegionNode {
                        ...
                        evidence_kind: TextReflowEvidenceKind::DeterministicFontShaping,
                    ...
                    add_edge(
                        ...
                        TextReflowEvidenceKind::DeterministicFontShaping,
    ...
    Ok(SemanticLayoutReport {
        ...
        edges,
        algorithms_used: vec![
            "native_renderer_semantic_text_model".into(),
            "canonical_xy_cut_projection_profiles".into(),
            "canonical_docstrum_style_spacing_estimate".into(),
            "editing_transactions_scene_graph_source_occurrence_links".into(),
        ],
        exact_vs_inferred: json!({
            "source_structure_facts": "NativeRenderer MCID/structure context is retained where present",
...
pub fn transaction_undo_report(input: &[u8], request: &GeometricReflowRequest) -> Result<Value> {
    ...
    let transaction = match request.requested_mode {
        ...
        TrueEditingMode::OperatorPreserving => {
            return Err(WellfriendError::UnsupportedFeature(
                "text_reflow owns only geometric_block and semantic_document reflow undo"
                    .to_string(),
            ));
        }
    };
    ...
    Ok(json!({
        ...
        "redo_deterministic": "not_exposed_by_text_reflow_session",
...
pub fn text_reflow_report(input: &[u8]) -> Result<Value> {
    ...
    Ok(json!({
        ...
        "operator_preserving_owner": "SourceEditing",
        "no_duplicate_architecture": true,
        "canonical_inputs": [
            "SourceEditing provenance/source selection",
            "EditingTransactions scene/snapshot/transaction/font identity",
            "canonical writer and AdvancedEditing/31 source mutation"
        ],
        ...
        "no_overlay_no_clipping": true,
        "unknown_objects_locked_by_default": true,
        "tables_formulas_boundary": "deferred_document_subsystems",
        "accessibility_repair_boundary": "deferred_document_security",
        "exact_limits": [
...
pub fn text_reflow_feature_matrix() -> Value {
    json!({
        "schema_version": TEXT_REFLOW_SCHEMA_VERSION,
        "rows": [
        ...
        "no_blocked_text_reflow_rows": false,
    })
}
...
mod tests {
    ...
    fn fixture_with_source_link(content: &[u8]) -> Vec<u8> {
        ...
        action.insert(
            "URI",
            PdfObject::String(b"https://example.invalid/text_reflow-source-link".to_vec()),
        );
    ...
    fn fixture_with_two_source_links(content: &[u8]) -> Vec<u8> {
        ...
        action.insert(
            "URI",
            PdfObject::String(b"https://example.invalid/text_reflow-second-source-link".to_vec()),
        );
        let mut link = crate::PdfDictionary::empty();
        link.insert("Type", PdfObject::Name("Annot".to_string()));
        link.insert("Subtype", PdfObject::Name("Link".to_string()));
    ...
    fn explicit_source_linked_downstream_vector_move_rewrites_and_undoes_atomically() {
        ...
        let transaction = report
            .editing_transactions_transaction
            .as_ref()
            .expect("EditingTransactions transaction evidence");
        assert!(transaction
            .affected_objects
    ...
    #[test]
    fn final_line_advance_matches_advanced_editing_shaping() {
        let text = "office fi";
        let layout =
            line_break_text(text, 400.0, 80.0, 14.0, Some("en"), None, false).expect("layout");
        assert_eq!(layout.lines.len(), 1);
        let shaped = crate::advanced_editing::analyze_advanced_text_reflow(
            text,
    ...
    #[test]
    fn semantic_layout_labels_inference_and_document_subsystems_boundary() {
        ...
        assert!(report
            .document_subsystems_boundaries
            .iter()
    ...
    fn reading_order_resolver_removes_lowest_confidence_cycle_edge_deterministically() {
        ...
        let edge = |id: &str, source: &str, target: &str, confidence: f64| SemanticRegionEdge {
            ...
            confidence,
            exact_inferred_or_user_supplied: TextReflowEvidenceKind::HeuristicInference,
            source_evidence: json!({}),
            alternatives: Vec::new(),
        };
