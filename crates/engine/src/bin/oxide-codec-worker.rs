...
fn run() -> Result<(), String> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--version" => {
                println!("{}", oxide_engine::codec_isolation::CODEC_WORKER_VERSION);
                return Ok(());
            }
    ...
    match std::env::var("OXIDE_CODEC_WORKER_SELF_TEST")
        .ok()
        .as_deref()
    {
    ...
    let mode = std::env::var("OXIDE_CODEC_WORKER_SELF_TEST").ok();
