...
//! and every mutation is routed through the existing writer/editor and Roadmap task
...
pub const FORM_ACTION_POLICY_SCHEMA_VERSION: &str =
...
pub struct CalculationResult {
    ...
    pub status: FormActionPolicySupportStatus,
...
pub fn form_javascript_inventory(
    ...
    Ok(FormJsInventoryReport {
        schema_version: FORM_ACTION_POLICY_SCHEMA_VERSION.to_string(),
...
fn add_action_entry(
    ...
    context.entries.push(ActionInventoryEntry {
        ...
        signature_impact: "inventory_none; mutation_requires_secure_mutation_closeout_policy"
            .to_string(),
...
pub fn form_js_sanitize_pdf(
    ...
    if options.mode == FormJsPolicyMode::FlattenCalculatedValuesThenRemove {
        ...
        return Ok((
            ...
            FormJsSanitizerReport {
                ...
                deterministic: true,
                exact_limits: form_action_policy_action_limits(),
            },
        ));
    }
    ...
    let output = rewrite_document_with_mode(reader, WriterMode::ClassicXref, |_, object| {
        sanitize_form_action_policy_object(object, reader, options, 0)
    })?;
    ...
    let forbidden_remaining_count = after
        .actions
        .iter()
        .filter(|entry| action_removed_by_form_action_policy(&entry.action_type, None, options))
        .count();
...
fn enforce_signature_policy(decision: &EditPolicyDecision, override_policy: bool) -> Result<()> {
    ...
    ) && !override_policy
    {
        return Err(WellfriendError::UnsupportedFeature(
            "secure mutation closeout signature policy blocks full-rewrite sanitization without an explicit override"
                .to_string(),
        ));
    }
    Ok(())
...
fn sanitize_form_action_policy_object(
    ...
    let action_type = object
        ...
        .map(str::to_string);
    if action_type.as_deref().is_some_and(|action| {
        action_removed_by_form_action_policy(action, object.as_dict(), options)
    }) {
        *object = PdfObject::Null;
        return;
    }
        ...
        PdfObject::Dictionary(dict) | PdfObject::Stream { dict, .. } => {
            if options.mode != FormJsPolicyMode::InventoryOnly {
                ...
                if let Some(aa) = dict.get_mut("AA") {
                    ...
                    sanitize_form_action_policy_object(aa, reader, options, depth + 1);
            ...
            for (key, value) in dict.entries_mut() {
                ...
                sanitize_form_action_policy_object(value, reader, options, depth + 1);
            }
        }
        PdfObject::Array(items) => {
            for item in items.iter_mut() {
                sanitize_form_action_policy_object(item, reader, options, depth + 1);
            }
        }
        _ => {}
    }
...
fn action_value_removed(
    ...
    if let Some(dict) = resolved.as_dict() {
        ...
        action_removed_by_form_action_policy(action, Some(dict), options)
...
fn action_removed_by_form_action_policy(
    action: &str,
...
pub fn flatten_calculated_values_pdf(
    ...
    for action in actions {
            ...
            .is_some_and(|field| cycle_fields.contains(field))
        {
            results.push(CalculationResult {
                ...
                status: FormActionPolicySupportStatus::UnsupportedReportedExact,
                instructions: 0,
        ...
        match evaluate_calculation_script(
            ...
            Ok((target, value, instructions)) => {
                ...
                results.push(CalculationResult {
                    ...
                    status: FormActionPolicySupportStatus::ImplementedWithLimits,
                    instructions,
                    diagnostic: None,
                });
            }
            Err(error) => results.push(CalculationResult {
                ...
                original_value: original,
                calculated_value: None,
                status: FormActionPolicySupportStatus::UnsupportedReportedSecurityPolicy,
                instructions: 0,
    ...
    let output = rewrite_document_with_mode(
        ...
        |_, object| {
            sanitize_form_action_policy_object(
                object,
                filled_engine.document().reader(),
                &sanitize_options,
                0,
    ...
    let unsupported_scripts = results
        ...
        .filter(|result| {
            !matches!(
                result.status,
                FormActionPolicySupportStatus::Implemented
                    | FormActionPolicySupportStatus::ImplementedWithLimits
            )
        })
        .count();
    Ok((
...
pub fn interactive_data_closeout_report(engine: &ContentEngine) -> Result<serde_json::Value> {
    ...
    let associated = crate::secure_mutation::associated_files_inventory(engine)?;
    let signatures = analyze_edit_policy(engine, EditOperation::IncrementalSave)?;
    Ok(json!({
        "schema_version": FORM_ACTION_POLICY_SCHEMA_VERSION,
        ...
        "consistency": {
            ...
            "popup_reply_relationships": "implemented_with_limits_annotation_media_redaction",
            "associated_file_ownership": "implemented_with_limits_secure_mutation_closeout",
            "coordinate_transforms": "implemented_with_rotation_limits",
            "sanitizer_disposition": "implemented_form_action_policy_action_policy",
...
pub fn form_action_policy_report(engine: &ContentEngine) -> Result<serde_json::Value> {
    ...
    Ok(json!({
        ...
        "feature": form_action_policy_feature_report_value(crate::sdk::REPORT_ENVELOPE_VERSION)
    }))
}

pub(crate) fn form_action_policy_feature_report_value(envelope_version: u32) -> serde_json::Value {
    json!({
        "schema_version": FORM_ACTION_POLICY_SCHEMA_VERSION,
        ...
        "unsupported_exact": form_action_policy_action_limits()
    })
}

pub fn form_action_policy_policy_matrix() -> serde_json::Value {
    let rows = [
        (
...
fn form_action_policy_action_limits() -> Vec<String> {
    vec![
        ...
        "full-rewrite sanitizer operations are subject to secure mutation closeout DocMDP/FieldMDP enforcement and explicit override policy".to_string(),
    ]
}
...
mod tests {
    ...
    fn policy_matrix_has_no_blocked_row() {
        let value = form_action_policy_policy_matrix();
        assert_eq!(value["rows"].as_array().unwrap().len(), 7);
        assert_eq!(
            form_action_policy_feature_report_value(1)["failure"]["blocked"],
            0
        );
    }
}
