...
impl PdfSource {
    fn from_path(path: impl AsRef<Path>) -> Result<Self> {
        ...
        let len = usize::try_from(len).map_err(|_| {
            WellfriendError::ResourceLimit("input file is too large for this platform".to_string())
        })?;
    ...
    fn read_at(&self, offset: usize, len: usize) -> Result<Vec<u8>> {
        let end = offset
            .checked_add(len)
            .ok_or_else(|| WellfriendError::MalformedPdf("read range overflows".to_string()))?;
        if end > self.len() {
            return Err(WellfriendError::ParseError(format!(
...
impl PdfReader {
    ...
    pub fn from_bytes_strict_with_password(data: Vec<u8>, password: &[u8]) -> Result<Self> {
        ...
        let trailer = trailer.ok_or_else(|| {
            WellfriendError::MalformedPdf("PDF did not contain a trailer dictionary".to_string())
        })?;
    ...
    pub fn get_object(&self, number: u32, generation: u16) -> Result<PdfObject> {
        ...
        match entry {
            XrefEntry::Free => Err(WellfriendError::MissingObject { number, generation }),
    ...
    pub(crate) fn content_stream_range(
        ...
        let length = usize::try_from(length)
            .map_err(|_| WellfriendError::MalformedPdf("stream Length is too large".to_string()))?;
        let stream_start = offset.checked_add(header.stream_start).ok_or_else(|| {
            WellfriendError::MalformedPdf("stream start offset overflows".to_string())
        })?;
        let stream_end = stream_start
            .checked_add(length)
            .ok_or_else(|| WellfriendError::MalformedPdf("stream Length overflows".to_string()))?;
    ...
    fn parse_object_stream(&self, stream_obj: u32) -> Result<HashMap<u32, (u32, PdfObject)>> {
        ...
        let (dict, raw) = stream.as_stream().ok_or_else(|| {
            WellfriendError::MalformedPdf(format!("object {stream_obj} 0 is not an object stream"))
        })?;
...
fn setup_encryption(
    ...
    let encrypt_dict = match resolve_encrypt_dict(source, xref, encrypt_obj)? {
        Some(dict) => dict,
        None => return Err(WellfriendError::EncryptedDocument),
    };

    let info = match EncryptionInfo::from_dict(&encrypt_dict) {
        Ok(info) => info,
        Err(_) => return Err(WellfriendError::EncryptedDocument),
    };
...
pub(crate) fn parse_xref_stream_entries(
    ...
    let entry_len = w0
        ...
        .ok_or_else(|| WellfriendError::MalformedPdf("xref entry width overflows".to_string()))?;
    if entry_len == 0 {
        return Err(WellfriendError::MalformedPdf(
    ...
    for (start, count) in ranges {
        for relative in 0..count {
            let end = cursor.checked_add(entry_len).ok_or_else(|| {
                WellfriendError::MalformedPdf("xref stream cursor overflows".to_string())
            })?;
            if end > raw.len() {
                return Err(WellfriendError::MalformedPdf(
            ...
            let relative = u32::try_from(relative).map_err(|_| {
                WellfriendError::MalformedPdf("xref stream /Index count exceeds u32".to_string())
            })?;
            let object_number = start.checked_add(relative).ok_or_else(|| {
                WellfriendError::MalformedPdf("xref stream object number overflows".to_string())
            })?;
            ...
            match field0 {
                ...
                1 => {
                    let generation = u16::try_from(field2).map_err(|_| {
                        WellfriendError::MalformedPdf(
                            "xref generation does not fit in u16".to_string(),
                        )
                    })?;
                    entries.push((
                        object_number,
...
pub(crate) fn parse_object_stream_data(
    ...
    for index in 0..n {
        ...
        table.push((
            ...
            u32::try_from(index).map_err(|_| {
                WellfriendError::MalformedPdf("object stream index does not fit in u32".to_string())
            })?,
    ...
    for (object_number, index, relative_offset) in table {
        let object_offset = first.checked_add(relative_offset).ok_or_else(|| {
            WellfriendError::MalformedPdf("object stream offset overflows".to_string())
        })?;
...
fn required_integer_array(dict: &PdfDictionary, key: &str) -> Result<Vec<i64>> {
    let array = dict.get_array(key).ok_or_else(|| {
        WellfriendError::MalformedPdf(format!("required dictionary key /{key} is missing"))
    })?;
...
fn parse_index_array(index: &[PdfObject]) -> Result<Vec<(u32, usize)>> {
    ...
    for pair in index.chunks(2) {
        let start = pair[0].as_integer().ok_or_else(|| {
            WellfriendError::MalformedPdf("xref /Index start must be an integer".to_string())
        })?;
        let count = pair[1].as_integer().ok_or_else(|| {
            WellfriendError::MalformedPdf("xref /Index count must be an integer".to_string())
        })?;
        if start < 0 || count < 0 {
            return Err(WellfriendError::MalformedPdf(
        ...
        ranges.push((
            u32::try_from(start).map_err(|_| {
                WellfriendError::MalformedPdf("xref /Index start does not fit in u32".to_string())
            })?,
            usize::try_from(count).map_err(|_| {
                WellfriendError::MalformedPdf("xref /Index count is too large".to_string())
            })?,
        ));
    }
    Ok(ranges)
    ...
    match dict.get(key) {
        Some(PdfObject::Integer(value)) => {
            ...
            Ok(Some(usize::try_from(*value).map_err(|_| {
                WellfriendError::MalformedPdf(format!("/{key} offset is too large"))
            })?))
        }
...
fn nonnegative_usize(value: i64, label: &str) -> Result<usize> {
    ...
    usize::try_from(value)
        .map_err(|_| WellfriendError::MalformedPdf(format!("{label} is too large")))
}
...
fn parse_header_version(data: &[u8]) -> Result<String> {
    ...
    let header_offset = data[..search_len]
        ...
        .ok_or_else(|| WellfriendError::MalformedPdf("missing PDF header".to_string()))?;
    ...
    let version = std::str::from_utf8(&data[version_start..version_end])
        .map_err(|err| WellfriendError::MalformedPdf(format!("PDF version is not UTF-8: {err}")))?;
...
fn find_startxref(data: &[u8]) -> Result<usize> {
    ...
    let marker_pos = rfind_marker_accelerated(data, marker)
        .ok_or_else(|| WellfriendError::MalformedPdf("missing startxref".to_string()))?;
