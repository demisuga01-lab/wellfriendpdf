...
fn fixture() -> Vec<u8> {
    ...
    builder.add("<< /Producer (Prompt18 fixture) >>");
    builder.add("<< /Title (Prompt18) >>");
    builder.build()
}
...
fn associated_file_add_extract_dedup_remove_and_rescan_are_real_mutations() {
    ...
    let (twice, second_report) =
        associated_files_add_pdf(&once, &request, b"PROMPT18-EVIDENCE").unwrap();
    ...
    assert_eq!(payload, b"PROMPT18-EVIDENCE");
...
fn signature_policy_separates_structural_crypto_and_incremental_prefix() {
    ...
    let (output, report) =
        incremental_metadata_update_pdf(&input, "Subject", "Prompt18", false).unwrap();
