...
fn export_fixture(name: &str, bytes: &[u8]) {
    let Some(root) = std::env::var_os("WELLFRIENDPDF_SECURE_MUTATION_CLOSEOUT_EXPORT_FIXTURES")
    else {
        return;
    };
...
fn image_fixture() -> Vec<u8> {
    ...
    builder.info("<< /Title (secure mutation closeout images) >>");
    builder.build()
}
...
fn owners_fixture() -> Vec<u8> {
    ...
    builder.info("<< /Title (secure mutation closeout owners) >>");
    builder.build()
}
...
fn signed_mutation_fixture() -> Vec<u8> {
    ...
    builder.info("<< /Title (secure mutation closeout signed edits) >>");
    builder.build()
}
...
#[test]
fn pades_ltv_signature_preserving_form_fill_plans_applies_and_revalidates() {
    let input = signed_mutation_fixture();
    ...
    assert!(
        ...
        "the fixture signature is intentionally non-cryptographic; Pades LTV must report, not fake, preservation"
    );
}

#[test]
fn secure_mutation_closeout_public_report_has_zero_blocked_or_security_failures() {
    let report: serde_json::Value = serde_json::from_str(
        &wellfriendpdf_engine::sdk::secure_mutation_closeout_report_json(&owners_fixture(), None)
            .unwrap(),
    )
    .unwrap();
    ...
    assert_eq!(
        feature["report"]["secure_mutation_closeout_advanced_secure_mutation_closure"]["status"],
