//! A **C-function-pointer OCR backend** for the Wellfriend seam.
//!
//! This lets a non-Rust integrator supply OCR to Wellfriend through the C ABI: they
//! provide a `recognize` function pointer (plus opaque `userdata`), Wellfriend renders
...
//!
//! ```c
//! // Wellfriend hands the callee this sink to report each recognized word. Call it
...
//! typedef void (*wellfriendpdf_ocr_emit_word_fn)(
...
//! // recognition failure (Wellfriend degrades that page to the placeholder). `gray`
//! // is width*height 8-bit grayscale, row-major, top-left origin.
//! typedef int (*wellfriendpdf_ocr_recognize_fn)(
//!     void* userdata,
//!     const uint8_t* gray, uint32_t width, uint32_t height,
//!     uint32_t dpi,
//!     void* sink, wellfriendpdf_ocr_emit_word_fn emit);
//!
//! typedef struct {
//!     void* userdata;                    // opaque, passed back to recognize
//!     wellfriendpdf_ocr_recognize_fn recognize;  // required
...
use wellfriendpdf_engine::{OcrEngine, OcrImage, OcrOptions, OcrPage, OcrWord, WellfriendError};

/// Sink callback Wellfriend passes to the C `recognize`; called once per word.
pub type WellfriendOcrEmitWordFn = extern "C" fn(
    sink: *mut c_void,
    text: *const c_char,
    x0: f64,
    y0: f64,
...
pub type WellfriendOcrRecognizeFn = extern "C" fn(
    userdata: *mut c_void,
    gray: *const u8,
    width: u32,
    height: u32,
...
#[repr(C)]
#[derive(Clone, Copy)]
pub struct WellfriendOcrBackend {
    pub userdata: *mut c_void,
    pub recognize: Option<WellfriendOcrRecognizeFn>,
...
impl CAbiOcrEngine {
    ...
    pub unsafe fn from_descriptor(backend: WellfriendOcrBackend) -> Option<Self> {
        backend.recognize?;
