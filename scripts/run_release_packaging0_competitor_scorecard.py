#!/usr/bin/env python3
"""Generate a bounded, evidence-first Release Readiness Benchmark competitor scorecard."""
...
def main() -> int:
    ...
    scorecard = {
        "schema_version": "release_readiness_benchmark.final-competitor-scorecard.v1",
        ...
        "wellfriendpdf": {
            ...
            "performance": "evidence from Release Readiness Benchmark performance harness",
    ...
    write_json(args.artifact_root / "competitor-tool-support-matrix.json", {"schema_version": "release_readiness_benchmark.competitor-tool-support.v1", "generated_at_utc": utc(), "tools": support, "verdict": "passed"})
    write_json(args.artifact_root / "final-competitor-scorecard.json", scorecard)
    write_json(args.artifact_root / "release-readiness-go-no-go.json", {"schema_version": "release_readiness_benchmark.release-readiness-go-no-go.v1", "generated_at_utc": utc(), "verdict": "release-ready, with documented boundaries" if scorecard["verdict"] == "passed" else "not_release_ready", "basis": ["bounded public-corpus scorecard", "Crypto Standards Fuzz-29 fuzz/corpus evidence", "Release Readiness Benchmark package/security gates"], "exact_limits": scorecard["wellfriendpdf"]["known_limits"]})
