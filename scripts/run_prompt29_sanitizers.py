#!/usr/bin/env python3
"""Prompt 29 bounded sanitizer runner.
...
def main() -> int:
    ...
    support_rows = [
        ...
        {"sanitizer": "thread", "available": False, "status": "unsupported_reported_exact", "reason": "TSan is tracked in CI posture; not run for this bounded Prompt 29 VPS gate"},
    ]
    runs = []
    ...
    write_json(
        ...
        {"schema_version": "prompt29.sanitizer-support-matrix.v1", "generated_at_utc": utc(), "rows": support_rows, "verdict": "passed"},
    )
    write_json(
        artifact_root / "sanitizer-run-results.json",
        {"schema_version": "prompt29.sanitizer-run-results.v1", "generated_at_utc": utc(), "runs": runs, "failure_count": len(failures), "verdict": "passed" if not failures else "failed"},
    )
    write_json(
        artifact_root / "sanitizer-failure-triage.json",
        {"schema_version": "prompt29.sanitizer-failure-triage.v1", "generated_at_utc": utc(), "findings": failures, "unclassified_count": len(failures), "verdict": "passed" if not failures else "failed"},
    )
