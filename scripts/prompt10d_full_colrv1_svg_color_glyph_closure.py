#!/usr/bin/env python3
"""Prompt 10D full COLRv1/SVG/bitmap color glyph closure harness."""
...
FIXTURE_DIR = OUT_DIR / "prompt10d-fixtures"
RENDER_DIR = OUT_DIR / "prompt10d-renders"
DIFF_DIR = OUT_DIR / "prompt10d-diffs"
LOG_DIR = OUT_DIR / "prompt10d-logs"
OXIDE_REPORT_DIR = OUT_DIR / "prompt10d-oxide-render-reports"
HTML_REPORT = OUT_DIR / "prompt10d-html-report" / "index.html"
TOOL_MANIFEST = OUT_DIR / "reference-tool-manifest-prompt10.json"

CLOSURE_AUDIT = OUT_DIR / "prompt10d-closure-audit.json"
RENDER_RESULTS = OUT_DIR / "multi-reference-render-results-prompt10d.json"
DIFF_METRICS = OUT_DIR / "multi-reference-diff-metrics-prompt10d.json"
DISAGREEMENT_SUMMARY = OUT_DIR / "reference-disagreement-summary-prompt10d.json"
PUBLIC_FEATURE_REPORT = OUT_DIR / "public-feature-report-prompt10d.json"
BINDING_REPORT = OUT_DIR / "prompt10d-binding-report-parity.json"

MATRIX_FILES = {
    "linear": OUT_DIR / "colrv1-linear-gradient-matrix-prompt10d.json",
    "radial": OUT_DIR / "colrv1-radial-gradient-matrix-prompt10d.json",
    "sweep": OUT_DIR / "colrv1-sweep-gradient-matrix-prompt10d.json",
    "gradient_results": OUT_DIR / "colrv1-gradient-reference-results-prompt10d.json",
    "clip": OUT_DIR / "colrv1-clip-matrix-prompt10d.json",
    "clip_results": OUT_DIR / "colrv1-clip-reference-results-prompt10d.json",
    "composite": OUT_DIR / "colrv1-composite-matrix-prompt10d.json",
    "composite_results": OUT_DIR / "colrv1-composite-reference-results-prompt10d.json",
    "svg": OUT_DIR / "svg-opentype-static-rendering-matrix-prompt10d.json",
    "svg_policy": OUT_DIR / "svg-opentype-security-policy-prompt10d.json",
    "svg_results": OUT_DIR / "svg-opentype-reference-results-prompt10d.json",
    "bitmap": OUT_DIR / "bitmap-color-glyph-nonpng-matrix-prompt10d.json",
    "cbdt_results": OUT_DIR / "cbdt-cblc-nonpng-results-prompt10d.json",
    "sbix_results": OUT_DIR / "sbix-nonpng-results-prompt10d.json",
}
...
def bootstrap_reference_manifest(dpi: int, timeout: int) -> dict[str, Any]:
    ...
    if missing:
        raise RuntimeError(f"Prompt 10D requires reference renderers: {', '.join(missing)}")
    return manifest
...
def generate_fixtures(p10b: Any) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    svg_font, svg_gid = make_rect_font(
        FIXTURE_DIR / "prompt10d-static-svg.ttf",
    ...
    svg_transform_font, svg_transform_gid = make_rect_font(
        FIXTURE_DIR / "prompt10d-static-svg-transform.ttf",
    ...
    sbix_jpeg_font, sbix_jpeg_gid = make_rect_font(
        FIXTURE_DIR / "prompt10d-sbix-jpeg.ttf",
        sbix_jpeg=True,
    )
    latin = Path(r"C:\Windows\Fonts\arial.ttf")
    if not latin.exists():
        raise RuntimeError(f"required Prompt 10D fixture base font missing: {latin}")
    sbix_png_font, sbix_png_gid = p10b.make_sbix_font(latin, FIXTURE_DIR / "prompt10d-sbix-png-regression.ttf")

    safe_svg_pdf = FIXTURE_DIR / "prompt10d-safe-static-svg.pdf"
    ...
    svg_transform_pdf = FIXTURE_DIR / "prompt10d-safe-static-svg-transform.pdf"
    p10b.make_identity_pdf(
        svg_transform_pdf,
        svg_transform_font,
        [svg_transform_gid],
    ...
    sbix_jpeg_pdf = FIXTURE_DIR / "prompt10d-sbix-jpeg.pdf"
    p10b.make_identity_pdf(
        sbix_jpeg_pdf,
        sbix_jpeg_font,
        [sbix_jpeg_gid],
    ...
    sbix_png_pdf = FIXTURE_DIR / "prompt10d-sbix-png-regression.pdf"
    p10b.make_identity_pdf(
        sbix_png_pdf,
        sbix_png_font,
        [sbix_png_gid],
    ...
    entries = [
        {
            "id": "prompt10d_svg_static_path_shape",
        ...
        {
            "id": "prompt10d_svg_static_transform",
            "category": "color_glyph/svg_static_subset",
        ...
        {
            "id": "prompt10d_sbix_png_regression",
            "category": "color_glyph/sbix_png_regression",
            "path": rel(sbix_png_pdf),
            "page": 1,
            "capabilities": ["sbix PNG regression", "Prompt 10B behavior preservation"],
        },
    ]
    metadata = {
        "generated_fonts": {
        ...
        "policy_only_rows": [
            "prompt10d_colrv1_linear_gradient",
            "prompt10d_colrv1_radial_gradient",
            "prompt10d_colrv1_sweep_gradient",
            "prompt10d_colrv1_clip",
            "prompt10d_colrv1_clip_box",
            "prompt10d_colrv1_non_source_over_composites",
            "prompt10d_svg_blocked_script",
            "prompt10d_svg_blocked_event",
            "prompt10d_svg_blocked_external_reference",
            "prompt10d_svg_blocked_foreign_object",
            "prompt10d_svg_blocked_animation",
            "prompt10d_svg_path_bomb",
            "prompt10d_cbdt_ambiguous_compressed_payload",
            "prompt10d_sbix_tiff_no_safe_decoder",
            "prompt10d_sbix_pdf_mask_unknown_tags",
        ],
    }
...
def render_compare(
    ...
    for entry in entries:
        ...
        classification = classify_prompt10d(raw, entry, pair_metrics)
        page = {
            "id": entry["id"],
            ...
            "prompt10d_classification": classification,
            "renders": renders,
    ...
    summary = {
        "schema_version": 1,
        "kind": "prompt10d_reference_disagreement_summary",
        "page_count": len(pages),
        "fixture_count": len(pages) + 15,
        "classification_counts": counts(page["prompt10d_classification"] for page in pages),
        "oxide_outlier_failures": sum(
            1
            for page in pages
            if page["prompt10d_classification"] in {"oxide_outlier_failure", "oxide_render_failure"}
        ),
        "unclassified_failures": sum(1 for page in pages if page["prompt10d_classification"] == "unclassified_failure"),
        "reference_disagreements": [
            {"id": page["id"], "classification": page["prompt10d_classification"]}
            for page in pages
            if "reference_disagreement" in page["prompt10d_classification"]
        ],
        "policy_only_rows": {
    ...
    results = {
        "schema_version": 1,
        "kind": "prompt10d_multi_reference_render_results",
        "dpi": dpi,
        "page_count": len(pages),
    ...
    metrics = {"schema_version": 1, "kind": "prompt10d_multi_reference_diff_metrics", "pages": metrics_pages}
...
def classify_prompt10d(raw: str, entry: dict[str, Any], pair_metrics: dict[str, Any]) -> str:
    ...
    oxide_matches = sum(1 for pair in oxide_pairs if pair.get("threshold_pass"))
    if oxide_matches >= 1:
        return "reference_disagreement_oxide_inside_cluster"
    if all(pair.get("status") == "computed" for pair in oxide_pairs):
        ...
        if max_mean <= 10.0 and max_changed8 <= 0.15:
            return "reference_disagreement_oxide_within_prompt10d_threshold"
...
def render_html(pages: list[dict[str, Any]], summary: dict[str, Any]) -> None:
    ...
    for page in pages:
        ...
        rows.append(
            ...
            f"<td>{html.escape(page['prompt10d_classification'])}</td>"
    ...
    HTML_REPORT.write_text(
        ...
        "<title>Prompt 10D Closure Harness</title>"
        ...
        "<h1>Prompt 10D Closure Harness</h1>"
        ...
        "<h2>Rendered Pages</h2><table><tr><th>Fixture</th><th>Category</th><th>Prompt 10D</th>"
...
def write_matrices(metadata: dict[str, Any], render_payload: dict[str, Any] | None) -> None:
    ...
    write_json(
        MATRIX_FILES["composite"],
        {
            **common,
            ...
            "reason": "non-SourceOver modes require isolated bounded glyph paint surfaces before Prompt 07 blend machinery can be reused",
        },
    )
    ...
    svg_rows = [
        {"id": "prompt10d_svg_static_path_shape", "status": pages.get("prompt10d_svg_static_path_shape", {}).get("prompt10d_classification", "not_run")},
        {"id": "prompt10d_svg_static_transform", "status": pages.get("prompt10d_svg_static_transform", {}).get("prompt10d_classification", "not_run")},
        {"id": "prompt10d_svg_blocked_script", "status": "unsupported_reported_security_policy", "reason": "script elements are blocked"},
        {"id": "prompt10d_svg_blocked_event", "status": "unsupported_reported_security_policy", "reason": "event handler attributes are blocked"},
        {"id": "prompt10d_svg_blocked_external_reference", "status": "unsupported_reported_security_policy", "reason": "network/file URLs and external images are blocked"},
        {"id": "prompt10d_svg_blocked_foreign_object", "status": "unsupported_reported_security_policy", "reason": "foreignObject is blocked"},
        {"id": "prompt10d_svg_blocked_animation", "status": "unsupported_reported_security_policy", "reason": "animation is blocked"},
        {"id": "prompt10d_svg_path_bomb", "status": "unsupported_reported_security_policy", "reason": "path/depth cap exceeded"},
    ]
    write_json(
        MATRIX_FILES["svg"],
        {
    ...
    write_json(
        MATRIX_FILES["svg_results"],
        {
            ...
            "rendered": [pages.get("prompt10d_svg_static_path_shape", {}), pages.get("prompt10d_svg_static_transform", {})],
    ...
    write_json(
        MATRIX_FILES["sbix_results"],
        {
            ...
            "jpeg": pages.get("prompt10d_sbix_jpeg", {}),
            "png_regression": pages.get("prompt10d_sbix_png_regression", {}),
...
def run_feature_report(timeout: int) -> dict[str, Any]:
    ...
    if PUBLIC_FEATURE_REPORT.exists():
        ...
        has_prompt10d = "prompt10d_full_colrv1_svg_color_glyph_closure" in payload.get("report", {})
    feature = {
        "status": "passed" if result["exit_status"] == 0 and has_prompt10d else "failed",
        "has_prompt10d_section": has_prompt10d,
