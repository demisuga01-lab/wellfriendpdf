#!/usr/bin/env python3
"""Prompt 12 prepress CMM, separation, and plate closure artifacts."""
...
HTML_REPORT = OUT_DIR / "prompt12-html-report" / "index.html"
CORPUS_DIR = OUT_DIR / "corpus"
STARTING_HEAD = "077e33d"
STARTING_STATUS = "clean"
EXPECTED_STARTING_COMMIT = "077e33d Close combined prompt 11B native littlecms cmm backend"

IMPLEMENTED_PUBLIC = "implemented_public"
IMPLEMENTED_INTERNAL = "implemented_internal"
UNSUPPORTED_REPORTED = "unsupported_reported"
NOT_IN_SCOPE = "not_in_prompt12_scope"
...
def prompt12_section(report: dict[str, Any] | None) -> dict[str, Any]:
    if not report:
        return {}
...
def write_corpus() -> dict[str, Any]:
    ...
    plate_path = CORPUS_DIR / "prompt12-spot-devicen-plates.pdf"
        ...
        entries.append(
            {
                ...
                else "synthetic_prompt12_profile_or_matrix_fixture",
    ...
    write_json(OUT_DIR / "prompt12-corpus-manifest.json", manifest)
    return manifest
...
def build_scope_matrix() -> list[dict[str, Any]]:
    return [
        ...
        row("proofing profile interaction", IMPLEMENTED_PUBLIC, ["Prompt 11B lcms2 proofing path plus Prompt 12 device-link ambiguity diagnostics"]),
...
def main() -> int:
    ...
    write_json(OUT_DIR / "prompt12-starting-audit.json", starting_audit)
    ...
    write_json(
        OUT_DIR / "device-link-icc-matrix-prompt12.json",
        {"kind": "device_link_icc_matrix_prompt12", "profiles": device_link_profiles},
    )
    write_json(
        OUT_DIR / "device-link-transform-results-prompt12.json",
        {
            "kind": "device_link_transform_results_prompt12",
    ...
    write_json(
        OUT_DIR / "device-link-fallback-results-prompt12.json",
        {
            "kind": "device_link_fallback_results_prompt12",
    ...
    write_json(
        OUT_DIR / "device-link-malformed-results-prompt12.json",
        {
            "kind": "device_link_malformed_results_prompt12",
    ...
    write_json(
        OUT_DIR / "multicolor-icc-matrix-prompt12.json",
        {"kind": "multicolor_icc_matrix_prompt12", "profiles": multicolor_profiles},
    )
    write_json(
        OUT_DIR / "multicolor-transform-results-prompt12.json",
        {
            "kind": "multicolor_transform_results_prompt12",
    ...
    write_json(
        OUT_DIR / "multicolor-devicen-interaction-prompt12.json",
        {
            "kind": "multicolor_devicen_interaction_prompt12",
    ...
    write_json(
        OUT_DIR / "multicolor-fallback-results-prompt12.json",
        {
            "kind": "multicolor_fallback_results_prompt12",
    ...
    bpc_matrix = {
        "kind": "bpc_rendering_intent_matrix_prompt12",
    ...
    write_json(OUT_DIR / "bpc-rendering-intent-matrix-prompt12.json", bpc_matrix)
    write_json(
        OUT_DIR / "bpc-native-results-prompt12.json",
        {
            "kind": "bpc_native_results_prompt12",
            "native_feature_report": prompt12_native or "not_run",
            "status": "wired_to_littlecms_blackpoint_compensation_flag_on_request",
        },
    )
    write_json(
        OUT_DIR / "bpc-fallback-results-prompt12.json",
        {
            "kind": "bpc_fallback_results_prompt12",
            "default_feature_report": prompt12_default or "not_run",
            "status": "bpc_unsupported_in_fallback",
        },
    )
    write_json(
        OUT_DIR / "intent-cache-separation-prompt12.json",
        {
            "kind": "intent_cache_separation_prompt12",
...
"""
    ...
    framebuffer_report = {
        "kind": "separation_framebuffer_matrix_prompt12",
    ...
    write_json(OUT_DIR / "separation-framebuffer-matrix-prompt12.json", framebuffer_report)
    write_json(
        OUT_DIR / "separation-framebuffer-memory-prompt12.json",
        {
            "kind": "separation_framebuffer_memory_prompt12",
    ...
    write_json(
        OUT_DIR / "separation-framebuffer-cache-prompt12.json",
        {
            "kind": "separation_framebuffer_cache_prompt12",
    ...
    write_json(
        OUT_DIR / "separation-framebuffer-equivalence-prompt12.json",
        {
            "kind": "separation_framebuffer_equivalence_prompt12",
            "focused_tests": compact_result(focused_test),
    ...
    write_json(OUT_DIR / "spot-plate-rendering-matrix-prompt12.json", {"kind": "spot_plate_rendering_matrix_prompt12", "rows": plate_rows[:2] + plate_rows[3:]})
    write_json(OUT_DIR / "devicen-plate-rendering-matrix-prompt12.json", {"kind": "devicen_plate_rendering_matrix_prompt12", "rows": [plate_rows[2], plate_rows[4]]})
    write_json(
        OUT_DIR / "tint-transform-results-prompt12.json",
        {
            "kind": "tint_transform_results_prompt12",
    ...
    write_json(
        OUT_DIR / "plate-preview-results-prompt12.json",
        {
            "kind": "plate_preview_results_prompt12",
    ...
    write_json(
        OUT_DIR / "plate-provenance-results-prompt12.json",
        {
    ...
    write_json(OUT_DIR / "multi-reference-render-results-prompt12.json", {"kind": "multi_reference_render_results_prompt12", "results": reference_results})
    write_json(
    ...
    write_json(
        OUT_DIR / "reference-disagreement-summary-prompt12.json",
        {
    ...
    write_json(
        OUT_DIR / "native-vs-fallback-cmm-deltas-prompt12.json",
        {
            "kind": "native_vs_fallback_cmm_deltas_prompt12",
            "default_report_command": default_cmd,
            "native_report_command": native_cmd,
            "default_prompt12": prompt12_default or "not_run",
            "native_prompt12": prompt12_native or "not_run",
    ...
    public_parity = {
        "kind": "public_report_parity_prompt12",
    ...
    write_json(OUT_DIR / "public-report-parity-prompt12.json", public_parity)
    write_json(
        OUT_DIR / "binding-smoke-results-prompt12.json",
        {
            "kind": "binding_smoke_results_prompt12",
    ...
    write_json(
        OUT_DIR / "cli-prepress-report-prompt12.json",
        {
...
<!doctype html>
<html lang="en">
<head><meta charset="utf-8"><title>Prompt 12 Prepress CMM Audit</title></head>
<body>
<h1>Prompt 12 Prepress CMM Audit</h1>
