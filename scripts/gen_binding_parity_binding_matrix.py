#!/usr/bin/env python3
"""Generate the roadmap closure 02 binding parity matrix.

The Binding Surface matrix remains the source for Rust/Python/C ABI/CLI feature
coverage. This generator adds Binding Parity surfaces (WASM, .NET, Java, docs, and
...
BINDING_SURFACE_JSON = ROOT / "target" / "binding_surface-binding-core" / "binding-gap-matrix.json"
OUT_DIR = ROOT / "target" / "binding_parity-binding-parity"
JSON_OUT = OUT_DIR / "binding-gap-matrix.json"
MD_OUT = ROOT / "docs" / "bindings_binding_parity_gap_matrix.md"
...
EXTRA_ROWS = [
    ...
    },
    {
        ...
        "category": "binding_parity-dotnet",
    ...
    },
    {
        ...
        "surfaces": {"wasm": DEF, "dotnet": DEF, "java": PUB, "docs": PUB, "packaging": PUB},
        "note": "pom.xml records Maven metadata and binds WellfriendPdfSmokeTest into mvn test; Java Packaging package smoke runs mvn test/package.",
    },
    {
    ...
    },
    {
        ...
        "category": "binding_parity-diagnostics",
        "feature": "cross-binding JSON envelope parity",
        "surfaces": {"wasm": PUB, "dotnet": PUB, "java": PUB, "docs": PUB, "packaging": PUB},
        "note": "All Binding Parity wrappers call shared facade or C ABI report functions.",
    },
    {
    ...
    },
    {
        ...
        "surfaces": {"wasm": UNSUP, "dotnet": UNSUP, "java": UNSUP, "docs": PUB, "packaging": PUB},
        "note": "Engine render internals can observe CancelToken, but Binding Parity WASM/.NET/Java report/output bindings expose no cancellable render or token-aware facade operation.",
    },
    {
        "id": "java_packaging.cabi.password_open",
        "category": "java_packaging-closure",
    ...
    {
        "id": "java_packaging.dotnet.password_open",
        "category": "java_packaging-closure",
    ...
    {
        "id": "java_packaging.java.password_open",
        "category": "java_packaging-closure",
    ...
    {
        "id": "java_packaging.java.maven_package",
        "category": "java_packaging-closure",
        "feature": "Java Maven package smoke",
        "surfaces": {"java": PUB, "docs": PUB, "packaging": PUB},
        "note": "scripts/java_packaging_java_package_smoke.ps1 runs Maven version/test/package with a target-local Maven fallback.",
    },
    {
        "id": "java_packaging.java.gradle_policy",
        "category": "java_packaging-closure",
        "feature": "Java Gradle package support",
        "surfaces": {"java": PUB, "docs": PUB, "packaging": PUB},
        "note": "Gradle Packaging adds build.gradle/settings.gradle plus a target-local Gradle 9.6.1 bootstrap that runs clean test, jar, build, JAR inspection, runtime smoke, and Maven/Gradle equivalence.",
    },
    {
        "id": "gradle_packaging.java.gradle_package",
        "category": "gradle_packaging-closure",
        "feature": "Java Gradle build/package/JAR smoke",
        "surfaces": {"java": PUB, "docs": PUB, "packaging": PUB},
        "note": "scripts/gradle_packaging_gradle_package_smoke.ps1 runs Gradle version/test/jar/build, smokes build/libs/wellfriendpdf-sdk-0.1.0.jar, and writes gradle-jar-smoke plus Maven/Gradle equivalence artifacts.",
    },
    {
        "id": "java_packaging.java.jar_verification",
        "category": "java_packaging-closure",
    ...
    {
        "id": "java_packaging.progress_posture",
        "category": "java_packaging-closure",
        "feature": "Java Packaging progress closure",
        "surfaces": {"wasm": UNSUP, "dotnet": UNSUP, "java": UNSUP, "docs": PUB, "packaging": PUB},
        "note": "Shared feature report records progress_not_supported; no binding exposes no-op callbacks.",
    },
    {
        "id": "java_packaging.cancellation_posture",
        "category": "java_packaging-closure",
        "feature": "Java Packaging cancellation closure",
        "surfaces": {"wasm": UNSUP, "dotnet": UNSUP, "java": UNSUP, "docs": PUB, "packaging": PUB},
    ...
    {
        "id": "java_packaging.memory_evidence",
        "category": "java_packaging-closure",
        "feature": "Java Packaging memory/leak evidence",
    ...
    {
        "id": "test.binding_parity_smokes",
        "category": "binding_parity-release",
        "feature": "Binding Parity binding smoke tests",
...
def action_for(row: dict, statuses: dict) -> str:
    ...
    if not gaps:
        return "No Binding Parity action; public or report-backed on WASM/.NET/Java."
    ...
    if any(statuses[name] == MISS for name in gaps):
        return "Future roadmap task must add a real shared-facade/C ABI entry point before exposing this surface."
...
def normalize_extra(row: dict) -> dict:
    ...
    return {
        ...
        "note": row["note"],
        "action": row["note"],
        "tests": "Binding Parity focused binding smokes and generated artifacts",
    }


def build_rows() -> list[dict]:
    binding_surface = json.loads(BINDING_SURFACE_JSON.read_text(encoding="utf-8"))
    rows = []
    for source in binding_surface["features"]:
...
def tests_for(fid: str, category: str, surfaces: dict) -> str:
    ...
    if fid.startswith(("diag.", "test.", "pkg.", "doc.")):
        return "Binding Parity matrix/docs plus focused smoke artifacts"
...
def write_json(rows: list[dict]) -> None:
    ...
    payload = {
        "schema_version": 1,
        "feature_area": "combined-02-wasm-dotnet-java-diagnostics-parity",
        "envelope_version": 1,
