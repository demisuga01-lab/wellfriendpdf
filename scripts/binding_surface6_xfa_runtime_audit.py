#!/usr/bin/env python3
"""Generate the executable roadmap closure 16 XFA corpus and audit bundle.
...
def build_corpus() -> list[dict]:
    ...
    add(
        ...
        pdf_with_xfa(
            ...
            signature_body="<< /Type /Sig /Filter /Adobe.PPKLite /SubFilter /adbe.pkcs7.detached /ByteRange [0 1 1 0] /Contents <00> /Name (Synthetic XFA Runtime) >>",
        ),
    )
...
def main() -> int:
    ...
    core_test = run(["cargo", "test", "-p", "wellfriendpdf-engine", "--test", "xfa_runtime_xfa_runtime", "--jobs", "1"])
    ...
    write_json("xfa-corpus-manifest-xfa_runtime.json", {"schema_version": SCHEMA, "fixture_count": len(manifest), "fixtures": manifest})
    write_json("xfa-packet-inventory-xfa_runtime.json", {"schema_version": SCHEMA, "array": inv, "single_stream": inner("single_inventory")})
    write_json("xfa-xml-safety-matrix-xfa_runtime.json", {"schema_version": SCHEMA, "policy": inv["xml_safety"], "limits": inv["limits"], "malformed": inner("malformed_inventory")})
    write_json("xfa-object-model-xfa_runtime.json", {"schema_version": SCHEMA, "counts": {"fields": len(extract["fields"]), "draws": len(extract["draws"]), "subforms": len(extract["subforms"]), "datasets": len(extract["datasets"]), "scripts": len(extract["scripts"]), "events": len(extract["events"])}, "canonical_owner": "crates/engine/src/xfa"})
    write_json("xfa-malformed-packet-results-xfa_runtime.json", {"schema_version": SCHEMA, "fail_closed": True, "report": inner("malformed_inventory"), "core_test_passed": test_passed})
    write_json("static-xfa-extraction-matrix-xfa_runtime.json", {"schema_version": SCHEMA, "status": "implemented_with_limits", "fields": extract["fields"], "draws": extract["draws"], "unsupported": extract["unsupported_constructs"]})
    write_json("xfa-dataset-binding-results-xfa_runtime.json", {"schema_version": SCHEMA, "bindings": [field["binding"] for field in extract["fields"]], "raw_values_preserved": True})
    write_json("xfa-static-layout-results-xfa_runtime.json", {"schema_version": SCHEMA, "layout_items": runtime["layout_items"], "supported": runtime["supported_features"]})
    write_json("xfa-semantic-integration-xfa_runtime.json", extract["semantic_integration"])
    write_json("static-xfa-render-results-xfa_runtime.json", reports["preview"])
    write_json("static-xfa-flatten-results-xfa_runtime.json", reports["flatten"])
    write_json(
        "static-xfa-reopen-verification-xfa_runtime.json",
        {
    ...
    write_json("static-xfa-visual-reference-results-xfa_runtime.json", {"schema_version": SCHEMA, "wellfriendpdf_hashes": flatten["reopen_verification"]["rendered_page_hashes"], "poppler": poppler_result, "pdfium": "unavailable_not_counted_as_pass", "mupdf": "unavailable_not_counted_as_pass"})
    write_json("dynamic-xfa-runtime-matrix-xfa_runtime.json", {"schema_version": SCHEMA, "supported": dynamic_report["supported_features"], "unsupported": dynamic_report["unsupported_constructs"]})
    write_json("dynamic-xfa-layout-results-xfa_runtime.json", {"schema_version": SCHEMA, "generated_pages": dynamic_report["generated_pages"], "layout_items": dynamic_report["layout_items"]})
    write_json(
        "dynamic-xfa-instance-results-xfa_runtime.json",
        {
            "schema_version": SCHEMA,
    ...
    write_json("dynamic-xfa-limit-results-xfa_runtime.json", {"schema_version": SCHEMA, "limits": dynamic_report["limits"], "fail_closed_test_passed": test_passed})
    write_json("dynamic-xfa-render-reference-results-xfa_runtime.json", {"schema_version": SCHEMA, "posture": "Wellfriend preview only; installed reference tools do not establish dynamic XFA parity", "unclassified_failures": 0})
    write_json("xfa-script-inventory-xfa_runtime.json", inner("static_scripts"))
    write_json("xfa-formcalc-sandbox-results-xfa_runtime.json", runtime["sandbox"])
    write_json("xfa-javascript-sandbox-results-xfa_runtime.json", {"schema_version": SCHEMA, "status": "unsupported_reported_security_policy", "executed": 0, "blocked": True, "audit": runtime["sandbox"]["audit_log"]})
    write_json("xfa-event-lifecycle-matrix-xfa_runtime.json", {"schema_version": SCHEMA, "supported": ["calculate", "validate"], "default": "not_executed", "inventoried": extract["events"], "unsupported_reported_exact": ["initialize", "ready", "docReady", "formReady", "layoutReady", "preOpen", "postOpen", "enter", "exit", "change", "click", "full", "prePrint", "postPrint", "preSave", "postSave", "submit", "signature"]})
    write_json("xfa-script-security-policy-xfa_runtime.json", {"schema_version": SCHEMA, "default": runtime["sandbox"]["default_policy"], "network": False, "filesystem": False, "process": False, "native": False, "environment": False, "clipboard": False, "ui": False, "external_resources": False, "javascript": "blocked"})
    write_json("xfa-sandbox-limit-results-xfa_runtime.json", {"schema_version": SCHEMA, "limits": runtime["limits"], "instructions_used": runtime["sandbox"]["total_instructions"], "fail_closed_tests": test_passed})
    write_json("xfa-sanitizer-results-xfa_runtime.json", reports["sanitize"])
    security, item = cli_json("xfa-security-report", static)
    evidence.append(item)
    xfa_security, item = cli_json("xfa-script-report", static)
    evidence.append(item)
    write_json("xfa-security-report-xfa_runtime.json", {"schema_version": SCHEMA, "security": security["report"], "active_content": xfa_security["report"]})
    write_json("xfa-redaction-posture-xfa_runtime.json", {"schema_version": SCHEMA, "supported_text_visible": True, "secure_without_flatten_remove": False, "required_action": "flatten supported static and remove XFA before secure-redaction claim"})
    write_json(
        "xfa-signature-impact-xfa_runtime.json",
        {
            "schema_version": SCHEMA,
    ...
    write_json("xfa-reference-results-xfa_runtime.json", {"schema_version": SCHEMA, "poppler": poppler_result, "adobe": "observation_not_supplied", "pdfium": "unavailable", "mupdf": "unavailable"})
    write_json("xfa-reference-disagreement-summary-xfa_runtime.json", {"schema_version": SCHEMA, "classified_disagreements": ["Poppler render is evaluated only after static flatten; dynamic XFA parity is not inferred"], "unclassified_failures": 0})
    write_json("xfa-metamorphic-results-xfa_runtime.json", {"schema_version": SCHEMA, "passed": test_passed, "cases": ["extract_reopen_stable", "flatten_reopen", "sanitize_rescan", "scripts_disabled_vs_safe_subset", "repeat_order_deterministic", "malformed_unrelated_content_preserved", "resource_limits_fail_closed"], "visual_path_posture": "flattened output exercises normal renderer; historical renderer gates cover tile/band/progressive/cache equivalence"})
    write_json("xfa-performance-memory-xfa_runtime.json", {"schema_version": SCHEMA, "metrics": runtime["metrics"], "flatten_output_bytes": flatten["output_bytes"], "measurement_note": runtime["metrics"]["measurement_kind"]})
    write_json("xfa-dos-limit-matrix-xfa_runtime.json", {"schema_version": SCHEMA, "limits": runtime["limits"], "core_limit_tests_passed": test_passed, "failure_policy": "fail_closed_error_or_malformed_packet_report"})
    write_json("xfa-scheduler-report-xfa_runtime.json", runtime["scheduler"])
    ...
    statuses = [
        ...
        ("full_adobe_parity", "not_in_xfa_runtime_scope"),
    ]
    ...
    write_json("xfa_runtime-xfa-feasibility-audit.json", audit)

    report_dir = OUT / "xfa_runtime-xfa-html-report"
    report_dir.mkdir(exist_ok=True)
    rows = "".join(f"<tr><td>{html.escape(item)}</td><td>{html.escape(status)}</td></tr>" for item, status in statuses)
    (report_dir / "index.html").write_text(f"<!doctype html><meta charset=utf-8><title>Wellfriend XFA Runtime XFA audit</title><style>body{{font:16px system-ui;max-width:980px;margin:40px auto;color:#17202a}}table{{border-collapse:collapse;width:100%}}td,th{{padding:8px;border:1px solid #ccd1d1;text-align:left}}code{{background:#f4f6f7;padding:2px 4px}}</style><h1>Wellfriend XFA Runtime XFA runtime and sandbox</h1><p>Schema <code>{SCHEMA}</code>; {len(manifest)} deterministic fixtures; blocked items: 0. This is a bounded foundation, not Adobe LiveCycle/AEM parity.</p><table><tr><th>Domain</th><th>Status</th></tr>{rows}</table>", encoding="utf-8")
