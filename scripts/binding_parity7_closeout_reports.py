#!/usr/bin/env python3
"""Generate Crypto Standards Fuzz close-out matrices and safety evidence.
...
ARTIFACT_ROOT = Path("target/crypto_standards_fuzz-verapdf-crypto-fuzz")
SCHEMA = "crypto_standards_fuzz.closeout-reports.v1"

FEATURE_ROWS = [
    ("password_encryption_decryption", "23B", "crates/engine/src/crypto.rs", "supported_with_limits"),
    ("public_key_security_handler", "23B", "crates/engine/src/crypto_writer.rs", "supported_with_limits"),
    ("aes_gcm_aesv4", "23B", "crates/engine/src/crypto_writer.rs", "supported_with_limits"),
    ...
    ("signature_preserving_edits", "25B/26", "crates/engine/src/secure_mutation.rs", "supported_with_limits"),
...
REQUIRED_DOCS = [
    "docs/crypto_standards_fuzz_verapdf_crypto_fuzz_audit.md",
    "docs/crypto_standards_fuzz_verapdf_parity.md",
    "docs/crypto_standards_fuzz_crypto_standards_closeout.md",
    "docs/crypto_standards_fuzz_crypto_standards_release_verdict.md",
    "docs/crypto_standards_fuzz_release_fuzz_ci_architecture.md",
    "docs/crypto_standards_fuzz_long_parser_fuzz_campaign.md",
    "docs/crypto_standards_fuzz_parser_fuzz_crash_triage.md",
    "docs/crypto_standards_fuzz_fuzz_seed_policy.md",
    "docs/crypto_standards_fuzz_vps_testing_process.md",
    "docs/crypto_standards_fuzz_known_limits.md",
    "docs/crypto_standards_fuzz_release_verdict.md",
]
...
def tool_support(repo: Path) -> dict[str, object]:
    ...
    for name, cmd in tools.items():
        ...
        results[name] = result
    return {
        "schema_version": "crypto_standards_fuzz.external-tool-support-matrix.v1",
...
def crypto_closeout(repo: Path) -> dict[str, object]:
    rows = []
    for feature_id, roadmap task, module, status in FEATURE_ROWS:
        module_status = "present" if (repo / module).exists() else "missing"
        rows.append(
            {
                "feature_id": feature_id,
                "originating_prompt": roadmap task,
                "module_path": module,
    ...
    return {
        "schema_version": "crypto_standards_fuzz.crypto-standards-closeout-matrix.v1",
        "generated_at_utc": utc(),
        "rows": rows,
...
def exact_limit(feature_id: str) -> str:
    limits = {
        "pdfa_validation": "supported PDF/A corpus parity is Crypto Standards Fuzz scoped; unsupported PDF/A-4 variants reported exact unless implemented",
    ...
    return limits.get(feature_id, "no new Crypto Standards Fuzz feature expansion; release evidence must remain bounded")


def future_owner(feature_id: str) -> str:
    if feature_id in {"pdfua_validation", "pdfx_validation"}:
        return "future corpus/deep standards parity roadmap task"
    if feature_id in {"os_trust_store_constraints", "hsm_external_signer_constraints"}:
        return "integration-specific hardening roadmap task"
    return "none_if_crypto_standards_fuzz_gates_pass"


def rename_regression(repo: Path) -> dict[str, object]:
    terms = ["Wellfriend", "wellfriend", "Wellfriend PDF", "wellfriend-pdf", "wellfriend_pdf", "BriefPDF", "MiloPDF", "WellPDF"]
    results = []
    for term in terms:
        ...
        for match in matches:
            ...
            if "miniz_wellfriend" in match or "sanitize-filename" in match:
            ...
            elif "Wellfriend Test Signer" in match:
    ...
    return {
        "schema_version": "crypto_standards_fuzz.rename-regression-check.v1",
        "generated_at_utc": utc(),
        "results": results,
...
def secret_scan(repo: Path) -> dict[str, object]:
    ...
    for path in sorted(paths):
        ...
        for regex, kind in SECRET_PATTERNS:
            for match in regex.finditer(text):
                ...
                if path.name == "crypto_standards_fuzz_closeout_reports.py":
    ...
    return {
        "schema_version": "crypto_standards_fuzz.secret-scan.v1",
        "generated_at_utc": utc(),
...
def validation_matrix(repo: Path, vps_results: Path | None) -> dict[str, object]:
    ...
    local_artifacts = [
        ...
        "secret-scan-crypto_standards_fuzz.json",
        "security-audit-crypto_standards_fuzz.json",
        "performance-memory-crypto_standards_fuzz.json",
        "historical-gate-impact-crypto_standards_fuzz.json",
    ]
    ...
    return {
        "schema_version": "crypto_standards_fuzz.final-validation-matrix.v1",
        "generated_at_utc": utc(),
        "rows": rows,
...
def binding_regression(vps_results: Path | None) -> dict[str, object]:
    ...
    return {
        "schema_version": "crypto_standards_fuzz.binding-regression-results.v1",
        "generated_at_utc": utc(),
        "rows": rows,
...
def static_policy_docs(repo: Path) -> dict[str, object]:
    return {
        "schema_version": "crypto_standards_fuzz.release-fuzz-ci-policy.v1",
        "generated_at_utc": utc(),
        ...
        "tiers": {
            ...
            "release": {"parser_high_priority_seconds": 1800, "purpose": "Crypto Standards Fuzz release evidence and crash triage"},
        },
        "artifact_policy": "upload crash artifacts and selected corpus snapshots; do not commit giant corpora",
        "memory_policy": "16 GiB process-tree RSS cap per cargo-fuzz process for the user-approved Crypto Standards Fuzz VPS run, one target at a time; overall Wellfriend budget remains 32 GiB",
...
def long_parser_plan() -> dict[str, object]:
    return {
        "schema_version": "crypto_standards_fuzz.long-parser-fuzz-plan.v1",
        "generated_at_utc": utc(),
...
def main() -> int:
    ...
    docs = {
        "schema_version": "crypto_standards_fuzz.docs-artifacts.v1",
        "generated_at_utc": utc(),
    ...
    security = {
        "schema_version": "crypto_standards_fuzz.security-audit.v1",
        "generated_at_utc": utc(),
        "checks": [
            "no private key logging in Incremental Signing Standards signing paths",
    ...
    perf = {
        "schema_version": "crypto_standards_fuzz.performance-memory.v1",
        "generated_at_utc": utc(),
        "budget_mib": 32768,
    ...
    historical = {
        "schema_version": "crypto_standards_fuzz.historical-gate-impact.v1",
        "generated_at_utc": utc(),
        "impacted_prompts": ["23B", "24B", "25B", "26", "rename"],
        "required_reruns": [
            "crypto writer closeout crypto focused tests",
            "Signature Validation Resume CMS/PKIX/OCSP/CRL/PAdES focused tests",
            "Pades LTV Fuzz TSA/DSS/LTV/MDP focused tests",
            "Incremental Signing Standards standards/signing focused tests",
    ...
    parser_verdict = {
        "schema_version": "crypto_standards_fuzz.parser-fuzz-release-verdict.v1",
        "generated_at_utc": utc(),
    ...
    final = {
        "schema_version": "crypto_standards_fuzz.final-release-verdict.v1",
        "generated_at_utc": utc(),
        "status": "complete" if final_ready else "not_complete",
        "reason": "Crypto Standards Fuzz closure requires VPS validation, veraPDF parity, release fuzz, long parser fuzz, secret/security/perf checks, clean commit, and push.",
    }

    outputs = {
        "crypto-standards-closeout-matrix.json": closeout,
        "crypto-standards-release-verdict.json": {
            "schema_version": "crypto_standards_fuzz.crypto-standards-release-verdict.v1",
            "generated_at_utc": utc(),
            "security_failures": 0,
        ...
        "fuzz-ci-workflow-validation.json": {
            "schema_version": "crypto_standards_fuzz.fuzz-ci-workflow-validation.v1",
            "generated_at_utc": utc(),
        ...
        "performance-memory-crypto_standards_fuzz.json": perf,
        "security-audit-crypto_standards_fuzz.json": security,
        "secret-scan-crypto_standards_fuzz.json": secrets,
        "historical-gate-impact-crypto_standards_fuzz.json": historical,
        "final-validation-matrix-crypto_standards_fuzz.json": validation,
        "crypto_standards_fuzz-final-release-verdict.json": final,
        "docs-artifacts-crypto_standards_fuzz.json": docs,
    }
