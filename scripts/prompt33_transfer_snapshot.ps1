...
$archive = Join-Path $repo "target/prompt33-geometric-semantic-reflow/prompt33-source-$stamp.tar.gz"
$manifest = Join-Path $repo "target/prompt33-geometric-semantic-reflow/prompt33-source-$stamp.manifest.txt"
...
try {
    ...
    # result-cache transfer and can exhaust the VPS temporary quota.  Prompt
    ...
    if (-not $files) {
        throw 'Prompt 33 transfer source manifest is empty.'
    }
    ...
    $remoteArchive = "$RemoteTempDir/prompt33-source-$stamp.tar.gz"
