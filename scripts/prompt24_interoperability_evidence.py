#!/usr/bin/env python3
"""Generate Prompt 24 interoperability evidence from installed local tools.
...
DOCS = ROOT / "docs"
SCHEMA = "prompt24.interoperability.v1"
...
def pkix_rows() -> list[dict[str, Any]]:
    ...
    checked(
        ...
        "Verify Prompt 24 AIA fixture certificate signatures and CA constraints",
...
def write_doc(all_payloads: dict[str, dict[str, Any]]) -> None:
    ...
    for name, payload in all_payloads.items():
        ...
        lines.append("")
    (DOCS / "prompt24_interoperability.md").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> int:
    ...
    write_json("cms-interoperability-prompt24.json", payloads["cms"])
    write_json("pkix-interoperability-prompt24.json", payloads["pkix"])
    write_json("ocsp-interoperability-prompt24.json", payloads["ocsp"])
    write_json("crl-interoperability-prompt24.json", payloads["crl"])
    write_json("pades-interoperability-prompt24.json", payloads["pades"])
    write_json("independent-tool-support-matrix-prompt24.json", support)
