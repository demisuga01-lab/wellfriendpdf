#!/usr/bin/env python3
"""Generate and compare the Transparency Rendering transparency compositing corpus."""
...
OUT_DIR = Path("target/transparency_rendering-transparency-compositing")
...
TOOL_MANIFEST = Path("target/native_renderer-renderer-native-replay/reference-tool-manifest-reference_renderer.json")
...
def generate_corpus() -> list[dict[str, Any]]:
        ...
        entries.append(
            {
                ...
                "generator": "scripts/transparency_rendering_transparency_compositing_audit.py",
            }
        )
    ...
    write_json(
        ...
        {
            "schema_version": 1,
            "kind": "transparency_rendering_transparency_corpus_manifest",
...
def load_tool_manifest(path: Path) -> dict[str, Any]:
    if not path.exists():
        raise RuntimeError(f"missing reference tool manifest: {path}; run scripts/reference_renderer_multi_reference_audit.ps1 first")
...
def run_phase(entries: list[dict[str, Any]], tools: dict[str, Any], base: list[str], phase: str, dpi: int, timeout: int) -> dict[str, Any]:
    ...
    return {
        "schema_version": 1,
        "kind": f"transparency_rendering_{phase}_render_results",
        "phase": phase,
        "dpi": dpi,
        "memory_cap_mb": 4096,
        "fixture_count": len(entries),
...
def write_summary_artifacts(entries: list[dict[str, Any]], results: dict[str, Any] | None) -> None:
    ...
    write_json(
        ...
        {
            "schema_version": 1,
            "kind": "transparency_rendering_reference_disagreement_summary",
            "fixture_count": len(entries),
            "classification_counts": classification_counts,
    ...
    write_json(
        ...
        {
            "schema_version": 1,
            "kind": "transparency_rendering_blend_mode_matrix",
            "blend_modes": [
                {
                    "mode": mode,
                    "implemented": True,
    ...
    write_json(
        ...
        {
            "schema_version": 1,
            "kind": "transparency_rendering_soft_mask_matrix",
            "features": [
                {
                    "id": e["id"],
                    "category": e["category"],
            ...
            "known_limits": [
                "Transparency Closeout closes common image /SMask /Matte and ExtGState /BC backdrop behavior",
                "Transparency Closeout closes DeviceGray/DeviceRGB/DeviceCMYK luminosity mask color-space coverage",
    ...
    write_json(
        ...
        {
            "schema_version": 1,
            "kind": "transparency_rendering_group_isolation_knockout_matrix",
            "features": [
                {
                    "id": e["id"],
                    "category": e["category"],
            ...
            "known_limits": [
                "Transparency Closeout closes interior knockout overlap for common vector/Form group cases",
                "text clipping plus pattern/shading paints inside knockout groups remain later roadmap task ownership",
            ],
        },
    )
    write_json(
        ...
        {
            "schema_version": 1,
            "kind": "transparency_rendering_fallback_taxonomy",
            "removed_or_reduced": ["transparency/later is no longer an unowned Reference Renderer category"],
            "remaining": [
                "pattern/later remains Advanced Rendering",
                "shading/later remains Advanced Rendering",
    ...
    write_json(
        ...
        {
            "schema_version": 1,
            "kind": "transparency_rendering_memory_budget_report",
            "memory_cap_mb": 4096,
    ...
    HTML_REPORT.write_text(
        ...
        "<title>Transparency Rendering Transparency Compositing Audit</title>"
        ...
        "<h1>Transparency Rendering Transparency Compositing Audit</h1>"
...
def main() -> int:
    ...
    if args.phase in {"baseline", "both"}:
        ...
        latest["baseline_note"] = "Transparency Rendering visual baseline for the transparency-focused corpus."
    ...
    if args.phase in {"post", "both"}:
        ...
        latest["starting_checkpoint"] = run_command(["git", "rev-parse", "--short", "HEAD"], 10)
        latest["post_note"] = "Post-implementation Transparency Rendering transparency corpus comparison."
