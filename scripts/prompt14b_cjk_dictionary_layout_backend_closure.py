#!/usr/bin/env python3
"""Generate Prompt 14B CJK dictionary/layout backend closure artifacts."""
...
OUT = ROOT / "target" / "prompt14-semantic-intelligence"
...
PROMPT14B_ITEMS = [
    ...
    ("cloud provider integration feasibility", "not_in_prompt14b_scope"),
    ("privacy policy", "implemented"),
    ("validation gates", "implemented"),
]

ENTRIES = [
    ("机器", "zh", 1, "prompt14b-fixture", 0.70),
    ("机器学习", "zh", 10, "prompt14b-fixture", 0.97),
    ("人工智能", "zh", 8, "prompt14b-fixture", 0.96),
    ("数据库", "zh", 6, "prompt14b-fixture", 0.95),
    ("検索エンジン", "ja", 9, "prompt14b-fixture", 0.96),
    ("形態素解析", "ja", 8, "prompt14b-fixture", 0.95),
    ("東京大学", "ja", 7, "prompt14b-fixture", 0.94),
    ("한국어", "ko", 8, "prompt14b-fixture", 0.95),
    ("자연어처리", "ko", 7, "prompt14b-fixture", 0.94),
    ("데이터베이스", "ko", 6, "prompt14b-fixture", 0.93),
]
...
def pack_manifest(entries_bytes: bytes) -> dict[str, Any]:
    return {
        ...
        "entries_path": "dictionary-pack-entries-prompt14b.tsv",
        "entry_count": len(ENTRIES),
        "generation_command": "python scripts/prompt14b_cjk_dictionary_layout_backend_closure.py",
...
def main() -> int:
    ...
    (OUT / "dictionary-pack-entries-prompt14b.tsv").write_bytes(entries)
    manifest = pack_manifest(entries)

    audit = matrix(PROMPT14B_ITEMS, "prompt14b_cjk_dictionary_layout_backend_closure")
    write_json(OUT / "prompt14b-closure-audit.json", audit)
    write_json(OUT / "dictionary-provider-matrix-prompt14b.json", audit)
    write_json(OUT / "dictionary-pack-manifest-prompt14b.json", manifest)
    write_json(
        OUT / "dictionary-load-report-prompt14b.json",
        {
            "schema_version": "prompt14b.cjk_dictionary_provider.v1",
    ...
    write_json(
        OUT / "dictionary-memory-index-report-prompt14b.json",
        {
            "schema_version": "prompt14b.cjk_dictionary_provider.v1",
    ...
    write_json(
        OUT / "cjk-segmentation-fixtures-prompt14b.json",
        {
            "fixtures": fixtures,
            "expected_tokens": [
    ...
    write_json(
        OUT / "cjk-dictionary-quality-prompt14b.json",
        {
    ...
    write_json(
        OUT / "cjk-search-rag-integration-prompt14b.json",
        {
            "status": "implemented_with_limits",
    ...
    write_json(
        OUT / "cjk-search-token-layer-prompt14b.json",
        {
            "query": "検索エンジン",
            "match_count": 1,
    ...
    write_json(
        OUT / "cjk-rag-token-layer-prompt14b.json",
        {
    ...
    write_json(
        OUT / "layout-backend-feasibility-prompt14b.json",
        {
            ...
            "safe_path": "Prompt 14 LayoutProposalSet schema plus local/cloud templates remain the integration layer for application-supplied runtimes.",
        },
    )
    write_json(
        OUT / "local-layout-backend-status-prompt14b.json",
        {
    ...
    write_json(
        OUT / "cloud-layout-backend-status-prompt14b.json",
        {
    ...
    write_json(
        OUT / "layout-proposal-merge-quality-prompt14b.json",
        {
            "status": "implemented",
