...
$ArtifactDir = Join-Path $Repo "target/binding_parity-binding-parity"
$ToolDir = Join-Path $Repo "target/gradle_packaging-tools"
$SmokeDir = Join-Path $Repo "target/gradle_packaging-gradle-package-smoke"
...
$env:WELLFRIENDPDF_BINDING_PARITY_ARTIFACT_DIR = $ArtifactDir
...
$gradleSmoke = [ordered]@{
    ...
    native_loading = "Gradle JAR smoke ran from target/gradle_packaging-gradle-package-smoke/run with WELLFRIENDPDF_NATIVE_LIBRARY unset and wellfriendpdf_capi copied to bindings/java/build/libs/runtimes/$rid/native."
}
...
if (!(Test-Path $mavenJar)) {
    Invoke-Checked "powershell" @("-ExecutionPolicy", "Bypass", "-File", (Join-Path $Repo "scripts/java_packaging_java_package_smoke.ps1")) "Maven package smoke for equivalence"
}
if (!(Test-Path $mavenJar)) {
...
$equivalence = [ordered]@{
    ...
    smoke_result = [ordered]@{
        maven = "passed in target/binding_parity-binding-parity/java-package-smoke.json"
        gradle = "passed in target/binding_parity-binding-parity/gradle-jar-smoke.json"
    }
