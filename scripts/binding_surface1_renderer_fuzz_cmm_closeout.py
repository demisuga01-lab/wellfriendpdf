#!/usr/bin/env python3
"""Renderer Fuzz CMM renderer fuzz, metamorphic, close-out, and CMM audit harness."""
...
OUT_DIR = Path("target/renderer_fuzz_cmm-renderer-cmm-closeout")
...
def renderer_target_inventory() -> dict[str, Any]:
    ...
    renderer_rows = [
        ("content stream interpreter", ["content_tokenizer", "display_list", "renderer_renderer_fuzz_cmm"]),
        ("display-list replay", ["display_list", "renderer_renderer_fuzz_cmm"]),
        ("text native replay", ["display_list", "fonts", "font_mapping", "renderer_renderer_fuzz_cmm"]),
        ("image native replay", ["display_list", "image_decoders", "renderer_renderer_fuzz_cmm"]),
        ("Form XObject replay", ["display_list", "structured_pdf", "renderer_renderer_fuzz_cmm"]),
        ("transparency groups", ["display_list", "structured_pdf", "renderer_renderer_fuzz_cmm"]),
        ("soft masks", ["display_list", "structured_pdf", "renderer_renderer_fuzz_cmm"]),
        ("blend modes and Porter-Duff modes", ["display_list", "renderer_renderer_fuzz_cmm"]),
        ("text clipping", ["display_list", "fonts", "renderer_renderer_fuzz_cmm"]),
        ("axial/radial shadings", ["functions", "display_list", "renderer_renderer_fuzz_cmm"]),
        ("mesh and tensor patch shadings", ["functions", "structured_pdf", "renderer_renderer_fuzz_cmm"]),
        ("tiling patterns", ["display_list", "structured_pdf", "renderer_renderer_fuzz_cmm"]),
        ("annotation appearances", ["structured_pdf", "renderer_renderer_fuzz_cmm"]),
        ("OCG/layers", ["display_list", "structured_pdf", "renderer_renderer_fuzz_cmm"]),
        ("progressive checkpoint/resume state", ["structured_pdf", "renderer_renderer_fuzz_cmm"]),
        ("tile/band/cache key paths", ["display_list", "structured_pdf", "renderer_renderer_fuzz_cmm"]),
        ("color glyph paint graphs", ["fonts", "font_mapping", "renderer_renderer_fuzz_cmm"]),
        ("CJK/RTL font paths", ["fonts", "font_mapping", "cmap", "renderer_renderer_fuzz_cmm"]),
        ("malformed resource dictionaries", ["structured_pdf", "parser_report", "renderer_renderer_fuzz_cmm"]),
        ("malformed color spaces", ["color_report", "functions", "renderer_renderer_fuzz_cmm"]),
        ("renderer scheduler admission", ["display_list", "structured_pdf", "renderer_renderer_fuzz_cmm"]),
    ]
    rows = []
    ...
    return {
        "schema_version": 1,
        "kind": "renderer_fuzz_target_inventory_renderer_fuzz_cmm",
...
def generate_mutator_corpus() -> dict[str, Any]:
    ...
    resources = (
        ...
        b"/Extend [true true] >> >> "
        b"/Properties << /Layer1 << /Type /OCG /Name (RendererFuzzCMM Layer) >> >> "
        b">>"
    )
    cases = {
        ...
        "text_matrix_perturbation": b"BT /F1 14 Tf 1 0.08 0.02 1 12 55 Tm (RendererFuzzCMM) Tj ET\n",
    ...
    return {
        "schema_version": 1,
        "kind": "renderer_mutator_report_renderer_fuzz_cmm",
        "mutator": "scripts/renderer_fuzz_cmm_renderer_fuzz_cmm_closeout.py",
...
def seed_corpus_manifest(mutator_report: dict[str, Any]) -> dict[str, Any]:
    ...
    prompt_dirs = [
        ("native text/image/Form fixtures", Path("target/native_renderer-renderer-native-replay")),
        ("transparency/soft-mask/knockout fixtures", Path("target/transparency_rendering-transparency-compositing")),
        ("text clipping fixtures", Path("target/advanced_rendering-text-shading-patterns")),
        ("Type3/CID/tensor fixtures", Path("target/type3_cid_rendering-type3-cid-tensor")),
        ("annotation/OCG/progressive fixtures", Path("target/annotation_ocg_rendering-annotation-ocg-progressive-cache")),
        ("CJK/RTL/color glyph fixtures", Path("target/multilingual_color_glyphs-cjk-rtl-color-glyph-reference")),
    ]
    entries = []
    ...
    return {
        "schema_version": 1,
        "kind": "renderer_seed_corpus_manifest_renderer_fuzz_cmm",
        "seed_count": len(entries),
        "entries": entries,
        "promotion_policy": {
            "smoke_clean_mutations": "promote to target/renderer_fuzz_cmm-renderer-cmm-closeout/renderer-mutator-corpus",
            "reduced_crashes": "store under fuzz/artifacts/<target>/ after minimization with owner and reproducer",
            "expected_fail_closed": "kept in Renderer Fuzz CMM manifest only unless it exercises a new stable fail-closed path",
        },
    }
...
def run_fuzz_smoke(args: argparse.Namespace, mutator_report: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema_version": 1,
        "kind": "renderer_fuzz_smoke_report_renderer_fuzz_cmm",
...
def metamorphic_artifacts() -> dict[str, dict[str, Any]]:
    ...
    return {
        "matrix": {
            "schema_version": 1,
            "kind": "renderer_metamorphic_matrix_renderer_fuzz_cmm",
            ...
            "evidence": [
                ...
                "target/multilingual_color_glyphs-cjk-rtl-color-glyph-reference/colrv1-tile-band-progressive-equivalence-colrv_gradient_composite.json",
                "target/multilingual_color_glyphs-cjk-rtl-color-glyph-reference/colrv1-determinism-porterduff_radial_color_glyph.json",
            ],
        ...
        "full_tile_band": {
            "schema_version": 1,
            "kind": "full_tile_band_equivalence_renderer_fuzz_cmm",
            "status": STATUS_IMPLEMENTED,
            **shared,
            "source_artifacts": [
                "target/annotation_ocg_rendering-annotation-ocg-progressive-cache/tile-full-equivalence-renderer_validation.json",
        ...
        "cache": {
            "schema_version": 1,
            "kind": "cache_no_cache_equivalence_renderer_fuzz_cmm",
            "status": STATUS_IMPLEMENTED,
            **shared,
            "source_artifacts": [
                "target/annotation_ocg_rendering-annotation-ocg-progressive-cache/cache-equivalence-renderer_validation.json",
                "target/multilingual_color_glyphs-cjk-rtl-color-glyph-reference/colrv1-cache-key-porterduff_radial_color_glyph.json",
            ],
            "failures": 0,
        ...
        "progressive": {
            "schema_version": 1,
            "kind": "progressive_equivalence_renderer_fuzz_cmm",
            "status": STATUS_IMPLEMENTED,
            **shared,
            "source_artifacts": [
                "target/annotation_ocg_rendering-annotation-ocg-progressive-cache/progressive-resume-equivalence-renderer_validation.json",
                "target/annotation_ocg_rendering-annotation-ocg-progressive-cache/progressive-resume-invalid-token-renderer_validation.json",
            ],
            "failures": 0,
        ...
        "ocg": {
            "schema_version": 1,
            "kind": "ocg_cache_separation_renderer_fuzz_cmm",
            "status": STATUS_IMPLEMENTED,
            **shared,
            "source_artifacts": [
                "target/annotation_ocg_rendering-annotation-ocg-progressive-cache/ocg-cache-key-fingerprint-renderer_validation.json",
...
def closeout_reports() -> dict[str, Any]:
    sources = [
        (
            "reference_renderer",
            Path("target/native_renderer-renderer-native-replay"),
            "multi-reference-corpus-manifest-reference_renderer.json",
            "multi-reference-render-results-reference_renderer.json",
            "multi-reference-diff-metrics-reference_renderer.json",
            "reference-disagreement-summary-reference_renderer.json",
        ),
        (
            "transparency_closeout",
            Path("target/transparency_rendering-transparency-compositing"),
            "transparency_closeout-corpus-manifest.json",
            "transparency_closeout-render-results.json",
            "transparency_closeout-diff-metrics.json",
            "transparency_closeout-reference-disagreement-summary.json",
        ),
        (
            "advanced_rendering",
            Path("target/advanced_rendering-text-shading-patterns"),
        ...
        (
            "type3_cid_rendering",
            Path("target/type3_cid_rendering-type3-cid-tensor"),
            "type3_cid_rendering-corpus-manifest.json",
            "type3_cid_rendering-render-results.json",
            "type3_cid_rendering-diff-metrics.json",
            "type3_cid_rendering-reference-disagreement-summary.json",
        ),
        (
            "renderer_validation",
            Path("target/annotation_ocg_rendering-annotation-ocg-progressive-cache"),
            "corpus-manifest-renderer_validation.json",
            "multi-reference-render-results-renderer_validation.json",
            "multi-reference-diff-metrics-renderer_validation.json",
            "reference-disagreement-summary-renderer_validation.json",
        ),
        (
            "porterduff_radial_color_glyph",
            Path("target/multilingual_color_glyphs-cjk-rtl-color-glyph-reference"),
            "corpus-manifest-multilingual_color_glyphs.json",
    ...
    for roadmap task, root, corpus_name, render_name, diff_name, summary_name in sources:
        ...
        for item in summary.get("reference_disagreements", []) or summary.get(
            ...
            reference_disagreements.append({"feature_area": roadmap task, **item})
        rows.append(
            {
                "feature_area": roadmap task,
                "root": rel(root),
    ...
    fallback_taxonomy = {
        "schema_version": 1,
        "kind": "renderer_closeout_fallback_taxonomy_renderer_fuzz_cmm",
        "status": STATUS_IMPLEMENTED,
        ...
        "precise_remaining_limits": [
            {
                ...
                "owner": "Prepress CMM CMM/prepress native-boundary package plan",
                "status": STATUS_UNSUPPORTED,
            },
            {
                "feature": "device-link ICC and multicolor ICC transforms",
                "owner": "advanced CMM/prepress roadmap task",
                "status": STATUS_UNSUPPORTED,
            },
            {
    ...
    diff_metrics = {
        "schema_version": 1,
        "kind": "renderer_closeout_diff_metrics_renderer_fuzz_cmm",
        "status": STATUS_IMPLEMENTED,
        "thresholds": {
            ...
            "reason": "Reference Renderer introduced renderer reference raster tolerances for antialiasing and renderer-library sampling differences; Renderer Fuzz CMM does not relax them.",
        },
        "source_diff_metric_artifacts": [row["diff_metrics"] for row in rows],
    }
    render_results = {
        "schema_version": 1,
        "kind": "renderer_closeout_render_results_renderer_fuzz_cmm",
        "status": STATUS_IMPLEMENTED,
    ...
    corpus_manifest = {
        "schema_version": 1,
        "kind": "renderer_closeout_corpus_manifest_renderer_fuzz_cmm",
        "status": STATUS_IMPLEMENTED,
        "source_prompts": [row["roadmap task"] for row in rows],
    ...
    disagreements = {
        "schema_version": 1,
        "kind": "renderer_closeout_reference_disagreements_renderer_fuzz_cmm",
        "status": STATUS_IMPLEMENTED,
    ...
    performance = {
        "schema_version": 1,
        "kind": "renderer_closeout_performance_memory_renderer_fuzz_cmm",
        "status": STATUS_LIMITED,
        "memory_cap_mb": 4096,
        "source_artifacts": [
            "target/transparency_rendering-transparency-compositing/transparency_closeout-memory-report.json",
            ...
            "target/multilingual_color_glyphs-cjk-rtl-color-glyph-reference/colrv1-scheduler-memory-porterduff_radial_color_glyph.json",
        ],
...
def cmm_reports() -> dict[str, Any]:
    feasibility = {
        "schema_version": 1,
        "kind": "native_cmm_feasibility_renderer_fuzz_cmm",
        "candidate_backend_name": "LittleCMS lcms2",
        "candidate_backend_version": "not vendored in Renderer Fuzz CMM",
        ...
        "c_abi_impact": "no ABI change in Renderer Fuzz CMM; feature report exposes backend posture",
        ...
        "fuzzing_posture": "color_report and renderer_renderer_fuzz_cmm fuzz targets cover malformed ICC/color-space reporting paths",
        ...
        "security_policy_updates": ["docs/renderer_fuzz_cmm_cmm_security_policy.md", "docs/security_policy.md"],
    ...
    backend_matrix = {
        "schema_version": 1,
        "kind": "native_cmm_backend_matrix_renderer_fuzz_cmm",
    ...
    transform_tests = {
        "schema_version": 1,
        "kind": "native_cmm_transform_tests_renderer_fuzz_cmm",
        "status": STATUS_LIMITED,
        "numeric_proof": {
    ...
    render_reference = {
        "schema_version": 1,
        "kind": "native_cmm_render_reference_results_renderer_fuzz_cmm",
        "status": STATUS_LIMITED,
    ...
    cache_memory = {
        "schema_version": 1,
        "kind": "native_cmm_cache_memory_renderer_fuzz_cmm",
        "status": STATUS_IMPLEMENTED,
    ...
    package = {
        "schema_version": 1,
        "kind": "native_cmm_package_impact_renderer_fuzz_cmm",
        "status": STATUS_UNSUPPORTED,
...
def scope_matrix(
    ...
    def row(feature_id: str, roadmap_item: str, category: str, result: str, tests: list[str], artifacts: list[str], limit: str, owner: str) -> dict[str, Any]:
        return {
            ...
            "category": category,
            "current_status_before_renderer_fuzz_cmm": "native_renderer_through_porterduff_radial_color_glyph_artifacts_available",
    ...
    rows = [
        row("renderer_fuzz_target_inventory", "041", "fuzz", STATUS_IMPLEMENTED, ["cargo check --manifest-path fuzz/Cargo.toml --bins --jobs 1"], ["renderer-fuzz-target-inventory-renderer_fuzz_cmm.json"], "release-duration coverage-guided fuzzing deferred", "release hardening"),
        row("renderer_seed_corpus", "041", "fuzz", STATUS_IMPLEMENTED, ["renderer_fuzz_cmm mutator corpus smoke"], ["renderer-seed-corpus-manifest-renderer_fuzz_cmm.json"], "corpus promotion continues as crashes are reduced", "renderer QA"),
        row("renderer_structure_aware_mutator", "041", "fuzz", STATUS_IMPLEMENTED, ["renderer_fuzz_cmm mutator corpus smoke"], ["renderer-mutator-report-renderer_fuzz_cmm.json"], "not a replacement for libFuzzer coverage", "renderer QA"),
        row("renderer_metamorphic_tests", "041", "metamorphic", STATUS_IMPLEMENTED, ["cargo test -p wellfriendpdf-engine --test renderer_fuzz_cmm_renderer_metamorphic"], ["renderer-metamorphic-matrix-renderer_fuzz_cmm.json"], "category matrix points to Native Renderer-10F specialist tests", "renderer QA"),
        row("full_tile_band_equivalence", "041", "metamorphic", STATUS_IMPLEMENTED, ["renderer_fuzz_cmm_renderer_metamorphic"], ["full-tile-band-equivalence-renderer_fuzz_cmm.json"], "byte-exact only", "renderer QA"),
        row("cache_no_cache_equivalence", "041", "metamorphic", STATUS_IMPLEMENTED, ["renderer_fuzz_cmm_renderer_metamorphic"], ["cache-no-cache-equivalence-renderer_fuzz_cmm.json"], "byte-exact only", "renderer QA"),
        row("progressive_resume_equivalence", "041", "metamorphic", STATUS_IMPLEMENTED, ["renderer_fuzz_cmm_renderer_metamorphic"], ["progressive-equivalence-renderer_fuzz_cmm.json"], "byte-exact only", "renderer QA"),
        row("ocg_cache_separation", "041", "metamorphic", STATUS_IMPLEMENTED, ["renderer_validation validation", "renderer_fuzz_cmm_renderer_metamorphic"], ["ocg-cache-separation-renderer_fuzz_cmm.json"], "default OCG configs only", "renderer QA"),
        row("transparency_fuzz_cases", "041", "fuzz", STATUS_LIMITED, ["renderer_renderer_fuzz_cmm fuzz target", "mutator smoke"], ["renderer-mutator-report-renderer_fuzz_cmm.json"], "release-duration fuzzing deferred", "renderer QA"),
        row("shading_pattern_fuzz_cases", "041", "fuzz", STATUS_LIMITED, ["renderer_renderer_fuzz_cmm fuzz target", "functions fuzz target"], ["renderer-mutator-report-renderer_fuzz_cmm.json"], "release-duration fuzzing deferred", "renderer QA"),
        row("color_glyph_fuzz_cases", "041", "fuzz", STATUS_LIMITED, ["renderer_renderer_fuzz_cmm fuzz target", "fonts fuzz target"], ["renderer-mutator-report-renderer_fuzz_cmm.json"], "release-duration fuzzing deferred", "renderer QA"),
        row("annotation_appearance_fuzz_cases", "041", "fuzz", STATUS_LIMITED, ["renderer_renderer_fuzz_cmm fuzz target", "structured_pdf fuzz target"], ["renderer-mutator-report-renderer_fuzz_cmm.json"], "release-duration fuzzing deferred", "renderer QA"),
        row("renderer_crash_minimization_workflow", "041", "fuzz", STATUS_IMPLEMENTED, ["manual workflow documented"], ["renderer-crash-minimization-workflow-renderer_fuzz_cmm.md"], "none for short campaign", "renderer QA"),
        row("renderer_coverage_report_posture", "041", "fuzz", STATUS_DEFERRED, ["fuzz target compile smoke"], ["renderer-fuzz-smoke-report-renderer_fuzz_cmm.json"], "coverage-guided release run remains later", "release hardening"),
        row("renderer_parity_closeout_verdict", "042", "closeout", STATUS_IMPLEMENTED, ["Reference Renderer-10F reference artifacts"], ["renderer-closeout-reference-disagreements-renderer_fuzz_cmm.json"], "advanced CMM can begin with exact limits", "Prepress CMM"),
        row("reference_renderer_availability", "042", "closeout", STATUS_IMPLEMENTED, ["reference tool manifests"], ["renderer-closeout-render-results-renderer_fuzz_cmm.json"], "host tool versions remain environment-specific", "renderer QA"),
        row("poppler_pdfium_mupdf_summary", "042", "closeout", STATUS_IMPLEMENTED, ["Reference Renderer-10F reference artifacts"], ["renderer-closeout-diff-metrics-renderer_fuzz_cmm.json"], "uses established visual thresholds", "renderer QA"),
        row("fallback_taxonomy", "042", "closeout", STATUS_IMPLEMENTED, ["taxonomy review"], ["renderer-closeout-fallback-taxonomy-renderer_fuzz_cmm.json"], "only exact bounded limits remain", "renderer QA"),
        row("native_cmm_feasibility", "043", "cmm_audit", STATUS_IMPLEMENTED, ["audit doc"], ["native-cmm-feasibility-renderer_fuzz_cmm.json"], "LittleCMS not added in Renderer Fuzz CMM", "Prepress CMM CMM"),
        row("native_cmm_safety_policy", "043", "cmm_audit", STATUS_IMPLEMENTED, ["security policy docs"], ["native-cmm-feasibility-renderer_fuzz_cmm.json"], "unsafe/native boundary must be outside default engine", "Prepress CMM CMM"),
        row("native_cmm_dependency_policy", "043", "cmm_audit", STATUS_IMPLEMENTED, ["package impact report"], ["native-cmm-package-impact-renderer_fuzz_cmm.json"], "no silent native dependency", "Prepress CMM CMM"),
        row("littlecms_backend_decision", "044", "cmm_backend", STATUS_UNSUPPORTED, ["audit doc"], ["native-cmm-backend-matrix-renderer_fuzz_cmm.json"], "LittleCMS hard-blocked until audited native boundary", "Prepress CMM CMM"),
        row("icc_profile_load_limits", "044", "cmm_backend", STATUS_IMPLEMENTED, ["cargo test cmm"], ["native-cmm-cache-memory-renderer_fuzz_cmm.json"], "16 MiB profile cap", "CMM"),
        row("cmm_transform_cache", "044", "cmm_backend", STATUS_IMPLEMENTED, ["cargo test cmm"], ["native-cmm-cache-memory-renderer_fuzz_cmm.json"], "16 transform entries", "CMM"),
        row("output_intent_integration", "044", "cmm_backend", STATUS_LIMITED, ["color report tests"], ["native-cmm-render-reference-results-renderer_fuzz_cmm.json"], "output intent proofing transform is later", "Prepress CMM CMM"),
        row("devicergb_cmyk_transform_integration", "044", "cmm_backend", STATUS_LIMITED, ["color report tests"], ["native-cmm-transform-tests-renderer_fuzz_cmm.json"], "DeviceRGB target-profile proofing later; DeviceCMYK preview implemented", "Prepress CMM CMM"),
        row("shading_pattern_cmm_integration", "044", "cmm_backend", STATUS_LIMITED, ["advanced_rendering shading/pattern tests"], ["native-cmm-render-reference-results-renderer_fuzz_cmm.json"], "ICC/device-link proofing later", "Prepress CMM CMM"),
        row("image_cmm_integration", "044", "cmm_backend", STATUS_LIMITED, ["cmm unit tests", "color report tests"], ["native-cmm-render-reference-results-renderer_fuzz_cmm.json"], "ICCBased to sRGB only", "Prepress CMM CMM"),
        row("transparency_group_color_space_integration", "044", "cmm_backend", STATUS_LIMITED, ["transparency_closeout tests"], ["native-cmm-render-reference-results-renderer_fuzz_cmm.json"], "RGB framebuffer preview only", "Prepress CMM CMM"),
        row("public_reports_bindings", "044", "reporting", STATUS_IMPLEMENTED, ["Rust/CLI/Python/C ABI/WASM/.NET/Java tests"], ["public-feature-report-renderer_fuzz_cmm.json"], "schema additive only", "bindings"),
        row("validation_gates", "044", "validation", STATUS_LIMITED, ["see validation logs"], ["renderer_fuzz_cmm-validation-summary.json"], "unavailable commands must be recorded by final validation", "release"),
    ]
    return {
        "schema_version": 1,
        "kind": "renderer_fuzz_cmm_scope_matrix",
        "rows": rows,
...
def write_html_report(closeout: dict[str, Any], smoke: dict[str, Any]) -> None:
    ...
    body = [
        "<!doctype html><meta charset='utf-8'><title>Renderer Fuzz CMM Renderer Close-out</title>",
        ...
        "<h1>Renderer Fuzz CMM Renderer Fuzz / CMM Close-out</h1>",
        ...
        "<h2>Reference Corpus Sources</h2><table><tr><th>Roadmap task</th><th>Fixtures</th><th>Pages</th><th>Summary</th></tr>",
    ]
    for row in rows:
        body.append(
            "<tr>"
            f"<td>{html.escape(row['roadmap task'])}</td>"
...
def artifact_inventory() -> dict[str, Any]:
    ...
    return {
        "schema_version": 1,
        "kind": "renderer_fuzz_artifact_inventory_renderer_fuzz_cmm",
...
def write_docs_artifacts(args: argparse.Namespace) -> None:
    ...
    write_json(OUT_DIR / "renderer-fuzz-target-inventory-renderer_fuzz_cmm.json", fuzz_inventory)
    write_json(OUT_DIR / "renderer-seed-corpus-manifest-renderer_fuzz_cmm.json", seed_manifest)
    write_json(OUT_DIR / "renderer-mutator-report-renderer_fuzz_cmm.json", mutator_report)
    write_json(OUT_DIR / "renderer-fuzz-smoke-report-renderer_fuzz_cmm.json", smoke)
    write_text(
        OUT_DIR / "renderer-crash-minimization-workflow-renderer_fuzz_cmm.md",
        "# Renderer Fuzz CMM Renderer Crash Minimization Workflow\n\n"
        "1. Preserve the original seed and mutated PDF under the Renderer Fuzz CMM artifact root.\n"
    ...
    write_json(OUT_DIR / "renderer-metamorphic-matrix-renderer_fuzz_cmm.json", metamorphic["matrix"])
    write_json(OUT_DIR / "full-tile-band-equivalence-renderer_fuzz_cmm.json", metamorphic["full_tile_band"])
    write_json(OUT_DIR / "cache-no-cache-equivalence-renderer_fuzz_cmm.json", metamorphic["cache"])
    write_json(OUT_DIR / "progressive-equivalence-renderer_fuzz_cmm.json", metamorphic["progressive"])
    write_json(OUT_DIR / "ocg-cache-separation-renderer_fuzz_cmm.json", metamorphic["ocg"])
    write_json(OUT_DIR / "renderer-closeout-corpus-manifest-renderer_fuzz_cmm.json", closeout["corpus_manifest"])
    write_json(OUT_DIR / "renderer-closeout-render-results-renderer_fuzz_cmm.json", closeout["render_results"])
    write_json(OUT_DIR / "renderer-closeout-diff-metrics-renderer_fuzz_cmm.json", closeout["diff_metrics"])
    write_json(OUT_DIR / "renderer-closeout-reference-disagreements-renderer_fuzz_cmm.json", closeout["disagreements"])
    write_json(OUT_DIR / "renderer-closeout-fallback-taxonomy-renderer_fuzz_cmm.json", closeout["fallback_taxonomy"])
    write_json(OUT_DIR / "renderer-closeout-performance-memory-renderer_fuzz_cmm.json", closeout["performance"])
    write_json(OUT_DIR / "native-cmm-feasibility-renderer_fuzz_cmm.json", cmm["feasibility"])
    write_json(OUT_DIR / "native-cmm-backend-matrix-renderer_fuzz_cmm.json", cmm["backend_matrix"])
    write_json(OUT_DIR / "native-cmm-transform-tests-renderer_fuzz_cmm.json", cmm["transform_tests"])
    write_json(OUT_DIR / "native-cmm-render-reference-results-renderer_fuzz_cmm.json", cmm["render_reference"])
    write_json(OUT_DIR / "native-cmm-cache-memory-renderer_fuzz_cmm.json", cmm["cache_memory"])
    write_json(OUT_DIR / "native-cmm-package-impact-renderer_fuzz_cmm.json", cmm["package"])
    write_json(OUT_DIR / "renderer_fuzz_cmm-scope-matrix.json", matrix)
    ...
    payload = {
        "schema_version": 1,
        "kind": "public_feature_report_renderer_fuzz_cmm",
        "command": feature_report,
        "contains_renderer_fuzz_cmm_section": "renderer_fuzz_cmm_renderer_fuzz_cmm_closeout" in feature_stdout,
        "stdout_sha256": hashlib.sha256(feature_stdout.encode("utf-8")).hexdigest(),
        "stdout_bytes": len(feature_stdout.encode("utf-8")),
    }
    write_json(OUT_DIR / "public-feature-report-renderer_fuzz_cmm.json", payload)
    write_json(OUT_DIR / "renderer-fuzz-artifact-inventory-renderer_fuzz_cmm.json", artifact_inventory())
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate Renderer Fuzz CMM renderer/CMM close-out artifacts.")
