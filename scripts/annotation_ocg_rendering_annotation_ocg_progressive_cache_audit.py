#!/usr/bin/env python3
"""Generate Annotation Ocg Rendering annotation/OCG/progressive/cache audit artifacts."""
...
def write_representative_fixtures() -> list[dict[str, Any]]:
    ...
    vector = CORPUS_DIR / "annotation_ocg_rendering_tile_band_progressive_vector.pdf"
    b = PdfBuilder()
    ...
    ocg = CORPUS_DIR / "annotation_ocg_rendering_ocg_marked_content_hidden.pdf"
    b = PdfBuilder()
    ...
    annot = CORPUS_DIR / "annotation_ocg_rendering_widget_ap_stream.pdf"
    b = PdfBuilder()
        ...
        rows.append(
            {
                ...
                "pdfium_image": None,
                "mupdf_image": None,
                "diff_metrics": "target/annotation_ocg_rendering-annotation-ocg-progressive-cache/visual-diff-metrics.json",
...
def reference_manifest() -> dict[str, Any]:
    if REFERENCE_RENDERER_TOOL_MANIFEST.exists():
    ...
    for name, exe in [("poppler", "pdftoppm"), ("pdfium", "pdfium_test"), ("mupdf", "mutool")]:
        ...
        tools[name] = {
            ...
            "policy": "Annotation Ocg Rendering partial if unavailable",
        }
...
def main() -> int:
    ...
    HTML_REPORT.write_text(
        "<!doctype html><meta charset='utf-8'><title>Annotation Ocg Rendering Audit</title>"
        "<h1>Annotation Ocg Rendering Annotation / OCG / Progressive / Cache Audit</h1>"
    ...
    write_json(OUT_DIR / "annotation_ocg_rendering-audit-summary.json", {"feature_report": feature, "html_report": rel(HTML_REPORT)})
    return 0
