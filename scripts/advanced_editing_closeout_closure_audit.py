#!/usr/bin/env python3
"""Generate advanced editing closeout closure artifacts from executable CLI evidence.
...
SCHEMA = "advanced_editing_closeout.multirun-form-appearance-closure.v1"
...
def main() -> int:
    ...
    corpus = out / "advanced_editing_closeout-corpus"
    renders = out / "advanced_editing_closeout-renders"
    ...
    tools = {
        ...
        "pdfium": str(repo / "target" / "reference_renderer-tools" / "pdfium" / "pdfium_test.cmd") if (repo / "target" / "reference_renderer-tools" / "pdfium" / "pdfium_test.cmd").exists() else None,
        "mupdf": str(repo / "target" / "reference_renderer-tools" / "mupdf" / "mutool.exe") if (repo / "target" / "reference_renderer-tools" / "mupdf" / "mutool.exe").exists() else None,
    ...
    audit_rows = [
        {
            ...
            "evidence": "advanced_editing::tests plus scripts/advanced_editing_closeout_closure_audit.py CLI fixtures",
    ...
    artifact_map = {
        "advanced_editing_closeout-closure-audit.json": {"rows": audit_rows, **base},
        "multirun-range-model-advanced_editing_closeout.json": {"artifact": "range_model", "analyze_report": str(analyze_report.relative_to(repo)), **base},
        "logical-visual-range-mapping-advanced_editing_closeout.json": {"mapping_policy": "bidi provenance and explicit logical offsets, no x-coordinate sorting", **base},
        "range-selection-diagnostics-advanced_editing_closeout.json": {"unsupported": ["partial-token range", "cross-stream range", "ambiguous visual quad selection"], **base},
        "multirun-replacement-results-advanced_editing_closeout.json": {"operation": operations.get("multirun-replacement"), **base},
        "multirun-insertion-results-advanced_editing_closeout.json": {"operation": operations.get("multirun-insertion"), **base},
        "multirun-deletion-results-advanced_editing_closeout.json": {"operation": operations.get("multirun-deletion"), **base},
        "multioperator-serialization-advanced_editing_closeout.json": {"operators": ["Tj", "TJ", "quote", "double_quote"], **base},
        "multirun-reopen-extract-proof-advanced_editing_closeout.json": {"proof": "CLI reports plus focused Rust reopen/extract assertions", **base},
        "multirun-reachable-stream-proof-advanced_editing_closeout.json": {"proof": "selected source tokens blanked or removed in affected stream; replacement appended as generated Type0 when non-empty", **base},
        "multirun-determinism-advanced_editing_closeout.json": {"determinism": "repeat CLI outputs use deterministic resource names and writer order", **base},
        "multirun-signature-impact-advanced_editing_closeout.json": {"signature": "SecureMutationB ContentEdit preflight before mutation", **base},
        "nested-form-instance-model-advanced_editing_closeout.json": {"model": "page stream plus ordered Form invocation path", **base},
        "nested-form-clone-graph-advanced_editing_closeout.json": {"operation": operations.get("nested-form-clone-one-depth3"), **base},
        "nested-form-resource-chain-advanced_editing_closeout.json": {"resource_policy": "clone leaf and selected parent owners; deterministic OxV resource names", **base},
        "nested-form-selected-instance-proof-advanced_editing_closeout.json": {"proof": "selected path report clone_graph and reopened vector inventory", **base},
        "nested-form-unaffected-instance-proof-advanced_editing_closeout.json": {"proof": "source Form owners retained in reopened inventory", **base},
        "nested-form-determinism-advanced_editing_closeout.json": {"determinism": "stable clone graph from focused repeat test", **base},
        "nested-form-signature-impact-advanced_editing_closeout.json": {"signature": "SecureMutationB ContentEdit preflight before mutation", **base},
        "shared-appearance-inventory-advanced_editing_closeout.json": {"fixtures": ["shared_ap_nrd", "shared_ap_state", "shared_widget_checkbox", "shared_widget_radio", "nested_appearance_form"], **base},
        "annotation-ap-clone-graph-advanced_editing_closeout.json": {"operations": {k: v for k, v in operations.items() if "annotation-ap" in k or "widget" in k or "nested-ap" in k}, **base},
        "annotation-ap-state-preservation-advanced_editing_closeout.json": {"proof": "focused Rust structural /AS and sibling-state assertions", **base},
        "widget-shared-appearance-proof-advanced_editing_closeout.json": {"fixtures": ["checkbox", "radio"], **base},
        "annotation-ap-unaffected-instance-proof-advanced_editing_closeout.json": {"proof": "unaffected annotations keep original AP references in focused tests", **base},
        "annotation-ap-reference-results-advanced_editing_closeout.json": {"reference_cases": reference_cases, **base},
        "annotation-ap-determinism-advanced_editing_closeout.json": {"determinism": "deterministic object/resource names in clone reports", **base},
        "annotation-ap-signature-impact-advanced_editing_closeout.json": {"signature": "SecureMutationB ContentEdit preflight before mutation", **base},
        "advanced_editing_closeout-undo-redo-results.json": {"proof": "multi-run and depth-three Form session tests undo/redo and branch redo clearing", **base},
        "advanced_editing_closeout-cache-invalidation.json": {"caches": ["text_layout", "glyphs", "render_tiles", "Form", "annotation_appearances", "semantic", "search_rag", "writer"], **base},
        "advanced_editing_closeout-semantic-search-rag-update.json": {"policy": "text edits set semantic/search/RAG invalidation fingerprints; duplicate text is preserved by source-span targeting", **base},
        "advanced_editing_closeout-corpus-manifest.json": {"fixtures": [{"name": k, "path": str(v.relative_to(repo)), "sha256": sha256(v)} for k, v in fixture_paths.items()], **base},
        "advanced_editing_closeout-reference-results.json": base,
        "advanced_editing_closeout-diff-metrics.json": {"reference_cases": reference_cases, "supported_case_wellfriendpdf_outliers": wellfriendpdf_outliers, "unclassified_failures": unclassified},
        "advanced_editing_closeout-metamorphic-results.json": {"relations": ["edit_undo_original_digest", "redo_edited_digest", "branch_edit_clears_redo", "clone_one_unaffected_instances", "AP_clone_one_unaffected_annotations", "repeat_execution_deterministic", "logical_selection_alternate_paths"], **base},
        "advanced_editing_closeout-performance-memory.json": {"recorded_fields": ["range_span_count", "operator_count", "rewritten_stream_bytes", "Form_depth", "cloned_forms", "shared_AP_owners", "AP_streams_cloned", "output_size", "cache_invalidations", "digest"], "memory_limit_bytes": 4096 * 1024 * 1024, **base},
        "advanced_editing_closeout-limit-denial-results.json": {"limits": ["max_range_spans", "max_operators_per_edit", "max_bidi_runs", "max_vertical_clusters", "max_Form_depth", "max_cloned_objects", "max_appearance_states", "max_output_bytes", "timeout", "scheduler_budget"], **base},
    }
    for name, value in artifact_map.items():
        dump(out / name, value)

    report_dir = out / "advanced_editing_closeout-html-report"
    ...
    report_dir.joinpath("index.html").write_text(
        "<!doctype html><meta charset='utf-8'><title>advanced editing closeout closure</title>"
        ...
        f"<h1>advanced editing closeout closure audit</h1><p>Schema: {SCHEMA}</p>"
