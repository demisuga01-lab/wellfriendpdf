#!/usr/bin/env python3
"""Generate Signature Validation's local AIA path-building integration fixture.
...
def name(common_name: str) -> x509.Name:
    return x509.Name(
        [
            ...
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Wellfriend SignatureValidation Test PKI"),
...
def main() -> None:
    ...
    root_subject = name("Wellfriend SignatureValidation AIA Root")
    ...
    intermediate_subject = name("Wellfriend SignatureValidation AIA Intermediate")
    ...
    leaf_subject = name("Wellfriend SignatureValidation AIA Leaf")
    leaf = (
        x509.CertificateBuilder()
    ...
    bad_usage_leaf = (
        x509.CertificateBuilder()
        .subject_name(name("Wellfriend SignatureValidation Key Usage Restricted Leaf"))
        .issuer_name(intermediate.subject)
    ...
    crl_signer = (
        x509.CertificateBuilder()
        .subject_name(name("Wellfriend SignatureValidation Delegated CRL Signer"))
        .issuer_name(intermediate.subject)
    ...
    indirect_leaf = (
        x509.CertificateBuilder()
        .subject_name(name("Wellfriend SignatureValidation Indirect CRL Leaf"))
        .issuer_name(intermediate.subject)
    ...
    ocsp_responder = (
        x509.CertificateBuilder()
        .subject_name(name("Wellfriend SignatureValidation Delegated OCSP Responder"))
        .issuer_name(intermediate.subject)
    ...
    bad_ocsp_responder = (
        x509.CertificateBuilder()
        .subject_name(name("Wellfriend SignatureValidation Wrong EKU OCSP Responder"))
        .issuer_name(intermediate.subject)
    ...
    temporary_p12 = FIXTURES / "_signature_validation_aia_fixture.p12"
    temporary_bad_usage_p12 = FIXTURES / "_signature_validation_aia_bad_usage_fixture.p12"
    temporary_indirect_leaf_p12 = FIXTURES / "_signature_validation_aia_indirect_leaf_fixture.p12"
    try:
        temporary_p12.write_bytes(
            pkcs12.serialize_key_and_certificates(
                name=b"wellfriendpdf-signature_validation-aia-leaf",
                key=leaf_key,
                cert=leaf,
                cas=None,
                encryption_algorithm=BestAvailableEncryption(b"signature_validation-aia-fixture"),
            )
        )
        signer = signers.SimpleSigner.load_pkcs12(
            str(temporary_p12), passphrase=b"signature_validation-aia-fixture"
        )
        writer = IncrementalPdfFileWriter(io.BytesIO((FIXTURES / "minimal.pdf").read_bytes()))
        append_signature_field(writer, SigFieldSpec(sig_field_name="SignatureValidationAia"))
        output = io.BytesIO()
        signers.sign_pdf(
            writer,
            signers.PdfSignatureMetadata(field_name="SignatureValidationAia", md_algorithm="sha256"),
            signer=signer,
            output=output,
        )
        ...
        temporary_bad_usage_p12.write_bytes(
            pkcs12.serialize_key_and_certificates(
                name=b"wellfriendpdf-signature_validation-aia-bad-usage-leaf",
        ...
        bad_usage_signer = signers.SimpleSigner.load_pkcs12(
            str(temporary_bad_usage_p12), passphrase=b"signature_validation-aia-fixture"
        )
        bad_usage_writer = IncrementalPdfFileWriter(
            io.BytesIO((FIXTURES / "minimal.pdf").read_bytes())
        )
        append_signature_field(bad_usage_writer, SigFieldSpec(sig_field_name="SignatureValidationResumeadUsage"))
        bad_usage_output = io.BytesIO()
        signers.sign_pdf(
            bad_usage_writer,
            signers.PdfSignatureMetadata(
                field_name="SignatureValidationResumeadUsage", md_algorithm="sha256"
            ),
        ...
        temporary_indirect_leaf_p12.write_bytes(
            pkcs12.serialize_key_and_certificates(
                name=b"wellfriendpdf-signature_validation-aia-indirect-leaf",
        ...
        indirect_leaf_signer = signers.SimpleSigner.load_pkcs12(
            str(temporary_indirect_leaf_p12), passphrase=b"signature_validation-aia-fixture"
        )
        indirect_leaf_writer = IncrementalPdfFileWriter(
            io.BytesIO((FIXTURES / "minimal.pdf").read_bytes())
        )
        append_signature_field(
            indirect_leaf_writer, SigFieldSpec(sig_field_name="SignatureValidationIndirectCrl")
        )
        indirect_leaf_output = io.BytesIO()
        signers.sign_pdf(
            indirect_leaf_writer,
            signers.PdfSignatureMetadata(
                field_name="SignatureValidationIndirectCrl", md_algorithm="sha256"
            ),
