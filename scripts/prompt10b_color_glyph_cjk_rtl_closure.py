...
OXIDE_REPORT_DIR = OUT_DIR / "prompt10b-oxide-render-reports"
...
def render_compare(entries: list[dict[str, Any]], manifest: dict[str, Any], oxide_bin: str | None, dpi: int, timeout: int) -> dict[str, Any]:
    p06 = load_prompt06b()
    ...
    summary = {
        ...
        "oxide_outlier_failures": sum(
            1 for page in pages if page["prompt10b_classification"] in {"oxide_outlier_failure", "oxide_render_failure"}
        ),
...
def render_html(pages: list[dict[str, Any]], summary: dict[str, Any]) -> None:
    ...
    HTML_REPORT.write_text(
        ...
        f"<p>Rendered pages: {summary['page_count']}. Oxide outliers: {summary['oxide_outlier_failures']}. "
...
def write_closure_audit(render_payload: dict[str, Any] | None) -> None:
    ...
    write_json(
        CLOSURE_AUDIT,
        {
            ...
            "status": "complete" if summary.get("oxide_outlier_failures", 1) == 0 and summary.get("unclassified_failures", 1) == 0 else "partial_blocker",
            "oxide_outlier_failures": summary.get("oxide_outlier_failures"),
