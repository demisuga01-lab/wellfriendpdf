#!/usr/bin/env python3
"""Generate Prompt 31 audit docs and machine-readable closeout artifacts."""
...
TARGET = ROOT / "target" / "prompt31-provenance-operator-editing"


DOCS = {
    "prompt31_true_editing_architecture_audit.md": "Prompt 31 True Editing Architecture Audit",
    "prompt31_feature_matrix.md": "Prompt 31 Feature Matrix",
    ...
    "prompt31_bindings.md": "Prompt 31 Bindings",
    "prompt31_fuzzing.md": "Prompt 31 Fuzzing",
    "prompt31_performance_security.md": "Prompt 31 Performance Security",
    "prompt31_known_limits.md": "Prompt 31 Known Limits",
    "prompt31_release_verdict.md": "Prompt 31 Release Verdict",
}


ARTIFACTS = [
    "prompt31-starting-state.json",
    "prompt31-gap-matrix.json",
    ...
    "historical-gate-impact-prompt31.json",
    "final-validation-matrix-prompt31.json",
    "prompt31-final-release-verdict.json",
    "PROMPT31_FINAL_REPORT.md",
]
...
def common(verdict: str, result_dir: str | None) -> dict[str, Any]:
    ...
    return {
        "schema": "prompt31.provenance-operator-editing.v1",
        ...
        "verdict": verdict,
        "gates": {
            "fmt": sidecar("prompt31_vps_fmt", result_dir),
            "check": sidecar("prompt31_vps_check", result_dir),
            "clippy": sidecar("prompt31_vps_clippy", result_dir),
            "test": sidecar("prompt31_vps_test", result_dir),
            "engine_focus": sidecar("prompt31_vps_engine_focus", result_dir),
        },
    }
...
def artifact_payload(name: str, base: dict[str, Any]) -> Any:
    rows = [
        {
            ...
            "prompt31_action": "mapped and routed into Prompt 31 reports",
        },
        {
            "capability": "content_instruction_graph",
            "status": "canonical_complete",
            "module": "crates/engine/src/prompt20.rs",
            "prompt31_action": "reused for Tj/TJ/quote/double-quote source edits",
        },
        {
            "capability": "operator_preserving_text_edit",
            "status": "verified",
            "module": "crates/engine/src/prompt31.rs",
            "prompt31_action": "source operator mutation, reopen validation, overlay refusal",
        },
        {
            "capability": "path_form_occurrence_edit",
            "status": "verified_with_limits",
            "module": "crates/engine/src/prompt20.rs",
            "prompt31_action": "source vector mutation and clone-edit-one-instance routing",
        },
        {
            "capability": "image_occurrence_edit",
            "status": "deferred_prompt32",
            "module": "crates/engine/src/prompt31.rs",
            "prompt31_action": "typed no-change refusal; no overlay fallback",
        },
    ]
    if name.endswith(".md"):
        return None
    ...
    if "matrix" in name or "map" in name or "audit" in name or "inventory" in name:
        ...
        payload["remaining_limits"] = [
            "stable display-list-to-instruction IDs are deferred to Prompt 32",
            "image occurrence source mutation is refused until Prompt 32 occurrence graph closure",
            "geometric block and semantic document reflow are deferred to Prompt 33",
        ]
    else:
...
Prompt 31 verdict: {base['verdict']}.
"""
...
def main() -> int:
    ...
    for name in ARTIFACTS:
        path = TARGET / name
        if name.endswith(".md"):
            path.write_text(doc_body("Prompt 31 Final Report", base), encoding="utf-8", newline="\n")
        else:
