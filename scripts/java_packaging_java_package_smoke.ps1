...
$ToolDir = Join-Path $Repo "target/java_packaging-tools"
$SmokeDir = Join-Path $Repo "target/java_packaging-package-smoke"
...
$payload = [ordered]@{
    ...
    native_loading = "JAR smoke ran from target/java_packaging-package-smoke/run with WELLFRIENDPDF_NATIVE_LIBRARY unset and wellfriendpdf_capi copied to bindings/java/target/runtimes/$rid/native."
    gradle_policy = "Java Packaging Maven package smoke preserved; Gradle Packaging adds authoritative Gradle build/package support."
    result = "passed"
}
