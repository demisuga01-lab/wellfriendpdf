#!/usr/bin/env python3
"""Decode Scheduler codec fuzz campaign harness.
...
LOGICAL_TARGETS = [
    ("filter_chain", "filters", "raw filter-chain selector bytes", "target/decode_scheduler-codec-closeout/hostile-corpus/raw"),
    ("image_inventory", "parser_report", "PDF wrapper bytes that drive stream and image inventory", "target/decode_scheduler-codec-closeout/hostile-corpus/pdf"),
    ("dct", "image_decoders", "image decoder selector plus DCT bytes", "target/decode_scheduler-codec-closeout/hostile-corpus/raw"),
    ("jpx", "image_decoders", "image decoder selector plus JPX bytes", "target/decode_scheduler-codec-closeout/hostile-corpus/raw"),
    ("jbig2", "image_decoders", "image decoder selector plus JBIG2 bytes", "target/decode_scheduler-codec-closeout/hostile-corpus/raw"),
    ("ccitt", "image_decoders", "image decoder selector plus CCITT bytes", "target/decode_scheduler-codec-closeout/hostile-corpus/raw"),
    ("predictor", "predictor", "predictor parameter bytes plus payload", "target/decode_scheduler-codec-closeout/hostile-corpus/raw"),
    ("pdf_wrapper", "parser_report", "whole-PDF wrapper corpus", "target/decode_scheduler-codec-closeout/hostile-corpus/pdf"),
    ("inline_image", "parser_report", "content stream with inline image ambiguity", "target/decode_scheduler-codec-closeout/hostile-corpus/pdf"),
    ("worker_protocol", "filters", "worker-compatible filter payloads and caps", "target/decode_scheduler-codec-closeout/hostile-corpus/raw"),
    ("scheduler_admission", "filters", "small and oversized decode jobs under scheduler caps", "target/decode_scheduler-codec-closeout/hostile-corpus/raw"),
]
        ...
        entries.append(
            {
                ...
                "memory_policy": "run under Decode Scheduler release harness memory cap; decode targets keep DecodeLimits caps enabled",
                "artifact_path": str((ARTIFACT_DIR / logical).as_posix()),
                "corpus_path": str((CORPUS_DIR / logical).as_posix()),
                "reproduction_command": f"cargo +nightly fuzz run {backing} -- target/decode_scheduler-codec-closeout/fuzz-artifacts/{logical}/crash",
                "minimize_command": f"cargo +nightly fuzz tmin {backing} target/decode_scheduler-codec-closeout/fuzz-artifacts/{logical}/crash",
                "promotion_path": f"target/decode_scheduler-codec-closeout/regressions/{logical}/",
            }
        )
    report = {
        "schema_version": 1,
        ...
        "targets": entries,
        "dictionary": {
            "path": "target/decode_scheduler-codec-closeout/fuzz-dictionary.txt",
            "tokens": [
                "/Filter",
                "/DecodeParms",
                "FlateDecode",
        ...
        commands.append(
            {
                ...
                "command": [
                    ...
                    "-dict=target/decode_scheduler-codec-closeout/fuzz-dictionary.txt",
                ],
            }
        )
    return commands
...
def smoke(mode: str, dry_run: bool, timeout_sec: int) -> dict[str, Any]:
    ...
    report = {
        ...
        "minimization_workflow": [
            "copy crash artifact into target/decode_scheduler-codec-closeout/fuzz-artifacts/<logical_target>/",
            "run the recorded cargo +nightly fuzz tmin command",
            "rerun the recorded reproduction command",
            "promote minimized bytes into target/decode_scheduler-codec-closeout/regressions/<logical_target>/ and add a manifest row",
        ],
    }
