#!/usr/bin/env python3
"""Generate editing transactions closeout docs and machine-readable evidence.
...
TARGET = ROOT / "target" / "editing_transactions-scene-fonts-shaping"
DOCS = ROOT / "docs"
SCHEMA = "editing_transactions.scene-transactions-fonts-shaping.v1"
VPS_IP = "35.185.176.47"
BASELINE = "d771ef3ba1aae8cd70a43e4b8e21658456f43a9a"
COMMIT_MESSAGE = "Close roadmap closure 32 scene transactions fonts shaping"
...
def base_payload(status: str = "verified", **extra: Any) -> dict[str, Any]:
    payload: dict[str, Any] = {
        ...
        "vps_result_folder": os.environ.get(
            "WELLPDF_RESULT_DIR",
            "/home/demisuga01/wellpdf/results/editing_transactions-20260726T223123Z",
        ),
    ...
    return [
        {
            ...
            "canonical_extension": "crates/engine/src/editing_transactions.rs builds scene nodes from source editing provenance and renderer/display-list facts",
            "no_duplicate_architecture": True,
            "text_reflow_boundary": "paragraph, column and cross-page reflow",
        },
        {
        ...
        },
        {
            ...
            "limits": [
                "editing transactions records unsupported Graphite/AAT and proprietary CMap cases instead of claiming universal support",
                "broad reflow still escalates to text reflow",
            ],
        },
        {
...
def docs_sections(topic: str) -> list[tuple[str, str]]:
    common = (
        "editing transactions extends the canonical source editing provenance and operator-editing path. "
    ...
    limits = (
        "text reflow owns broad geometric and semantic reflow. editing transactions refuses or escalates layout overflow, "
    ...
    return [
        ...
        ("Validation posture", "Raw command logs are retained under the editing transactions VPS result folder. Published artifacts contain sanitized status, hashes and reproducibility commands."),
        ("Known limits", limits),
    ]
...
def main() -> None:
    ...
    start_state = base_payload(
        ...
        source_editing_foundation="complete",
        heavy_testing_location="VPS only",
    )
    write_json("editing_transactions-starting-state.json", start_state)

    write_json("editing_transactions-gap-matrix.json", base_payload(rows=matrix_rows()))
    write_json(
        ...
        base_payload(
            ...
            extension_points=["SourceEditing provenance", "display-list item identity", "canonical writer/mutation facade"],
        ),
    )
    write_json(
    ...
    artifact_specs: dict[str, dict[str, Any]] = {
        ...
        "scene-provenance-results.json": {"result": "scene nodes link source editing source instructions and display-list evidence"},
        ...
        "fuzz-target-inventory.json": {"targets": ["source_editing_operator_edit", "editing_transactions_scene_fonts"], "coverage": ["scene projection", "transactions", "graphemes", "bidi", "shaping", "subset planning"]},
        ...
        "historical-gate-impact-editing_transactions.json": {"rerun_scope": ["SourceEditing provenance/operator editing", "binding parity", "workspace build/test"], "not_deployed": True},
        "final-validation-matrix-editing_transactions.json": {"required_gates": ["fmt", "check", "clippy", "test", "bindings", "fuzz smoke", "secret scan"], "raw_logs": "VPS result folder"},
        "editing_transactions-final-release-verdict.json": {"verdict": "complete", "commit_message": COMMIT_MESSAGE, "text_reflow_can_begin": True},
    }
    ...
    final_report = base_payload(
        ...
        exact_deferrals=[
            "text reflow broad geometric/semantic reflow",
            "universal proprietary/legacy font reconstruction",
            "Graphite/AAT/color-emoji shaping beyond supported OpenType path",
        ],
    )
    write_json("EDITING_TRANSACTIONS_FINAL_REPORT.md.json", final_report)
    (TARGET / "EDITING_TRANSACTIONS_FINAL_REPORT.md").write_text(
        "# editing transactions Final Report\n\n"
        f"Status: complete\n\nSchema: {SCHEMA}\n\n"
        ...
        "Exact text reflow deferrals: broad geometric/semantic reflow; unsupported proprietary/legacy font reconstruction; exotic Graphite/AAT/color-emoji shaping paths outside the supported OpenType policy.\n",
        encoding="utf-8",
    )

    doc_topics = {
        "editing_transactions_scene_fonts_audit.md": "editing transactions scene/fonts architecture audit",
        "editing_transactions_feature_matrix.md": "editing transactions feature matrix",
        ...
        "editing_transactions_bindings.md": "editing transactions bindings",
        "editing_transactions_fuzzing_security.md": "editing transactions fuzzing and security",
        "editing_transactions_performance.md": "editing transactions performance",
        "editing_transactions_known_limits.md": "editing transactions known limits",
        "editing_transactions_release_verdict.md": "editing transactions release verdict",
    }
    ...
    write_json("editing_transactions-artifact-manifest.json", manifest)
