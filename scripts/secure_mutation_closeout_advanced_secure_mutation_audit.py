#!/usr/bin/env python3
"""Generate the secure mutation closeout closure bundle from serial executable gates."""
...
SCHEMA = "secure_mutation_closeout.advanced-secure-mutation-closure.v1"
...
ARTIFACTS = [
    "packed-stencil-redaction-secure_mutation_closeout.json", "indexed-image-redaction-secure_mutation_closeout.json",
    "packed-sample-reopen-proof-secure_mutation_closeout.json", "packed-sample-reachable-stream-proof-secure_mutation_closeout.json",
    "iccbased-redaction-matrix-secure_mutation_closeout.json", "iccbased-mask-redaction-secure_mutation_closeout.json",
    "iccbased-softmask-redaction-secure_mutation_closeout.json", "iccbased-reachable-stream-proof-secure_mutation_closeout.json",
    "inline-decodeparms-matrix-secure_mutation_closeout.json", "inline-predictor-redaction-secure_mutation_closeout.json",
    "inline-imagemask-redaction-secure_mutation_closeout.json", "inline-content-reparse-proof-secure_mutation_closeout.json",
    "inline-promotion-selection-secure_mutation_closeout.json", "inline-promotion-resource-update-secure_mutation_closeout.json",
    "inline-promotion-redaction-proof-secure_mutation_closeout.json", "inline-promotion-determinism-secure_mutation_closeout.json",
    "associated-file-owner-matrix-secure_mutation_closeout.json", "associated-file-owner-mutation-secure_mutation_closeout.json",
    "associated-file-relationship-preservation-secure_mutation_closeout.json", "associated-file-orphan-cleanup-secure_mutation_closeout.json",
    "associated-file-owner-reopen-secure_mutation_closeout.json", "signature-policy-enforcement-secure_mutation_closeout.json",
    "incremental-form-edit-proof-secure_mutation_closeout.json", "incremental-annotation-edit-proof-secure_mutation_closeout.json",
    "incremental-page-property-proof-secure_mutation_closeout.json", "docmdp-fieldmdp-blocking-secure_mutation_closeout.json",
    "incremental-prefix-revision-proof-secure_mutation_closeout.json", "post-save-signature-impact-secure_mutation_closeout.json",
    "secure_mutation_closeout-corpus-manifest.json", "secure_mutation_closeout-reference-results.json",
    "secure_mutation_closeout-diff-metrics.json", "secure_mutation_closeout-metamorphic-results.json",
    "secure_mutation_closeout-performance-memory.json", "secure_mutation_closeout-limit-denial-results.json",
]
...
def run(*args: str) -> dict:
    ...
    env.update({
        ...
        "RAYON_NUM_THREADS": "1",
        "RUST_TEST_THREADS": "1",
        "WELLFRIENDPDF_SECURE_MUTATION_CLOSEOUT_EXPORT_FIXTURES": str(OUT / "fixtures"),
    })
...
def reference_proof(tools: dict, wellfriendpdf: pathlib.Path) -> dict:
    ...
    render_dir = OUT / "secure_mutation_closeout-reference-renders"
...
def main() -> int:
    ...
    write("secure_mutation_closeout-starting-state.json", start)

    gates = [
        run("cargo", "test", "-p", "wellfriendpdf-engine", "--test", "secure_mutation_secure_mutation", "--jobs", "1"),
        run("cargo", "test", "-p", "wellfriendpdf-engine", "--test", "secure_mutation_closeout_advanced_secure_mutation", "--jobs", "1"),
        run("cargo", "build", "-p", "wellfriendpdf-cli", "--jobs", "1"),
    ]
    if not all(gate["passed"] for gate in gates):
        write("secure_mutation_closeout-focused-failure.json", {"schema_version": SCHEMA, "gates": gates})
        return 1

    feature_path = OUT / "public-feature-report-secure_mutation_closeout.json"
    ...
    section = feature["report"]["secure_mutation_closeout_advanced_secure_mutation_closure"]

    tools = {
        "poppler": tool("pdftoppm", ["target/roadmap task*-tools/**/pdftoppm.exe"]),
        "pdfium": tool("pdfium_test", ["target/roadmap task*-tools/**/pdfium_test.cmd"]),
        "mupdf": tool("mutool", ["target/roadmap task*-tools/**/mutool.exe"]),
        "qpdf": tool("qpdf", ["target/roadmap task*-tools/**/qpdf.exe"]),
        "pdfbox": tool(None, ["target/roadmap task*-tools/**/pdfbox*.jar"]),
    }
    reference = reference_proof(tools, ROOT / "target" / "debug" / "wellfriendpdf.exe")
    if reference["status"] != "passed":
        write("secure_mutation_closeout-reference-failure.json", reference)
        return 1
    matrix = {
    ...
    write("secure_mutation_closeout-closure-audit.json", matrix)

    common = {
        "schema_version": SCHEMA,
        "status": "passed",
        "focused_gates": gates,
        "fixture": "crates/engine/tests/secure_mutation_closeout_advanced_secure_mutation.rs",
    ...
    for name in ARTIFACTS:
        ...
        if name == "secure_mutation_closeout-reference-results.json":
        ...
        if name == "secure_mutation_closeout-diff-metrics.json":
            payload["reference_metrics"] = reference["metrics"]
        write(name, payload)

    html = OUT / "secure_mutation_closeout-html-report" / "index.html"
    html.parent.mkdir(parents=True, exist_ok=True)
    html.write_text(
        "<!doctype html><meta charset='utf-8'><title>secure mutation closeout closure</title>"
        "<h1>secure mutation closeout advanced secure mutation closure</h1>"
    ...
    write("secure_mutation_closeout-artifact-manifest.json", {
        "schema_version": SCHEMA,
        "files": sorted(path.name for path in OUT.glob("*secure_mutation_closeout*.json")),
