param(
    [string]$OutDir = "target/codec_boundary-codec-boundary-scheduler"
)
...
$rlboxReport = [ordered]@{
    schema_version = 1
    roadmap task = "combined_codec_boundary"
    status = "hard_blocked"
    verdict = "RLBox/WASM codec sandbox production integration is not viable in this repository in Codec Boundary; keep OS subprocess isolation as the practical codec sandbox boundary."
    head = $head
    dirty_entries = $status
    dependency_feasibility = [ordered]@{
        cargo_search_rlbox = $rlboxSearch
        cargo_search_rlbox_wasm = $rlboxWasmSearch
        existing_repo_integration = "No rlbox, wasmtime, wasmer, or wasm sandbox runtime integration was present in Cargo manifests or engine source during Codec Boundary inventory."
    }
    toolchain_feasibility = [ordered]@{
        ...
        linux_result = "not locally proven in this Windows Codec Boundary run; would require CI job with RLBox C++ toolchain and wasm runtime."
        macos_result = "not locally proven in this Windows Codec Boundary run; would require CI job with RLBox C++ toolchain and wasm runtime."
    ...
    performance_estimate = "No RLBox stub benchmark was run because the prototype was hard-blocked before build; subprocess isolation overhead remains measured by Release Packaging release gate and codec isolation tests."
...
$generator = Invoke-Capture "codec_boundary rust report generator" "cargo" @(
    "run",
    "-p",
    "wellfriendpdf-engine",
    "--example",
    "codec_boundary_reports",
    "--",
    $OutPath
)
...
if ($generator.exit_code -ne 0) {
    throw "Codec Boundary report generator failed"
}

Write-Host "Codec Boundary reports written to $OutPath"
