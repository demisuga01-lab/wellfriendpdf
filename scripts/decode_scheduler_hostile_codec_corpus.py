#!/usr/bin/env python3
"""Decode Scheduler hostile codec corpus generator and runner.

The committed artifact is this deterministic generator, not opaque hostile PDF
blobs. Generated fixtures live under target/decode_scheduler-codec-closeout and are safe
to delete/recreate.
"""
...
def pdf_bytes(hostile_stream: bytes, fixture_id: str) -> bytes:
    ...
    objects = [
        ...
        (4, empty_content),
        (5, hostile_stream),
        (6, f"<< /Producer (Wellfriend DecodeScheduler) /Fixture ({fixture_id}) >>".encode("ascii")),
    ]
    out = bytearray(b"%PDF-1.7\n% Wellfriend DecodeScheduler generated fixture\n")
        ...
        entries.append(
            {
                ...
                "generator_command": "python scripts/decode_scheduler_hostile_codec_corpus.py generate",
