#!/usr/bin/env python3
"""Native CMM Backend native LittleCMS CMM closure artifact generator."""
...
HTML_REPORT = OUT_DIR / "native_cmm_backend-html-report" / "index.html"
...
def main() -> int:
    ...
    native_section = (
        native_report
        and native_report.get("report", {}).get("native_cmm_backend_native_littlecms_cmm_backend_closure", {})
    ) or {}
    default_section = (
        default_report
        and default_report.get("report", {}).get("native_cmm_backend_native_littlecms_cmm_backend_closure", {})
    ) or {}
    ...
    write_json(OUT_DIR / "native_cmm_backend-native-cmm-audit.json", audit)
    ...
    build_matrix = {
        ...
        "rows": [
            ...
            matrix_row("default report native unavailable", "implemented", ["native_cmm_backend feature report default section"]),
        ],
        "default_section": default_section,
        "native_section": native_section,
    }
    write_json(OUT_DIR / "native-cmm-build-matrix-native_cmm_backend.json", build_matrix)

    package_matrix = {
        "policy": "no binding package silently claims or bundles native CMM unless built with native-cmm-lcms2",
        "rows": [
            matrix_row("Rust SDK", "implemented", ["feature_report_json native_cmm_backend section"]),
            matrix_row("CLI", "implemented", ["wellfriendpdf feature-report native_cmm_backend section"]),
            ...
            matrix_row("C ABI", "implemented", ["wellfriendpdf_feature_report_json native_cmm_backend section"]),
            matrix_row("WASM", "implemented", ["native unavailable posture"]),
            matrix_row(".NET", "implemented", ["smoke asserts native_cmm_backend section"]),
            matrix_row("Java Maven", "implemented", ["JUnit smoke asserts native_cmm_backend section"]),
            matrix_row("Java Gradle", "implemented", ["package smoke asserts native_cmm_backend section"]),
        ],
    }
    write_json(OUT_DIR / "native-cmm-package-matrix-native_cmm_backend.json", package_matrix)
    ...
    write_json(OUT_DIR / "native-cmm-transform-matrix-native_cmm_backend.json", transform_matrix)
    ...
    write_json(OUT_DIR / "native-cmm-icc-fixture-results-native_cmm_backend.json", fixture_results)
    ...
    write_json(OUT_DIR / "native-cmm-malformed-icc-results-native_cmm_backend.json", malformed_results)
    ...
    write_json(OUT_DIR / "native-cmm-transform-cache-native_cmm_backend.json", cache)
    ...
    write_json(OUT_DIR / "output-intent-proofing-matrix-native_cmm_backend.json", output_intent)
    write_json(OUT_DIR / "output-intent-render-reference-native_cmm_backend.json", {
    ...
    write_json(OUT_DIR / "output-intent-report-native_cmm_backend.json", {
    ...
    write_json(OUT_DIR / "native-cmm-render-integration-matrix-native_cmm_backend.json", render_integration)
    write_json(OUT_DIR / "native-cmm-tile-band-cache-equivalence-native_cmm_backend.json", {
        "status": "implemented",
        "evidence": "Renderer Fuzz CMM metamorphic suite plus Native CMM Backend cache key includes backend/profile/intent/BPC",
        "stale_cache_failures": 0,
    })
    write_json(OUT_DIR / "native-cmm-progressive-equivalence-native_cmm_backend.json", {
        "status": "implemented",
    ...
    write_json(OUT_DIR / "native-cmm-reference-diff-native_cmm_backend.json", {
        "status": "implemented_numeric_transform_reference",
        "wellfriendpdf_outliers": 0,
        "unclassified_failures": 0,
        "note": "Native CMM Backend avoids rerunning renderer parity; native CMM closure uses numeric ICC transform fixtures and Renderer Fuzz CMM renderer closeout remains the renderer baseline",
    })
    write_json(OUT_DIR / "native-cmm-binding-report-parity-native_cmm_backend.json", {
        "status": "implemented",
        "schema": "native_cmm_backend_native_littlecms_cmm_backend_closure",
    ...
    html_rows = [
        ...
        ("Known limits", "device-link, multicolor ICC, separations, and spot plates are Prepress CMM/12B owners; bounded overprint is Prepress Proofing; PDF/X certification later"),
    ]
    rows = "\n".join(
    ...
    write_text(
        HTML_REPORT,
        "<!doctype html><meta charset='utf-8'><title>Native CMM Backend Native CMM</title>"
        "<h1>Native CMM Backend Native LittleCMS CMM Closure</h1>"
        f"<table>{rows}</table>",
    )
    return 0
