...
$ToolRoot = Join-Path $Repo "target/release_packaging-tools/wasm-pack-$WasmPackVersion"
...
$WasmCargoTarget = Join-Path $Repo "target/release_packaging-tools/wasm-cargo-target"
...
Invoke-Logged "packaged node smoke" "node" @("scripts/wasm_packaging_wasm_pack_node_smoke.mjs", $NodePackage, $Fixture, $nodeSmoke) (Join-Path $EvidenceDir "wasm-pack-node-smoke-log.txt")
...
Write-Host "wasm-pack Wasm Packaging gate passed: $EvidenceDir"
