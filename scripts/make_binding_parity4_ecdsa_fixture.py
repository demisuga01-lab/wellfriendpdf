#!/usr/bin/env python3
"""Generate the independent ECDSA PDF fixture used by Signature Validation tests.
...
def main() -> None:
    ...
    name = x509.Name(
        [
            x509.NameAttribute(NameOID.COMMON_NAME, "Wellfriend SignatureValidation ECDSA Signer"),
    ...
    temporary_p12 = FIXTURES / "_signature_validation_ecdsa_fixture.p12"
    try:
        temporary_p12.write_bytes(
            pkcs12.serialize_key_and_certificates(
                name=b"wellfriendpdf-signature_validation-ecdsa",
                key=key,
                cert=cert,
                cas=None,
                encryption_algorithm=BestAvailableEncryption(b"signature_validation-fixture"),
            )
        )
        signer = signers.SimpleSigner.load_pkcs12(
            str(temporary_p12), passphrase=b"signature_validation-fixture"
        )
        ...
        sign(OUT_PDF, "SignatureValidationEcdsa")
        sign(OUT_PADES_PDF, "SignatureValidationPadesEcdsa", SigSeedSubFilter.PADES)
