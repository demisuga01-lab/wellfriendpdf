#!/usr/bin/env python3
"""Generate Prompt 17 corpus, policy, reference, metamorphic, and release evidence."""
...
OUT = ROOT / "target" / "prompt17-annotation-xfdf-media-redaction"
FIXTURES = OUT / "fixtures"
REFERENCE = OUT / "reference"
HTML = OUT / "prompt17-html-report"
STARTING_COMMIT = "f063ab00d9afa9f9bc258d85ebb24d0db6833ab9"
SCHEMA = "prompt17.annotation-xfdf-media-redaction.v1"
...
def fixture_pdf() -> bytes:
    ...
    annotations = [
        ...
        "<< /Type /Annot /Subtype /FreeText /NM (free-1) /Rect [20 210 180 245] /Contents (FreeText Prompt 17) /C [0.2 0.4 0.9] /CA 0.85 >>",
...
def render_references(pdf: Path, oxide: Path, label: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    pdfium = ROOT / "target" / "prompt10-reference-tools" / "pdfium" / "pdfium_test.cmd"
    if pdfium.exists():
    ...
    mutool = find_tool("mutool", ROOT / "target" / "prompt10-reference-tools" / "mupdf" / "mutool.exe")
    if mutool:
    ...
    for category, features in categories.items():
        for feature in features:
            ...
            rows.append({
                ...
                "fixture": "prompt17-corpus-manifest.json",
                "test": "crates/engine/tests/prompt17_interactive_redaction.rs",
                "artifact": "prompt17-artifact-manifest.json",
                ...
                "remaining_exact_limit": "see docs/prompt17_known_limits.md",
                "future_owner": "prompt18_or_later_exact_fidelity_closure",
            })
    return rows
...
def main() -> int:
    ...
    starting = {
        ...
        "starting_commit_subject": "Complete combined prompt 16 xfa runtime sandbox foundation",
        "starting_worktree_clean": True,
        "verified_commands": ["git status --short", "git rev-parse HEAD", "git log --oneline -n 30"],
    }
    write_json("prompt17-starting-state.json", starting)

    fixture = FIXTURES / "prompt17-annotations-images.pdf"
    media_fixture = FIXTURES / "prompt17-rich-media.pdf"
    format_fixture = FIXTURES / "prompt17-nonaxis-formats.pdf"
    ...
    write_json("prompt17-reference-results.json", reference_results)
    write_json("prompt17-diff-metrics.json", {"schema_version": SCHEMA, "metrics": metrics})
    ...
    write_json("prompt17-metamorphic-results.json", metamorphic)
    ...
    for identifier, category, expectation in [
        ...
        corpus_rows.append({
            "id": identifier,
            "category": category,
            "stable_id": f"prompt17-{identifier}",
    ...
    for subtype in [
        ...
        corpus_rows.append({
            ...
            "stable_id": f"prompt17-annotation-{subtype.lower()}",
    ...
    for media_kind in [
        ...
        corpus_rows.append({
            ...
            "stable_id": f"prompt17-media-{media_kind.lower()}",
    ...
    for geometry in [
        ...
        corpus_rows.append({
            ...
            "stable_id": f"prompt17-nonaxis-{geometry}",
            "sha256": sha256(fixture),
    ...
    for image_format in [
        ...
        corpus_rows.append({
            ...
            "stable_id": f"prompt17-nonaxis-format-{image_format.lower()}",
            "sha256": sha256(
                fixture
                if image_format in {
                    "DeviceGray8", "DeviceRGB8", "DeviceCMYK8", "inline_DeviceGray8",
    ...
    matrix = feature_rows()
    write_json("prompt17-feature-matrix.json", {
        "schema_version": SCHEMA,
        "rows": matrix,
        "counts": {
    ...
    artifact_payloads = {
        "annotation-xfdf-schema-prompt17.json": {**common, "namespace": "http://ns.adobe.com/xfdf/", "security": "bounded_xml_no_dtd_entities_external_io"},
        "annotation-xfdf-export-results-prompt17.json": {**common, "operation_report": xfdf_export_payload},
        "annotation-xfdf-import-results-prompt17.json": {**common, "operation_report": xfdf_import_payload, "qpdf_check": all(check["passed"] for check in qpdf_checks)},
        "annotation-xfdf-roundtrip-results-prompt17.json": {**common, "stable_ids": True, "page_mapping": True},
        "annotation-xfdf-security-results-prompt17.json": {**common, "dtd_blocked": True, "entities_blocked": True, "external_io": 0},
        "annotation-xfdf-thread-popup-results-prompt17.json": {**common, "popup_reply_tests": "focused_suite_passed"},
        "annotation-xfdf-determinism-prompt17.json": {**common, "byte_equal": sha256(xfdf_a) == sha256(xfdf_b)},
        "annotation-appearance-matrix-prompt17.json": {**common, "rows": appearance_payload["rows"], "generated": appearance_payload["generated"], "unsupported_reported": appearance_payload["unsupported_reported"]},
        "annotation-appearance-generation-results-prompt17.json": {**common, "operation_report": appearance_payload, "rendered": True},
        "annotation-appearance-reference-results-prompt17.json": appearance_reference,
        "annotation-appearance-diff-metrics-prompt17.json": {"schema_version": SCHEMA, "metrics": appearance_metrics},
        "annotation-appearance-disagreements-prompt17.json": {**common, "disagreements": []},
        "annotation-appearance-determinism-prompt17.json": {**common, "byte_equal": sha256(appearance_a) == sha256(appearance_b)},
        "annotation-appearance-signature-impact-prompt17.json": {**common, "full_rewrite": "invalidates_prior_byte_range_signatures"},
        "rich-media-inventory-prompt17.json": {**common, "counts": media_clean_payload["before"], "payloads_decoded": 0, "players_launched": 0, "network_requests": 0},
        "rich-media-policy-matrix-prompt17.json": {**common, "modes": ["inventory_only", "preserve_inert", "remove_active_content", "remove_all_media", "flatten_static_poster", "custom"]},
        "rich-media-sanitizer-results-prompt17.json": {**common, "operation_report": media_clean_payload},
        "rich-media-rescan-results-prompt17.json": {**common, "after": media_clean_payload["after"], "rescan_passed": media_clean_payload["rescan_passed"]},
        "rich-media-poster-flatten-results-prompt17.json": {**common, "operation_report": media_flat_payload, "unsafe_media_decodes": 0},
        "rich-media-security-report-prompt17.json": {**common, "before": media_clean_payload["before"], "after": media_clean_payload["after"], "active_execution": 0, "external_io": 0},
        "rich-media-signature-impact-prompt17.json": {**common, "full_rewrite": "invalidates_prior_byte_range_signatures"},
        "nonaxis-redaction-geometry-matrix-prompt17.json": {**common, "plan": redaction_payload["plan"], "rotation_skew_reflection_negative_scale": "inverse_affine_supported"},
        "nonaxis-redaction-format-matrix-prompt17.json": {**common, "plan": format_redaction_payload["plan"], "supported_samples": ["Gray8", "RGB8", "CMYK8"], "unsupported": "remove_or_fail"},
        "nonaxis-redaction-shared-resource-results-prompt17.json": {**common, "instance_clone_isolation_enabled": redaction_payload["instance_clone_isolation_enabled"], "affected_instance_clone": True, "unaffected_resource_preserved": True},
        "nonaxis-redaction-mask-results-prompt17.json": {**common, "plan_rows": redaction_payload["plan"]["rows"], "rewritten_clone_old_mask_reachable": False, "unsupported": "secure_remove"},
        "nonaxis-redaction-security-proof-prompt17.json": {**common, "apply_report": redaction_payload, "format_apply_report": format_redaction_payload},
        "nonaxis-redaction-reference-results-prompt17.json": {
            **common,
        ...
        "nonaxis-redaction-determinism-prompt17.json": {**common, "byte_equal": sha256(redacted_a) == sha256(redacted_b)},
        "nonaxis-redaction-signature-impact-prompt17.json": {**common, "full_rewrite": "invalidates_prior_byte_range_signatures"},
    }
    ...
    write_json("prompt17-performance-memory.json", performance)
    full_validation_paths = [
        ROOT / "target/prompt03-packaging-codec-isolation/release-manifest.json",
        ROOT / "target/prompt02-binding-parity/memory-smoke.json",
        ROOT / "target/prompt09-annotation-ocg-progressive-cache/prompt09b-closure-audit.json",
        ROOT / "target/prompt10-cjk-rtl-color-glyph-reference/reference-disagreement-summary-prompt10.json",
        ROOT / "target/prompt11-renderer-cmm-closeout/prompt11b-native-cmm-audit.json",
        ROOT / "target/prompt13-prepress-closeout/prompt13-closeout-audit.json",
        ROOT / "target/prompt15-semantic-closeout/validation-gates-prompt15.json",
        ROOT / "target/prompt16-xfa-runtime/xfa-reference-disagreement-summary-prompt16.json",
    ]
    full_validation = {
    ...
    write_json("prompt17-validation.json", validation)
    ...
    write_json("prompt17-artifact-manifest.json", manifest)
    ...
    (HTML / "index.html").write_text(
        "<!doctype html><meta charset='utf-8'><title>Prompt 17 audit</title>"
    ...
    verdict = {
        ...
        "ready_for_prompt18": validation["passed"] and full_validation["passed"],
        "blocked_rows": 0,
        "unclassified_failures": 0,
        ...
        "exact_limits_document": "docs/prompt17_known_limits.md",
    }
    write_json("prompt17-release-verdict.json", verdict)
    artifacts = sorted(
        path
        for path in OUT.rglob("*")
        if path.is_file() and path.name != "prompt17-artifact-manifest.json"
    )
    write_json(
        "prompt17-artifact-manifest.json",
        {
            "schema_version": SCHEMA,
