param(
    [string]$ToolsDir = "target/reference_renderer-tools",
    [string]$ManifestPath = "target/native_renderer-renderer-native-replay/reference-tool-manifest-reference_renderer.json",
    [int]$Dpi = 72,
...
function Bootstrap-Pdfium {
    ...
    $renderScript = Join-Path $RepoRoot "scripts/reference_renderer_pdfium_render.py"
...
if ($popplerPath) {
    ...
    $tools.poppler = [ordered]@{
        ...
        source_bootstrap_strategy = "not_bootstrapped_reference_renderer_requires_existing_pdftoppm"
...
$manifestPayload = [ordered]@{
    schema_version = 1
    kind = "reference_renderer_reference_tool_manifest"
    host = [ordered]@{
...
if ($tools.poppler.availability -ne "available" -or $tools.pdfium.availability -ne "available" -or $tools.mupdf.availability -ne "available") {
    Write-Error "Reference Renderer reference bootstrap did not make all required tools available. See $manifest"
    exit 1
}

Write-Host "Reference Renderer reference renderer manifest: $manifest"
