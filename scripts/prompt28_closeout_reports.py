#!/usr/bin/env python3
"""Generate Prompt 28 closeout artifacts from VPS evidence."""
...
ARTIFACT_ROOT = Path("target/prompt28-long-fuzz-safedocs")
REQUIRED_DOCS = [
    "docs/prompt28_long_fuzz_safedocs_audit.md",
    "docs/prompt28_feature_matrix.md",
    ...
    "docs/prompt28_known_limits.md",
    "docs/prompt28_release_verdict.md",
]

REQUIRED_ARTIFACT_FILES = [
    "prompt28-starting-state.json",
    "vps-test-plan.json",
    "vps-toolchain-inventory.json",
    "vps-provisioning-log.json",
    "prompt28-feature-matrix.json",
    "prompt28-campaign-plan.json",
    ...
    "prompt28-crash-triage-master.json",
    "binding-regression-results.json",
    "performance-memory-results.json",
    "security-audit-results.json",
    "secret-scan-results.json",
    "historical-gate-impact-prompt28.json",
    "final-validation-matrix-prompt28.json",
    "prompt28-final-release-verdict.json",
    "PROMPT28_FINAL_REPORT.md",
]
...
def artifact_from_phases(group: str, kind: str, rows: list[dict[str, object]], status_name: str) -> dict[str, object]:
    ...
    return {
        "schema_version": f"prompt28.{group}.{kind}.v1",
...
def crash_triage(group: str, summary: dict[str, object]) -> dict[str, object]:
    ...
    return {
        "schema_version": f"prompt28.{group}.crash-triage.v1",
        "generated_at_utc": utc(),
        "findings": findings,
...
def promoted_seeds(group: str) -> dict[str, object]:
    return {
        "schema_version": f"prompt28.{group}.promoted-seeds.v1",
        "generated_at_utc": utc(),
...
def secret_scan(repo: Path) -> dict[str, object]:
    ...
    def classify_hit(rel: str, path: Path, kind: str, text: str, line: int, matched: str) -> str:
        ...
        rel_lower = rel.lower()
        if path.name in {"prompt28_closeout_reports.py", "prompt27_closeout_reports.py"}:
    ...
    return {
        "schema_version": "prompt28.secret-scan.v1",
        "generated_at_utc": utc(),
...
def binding_results(vps_results: Path | None) -> dict[str, object]:
    ...
    return {
        "schema_version": "prompt28.binding-regression-results.v1",
        "generated_at_utc": utc(),
        "rows": rows,
        "failed": failed,
...
def validation_matrix(repo: Path, artifact_root: Path, vps_results: Path | None, required_artifacts: list[str]) -> dict[str, object]:
    ...
    return {
        "schema_version": "prompt28.final-validation-matrix.v1",
        "generated_at_utc": utc(),
        "rows": rows,
        "failed": failed,
...
def docs_status(repo: Path) -> dict[str, object]:
    ...
    return {
        "schema_version": "prompt28.docs-artifacts.v1",
        "generated_at_utc": utc(),
        "docs": rows,
...
def external_tool_support(vps_results: Path | None) -> dict[str, object]:
    ...
    return {
        "schema_version": "prompt28.vps-toolchain.v1",
        "generated_at_utc": utc(),
...
def artifact_presence(artifact_root: Path) -> dict[str, object]:
    ...
    return {
        "schema_version": "prompt28.required-artifact-presence.v1",
        "generated_at_utc": utc(),
        "rows": rows,
        "missing": missing,
...
def performance_memory(codec: dict[str, object], renderer: dict[str, object], writer: dict[str, object], safedocs: dict[str, object]) -> dict[str, object]:
    ...
    return {
        "schema_version": "prompt28.performance-memory.v1",
        "generated_at_utc": utc(),
...
def security_audit(secrets: dict[str, object], master: dict[str, object]) -> dict[str, object]:
    return {
        "schema_version": "prompt28.security-audit.v1",
        "generated_at_utc": utc(),
        "checks": [
...
def historical_impact() -> dict[str, object]:
    return {
        "schema_version": "prompt28.historical-gate-impact.v1",
        "generated_at_utc": utc(),
        "impacted_prompts": ["05", "10", "11", "18", "20", "21", "22", "23", "25", "26", "27"],
        "rationale": "Prompt 28 changes fuzz/corpus/release-hardening tooling and only bugfixes Prompt 28-owned findings if any are discovered. Full workspace and binding gates are rerun on VPS; subsystem campaign fuzzing covers codec, renderer, writer/edit, and Prompt 27 release-fuzz architecture reuse.",
        "verdict": "passed",
    }
...
def final_report(result_path: Path, final: dict[str, object], outputs: dict[str, dict[str, object]]) -> None:
    ...
    matrix = outputs.get("final-validation-matrix-prompt28.json", {})
...
def main() -> int:
    ...
    master = {
        "schema_version": "prompt28.crash-triage-master.v1",
        "generated_at_utc": utc(),
    ...
    outputs: dict[str, dict[str, object]] = {
        ...
        "codec-long-campaign-results.json": {
            "schema_version": "prompt28.codec.long-campaign-results.v1",
            "generated_at_utc": utc(),
        ...
        "renderer-long-campaign-results.json": {
            "schema_version": "prompt28.renderer.long-campaign-results.v1",
            "generated_at_utc": utc(),
        ...
        "renderer-metamorphic-results.json": {
            "schema_version": "prompt28.renderer.metamorphic-results.v1",
            "generated_at_utc": utc(),
            "status": "covered_by_display_list_renderer_prompt11_structured_pdf_fuzz",
        ...
        "writer-edit-long-campaign-results.json": {
            "schema_version": "prompt28.writer-edit.long-campaign-results.v1",
            "generated_at_utc": utc(),
        ...
        "historical-gate-impact-prompt28.json": historical,
        "docs-artifacts-prompt28.json": docs,
    ...
    outputs["final-validation-matrix-prompt28.json"] = validation
    ...
    final = {
        "schema_version": "prompt28.final-release-verdict.v1",
        "generated_at_utc": utc(),
    ...
    outputs["prompt28-final-release-verdict.json"] = final
    for name, payload in outputs.items():
        write_json(artifact_root / name, payload)
    final_report(artifact_root / "PROMPT28_FINAL_REPORT.md", final, outputs)
    presence = artifact_presence(artifact_root)
    outputs["prompt28-required-artifact-presence.json"] = presence
    write_json(artifact_root / "prompt28-required-artifact-presence.json", presence)
    if presence["verdict"] != "passed" and final["status"] == "complete":
        ...
        outputs["prompt28-final-release-verdict.json"] = final
        write_json(artifact_root / "prompt28-final-release-verdict.json", final)
        final_report(artifact_root / "PROMPT28_FINAL_REPORT.md", final, outputs)
