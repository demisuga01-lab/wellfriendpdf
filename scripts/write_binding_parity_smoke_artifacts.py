#!/usr/bin/env python3
"""Write Binding Parity smoke/parity artifacts from binding test outputs."""
...
OUT = ROOT / "target" / "binding_parity-binding-parity"
...
def wasm_smoke() -> dict:
    ...
    return {
        ...
        "runtime_smoke": runtime or {
            "status": "not_run",
            "reason": "Node wasm-bindgen glue not generated under target/binding_parity-binding-parity/wasm-node-pkg",
        },
    }
...
def run_wasm_node_smoke(node_glue: Path) -> dict:
    js = """
const fs = require('fs');
const wasm = require('./target/binding_parity-binding-parity/wasm-node-pkg/wellfriendpdf_wasm.js');
...
def compare_reports(dotnet: dict | None, java: dict | None) -> dict:
    ...
    exclusions = {
        "advanced_chunks": "Semantic Closeout binding smokes intentionally use different semantic fixtures",
        "semantic_bundle": "Semantic Closeout binding smokes intentionally use different semantic fixtures",
        "semantic_search": "Semantic Closeout binding smokes intentionally use different semantic fixtures",
    ...
    if not dotnet:
        write_json(
            OUT / "dotnet-smoke.json",
            {
                "surface": "dotnet",
                "status": "missing",
                "reason": "Run dotnet tests with WELLFRIENDPDF_BINDING_PARITY_ARTIFACT_DIR=target/binding_parity-binding-parity",
            },
        )
    if not java:
        write_json(
            OUT / "java-smoke.json",
            {
                "surface": "java",
                "status": "missing",
                "reason": "Run Java smoke with WELLFRIENDPDF_BINDING_PARITY_ARTIFACT_DIR=target/binding_parity-binding-parity",
            },
        )
    write_json(OUT / "cross-binding-parity.json", compare_reports(dotnet, java))
    print(f"wrote Binding Parity smoke artifacts under {OUT}")
