#!/usr/bin/env python3
"""Color Glyph Hinting color glyph, hinting, and exotic CFF closure harness."""
...
FIXTURE_DIR = OUT_DIR / "color_glyph_hinting-fixtures"
RENDER_DIR = OUT_DIR / "color_glyph_hinting-renders"
DIFF_DIR = OUT_DIR / "color_glyph_hinting-diffs"
LOG_DIR = OUT_DIR / "color_glyph_hinting-logs"
WELLFRIENDPDF_REPORT_DIR = OUT_DIR / "color_glyph_hinting-wellfriendpdf-render-reports"
HTML_REPORT = OUT_DIR / "color_glyph_hinting-html-report" / "index.html"
TOOL_MANIFEST = OUT_DIR / "reference-tool-manifest-multilingual_color_glyphs.json"

CLOSURE_AUDIT = OUT_DIR / "color_glyph_hinting-closure-audit.json"
RENDER_RESULTS = OUT_DIR / "multi-reference-render-results-color_glyph_hinting.json"
DIFF_METRICS = OUT_DIR / "multi-reference-diff-metrics-color_glyph_hinting.json"
DISAGREEMENT_SUMMARY = OUT_DIR / "reference-disagreement-summary-color_glyph_hinting.json"
PUBLIC_FEATURE_REPORT = OUT_DIR / "public-feature-report-color_glyph_hinting.json"

MATRIX_FILES = {
    "colrv1": OUT_DIR / "color-glyph-colrv1-matrix-color_glyph_hinting.json",
    "colrv1_results": OUT_DIR / "color-glyph-colrv1-reference-results-color_glyph_hinting.json",
    "svg": OUT_DIR / "color-glyph-svg-static-subset-matrix-color_glyph_hinting.json",
    "svg_policy": OUT_DIR / "color-glyph-svg-security-policy-color_glyph_hinting.json",
    "svg_results": OUT_DIR / "color-glyph-svg-reference-results-color_glyph_hinting.json",
    "bitmap": OUT_DIR / "color-glyph-bitmap-payload-matrix-color_glyph_hinting.json",
    "cbdt_results": OUT_DIR / "color-glyph-cbdt-cblc-results-color_glyph_hinting.json",
    "sbix_results": OUT_DIR / "color-glyph-sbix-results-color_glyph_hinting.json",
    "hinting": OUT_DIR / "hinting-posture-color_glyph_hinting.json",
    "cff": OUT_DIR / "cid-keyed-cff-clipping-matrix-color_glyph_hinting.json",
    "cff_results": OUT_DIR / "cid-keyed-cff-clipping-reference-results-color_glyph_hinting.json",
}
...
def load_cjk_rtl_color_glyph_closeout() -> Any:
    script = Path("scripts/cjk_rtl_color_glyph_closeout_color_glyph_cjk_rtl_closure.py")
    spec = importlib.util.spec_from_file_location("cjk_rtl_color_glyph_closeout_color_glyph_cjk_rtl_closure", script)
...
def bootstrap_reference_manifest(dpi: int, timeout: int) -> dict[str, Any]:
    ...
    if missing:
        raise RuntimeError(f"Color Glyph Hinting requires reference renderers: {', '.join(missing)}")
    return manifest
...
def generate_fixtures(p10b: Any) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for name, path in fonts.items():
        if not path.exists():
            raise RuntimeError(f"required Color Glyph Hinting fixture font missing: {name} {path}")
    ...
    sbix_font, sbix_gid = p10b.make_sbix_font(fonts["latin"], FIXTURE_DIR / "color_glyph_hinting-sbix-png.ttf")

    colrv1_pdf = FIXTURE_DIR / "color_glyph_hinting-colrv1-solid-transform.pdf"
    colrv1_content = (
        "0 0 0 rg\n"
    ...
    korean_pdf = FIXTURE_DIR / "color_glyph_hinting-korean-hinting-regression.pdf"
    korean_content = "".join(
    ...
    hebrew_pdf = FIXTURE_DIR / "color_glyph_hinting-hebrew-hinting-regression.pdf"
    hebrew_content = "".join(
    ...
    sbix_pdf = FIXTURE_DIR / "color_glyph_hinting-sbix-png-regression.pdf"
    sbix_content = (
    ...
    entries = [
        {
            "id": "color_glyph_hinting_colrv1_solid_transform",
        ...
        {
            "id": "color_glyph_hinting_korean_hinting_regression",
        ...
        {
            "id": "color_glyph_hinting_hebrew_hinting_regression",
        ...
        {
            "id": "color_glyph_hinting_sbix_png_regression",
    ...
    if cff_fixture.exists():
        entries.append(
            {
                "id": "color_glyph_hinting_cid_keyed_cff_clip",
    ...
    metadata = {
        ...
        "policy_only_rows": [
            "color_glyph_hinting_colrv1_unsupported_gradient_operator",
            "color_glyph_hinting_svg_static_subset_classifier",
            "color_glyph_hinting_svg_security_blocked_script",
            "color_glyph_hinting_bitmap_non_png_payload_policy",
        ],
    }
...
def render_compare(
    ...
    for entry in entries:
        ...
        classification = classify_color_glyph_hinting(raw, entry, pair_metrics)
        page = {
            "id": entry["id"],
            ...
            "color_glyph_hinting_classification": classification,
            "renders": renders,
    ...
    summary = {
        "schema_version": 1,
        "kind": "color_glyph_hinting_reference_disagreement_summary",
        "page_count": len(pages),
        "fixture_count": len(pages) + 4,
        "classification_counts": counts(page["color_glyph_hinting_classification"] for page in pages),
        "wellfriendpdf_outlier_failures": sum(
            1
            for page in pages
            if page["color_glyph_hinting_classification"] in {"wellfriendpdf_outlier_failure", "wellfriendpdf_render_failure"}
        ),
        "unclassified_failures": sum(1 for page in pages if page["color_glyph_hinting_classification"] == "unclassified_failure"),
        "reference_disagreements": [
            {"id": page["id"], "classification": page["color_glyph_hinting_classification"]}
            for page in pages
            if "reference_disagreement" in page["color_glyph_hinting_classification"]
        ],
        "policy_only_rows": {
    ...
    results = {
        "schema_version": 1,
        "kind": "color_glyph_hinting_multi_reference_render_results",
        "dpi": dpi,
        "page_count": len(pages),
    ...
    metrics = {"schema_version": 1, "kind": "color_glyph_hinting_multi_reference_diff_metrics", "pages": metrics_pages}
...
def classify_color_glyph_hinting(raw: str, entry: dict[str, Any], pair_metrics: dict[str, Any]) -> str:
...
def render_html(pages: list[dict[str, Any]], summary: dict[str, Any]) -> None:
    ...
    for page in pages:
        ...
        rows.append(
            ...
            f"<td>{html.escape(page['color_glyph_hinting_classification'])}</td>"
    ...
    HTML_REPORT.write_text(
        ...
        "<title>Color Glyph Hinting Closure Harness</title>"
        ...
        "<h1>Color Glyph Hinting Closure Harness</h1>"
        ...
        "<h2>Rendered Pages</h2><table><tr><th>Fixture</th><th>Category</th><th>Color Glyph Hinting</th>"
...
def write_matrices(metadata: dict[str, Any], render_payload: dict[str, Any] | None) -> None:
    ...
    write_json(
        MATRIX_FILES["colrv1"],
        {
            **common,
            ...
            "fixtures": ["color_glyph_hinting_colrv1_solid_transform", "color_glyph_hinting_colrv1_unsupported_gradient_operator"],
        },
    )
    write_json(MATRIX_FILES["colrv1_results"], pages.get("color_glyph_hinting_colrv1_solid_transform", {}))
    svg_rows = [
        {"id": "color_glyph_hinting_svg_static_path", "status": "static_subset_candidate", "reason": "path-only SVG classified without execution"},
        {"id": "color_glyph_hinting_svg_static_shape", "status": "static_subset_candidate", "reason": "basic shape SVG classified without execution"},
        {"id": "color_glyph_hinting_svg_transform", "status": "static_subset_candidate", "reason": "finite transform candidate classified"},
        {"id": "color_glyph_hinting_svg_blocked_script", "status": "unsupported_reported_security_policy", "reason": "script elements are blocked"},
        {"id": "color_glyph_hinting_svg_blocked_external", "status": "unsupported_reported_security_policy", "reason": "network/file URLs are blocked"},
        {"id": "color_glyph_hinting_svg_blocked_foreign_object", "status": "unsupported_reported_security_policy", "reason": "foreignObject is blocked"},
        {"id": "color_glyph_hinting_svg_path_bomb", "status": "unsupported_reported_security_policy", "reason": "path/depth cap blocks oversized SVG"},
    ]
    ...
    write_json(MATRIX_FILES["sbix_results"], pages.get("color_glyph_hinting_sbix_png_regression", {}))
    write_json(
        MATRIX_FILES["hinting"],
        {
            ...
            "corpus": ["color_glyph_hinting_korean_hinting_regression", "color_glyph_hinting_hebrew_hinting_regression", "color_glyph_hinting_colrv1_solid_transform"],
        },
    )
    write_json(
        MATRIX_FILES["cff"],
        {
            **common,
            ...
            "fixtures": ["color_glyph_hinting_cid_keyed_cff_clip"] if "color_glyph_hinting_cid_keyed_cff_clip" in pages else [],
        },
    )
    write_json(MATRIX_FILES["cff_results"], pages.get("color_glyph_hinting_cid_keyed_cff_clip", {"status": "fixture_unavailable"}))
...
def write_closure_audit(render_payload: dict[str, Any] | None) -> None:
    ...
    rows = [
        ...
        ("native hinting backend", "not_in_multilingual_color_glyphs_scope", rel(MATRIX_FILES["hinting"])),
    ...
    write_json(
        ...
        {
            "schema_version": 1,
            "kind": "color_glyph_hinting_closure_audit",
...
def run_feature_report(timeout: int) -> dict[str, Any]:
    ...
    has_color_glyph_hinting = False
    if PUBLIC_FEATURE_REPORT.exists():
        payload = json.loads(PUBLIC_FEATURE_REPORT.read_text(encoding="utf-8"))
        has_color_glyph_hinting = "color_glyph_hinting_color_glyph_hinting_cff_closure" in payload.get("report", {})
    return {
        "status": "passed" if result["exit_status"] == 0 and has_color_glyph_hinting else "failed",
        "has_color_glyph_hinting_section": has_color_glyph_hinting,
...
def main() -> int:
    ...
    if not args.skip_feature_report:
        ...
        write_json(OUT_DIR / "color_glyph_hinting-binding-report-parity.json", feature)
