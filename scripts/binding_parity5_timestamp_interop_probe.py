"""Generate independent Pades LTV RFC 3161 timestamp interoperability evidence."""
...
def cert_name(common_name: str) -> x509.Name:
    return x509.Name(
        [
            ...
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Wellfriend PadesLTV Test"),
...
def build_certificates(fixed_time: datetime) -> dict[str, bytes]:
    ...
    root_subject = cert_name("PadesLTV Interop Timestamp Root")
    tsa_subject = cert_name("PadesLTV Interop TSA")
...
async def main() -> int:
    ...
    parser.add_argument("--output", default=str(ARTIFACT_ROOT / "timestamp-interoperability-pades_ltv.json"))
    ...
    signature_value = b"pades_ltv independent timestamp interoperability signature value"
    ...
    payload = {
        "schema": "pades_ltv.timestamp-interoperability.v1",
