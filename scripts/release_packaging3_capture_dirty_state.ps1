param(
    [string]$OutputDirectory = "target/text_reflow-geometric-semantic-reflow"
)
...
function Get-TextReflowClassification([string]$Path) {
    if ($Path -like 'bindings/*') { return 'intended_text_reflow_binding' }
    if ($Path -like 'docs/*') { return 'intended_text_reflow_doc' }
    if ($Path -like 'scripts/*') { return 'intended_text_reflow_script' }
    if ($Path -like 'fuzz/*' -or $Path -like '*tests/*') { return 'intended_text_reflow_test' }
    if ($Path -eq 'Cargo.lock' -or $Path -like 'crates/*') { return 'intended_text_reflow_source' }
    return 'uncertain'
}
...
try {
    ...
    $entries = foreach ($path in @($tracked + $untracked | Sort-Object -Unique)) {
        ...
        [ordered]@{
            ...
            classification = Get-TextReflowClassification $path
            bytes = $item.Length
    ...
    $common = [ordered]@{
        captured_at_utc = $now
        baseline = $baseline
        authorized_dirty_snapshot_at_text_reflowc_start = $authorizedStartingSnapshot
        current_head = $head
    ...
    $state = [ordered]@{
        schema_version = 'text_reflow.authorized-dirty-state.v9'
        status = 'authorized_continuation_from_existing_dirty_text_reflow_worktree'
        original_clean_editing_transactions_baseline = $baseline
        authorized_dirty_snapshot_at_text_reflowc_start = $authorizedStartingSnapshot
        changed_path_count = @($entries).Count
    }
    foreach ($property in $common.GetEnumerator()) { $state[$property.Key] = $property.Value }
    $manifest = [ordered]@{ schema_version = 'text_reflow.worktree-manifest.v9' }
    foreach ($property in $common.GetEnumerator()) { $manifest[$property.Key] = $property.Value }
    $gaps = [ordered]@{
        schema_version = 'text_reflow.remaining-gap-matrix.v10'
        captured_at_utc = $now
        baseline = $baseline
        ...
        implemented_boundaries = @(
            ...
            'native_renderer_xycut_semantic_runtime_region_graph',
    ...
    foreach ($record in @(
        @('text_reflow-authorized-dirty-state.json', $state),
        @('text_reflow-worktree-manifest.json', $manifest),
        @('text_reflow-remaining-gap-matrix.json', $gaps)
    )) {
