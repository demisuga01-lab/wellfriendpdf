#!/usr/bin/env python3
"""Multilingual Color Glyphs direct CJK/RTL/color-glyph reference-renderer harness."""
...
CORPUS_MANIFEST = OUT_DIR / "corpus-manifest-multilingual_color_glyphs.json"
CAPABILITY_MATRIX = OUT_DIR / "multilingual_color_glyphs-capability-matrix.json"
RENDER_RESULTS = OUT_DIR / "multi-reference-render-results-multilingual_color_glyphs.json"
DIFF_METRICS = OUT_DIR / "multi-reference-diff-metrics-multilingual_color_glyphs.json"
DISAGREEMENT_SUMMARY = OUT_DIR / "reference-disagreement-summary-multilingual_color_glyphs.json"
PUBLIC_FEATURE_REPORT = OUT_DIR / "public-feature-report-multilingual_color_glyphs.json"
BINDING_PARITY = OUT_DIR / "binding-report-parity-multilingual_color_glyphs.json"
    ...
    rows = [
        ...
        (
            ...
            [
                "text clipping",
                "CFF CID outlines",
                "Type3 CID Rendering regression",
        ...
        (
            ...
            "tests/corpus/pdfs/pdfjs/korean-hangul-multilingual_color_glyphs.pdf",
        ...
        (
            ...
            "tests/corpus/pdfs/pdfjs/hebrew-multilingual_color_glyphs.pdf",
        ...
        (
            ...
            "tests/corpus/pdfs/pdfjs/color-colr-cpal-multilingual_color_glyphs.pdf",
        ...
        (
            ...
            "tests/corpus/pdfs/pdfjs/color-bitmap-svg-multilingual_color_glyphs.pdf",
        ...
        entries.append(
            {
                ...
                "expected_multilingual_color_glyphs_classification": expected,
            }
        )
    return entries
...
def write_corpus_manifest(entries: list[dict[str, Any]]) -> None:
    ...
    write_json(
        ...
        {
            "schema_version": 1,
            "kind": "multilingual_color_glyphs_cjk_rtl_color_glyph_corpus_manifest",
...
def write_capability_matrix(entries: list[dict[str, Any]], summary: dict[str, Any] | None = None) -> None:
    ...
    write_json(
        ...
        {
            "schema_version": 1,
            "kind": "multilingual_color_glyphs_capability_matrix",
            "summary": summary or {},
            "rows": matrix,
        },
    )
...
def bootstrap_reference_manifest(dpi: int, timeout: int, allow_missing: bool) -> dict[str, Any]:
    if not TOOL_MANIFEST.exists():
        ...
        if not TOOL_MANIFEST.exists():
            raise RuntimeError(f"Multilingual Color Glyphs reference bootstrap did not write manifest: {bootstrap}")
        if bootstrap["exit_status"] != 0 and not allow_missing:
            raise RuntimeError(f"Multilingual Color Glyphs reference bootstrap failed: {bootstrap}")

    payload = json.loads(TOOL_MANIFEST.read_text(encoding="utf-8-sig"))
    payload["kind"] = "multilingual_color_glyphs_reference_tool_manifest"
    payload["multilingual_color_glyphs_command_normalization"] = {
        "dpi": dpi,
...
def render_compare(
    ...
    multilingual_color_glyphs_counts: dict[str, int] = {}
    ...
    for entry in available:
        ...
        multilingual_color_glyphs = classify_multilingual_color_glyphs(raw, entry, pair_metrics)
        multilingual_color_glyphs_counts[multilingual_color_glyphs] = multilingual_color_glyphs_counts.get(multilingual_color_glyphs, 0) + 1
        page = {
            "id": entry["id"],
            ...
            "expected_multilingual_color_glyphs_classification": entry["expected_multilingual_color_glyphs_classification"],
            "raw_reference_renderer_classification": raw,
            "multilingual_color_glyphs_classification": multilingual_color_glyphs,
            "renders": renders,
    ...
    policy_rows = [
        {
            ...
            "capabilities": entry["capabilities"],
            "expected_multilingual_color_glyphs_classification": entry["expected_multilingual_color_glyphs_classification"],
            "multilingual_color_glyphs_classification": entry["expected_multilingual_color_glyphs_classification"],
    ...
    for row in policy_rows:
        key = row["multilingual_color_glyphs_classification"]
        multilingual_color_glyphs_counts[key] = multilingual_color_glyphs_counts.get(key, 0) + 1

    summary = {
        "schema_version": 1,
        "kind": "multilingual_color_glyphs_reference_disagreement_summary",
        "page_count": len(pages),
        "policy_row_count": len(policy_rows),
        "classification_counts": multilingual_color_glyphs_counts,
        ...
        "reference_disagreements": [
            {
                ...
                "multilingual_color_glyphs_classification": page["multilingual_color_glyphs_classification"],
            }
            for page in pages
            if page["multilingual_color_glyphs_classification"] != "all_references_agree_wellfriendpdf_pass"
        ],
        "policy_rows": policy_rows,
    }
    results = {
        "schema_version": 1,
        "kind": "multilingual_color_glyphs_multi_reference_render_results",
        "dpi": dpi,
        "page_count": len(pages),
        "policy_row_count": len(policy_rows),
    ...
    metrics = {"schema_version": 1, "kind": "multilingual_color_glyphs_multi_reference_diff_metrics", "pages": metrics_pages}
...
def classify_multilingual_color_glyphs(raw: str, entry: dict[str, Any], pair_metrics: dict[str, Any]) -> str:
    ...
    if raw == "all_references_agree_wellfriendpdf_pass":
        return raw
    if entry["expected_multilingual_color_glyphs_classification"] == "unsupported_policy_accepted":
    ...
    if entry["expected_multilingual_color_glyphs_classification"] in {
        "unsupported_policy_accepted",
        "fixture_gap_policy_reported",
    }:
        return entry["expected_multilingual_color_glyphs_classification"]
    return raw
...
def count_outliers(pages: list[dict[str, Any]]) -> int:
    return sum(
        1
        for page in pages
        if page["multilingual_color_glyphs_classification"]
...
def count_unclassified(pages: list[dict[str, Any]]) -> int:
    return sum(
        1
        for page in pages
        if page["multilingual_color_glyphs_classification"] in {"needs_manual_review", "reference_tool_failure"}
    )
...
def render_html(pages: list[dict[str, Any]], summary: dict[str, Any]) -> None:
    ...
    for page in pages:
        ...
        rows.append(
            ...
            f"<td>{html.escape(page['multilingual_color_glyphs_classification'])}</td>"
    ...
    policy_rows = [
        ...
        f"<td>{html.escape(row['multilingual_color_glyphs_classification'])}</td>"
    ...
    HTML_REPORT.write_text(
        ...
        "<title>Multilingual Color Glyphs CJK/RTL/Color Glyph Reference Harness</title>"
        ...
        "<h1>Multilingual Color Glyphs CJK/RTL/Color Glyph Reference Harness</h1>"
        ...
        "<h2>Pages</h2><table><tr><th>Fixture</th><th>Category</th><th>Multilingual Color Glyphs</th>"
...
def run_feature_report(timeout: int) -> dict[str, Any]:
    ...
    has_multilingual_color_glyphs = False
    if PUBLIC_FEATURE_REPORT.exists():
        try:
            payload = json.loads(PUBLIC_FEATURE_REPORT.read_text(encoding="utf-8"))
            has_multilingual_color_glyphs = "multilingual_color_glyphs_cjk_rtl_color_glyph_reference_harness" in payload.get("report", {})
        except json.JSONDecodeError:
            has_multilingual_color_glyphs = False
    return {
        "status": "passed" if result["exit_status"] == 0 and has_multilingual_color_glyphs else "failed",
        "has_multilingual_color_glyphs_section": has_multilingual_color_glyphs,
...
def write_binding_parity(feature_report: dict[str, Any] | None) -> None:
    write_json(
        ...
        {
            "schema_version": 1,
            "kind": "multilingual_color_glyphs_binding_report_parity",
