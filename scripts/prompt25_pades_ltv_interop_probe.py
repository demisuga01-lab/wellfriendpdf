"""Generate Prompt 25 PAdES B-T/B-LT interoperability evidence."""
...
def build_pki() -> dict[str, bytes]:
    ...
    root_subject = name("Prompt25 PAdES Root")
    intermediate_subject = name("Prompt25 PAdES Intermediate")
    signer_subject = name("Prompt25 PAdES Signer")
    tsa_subject = name("Prompt25 PAdES TSA")
...
def make_pdf(pki: dict[str, bytes]) -> tuple[bytes, bytes]:
    ...
    append_signature_field(writer, SigFieldSpec(sig_field_name="Prompt25PadesBT"))
    signed = io.BytesIO()
    signers.sign_pdf(
        writer,
        signers.PdfSignatureMetadata(
            field_name="Prompt25PadesBT",
            md_algorithm="sha256",
...
def prompt25_summary(parsed: dict | None) -> dict:
    try:
        ...
        return {
            ...
            "timestamp_status": sig.get("prompt25", {}).get("signature_timestamp_status"),
            "ltv_status": sig.get("prompt25", {}).get("ltv_status"),
            "achieved_pades_level": sig.get("prompt25", {}).get("achieved_pades_level"),
            "dss_status": sig.get("prompt25", {}).get("dss", {}).get("status"),
            "vri_matched": sig.get("prompt25", {}).get("dss", {}).get("vri_matched"),
        }
...
def main() -> int:
    ...
    parser.add_argument("--output", default=str(ARTIFACT_ROOT / "pades-ltv-interoperability-prompt25.json"))
