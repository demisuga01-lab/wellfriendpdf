...
$OutDir = Join-Path $Repo "target/release_packaging-packaging-codec-isolation"
...
Invoke-GateStep "rust example codec isolation" "cargo" @("run", "-p", "wellfriendpdf-engine", "--example", "release_packaging_codec_isolation", "--", "in_process")
...
if ((Get-Command java -ErrorAction SilentlyContinue) -and (Get-Command javac -ErrorAction SilentlyContinue)) {
    Invoke-GateStep "java maven package smoke" "powershell" @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts/java_packaging_java_package_smoke.ps1") $false
    Invoke-GateStep "java gradle package smoke" "powershell" @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts/gradle_packaging_gradle_package_smoke.ps1") $false
} else {
    Add-Step "java maven package smoke" "scripts/java_packaging_java_package_smoke.ps1" "unavailable" 127 "" "java or javac is not on PATH"
    Add-Step "java gradle package smoke" "scripts/gradle_packaging_gradle_package_smoke.ps1" "unavailable" 127 "" "java or javac is not on PATH"
}

Invoke-GateStep "wasm-pack package" "powershell" @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts/wasm_packaging_wasm_pack_gate.ps1", "-OutDir", $OutDir)
...
Add-Artifact "rust codec isolation example" (Join-Path $Repo "crates/engine/examples/release_packaging_codec_isolation.rs") "rust"
...
Add-Artifact "dotnet codec isolation example" (Join-Path $Repo "bindings/dotnet/examples/ReleasePackagingCodecIsolation.cs") "dotnet"
Add-Artifact "java codec isolation example" (Join-Path $Repo "bindings/java/examples/ReleasePackagingCodecIsolation.java") "java"

$examplesMatrix = [ordered]@{
    schema_version = 1
    roadmap task = "combined_release_packaging"
    surfaces = @(
        @{ surface = "rust"; example = "crates/engine/examples/sdk_reports.rs"; codec_isolation = "crates/engine/examples/release_packaging_codec_isolation.rs"; package_command = "cargo package -p wellfriendpdf-engine --allow-dirty" },
        ...
        @{ surface = "wasm"; example = "crates/wellfriendpdf-wasm/examples/browser"; codec_isolation = "crates/wellfriendpdf-wasm/examples/browser/codec_isolation_report.mjs"; package_command = "scripts/wasm_packaging_wasm_pack_gate.ps1"; package_status = "implemented_public" },
        @{ surface = "dotnet"; example = "bindings/dotnet/examples/BindingParityReports.cs"; codec_isolation = "bindings/dotnet/examples/ReleasePackagingCodecIsolation.cs"; package_command = "dotnet pack bindings/dotnet/WellfriendPdf/WellfriendPdf.csproj" },
        @{ surface = "java_maven"; example = "bindings/java/examples/BindingParityReports.java"; codec_isolation = "bindings/java/examples/ReleasePackagingCodecIsolation.java"; package_command = "scripts/java_packaging_java_package_smoke.ps1" },
        @{ surface = "java_gradle"; example = "bindings/java/examples/BindingParityReports.java"; codec_isolation = "bindings/java/examples/ReleasePackagingCodecIsolation.java"; package_command = "scripts/gradle_packaging_gradle_package_smoke.ps1" }
    )
    workflows = @(
...
$manifest = [ordered]@{
    ...
    artifacts = $Artifacts
    docs = @(
        "docs/combined_release_packaging_audit.md",
        "docs/multi_language_examples_release_packaging.md",
        "docs/binding_release_packaging_gate_release_packaging.md",
        "docs/package_artifact_manifest_release_packaging.md",
        "docs/codec_threat_model_release_packaging.md",
        "docs/codec_isolation_design_release_packaging.md",
        "docs/codec_isolation_user_guide_release_packaging.md",
        "docs/bindings_wasm_packaging_wasm_pack_closure.md",
        "docs/packaging_gate_release_packaging.md",
        "docs/package_platform_matrix_release_packaging.md",
...
if ($HadFailure -and !$ContinueOnFailure) {
    throw "Release Packaging release gate had required failures; inspect target/release_packaging-packaging-codec-isolation/release-manifest.json"
}
