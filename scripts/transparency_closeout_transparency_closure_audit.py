#!/usr/bin/env python3
"""Generate and compare the Transparency Closeout transparency closure corpus."""
...
import transparency_rendering_transparency_compositing_audit as p07


OUT_DIR = p07.OUT_DIR
FIXTURE_DIR = p07.FIXTURE_DIR
TOOL_MANIFEST_OUT = OUT_DIR / "transparency_closeout-reference-tool-manifest.json"
CORPUS_OUT = OUT_DIR / "transparency_closeout-corpus-manifest.json"
RESULTS_OUT = OUT_DIR / "transparency_closeout-render-results.json"
DIFF_METRICS_OUT = OUT_DIR / "transparency_closeout-diff-metrics.json"
DISAGREEMENT_OUT = OUT_DIR / "transparency_closeout-reference-disagreement-summary.json"
MATRIX_OUT = OUT_DIR / "transparency_closeout-transparency-matrix.json"
MEMORY_OUT = OUT_DIR / "transparency_closeout-memory-report.json"
CLOSURE_OUT = OUT_DIR / "transparency_closeout-closure-audit.json"
HTML_OUT = OUT_DIR / "transparency_closeout-html-report" / "index.html"
    ...
    entries.append(
        {
            ...
            "generator": "scripts/transparency_closeout_transparency_closure_audit.py",
        }
    )
...
def generate_transparency_closeout_corpus() -> list[dict[str, Any]]:
    entries = p07.generate_corpus()
    add_entry(
        entries,
        "image_smask_matte",
        "closure/softmask_matte_background",
        "transparency_closeout_image_smask_matte.pdf",
    ...
    add_entry(
        ...
        "closure/softmask_matte_background",
        "transparency_closeout_softmask_alpha_bc_background.pdf",
    ...
    add_entry(
        ...
        "transparency_closeout_softmask_luminosity_devicegray.pdf",
    ...
    add_entry(
        ...
        "closure/luminosity_colorspace",
        "transparency_closeout_softmask_luminosity_devicergb.pdf",
    ...
    add_entry(
        ...
        "closure/luminosity_colorspace",
        "transparency_closeout_softmask_luminosity_devicecmyk.pdf",
    ...
    add_entry(
        ...
        "transparency_closeout_group_colorspace_devicegray.pdf",
    ...
    add_entry(
        ...
        "closure/group_colorspace",
        "transparency_closeout_group_colorspace_devicergb.pdf",
    ...
    add_entry(
        ...
        "closure/group_colorspace",
        "transparency_closeout_group_colorspace_devicecmyk.pdf",
    ...
    add_entry(
        ...
        "transparency_closeout_knockout_overlap_exact.pdf",
    ...
    add_entry(
        ...
        "closure/knockout_overlap",
        "transparency_closeout_knockout_overlap_nested_form.pdf",
    ...
    p07.write_json(
        ...
        {
            "schema_version": 1,
            "kind": "transparency_closeout_transparency_closure_corpus_manifest",
            "fixture_count": len(entries),
            "base_transparency_rendering_fixture_count": 37,
...
def transparency_closeout_classification(raw: str) -> str:
    mapping = {
...
def add_transparency_closeout_classifications(results: dict[str, Any]) -> None:
    counts: dict[str, int] = {}
    for page in results.get("pages", []):
        final = transparency_closeout_classification(page.get("classification", "unknown"))
        page["transparency_closeout_classification"] = final
        counts[final] = counts.get(final, 0) + 1
    results["transparency_closeout_classification_counts"] = counts
...
def closure_rows(results: dict[str, Any]) -> list[dict[str, Any]]:
    def status_for(ids: list[str]) -> tuple[str, list[str]]:
        classes = [
            page_by_id(results, ident).get("transparency_closeout_classification", "not_run")
    ...
    specs = [
        ...
        (
            ...
            "Poppler/PDFium/MuPDF/Wellfriend audit rerun with Transparency Rendering plus Transparency Closeout closure fixtures",
        ...
        rows.append(
            {
                ...
                "fixture_ids": ids,
                "transparency_closeout_classifications": classes,
                "tests_artifacts": [
...
def write_transparency_closeout_artifacts(entries: list[dict[str, Any]], tools_payload: dict[str, Any], results: dict[str, Any]) -> None:
    ...
    p07.write_json(
        ...
        {
            "schema_version": 1,
            "kind": "transparency_closeout_diff_metrics",
            "metrics": [
                {
                    "id": page["id"],
                    "category": page["category"],
                    "classification": page["classification"],
                    "transparency_closeout_classification": page["transparency_closeout_classification"],
    ...
    disagreement_pages = [
        ...
        for page in results.get("pages", [])
        if page["transparency_closeout_classification"]
        in {
    ...
    p07.write_json(
        ...
        {
            "schema_version": 1,
            "kind": "transparency_closeout_reference_disagreement_summary",
            "fixture_count": len(entries),
            "classification_counts": results.get("transparency_closeout_classification_counts", {}),
    ...
    p07.write_json(
        ...
        {
            "schema_version": 1,
            "kind": "transparency_closeout_transparency_matrix",
            "rows": rows,
    ...
    p07.write_json(
        ...
        {
            "schema_version": 1,
            "kind": "transparency_closeout_closure_audit",
            "rows": rows,
    ...
    p07.write_json(
        ...
        {
            "schema_version": 1,
            "kind": "transparency_closeout_memory_report",
            "memory_cap_mb": 4096,
    ...
    for page in results.get("pages", []):
        table_rows.append(
            ...
            f"<td>{html.escape(page['transparency_closeout_classification'])}</td>"
            f"<td>{html.escape(page['classification'])}</td>"
            "</tr>"
        )
    HTML_OUT.write_text(
        "<!doctype html><meta charset='utf-8'>"
        "<title>Transparency Closeout Transparency Closure Audit</title>"
        ...
        "<h1>Transparency Closeout Transparency Closure Audit</h1>"
        f"<p>Fixtures: {len(entries)}. Memory cap: 4096 MB.</p>"
        "<h2>Classification Counts</h2><pre>"
        f"{html.escape(json.dumps(results.get('transparency_closeout_classification_counts', {}), indent=2, sort_keys=True))}</pre>"
        "<h2>Closure Rows</h2><pre>"
        f"{html.escape(json.dumps(rows, indent=2, sort_keys=True))}</pre>"
        "<h2>Pages</h2><table><tr><th>Fixture</th><th>Category</th><th>Transparency Closeout classification</th><th>Raw classification</th></tr>"
        + "\n".join(table_rows)
        + "</table>",
        encoding="utf-8",
    )
...
def main() -> int:
    ...
    entries = generate_transparency_closeout_corpus()
    base = p07.wellfriendpdf_base_command(args.wellfriendpdf_bin)
    results = p07.run_phase(entries, manifest_payload["tools"], base, "transparency_closeout", args.dpi, args.timeout)
    results["starting_checkpoint"] = p07.run_command(["git", "rev-parse", "--short", "HEAD"], 10)
    results["transparency_closeout_note"] = "Transparency Closeout closure audit for alpha image, matte/background, luminosity color spaces, group color spaces, and knockout overlap."
    add_transparency_closeout_classifications(results)
    write_transparency_closeout_artifacts(entries, manifest_payload, results)
    print(
        json.dumps(
            {
