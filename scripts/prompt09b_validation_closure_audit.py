#!/usr/bin/env python3
"""Generate Prompt 09B validation-closure artifacts."""
...
CORPUS_DIR = OUT_DIR / "prompt09b-corpus"
RENDER_DIR = OUT_DIR / "prompt09b-renders"
DIFF_DIR = OUT_DIR / "prompt09b-diffs"
LOG_DIR = OUT_DIR / "prompt09b-logs"
OXIDE_REPORT_DIR = OUT_DIR / "prompt09b-oxide-reports"
HTML_REPORT = OUT_DIR / "prompt09b-html-report" / "index.html"
...
ARTIFACTS = {
    "closure": OUT_DIR / "prompt09b-closure-audit.json",
    "annotation_matrix": OUT_DIR / "annotation-appearance-matrix-prompt09b.json",
    "annotation_results": OUT_DIR / "annotation-reference-results-prompt09b.json",
    "annotation_metrics": OUT_DIR / "annotation-diff-metrics-prompt09b.json",
    "annotation_disagreements": OUT_DIR / "annotation-reference-disagreements-prompt09b.json",
    "ocg_matrix": OUT_DIR / "ocg-layer-matrix-prompt09b.json",
    "ocg_cache": OUT_DIR / "ocg-cache-key-fingerprint-prompt09b.json",
    "ocg_results": OUT_DIR / "ocg-reference-results-prompt09b.json",
    "progressive_equivalence": OUT_DIR / "progressive-resume-equivalence-prompt09b.json",
    "progressive_invalid": OUT_DIR / "progressive-resume-invalid-token-prompt09b.json",
    "progressive_memory": OUT_DIR / "progressive-resume-memory-prompt09b.json",
    "tile_equivalence": OUT_DIR / "tile-full-equivalence-prompt09b.json",
    "band_equivalence": OUT_DIR / "band-full-equivalence-prompt09b.json",
    "cache_equivalence": OUT_DIR / "cache-equivalence-prompt09b.json",
    "performance": OUT_DIR / "tile-band-cache-performance-prompt09b.json",
    "memory": OUT_DIR / "tile-band-cache-memory-prompt09b.json",
    "multi_reference": OUT_DIR / "multi-reference-render-results-prompt09b.json",
    "multi_reference_metrics": OUT_DIR / "multi-reference-diff-metrics-prompt09b.json",
    "reference_summary": OUT_DIR / "reference-disagreement-summary-prompt09b.json",
    "feature_report": OUT_DIR / "public-feature-report-prompt09b.json",
    "corpus_manifest": OUT_DIR / "corpus-manifest-prompt09b.json",
    "tool_manifest": OUT_DIR / "reference-tool-manifest-prompt09b.json",
    "binding_parity": OUT_DIR / "binding-report-parity-prompt09b.json",
}
...
REQUIRED_DOCS = [
    "docs/prompt09b_validation_closure_audit.md",
    "docs/prompt09_annotation_appearance_parity.md",
    "docs/prompt09_ocg_layer_validation.md",
    "docs/prompt09_progressive_resume.md",
    "docs/prompt09_tile_band_cache_performance.md",
    "docs/prompt09_known_limits.md",
    "docs/prompt09_renderer_failure_taxonomy.md",
    "docs/prompt09_reference_disagreement_policy.md",
]
...
def write_pdf(name: str, builder: PdfBuilder, category: str, scope: list[str], expected: str) -> dict[str, Any]:
    ...
    return {
        "id": path.stem.replace("prompt09b_", ""),
        "category": category,
        "path": rel(path),
        "page": 1,
        "scope": scope,
        "expected_prompt09b_classification": expected,
...
def write_fixtures() -> list[dict[str, Any]]:
    ...
    write_json(
        ...
        {
            "schema_version": 1,
            "kind": "prompt09b_corpus_manifest",
...
def load_prompt06b() -> Any:
    ...
    for path in [RENDER_DIR, DIFF_DIR, LOG_DIR, OXIDE_REPORT_DIR]:
        path.mkdir(parents=True, exist_ok=True)
    return module


def reference_manifest() -> dict[str, Any]:
    if not PROMPT06B_TOOL_MANIFEST.exists():
        bootstrap = run_command(["powershell", "-NoProfile", "-File", "scripts/prompt06b_bootstrap_reference_renderers.ps1"], timeout=300)
        if not PROMPT06B_TOOL_MANIFEST.exists():
            raise RuntimeError(f"Prompt 06B reference manifest missing: {bootstrap}")
...
def run_feature_report(timeout: int) -> dict[str, Any]:
    ...
    if ARTIFACTS["feature_report"].exists():
        try:
            ...
            has_prompt09b = "prompt09b_annotation_progressive_cache_validation" in payload.get("report", {})
        except json.JSONDecodeError:
            has_prompt09b = False
    return {
        "status": status if has_prompt09b else "failed_missing_prompt09b_section",
        "has_prompt09b_section": has_prompt09b,
        "artifact": rel(ARTIFACTS["feature_report"]) if ARTIFACTS["feature_report"].exists() else None,
        "command": result,
    }


def render_compare(fixtures: list[dict[str, Any]], manifest: dict[str, Any], oxide_bin: str | None, dpi: int, timeout: int) -> dict[str, Any]:
    p06 = load_prompt06b()
    ...
    for entry in fixtures:
        ...
        closure = classify_prompt09b(raw, entry)
        prompt09b_counts[closure] = prompt09b_counts.get(closure, 0) + 1
        page = {
            "id": entry["id"],
            ...
            "input": entry["path"],
            "scope": entry["scope"],
            "expected_prompt09b_classification": entry["expected_prompt09b_classification"],
            "raw_prompt06b_classification": raw,
            "prompt09b_classification": closure,
            "renders": renders,
    ...
    results = {
        "schema_version": 1,
        "kind": "prompt09b_multi_reference_render_results",
        "dpi": dpi,
    ...
    metrics = {"schema_version": 1, "kind": "prompt09b_multi_reference_diff_metrics", "pages": metrics_pages}
    summary = {
        "schema_version": 1,
        "kind": "prompt09b_reference_disagreement_summary",
        "page_count": len(pages),
        "classification_counts": prompt09b_counts,
        "raw_prompt06b_classification_counts": raw_counts,
        "oxide_outlier_failures": count_outliers(pages),
        ...
        "reference_disagreements": [
            {
                ...
                "prompt09b_classification": page["prompt09b_classification"],
                "policy": page["expected_prompt09b_classification"],
            }
            for page in pages
            if page["prompt09b_classification"] != "all_references_agree_oxide_pass"
        ],
    }
...
def classify_prompt09b(raw: str, entry: dict[str, Any]) -> str:
    if entry["expected_prompt09b_classification"] == "policy_reported_expected":
        if raw in {"oxide_render_failure", "reference_tool_failure", "dimension_mismatch"}:
            return raw
...
def count_outliers(pages: list[dict[str, Any]]) -> int:
    return sum(
        1
        for page in pages
        if page["prompt09b_classification"]
        in {"all_references_agree_oxide_mismatch", "oxide_render_failure", "reference_tool_failure", "dimension_mismatch"}
    )


def count_unclassified(pages: list[dict[str, Any]]) -> int:
    return sum(1 for page in pages if page["prompt09b_classification"] == "needs_manual_review")
...
def render_html(pages: list[dict[str, Any]], summary: dict[str, Any]) -> None:
    ...
    for page in pages:
        ...
        rows.append(
            ...
            f"<td>{html.escape(page['prompt09b_classification'])}</td>"
    ...
    HTML_REPORT.write_text(
        ...
        "<title>Prompt 09B Annotation Progressive Cache Validation</title>"
        ...
        "<h1>Prompt 09B Annotation Progressive Cache Validation</h1>"
        f"<p>Pages: {len(pages)}. Oxide outliers: {summary['oxide_outlier_failures']}. "
        ...
        "<h2>Pages</h2><table><tr><th>Fixture</th><th>Category</th><th>Prompt 09B</th>"
...
def annotation_rows() -> list[dict[str, Any]]:
    rows = [
        ...
        ("Widget annotation appearance", "appearance_stream_rendered", "prompt09b_widget_ap_stream"),
        ...
        ("/AP normal appearance stream", "appearance_stream_rendered", "prompt09b_widget_ap_stream"),
        ("/AP rollover/down status if present", "deferred_with_owner", "interactive_viewer_state_phase"),
        ("missing /AP fallback policy", "generated_appearance_rendered", "prompt09b_widget_missing_ap_generated"),
        ...
        ("Form XObject annotation appearance streams", "appearance_stream_rendered", "prompt09b_widget_ap_stream"),
        ("OCG-gated annotations", "native_rendered", "prompt09b_annotation_ocg_hidden"),
    ]
    return [
        {
...
def ocg_rows() -> list[dict[str, Any]]:
    rows = [
        ("OCProperties discovery", "implemented_and_proven", "prompt09b_ocg_marked_content_hidden"),
        ("OCG inventory", "implemented_and_proven", "prompt09b_ocg_marked_content_hidden"),
        ("OCMD inventory", "implemented_and_proven", "prompt09b_ocmd_allon_hidden"),
        ("default configuration", "implemented_and_proven", "prompt09b_ocg_marked_content_hidden"),
        ("alternate configurations", "unsupported_reported", "parsed_report_only_no_public_selector"),
        ("BaseState and ON/OFF arrays", "implemented_and_proven", "prompt09b_ocg_marked_content_hidden"),
        ("Intent matching", "implemented_and_proven", "prompt09b_ocg_marked_content_hidden"),
        ...
        ("OCMD visibility policies", "implemented_and_proven", "prompt09b_ocmd_allon_hidden"),
        ("AnyOn/AllOn/AnyOff/AllOff", "implemented_and_proven", "unit_and_fixture_coverage"),
        ("marked content visibility", "implemented_and_proven", "prompt09b_ocg_marked_content_hidden"),
        ("XObject visibility", "implemented_and_proven", "prompt09b_xobject_ocg_hidden"),
        ("annotation visibility", "implemented_and_proven", "prompt09b_annotation_ocg_hidden"),
        ("pattern/shading visibility interaction", "implemented_and_proven", "prompt09b_pattern_shading_ocg_hidden"),
...
def write_matrices(rendered: dict[str, Any]) -> None:
    ...
    write_json(
        ...
        {
            "schema_version": 1,
            "oxide_outlier_failures": count_outliers(annotation_pages),
...
def write_equivalence_artifacts(rendered: dict[str, Any]) -> None:
    ...
    vector_hash = vector_page["pair_metrics"]["oxide_vs_poppler"].get("visual_hash_a")
    write_json(
        ARTIFACTS["progressive_equivalence"],
        {
            ...
            "fixture": "prompt09b_tile_band_progressive_vector.pdf",
            "tile_size": [25, 25],
            "oxide_visual_hash": vector_hash,
    ...
    write_json(
        ARTIFACTS["tile_equivalence"],
        {
            ...
            "ocg_visibility_changes": "covered_by_ocg_cache_key_fingerprint_prompt09b",
        },
    )
    write_json(
    ...
    write_json(
        ARTIFACTS["performance"],
        {
            ...
            "elapsed_ms_by_fixture": {
                page["id"]: page["renders"]["oxide"]["render_command"]["elapsed_ms"]
    ...
    write_json(
        ARTIFACTS["memory"],
        {
            ...
            "source": "unit tests plus Prompt 09B render command timing",
        },
    )
...
def write_binding_parity(feature_report: dict[str, Any]) -> None:
    write_json(
        ARTIFACTS["binding_parity"],
        {
            ...
            "bindings": {
                "rust_sdk": "feature_report_json exposes prompt09b section",
                "cli": "oxide feature-report exposes prompt09b section",
                "python": "binding smoke must parse same feature_report_json envelope",
                "c_abi": "oxide_feature_report_json exposes same envelope",
...
def prompt09_files_changed() -> list[str]:
...
def write_closure_audit(
    ...
    missing_gates = [
        ...
        {"item": "CJK_RTL_color_glyph", "status": "not_in_prompt09_scope"},
    ]
    write_json(
        ARTIFACTS["closure"],
        {
            "schema_version": 1,
            "kind": "prompt09b_validation_closure_audit",
            "starting_commit": starting_head,
            "starting_worktree_status": starting_worktree_status,
            "prompt09_files_changed": prompt09_files_changed(),
            "fixture_categories": rendered["results"].get("categories", {}),
            ...
            "oxide_outlier_failures": rendered["summary"]["oxide_outlier_failures"],
...
def main() -> int:
    ...
    rendered = render_compare(fixtures, manifest, args.oxide_bin, args.dpi, args.timeout)
    ...
    status = "passed"
    if rendered["summary"]["oxide_outlier_failures"] or rendered["summary"]["unclassified_failures"]:
        status = "failed"
    print(
        json.dumps(
            {
