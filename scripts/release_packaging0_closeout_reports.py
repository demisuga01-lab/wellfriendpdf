#!/usr/bin/env python3
"""Release Readiness Benchmark audit, API inventory, package gate, and closeout generator.
...
ARTIFACT = "target/release_readiness_benchmark-release-readiness"
REQUIRED_DOCS = (
    "docs/release_readiness_benchmark_release_readiness_audit.md",
    "docs/release_readiness_benchmark_feature_matrix.md",
    ...
    "docs/release_readiness_benchmark_known_limits.md",
    "docs/release_readiness_benchmark_release_verdict.md",
)
REQUIRED_ARTIFACTS = (
    "release_readiness_benchmark-starting-state.json", "vps-toolchain-inventory.json", "vps-provisioning-log.json", "release_readiness_benchmark-feature-matrix.json",
    ...
    "historical-gate-impact-release_readiness_benchmark.json", "final-validation-matrix-release_readiness_benchmark.json", "release_readiness_benchmark-final-release-verdict.json", "RELEASE_READINESS_BENCHMARK_FINAL_REPORT.md",
)
VPS_LOGS = {
...
def start_state(repo: Path, artifact: Path) -> None:
    ...
    write_json(artifact / "release_readiness_benchmark-starting-state.json", {"schema_version": "release_readiness_benchmark.starting-state.v1", "captured_at_utc": utc(), "git": actual, "clean": not actual["status_short"], "expected_malformed_coverage_ancestor": ancestry["123bc0179ebd9f7b81f46453d2403698bc4ad984"], "prior_prompt_ancestry": ancestry, "artifact_policy": "target artifacts are ignored; VPS results retain raw logs"})
...
def inventories(repo: Path, artifact: Path) -> dict[str, dict[str, object]]:
    ...
    rows = {
        "rust": {"schema_version": "release_readiness_benchmark.api-rust.v1", "surface": "wellfriendpdf-engine public declarations", "items": rust_exports, "verdict": "passed" if rust_exports else "failed"},
        "cli": {"schema_version": "release_readiness_benchmark.api-cli.v1", "surface": "wellfriendpdf CLI commands", "items": cli_commands, "verdict": "passed" if cli_commands else "failed"},
        "python": {"schema_version": "release_readiness_benchmark.api-python.v1", "surface": "wellfriendpdf Python bindings", "items": source_lines(py, r"#\[pyfunction\]|^fn [a-zA-Z_]") , "verdict": "passed" if py.is_file() else "failed"},
        "cabi": {"schema_version": "release_readiness_benchmark.api-cabi.v1", "surface": "wellfriendpdf C ABI", "items": source_lines(capi, r"wellfriendpdf_[a-zA-Z0-9_]+\("), "verdict": "passed" if capi.is_file() else "failed"},
        "wasm": {"schema_version": "release_readiness_benchmark.api-wasm.v1", "surface": "wellfriendpdf WASM exports", "items": source_lines(wasm, r"wasm_bindgen|^pub fn"), "verdict": "passed" if wasm.is_file() else "failed"},
        "dotnet": {"schema_version": "release_readiness_benchmark.api-dotnet.v1", "surface": "WellfriendPdf .NET", "items": [line for path in dotnet_files for line in source_lines(path, r"^public (?:class|sealed class|interface|enum|.*\()")], "verdict": "passed" if dotnet_files else "failed"},
        "java": {"schema_version": "release_readiness_benchmark.api-java.v1", "surface": "io.wellfriendpdf Java", "items": [line for path in java_files for line in source_lines(path, r"^public (?:class|interface|enum|.*\()")], "verdict": "passed" if java_files else "failed"},
    }
...
def secret_scan(repo: Path) -> dict[str, object]:
    ...
    return {"schema_version": "release_readiness_benchmark.secret-scan.v1", "generated_at_utc": utc(), "finding_count": len(findings), "blocker_count": len(blockers), "findings": findings, "verdict": "passed" if not blockers else "failed"}
...
def normalize(value: object) -> str:
    return "passed" if str(value) in {"passed", "complete", "verified", "verified_with_limits", "release_ready", "release-ready, with documented boundaries"} else str(value)
...
def generate(repo: Path, artifact: Path, vps_results: Path | None) -> int:
    ...
    dependency = {"schema_version": "release_readiness_benchmark.dependency-audit.v1", "generated_at_utc": utc(), "cargo_metadata_status": metadata["status"], "package_count": len(packages), "packages": [{"name": item.get("name"), "version": item.get("version"), "license": item.get("license")} for item in packages], "verdict": "passed" if metadata["status"] == "passed" else "failed"}
    license_missing = [item["name"] for item in dependency["packages"] if not item.get("license")]
    license_audit = {"schema_version": "release_readiness_benchmark.license-audit.v1", "generated_at_utc": utc(), "missing_license_packages": license_missing, "license_expression_policy": "MIT for maintained Wellfriend packages", "verdict": "passed" if not license_missing else "failed"}
    ...
    vulnerability = {"schema_version": "release_readiness_benchmark.vulnerability-audit.v1", "generated_at_utc": utc(), "cargo_audit": audit_status, "evidence": audit_log, "verdict": audit_verdict}
    sbom = {"schema_version": "cyclonedx-fallback.release_readiness_benchmark.v1", "generated_at_utc": utc(), "format": "cargo_metadata_fallback", "components": dependency["packages"], "limitations": ["transitive dependency resolution and native toolchain packages require a dedicated SBOM generator for a distribution release"], "verdict": "verified_with_limits"}
    unsafe_rows = []
    ...
    package_gate = {"schema_version": "release_readiness_benchmark.package-release-gate.v1", "generated_at_utc": utc(), "rows": package_logs, "failed": package_failures, "verdict": "passed" if not package_failures else "failed"}
    examples = {"schema_version": "release_readiness_benchmark.examples-docs-gate.v1", "generated_at_utc": utc(), "required_docs": [{"path": path, "status": "present" if (repo / path).is_file() and (repo / path).stat().st_size > 0 else "missing"} for path in REQUIRED_DOCS], "verdict": "passed" if all((repo / path).is_file() for path in REQUIRED_DOCS) else "failed"}
    release_gate = {"schema_version": "release_readiness_benchmark.release-gate-verdict.v1", "generated_at_utc": utc(), "api": api_stability["verdict"], "package": package_gate["verdict"], "examples_docs": examples["verdict"], "verdict": "passed" if all(x == "passed" for x in (api_stability["verdict"], package_gate["verdict"], examples["verdict"])) else "failed"}
    security_package = {"schema_version": "release_readiness_benchmark.security-audit-package.v1", "generated_at_utc": utc(), "threat_model": threat["verdict"], "dependency_audit": dependency["verdict"], "license_audit": license_audit["verdict"], "vulnerability_audit": vulnerability["verdict"], "sbom": sbom["verdict"], "secret_scan": secret["verdict"], "unsafe_native": unsafe["verdict"], "verdict": "passed" if all(x in {"passed", "verified_with_limits"} for x in (dependency["verdict"], license_audit["verdict"], vulnerability["verdict"], sbom["verdict"], secret["verdict"], unsafe["verdict"])) else "failed"}
    historical = {"schema_version": "release_readiness_benchmark.historical-gate-impact.v1", "generated_at_utc": utc(), "rows": [{"feature_area": "27-29", "status": "prior artifacts retained; ReleaseReadinessBenchmark reuses corpus/fuzz/coverage evidence without changing engine behavior"}, {"feature_area": "24-26", "status": "workspace and binding gates rerun; signing/standards public surfaces unchanged"}], "verdict": "passed"}
    vps_toolchain = {"schema_version": "release_readiness_benchmark.vps-toolchain-inventory.v1", "generated_at_utc": utc(), "raw_inventory_path": str(vps_results / "toolchain-inventory.txt") if vps_results else None, "raw_inventory_present": bool(vps_results and (vps_results / "toolchain-inventory.txt").is_file()), "verdict": "passed" if vps_results and (vps_results / "toolchain-inventory.txt").is_file() else "failed"}
    vps_provision = {"schema_version": "release_readiness_benchmark.vps-provisioning-log.v1", "generated_at_utc": utc(), "vps": "35.185.176.47", "result_root": str(vps_results) if vps_results else None, "actions": ["isolated result and temporary directories only", "no production-service action", "explicit serial Cargo jobs"], "verdict": "passed" if vps_results else "failed"}
    for name, value in {"vps-toolchain-inventory.json": vps_toolchain, "vps-provisioning-log.json": vps_provision, "threat-model.json": threat, "dependency-audit-results.json": dependency, "license-audit-results.json": license_audit, "vulnerability-audit-results.json": vulnerability, "sbom.json": sbom, "unsafe-native-audit.json": unsafe, "secret-scan-results.json": secret, "api-stability-report.json": api_stability, "package-release-gate-results.json": package_gate, "examples-docs-gate-results.json": examples, "release-gate-verdict.json": release_gate, "security-audit-package.json": security_package, "historical-gate-impact-release_readiness_benchmark.json": historical}.items():
    ...
    write_json(artifact / "release_readiness_benchmark-feature-matrix.json", {"schema_version": "release_readiness_benchmark.feature-matrix.v1", "generated_at_utc": utc(), "rows": feature_rows, "verdict": "passed"})
    return 0
...
def finalise(repo: Path, artifact: Path, vps_results: Path | None) -> int:
    ...
    failing = [row for row in rows if row["status"] not in {"passed", "present", "verified", "verified_with_limits", "release_ready", "release-ready, with documented boundaries"}]
    validation = {"schema_version": "release_readiness_benchmark.final-validation-matrix.v1", "generated_at_utc": utc(), "rows": rows, "failed": failing, "verdict": "complete" if not failing else "not_complete"}
    write_json(artifact / "final-validation-matrix-release_readiness_benchmark.json", validation)
    final = {"schema_version": "release_readiness_benchmark.final-release-verdict.v1", "generated_at_utc": utc(), "status": validation["verdict"], "verdict": validation["verdict"], "failed_count": len(failing)}
    write_json(artifact / "release_readiness_benchmark-final-release-verdict.json", final)
    report = ["# Release Readiness Benchmark Final Report", "", f"- Final status: `{final['status']}`", f"- Artifact root: `{artifact}`", f"- VPS evidence: `{vps_results}`", "- Raw corpus, benchmark, and audit logs are retained in the VPS result folder and intentionally omitted from this report.", "- Release readiness is tracked in `release-readiness-go-no-go.json`; it is not a claim that every external tool or deployment policy is universally supported."]
    (artifact / "RELEASE_READINESS_BENCHMARK_FINAL_REPORT.md").write_text("\n".join(report) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    if args.mode == "start":
        ...
        print(json.dumps({"status": "passed", "artifact": str(artifact / "release_readiness_benchmark-starting-state.json")}, sort_keys=True))
        return 0
