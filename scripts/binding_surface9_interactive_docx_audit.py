#!/usr/bin/env python3
"""Generate the authoritative Combined form action policy evidence bundle.
...
OUT = ROOT / "target" / "form_action_policy-interactive-docx"
CLI = ROOT / "target" / "debug" / ("wellfriendpdf.exe" if os.name == "nt" else "wellfriendpdf")
SCHEMA = "form_action_policy.form-js-interactive-docx-layout.v1"
...
def main() -> int:
    ...
    starting = {
        "schema_version": "form_action_policy.starting-state.v1",
    ...
    write_json("form_action_policy-starting-state.json", starting)
    if actual_head != START_HEAD:
        raise RuntimeError(f"form action policy checkpoint mismatch: {actual_head}")
    ...
    feature_report_path = OUT / "feature-report-form_action_policy.json"
    ...
    write_json("form-js-inventory-form_action_policy.json", inventory)

    form_action_policy_envelope = OUT / "form_action_policy-report-envelope.json"
    run([str(CLI), "form_action_policy-report", str(actions_pdf), "--output", str(form_action_policy_envelope)], timeout=600)
    form_action_policy = read_json(form_action_policy_envelope)["report"]
    graph = form_action_policy["action_graph"]
    write_json("form-js-action-graph-form_action_policy.json", graph)
    write_json("form-js-policy-matrix-form_action_policy.json", policy_matrix())
    write_json(
        "form-js-safe-subset-results-form_action_policy.json",
        {
    ...
    write_json(
        "form-js-cycle-diagnostics-form_action_policy.json",
        {
            "schema_version": SCHEMA,
    ...
    sanitizer_report_path = OUT / "form-js-sanitizer-results-form_action_policy.json"
    run(
        [
            str(CLI),
            "form-js-sanitize",
    ...
    rescan_path = OUT / "form-js-rescan-results-form_action_policy.json"
    run([str(CLI), "form-js-report", str(sanitized_pdf), "--output", str(rescan_path)])
    rescan = read_json(rescan_path)["report"]
    write_json("form-js-rescan-results-form_action_policy.json", rescan)
    write_json(
        "form-js-security-report-form_action_policy.json",
        {
            "schema_version": SCHEMA,
    ...
    write_json("form-js-signature-impact-form_action_policy.json", sanitizer["signature_impact"])

    flattened_pdf = OUT / "form-actions-flattened.pdf"
    flatten_report_path = OUT / "form-js-calculation-flatten-results-form_action_policy.json"
    run(
        [
            str(CLI),
    ...
    write_json("form-js-calculation-flatten-results-form_action_policy.json", flatten)
    ...
    write_json("docx-page-section-results-form_action_policy.json", {
        "schema_version": SCHEMA,
    ...
    write_json("docx-positioned-text-results-form_action_policy.json", {"schema_version": SCHEMA, "results": {key: {"text_box_count": value["text_box_count"], "anchor_count": value["anchor_count"]} for key, value in docx_results.items()}})
    write_json("docx-flowing-paragraph-results-form_action_policy.json", {"schema_version": SCHEMA, "flowing": docx_results["flowing"], "posture": "native_paragraphs_with_explicit_source_page_sections"})
    write_json("docx-image-layout-results-form_action_policy.json", {"schema_version": SCHEMA, "results": {key: value["image_count"] for key, value in docx_results.items()}, "dedup_posture": "sha256_stable_media_names"})
    write_json("docx-table-layout-results-form_action_policy.json", {"schema_version": SCHEMA, "results": {key: {"tables": value["table_count"], "merges": value["merge_count"]} for key, value in docx_results.items()}, "supported": ["grid_widths", "gridSpan", "vMerge", "repeated_header", "cantSplit"]})
    write_json("docx-header-footer-results-form_action_policy.json", {"schema_version": SCHEMA, "status": "implemented_with_limits", "posture": "detected_furniture_preserved_as_page_relative_positioned_content; dedicated_parts_not_inferred", "header_parts": 0, "footer_parts": 0})
    write_json("docx-link-bookmark-results-form_action_policy.json", {"schema_version": SCHEMA, "hyperlinks": "implemented_external_http_https_mailto_relationships", "bookmarks": "unsupported_reported_exact_without_named_semantic_source", "fixture_result": docx_results["hybrid"]["hyperlink_count"]})
    write_json("docx-determinism-results-form_action_policy.json", {"schema_version": SCHEMA, "results": {key: {"sha256": value["sha256"], "repeat_sha256": value["repeat_sha256"], "match": value["deterministic_repeat_match"]} for key, value in docx_results.items()}, "fixed_metadata_clock": "1980-01-01T00:00:00Z"})

    taxonomy = pagination_taxonomy()
    corpus = corpus_manifest(actions_pdf, pagination_pdf)
    baseline = {
        "schema_version": SCHEMA,
        "baseline": "type3_cid_rendering_single_hardcoded_A4_section",
    ...
    write_json("word-pagination-failure-taxonomy-form_action_policy.json", taxonomy)
    write_json("word-pagination-corpus-manifest-form_action_policy.json", corpus)
    write_json("word-pagination-baseline-results-form_action_policy.json", baseline)
    ...
    write_json("word-pagination-word-results-form_action_policy.json", word)
    write_json("word-pagination-libreoffice-results-form_action_policy.json", libreoffice)
    ...
    write_json("word-pagination-reference-diff-form_action_policy.json", {
        "schema_version": SCHEMA,
        "structural": improved,
        "word": word,
    ...
    write_json("form_action_policy-corpus-manifest.json", corpus)
    write_json("form_action_policy-metamorphic-results.json", metamorphic)
    write_json("form_action_policy-differential-results.json", differential)

    matrix = feature_matrix(word, libreoffice)
    blocked = sum(1 for row in matrix if row["implementation_status"] == "blocked")
    write_json("form_action_policy-feature-matrix.json", {
        "schema_version": SCHEMA,
    ...
    if blocked:
        raise RuntimeError(f"form action policy feature matrix has {blocked} blocked row(s)")
    ...
    write_json("form_action_policy-performance-memory.json", performance)
    write_json("form_action_policy-limit-denial-results.json", {
        "schema_version": SCHEMA,
    ...
    for path in sorted(OUT.rglob("*")):
        if path.is_file() and path.name != "form_action_policy-artifact-manifest.json":
            artifact_manifest.append({"path": str(path.relative_to(ROOT)), "bytes": path.stat().st_size, "sha256": sha256(path)})
    write_json("form_action_policy-artifact-manifest.json", {"schema_version": SCHEMA, "artifacts": artifact_manifest})
...
def corpus_manifest(actions_pdf: Path, pagination_pdf: Path) -> dict[str, Any]:
    rows = [
        ...
        {"id": "secure_mutation-associated", "path": "target/secure_mutation-mask-inline-associated-signatures", "covers": ["associated_files", "signed_mutation", "redaction"]},
    ]
...
def feature_matrix(word: dict[str, Any], libreoffice: dict[str, Any]) -> list[dict[str, Any]]:
    capabilities = [
        ...
        ("signature_policy", "security", "secure mutation closeout DocMDP/FieldMDP enforcement", "implemented_with_limits"),
    ...
    for feature_id, category, capability, status in capabilities:
        rows.append({
            ...
            "dotnet": "yes",
            "java": "yes",
            "fixture": "form_action_policy-corpus-manifest.json",
            "test": "form_action_policy_interactive_docx",
            ...
            "future_owner": "advanced_editing_or_later" if status.startswith("unsupported") else "form_action_policy_closed",
        })
    return rows
...
def artifact_for(feature_id: str) -> str:
    if feature_id.startswith("js") or feature_id in {"open_action_aa", "action_chains", "safe_subset"}:
        return "form-js-inventory-form_action_policy.json"
    if feature_id in {"sanitizer_rescan", "signature_policy"}:
        return "form-js-sanitizer-results-form_action_policy.json"
    if feature_id == "interactive_scorecard":
        return "interactive-data-scorecard.json"
    if feature_id.startswith("word"):
        return "word-pagination-word-results-form_action_policy.json"
    if feature_id.startswith("libreoffice"):
        return "word-pagination-libreoffice-results-form_action_policy.json"
    return "docx-page-section-results-form_action_policy.json"
...
def write_html(matrix: list[dict[str, Any]], performance: dict[str, Any], word: dict[str, Any], libreoffice: dict[str, Any], metamorphic: dict[str, Any]) -> None:
    target = OUT / "form_action_policy-html-report" / "index.html"
    ...
    target.write_text(
        "<!doctype html><html><head><meta charset='utf-8'><title>form action policy Evidence</title>"
        ...
        "</head><body><h1>Combined form action policy evidence</h1>"
...
if __name__ == "__main__":
    try:
        ...
        print(f"form_action_policy audit failed: {exc}", file=sys.stderr)
        raise
