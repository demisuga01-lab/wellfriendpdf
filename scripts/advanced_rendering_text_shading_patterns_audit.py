#!/usr/bin/env python3
"""Generate and compare the Advanced Rendering text clipping/shading/pattern corpus."""
...
OUT_DIR = Path("target/advanced_rendering-text-shading-patterns")
    ...
    entries.append(
        {
            ...
            "owner_prompt": "combined_advanced_rendering",
            "expected_feature_coverage": expected,
            "expected_reference_behavior": "multi_reference_classified_by_advanced_rendering_audit",
            "generator": "scripts/advanced_rendering_text_shading_patterns_audit.py",
        }
    )
...
def configure_reference_renderer_runner() -> None:
    p06.OUT_DIR = OUT_DIR
...
def advanced_rendering_classification(raw: str, category: str, metrics: dict[str, Any]) -> tuple[str, str | None]:
    ...
    if raw == "all_references_agree_wellfriendpdf_mismatch" and wellfriendpdf_within_reference_spread(metrics):
        return (
            ...
            "advanced_rendering_cluster_tolerance: wellfriendpdf matched one reference and stayed within reference changed-pixel spread",
        )
    if raw in {
...
def copy_tool_manifest() -> dict[str, Any]:
    ...
    if not src.exists():
        raise RuntimeError(
            "Missing target-local Poppler/PDFium/MuPDF manifest; run Reference Renderer bootstrap first"
        )
...
def write_starting_state() -> None:
    write_json(
        STARTING_OUT,
        {
            "kind": "advanced_rendering_starting_state",
...
def write_static_matrices(entries: list[dict[str, Any]]) -> None:
    ...
    write_json(TEXT_MATRIX_OUT, {"kind": "advanced_rendering_text_clipping_matrix", "entries": by_category.get("text_clipping", [])})
    write_json(AXIAL_MATRIX_OUT, {"kind": "advanced_rendering_axial_radial_shading_matrix", "entries": by_category.get("axial_radial_shading", [])})
    write_json(MESH_MATRIX_OUT, {"kind": "advanced_rendering_mesh_patch_shading_matrix", "entries": by_category.get("mesh_patch_shading", [])})
    write_json(PATTERN_MATRIX_OUT, {"kind": "advanced_rendering_tiling_pattern_matrix", "entries": by_category.get("tiling_pattern", [])})
    write_json(
        FALLBACK_OUT,
        {
            "kind": "advanced_rendering_fallback_taxonomy",
    ...
    write_json(
        MEMORY_OUT,
        {
            "kind": "advanced_rendering_memory_scheduler_report",
            "memory_cap_mb": 4096,
...
def render_and_compare(
    ...
    for entry in entries:
        ...
        classification, classification_note = advanced_rendering_classification(
    ...
    results = {
        "schema_version": 1,
        "kind": "advanced_rendering_multi_reference_render_results",
        "dpi": dpi,
    ...
    summary = {
        "schema_version": 1,
        "kind": "advanced_rendering_reference_disagreement_summary",
        "page_count": len(entries),
        "total_pairwise_comparisons": len(entries) * len(p06.PAIR_NAMES),
        "classification_counts": classification_counts,
        "raw_reference_renderer_classification_counts": raw_classification_counts,
        ...
        "advanced_rendering_cluster_tolerance_acceptances": sum(
            1 for page in pages if "classification_note" in page
        ),
    }
    write_json(RESULTS_OUT, results)
    write_json(DIFF_METRICS_OUT, {"kind": "advanced_rendering_visual_diff_metrics", "pages": metrics_pages})
    ...
    HTML_OUT.write_text(
        HTML_OUT.read_text(encoding="utf-8").replace(
            ...
            "Advanced Rendering Text Clipping, Shading, and Pattern Audit",
        ),
        encoding="utf-8",
    )
...
def write_feature_report(wellfriendpdf_bin: str | None, timeout: int) -> None:
    ...
    payload: dict[str, Any] = {
        "kind": "advanced_rendering_public_feature_report",
        "command": result,
    }
    try:
...
def main() -> int:
    ...
    configure_reference_renderer_runner()
    ...
    write_json(
        ...
        {
            "schema_version": 1,
            "kind": "advanced_rendering_corpus_manifest",
            "page_count": len(entries),
