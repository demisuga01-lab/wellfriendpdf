#!/usr/bin/env python3
"""Generate and compare the Type3 CID Rendering Type3/CID/tensor closure corpus."""
...
import reference_renderer_render_compare as p06
import advanced_rendering_text_shading_patterns_audit as p08


OUT_DIR = Path("target/type3_cid_rendering-type3-cid-tensor")
FIXTURE_DIR = OUT_DIR / "corpus"
TOOL_MANIFEST_OUT = OUT_DIR / "type3_cid_rendering-reference-tool-manifest.json"
CORPUS_OUT = OUT_DIR / "type3_cid_rendering-corpus-manifest.json"
RESULTS_OUT = OUT_DIR / "type3_cid_rendering-render-results.json"
DIFF_METRICS_OUT = OUT_DIR / "type3_cid_rendering-diff-metrics.json"
DISAGREEMENT_OUT = OUT_DIR / "type3_cid_rendering-reference-disagreement-summary.json"
TEXT_MATRIX_OUT = OUT_DIR / "type3_cid_rendering-text-clipping-matrix.json"
TYPE3_MATRIX_OUT = OUT_DIR / "type3_cid_rendering-type3-clip-matrix.json"
CID_MATRIX_OUT = OUT_DIR / "type3_cid_rendering-cid-clip-matrix.json"
TYPE7_MATRIX_OUT = OUT_DIR / "type3_cid_rendering-type7-tensor-matrix.json"
FALLBACK_OUT = OUT_DIR / "type3_cid_rendering-fallback-taxonomy.json"
MEMORY_OUT = OUT_DIR / "type3_cid_rendering-memory-scheduler-report.json"
FEATURE_OUT = OUT_DIR / "type3_cid_rendering-public-feature-report.json"
HTML_OUT = OUT_DIR / "type3_cid_rendering-html-report" / "index.html"
...
TRANSPARENCY_CLOSEOUT_TOOL_MANIFEST = Path(
    "target/transparency_rendering-transparency-compositing/transparency_closeout-reference-tool-manifest.json"
)
...
def add_entry(
    ...
    expected_reference: str = "multi_reference_classified_by_type3_cid_rendering_audit",
) -> None:
    ...
    entries.append(
        {
            ...
            "owner_prompt": "combined_type3_cid_rendering",
            "expected_feature_coverage": expected,
            "expected_reference_behavior": expected_reference,
            "generator": "scripts/type3_cid_rendering_type3_cid_tensor_audit.py",
        }
    )
...
def copy_tool_manifest() -> dict[str, Any]:
    src = REFERENCE_RENDERER_TOOL_MANIFEST if REFERENCE_RENDERER_TOOL_MANIFEST.exists() else TRANSPARENCY_CLOSEOUT_TOOL_MANIFEST
    if not src.exists():
        raise RuntimeError("Missing target-local Poppler/PDFium/MuPDF manifest; run Reference Renderer bootstrap")
...
def type3_cid_rendering_classification(raw: str, category: str, metrics: dict[str, Any]) -> tuple[str, str | None]:
    ...
    if raw == "all_references_agree_wellfriendpdf_mismatch" and p08.wellfriendpdf_within_reference_spread(metrics):
        return "all_references_agree_wellfriendpdf_passes", "type3_cid_rendering_cluster_tolerance"
    if raw in {
...
def render_and_compare(entries: list[dict[str, Any]], manifest: dict[str, Any], wellfriendpdf_bin: str | None, dpi: int, timeout: int) -> dict[str, Any]:
    ...
    for entry in entries:
        ...
        classification, note = type3_cid_rendering_classification(raw, entry["category"], pair_metrics)
    ...
    results = {
        "schema_version": 1,
        "kind": "type3_cid_rendering_multi_reference_render_results",
        "dpi": dpi,
    ...
    summary = {
        "schema_version": 1,
        "kind": "type3_cid_rendering_reference_disagreement_summary",
        "page_count": len(entries),
    ...
    write_json(DIFF_METRICS_OUT, {"schema_version": 1, "kind": "type3_cid_rendering_diff_metrics", "pages": metrics_pages})
    write_json(DISAGREEMENT_OUT, summary)
    p06.render_html(results, summary)
    HTML_OUT.write_text(
        HTML_OUT.read_text(encoding="utf-8").replace(
            "Reference Renderer Multi-Reference Renderer Audit",
            "Type3 CID Rendering Type3, CID, and Tensor Closure Audit",
        ),
        encoding="utf-8",
    )
    return summary
...
def write_static_artifacts(entries: list[dict[str, Any]]) -> None:
    ...
    write_json(TEXT_MATRIX_OUT, {"kind": "type3_cid_rendering_text_clipping_matrix", "entries": entries})
    write_json(TYPE3_MATRIX_OUT, {"kind": "type3_cid_rendering_type3_clip_matrix", "entries": by_category.get("type3_text_clipping", []) + by_category.get("type3_text_clipping_unsupported", [])})
    write_json(CID_MATRIX_OUT, {"kind": "type3_cid_rendering_cid_clip_matrix", "entries": by_category.get("cid_text_clipping", []) + by_category.get("cid_text_clipping_unsupported", [])})
    write_json(TYPE7_MATRIX_OUT, {"kind": "type3_cid_rendering_type7_tensor_matrix", "entries": by_category.get("type7_tensor_patch", []) + by_category.get("type7_tensor_patch_unsupported", [])})
    write_json(
        FALLBACK_OUT,
        {
            "schema_version": 1,
            "kind": "type3_cid_rendering_fallback_taxonomy",
    ...
    write_json(
        ...
        {
            "schema_version": 1,
            "kind": "type3_cid_rendering_memory_scheduler_report",
            "memory_cap_mb": 4096,
            ...
            "offscreen_surfaces": "Transparency Rendering/08 scheduler-bounded posture is unchanged",
        },
    )
...
def write_feature_report(wellfriendpdf_bin: str | None, timeout: int) -> None:
    ...
    payload: dict[str, Any] = {"kind": "type3_cid_rendering_public_feature_report", "command": result}
    try:
...
def main() -> int:
    ...
    write_json(
        ...
        {
            "schema_version": 1,
            "kind": "type3_cid_rendering_corpus_manifest",
