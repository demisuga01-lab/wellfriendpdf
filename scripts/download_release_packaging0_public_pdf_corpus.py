#!/usr/bin/env python3
"""Download a small, public-domain Release Readiness Benchmark PDF benchmark corpus.
...
def download(source: dict[str, str], out_dir: Path, timeout: float, max_bytes: int) -> dict[str, object]:
    request = urllib.request.Request(
        source["url"],
        headers={"User-Agent": "WellfriendPDFSDK-ReleaseReadinessBenchmark/0.1 public-corpus-runner"},
    )
...
def main() -> int:
    ...
    manifest = {
        "schema_version": "release_readiness_benchmark.public-pdf-corpus-manifest.v1",
    ...
    results = {
        "schema_version": "release_readiness_benchmark.public-pdf-download-results.v1",
        "generated_at_utc": utc(),
