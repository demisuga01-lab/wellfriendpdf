#!/usr/bin/env python3
"""Generate Prompt 28 campaign planning artifacts.
...
classifies the existing cargo-fuzz targets into Prompt 28 ownership groups,
...
RENDERER_TARGETS = ["display_list", "renderer_prompt11", "functions", "structured_pdf"]
...
def status_for_row(group: str, label: str, inventory: dict[str, object]) -> tuple[str, str]:
    ...
    if group == "renderer":
        if any(term in text for term in ["display", "path", "image", "text", "annotation", "ocg", "tile"]):
            return ("implemented_with_limits", "display_list,renderer_prompt11,structured_pdf")
        if any(term in text for term in ["shading", "pattern", "blend", "soft-mask", "cmm", "color"]):
            return ("implemented_with_limits", "functions,renderer_prompt11")
...
def feature_matrix(repo: Path, inventory: dict[str, object]) -> dict[str, object]:
    ...
    for group, labels in PROMPT28_ROWS.items():
        for label in labels:
            ...
            rows.append(
                {
                    ...
                    "status": status,
                    "evidence": evidence,
                    "notes": "final status resolved by VPS Prompt 28 closeout artifacts",
                }
            )
    return {
        "schema_version": "prompt28.feature-matrix.v1",
...
def campaign_command(group: str, targets: list[str], seconds: int) -> list[str]:
    return [
        ...
        ",".join(targets),
        "--artifact-root",
        f"target/prompt28-long-fuzz-safedocs/fuzz-artifacts/{group}",
        "--json-output",
        f"target/prompt28-long-fuzz-safedocs/{group}-fuzz-runner.json",
        "--markdown-output",
        f"target/prompt28-long-fuzz-safedocs/{group}-fuzz-runner.md",
        "--memory-mb",
        str(MEMORY_CAP_MIB),
        "--smoke-runs",
        str(SMOKE_RUNS),
...
def campaign_plan(repo: Path) -> dict[str, object]:
    return {
        "schema_version": "prompt28.campaign-plan.v1",
        "generated_at_utc": utc(),
        "memory_policy": {
        ...
        "safedocs": {
            ...
            "command": [
                ...
                ".",
                "--result-root",
                "target/prompt28-long-fuzz-safedocs",
                "--wellfriendpdf-bin",
                "$WELLPDF_TMP_DIR/prompt28-bin/wellfriendpdf",
                "--timeout-seconds",
                "20",
                "--memory-mb",
                "2048",
...
def seed_manifest(repo: Path, group: str, targets: list[str]) -> dict[str, object]:
    ...
    return {
        "schema_version": f"prompt28.{group}.seed-corpus-manifest.v1",
        "generated_at_utc": utc(),
        "group": group,
...
def write_docs(repo: Path, artifact_root: Path) -> None:
    docs = {
        "docs/prompt28_long_fuzz_safedocs_audit.md": (
            "Prompt 28 long fuzz and SafeDocs audit",
            [
                "Prompt 28 covers long codec, renderer, writer/edit fuzz campaigns and SafeDocs corpus execution.",
        ...
        "docs/prompt28_feature_matrix.md": (
            "Prompt 28 feature matrix",
            [
                "The machine-readable matrix is `target/prompt28-long-fuzz-safedocs/prompt28-feature-matrix.json`.",
        ...
        "docs/fuzz_campaign_artifacts.md": (
            ...
            [
                "Prompt 28 artifacts are generated under `target/prompt28-long-fuzz-safedocs/` and copied to the VPS result folder.",
        ...
        "docs/prompt28_known_limits.md": (
            "Prompt 28 known limits",
            [
                "Prompt 28 does not claim full differential rendering parity, full sanitizer coverage across every subsystem, or Prompt 30 security audit completion.",
...
def starting_state(repo: Path) -> dict[str, object]:
    return {
        "schema_version": "prompt28.starting-state.v1",
        "generated_at_utc": utc(),
        ...
        "prompt27_baseline": "3e6ed708f43fb27f7b7057e4900e736b34c67717",
        "repo_identity": {
...
def main() -> int:
    ...
    outputs = {
        "prompt28-starting-state.json": starting_state(repo),
        "prompt28-feature-matrix.json": feature_matrix(repo, inventory),
        "prompt28-campaign-plan.json": campaign_plan(repo),
        "codec-fuzz-target-inventory.json": {
            "schema_version": "prompt28.codec-fuzz-target-inventory.v1",
            "generated_at_utc": utc(),
            "targets": [row for row in inventory["targets"] if row["name"] in CODEC_TARGETS],
            "verdict": "complete",
        },
        "renderer-fuzz-target-inventory.json": {
            "schema_version": "prompt28.renderer-fuzz-target-inventory.v1",
            "generated_at_utc": utc(),
            "targets": [row for row in inventory["targets"] if row["name"] in RENDERER_TARGETS],
            "verdict": "complete",
        },
        "writer-edit-fuzz-target-inventory.json": {
            "schema_version": "prompt28.writer-edit-fuzz-target-inventory.v1",
            "generated_at_utc": utc(),
