#!/usr/bin/env bash
# Run a bounded, serial focused text reflow validation in a transferred source tree.
...
# `/home` may hold retained text reflow evidence and therefore be full. Callers
...
export CARGO_TARGET_DIR="${WELLPDF_TEXT_REFLOW_CARGO_TARGET_DIR:-$temp_root/target}"
...
cd "$source_root"
run_stage engine-text_reflow cargo test -p wellfriendpdf-engine --lib text_reflow -- --nocapture
run_stage capi-text_reflow cargo test -p wellfriendpdf-capi capi_text_reflow_reflow_surfaces_return_owned_outputs -- --nocapture
