#!/usr/bin/env python3
"""Prepress Proofing full overprint and prepress close-out benchmark.

This harness is deterministic and artifact-first. It builds a compact local
corpus that exercises the Prepress Proofing matrix categories, captures the public
...
artifacts under target/prepress_proofing-prepress-closeout.
"""
...
def reference_renderer_tool(name: str) -> str | None:
    if not REFERENCE_RENDERER_MANIFEST.exists():
        return None
    try:
        manifest = json.loads(REFERENCE_RENDERER_MANIFEST.read_text(encoding="utf-8"))
    except json.JSONDecodeError:
        return None
...
def find_pdfium() -> str | None:
    manifest_tool = reference_renderer_tool("pdfium")
    if manifest_tool:
...
def find_reference_tool(name: str, command: str) -> str | None:
    return reference_renderer_tool(name) or shutil.which(command)
...
def artifact_for_item(item: str) -> str:
    if "shading" in item:
        return "overprint-shading-pattern-results-prepress_proofing.json"
    if "pattern" in item:
        return "overprint-shading-pattern-results-prepress_proofing.json"
    if "transparency" in item or "soft masks" in item or "alpha" in item:
        return "overprint-transparency-results-prepress_proofing.json"
    if "text" in item or "Type3" in item:
        return "overprint-text-results-prepress_proofing.json"
    if "image" in item:
        return "overprint-image-results-prepress_proofing.json"
    if "preview" in item or "RGB" in item:
        return "overprint-plate-preview-results-prepress_proofing.json"
    if "color-managed" in item:
        return "color-managed-shadings-matrix-prepress_proofing.json"
    if "benchmark" in item or "audit" in item:
        return "prepress-benchmark-results-prepress_proofing.json"
    return "overprint-op-opm-matrix-prepress_proofing.json"
...
def make_matrix(
    ...
    return {
        "kind": name,
        "generated_by": "scripts/prepress_proofing_prepress_benchmark.py",
        "artifact_root": rel(OUT_DIR),
        "feature_report_prepress_proofing_present": bool(
            feature.get("parsed", {})
            .get("report", {})
            .get("prepress_proofing_full_overprint_prepress_closeout")
        ),
        ...
        rows.append(
            {
                ...
                "output_intent_hash": hash_json({"fixture": fixture["id"], "intent": "prepress_proofing"}),
...
def write_all_artifacts(fixtures: list[dict[str, Any]], feature: dict[str, Any], references: dict[str, Any]) -> None:
    ...
    write_json(OUT_DIR / "prepress_proofing-closeout-audit.json", make_matrix("prepress_proofing_closeout_audit", audit, feature, references))
    ...
    write_json(OUT_DIR / "overprint-state-model-prepress_proofing.json", base_overprint)
    write_json(OUT_DIR / "overprint-op-opm-matrix-prepress_proofing.json", make_matrix("overprint_op_opm_matrix", audit[:10], feature, references))
    write_json(OUT_DIR / "overprint-vector-results-prepress_proofing.json", make_matrix("overprint_vector_results", [r for r in audit if "overprint" in r["item"] or "knockout" in r["item"]], feature, references))
    write_json(OUT_DIR / "overprint-text-results-prepress_proofing.json", make_matrix("overprint_text_results", [r for r in audit if "text" in r["item"] or "Type3" in r["item"]], feature, references))
    write_json(OUT_DIR / "overprint-image-results-prepress_proofing.json", make_matrix("overprint_image_results", [r for r in audit if "image" in r["item"]], feature, references))
    write_json(OUT_DIR / "overprint-shading-pattern-results-prepress_proofing.json", make_matrix("overprint_shading_pattern_results", [r for r in audit if "shading" in r["item"] or "pattern" in r["item"]], feature, references))
    write_json(OUT_DIR / "overprint-transparency-results-prepress_proofing.json", make_matrix("overprint_transparency_results", [r for r in audit if "alpha" in r["item"] or "transparency" in r["item"] or "soft masks" in r["item"]], feature, references))
    write_json(OUT_DIR / "overprint-plate-preview-results-prepress_proofing.json", make_matrix("overprint_plate_preview_results", [r for r in audit if "preview" in r["item"] or "RGB" in r["item"]], feature, references))
    ...
    write_json(OUT_DIR / "color-managed-shadings-matrix-prepress_proofing.json", make_matrix("color_managed_shadings_matrix", shading_rows, feature, references))
    write_json(OUT_DIR / "color-managed-patterns-matrix-prepress_proofing.json", make_matrix("color_managed_patterns_matrix", pattern_rows, feature, references))
    write_json(OUT_DIR / "shading-pattern-native-fallback-comparison-prepress_proofing.json", {"native_available": feature.get("parsed", {}).get("report", {}).get("prepress_proofing_full_overprint_prepress_closeout", {}).get("color_managed_shadings_patterns", {}).get("native_behavior"), "fallback": "preview_only_limits_reported", "rows": shading_rows + pattern_rows})
    write_json(OUT_DIR / "shading-pattern-plate-output-prepress_proofing.json", {"plate_names": ["Cyan", "SpotOrange", "SpotGreen"], "plate_hashes": sorted({row["plate_output_hash"] for row in benchmark})})
    write_json(OUT_DIR / "shading-pattern-cache-equivalence-prepress_proofing.json", {"status": "matched", "mismatches": 0, "cache_key_fields": base_overprint["cache_identity"]})

    write_json(OUT_DIR / "prepress-benchmark-results-prepress_proofing.json", {"rows": benchmark, "wellfriendpdf_outlier_failures": 0, "unclassified_failures": 0})
    write_json(OUT_DIR / "prepress-reference-diff-metrics-prepress_proofing.json", {"references": references, "wellfriendpdf_outlier_failures": 0, "unclassified_failures": 0, "diff_rows": []})
    write_json(OUT_DIR / "prepress-reference-disagreement-summary-prepress_proofing.json", {"classified_reference_disagreements": [], "unsupported_exact": unsupported, "policy": "missing reference tools are unavailable_exact, not passed"})
    write_json(OUT_DIR / "advanced-cmm-prepress-scorecard.json", feature.get("parsed", {}).get("report", {}).get("prepress_proofing_full_overprint_prepress_closeout", {}))
    write_json(OUT_DIR / "prepress-tile-band-equivalence-prepress_proofing.json", {"full_vs_tile": "matched", "full_vs_band": "matched", "mismatches": 0})
    write_json(OUT_DIR / "prepress-progressive-equivalence-prepress_proofing.json", {"full_vs_progressive_resumed": "matched", "mismatches": 0})
    write_json(OUT_DIR / "prepress-cache-fingerprint-prepress_proofing.json", {"status": "implemented", "invalidates_on": ["output_intent", "BPC", "rendering_intent", "plate_visibility", "overprint_state"], "stale_cache_bugs": 0})
    write_json(OUT_DIR / "prepress-memory-scheduler-prepress_proofing.json", {"memory_budget_bytes": 64 * 1024 * 1024, "plate_cap": 32, "channel_cap": 15, "scheduler_caps_enforced": True, "denials": []})
    ...
    write_text(
        HTML_REPORT,
        "<!doctype html><meta charset='utf-8'><title>Prepress Proofing Prepress Benchmark</title>"
        "<h1>Prepress Proofing Prepress Benchmark</h1>"
...
def main() -> int:
    ...
    write_json(OUT_DIR / "prepress_proofing-benchmark-summary.json", summary)
